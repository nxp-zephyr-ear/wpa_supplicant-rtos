...
int main(int argc, char *argv[])
{
    ...
    if (argc != 4)
    {
        fprintf(stderr, "usage: sae_pk_gen <DER ECPrivateKey file> <Sec:3|5> <SSID>\n");
        goto fail;
    }
    ...
    if (sec != 3 && sec != 5)
    {
        fprintf(stderr, "Invalid Sec value (allowed values: 3 and 5)\n");
        goto fail;
    }
    sec_1b   = sec == 3;
    expected = 1;
    ...
    for (i = 0;; i++)
    {
        ...
        {
            if ((hash[2] & 0xf0) == 0)
                fprintf(stderr, "\r%3.2f%%", 100.0 * (double)i / (double)expected);
            for (j = 2; j < sec; j++)
            {
                if (hash[j])
                    break;
    ...
    b64 = wpa_base64_encode(der, der_len, NULL);
    if (!b64)
        goto fail;
    src = pos = b64;
    while (*src)
    ...
    *pos = '\0';
    ...
    if (wpa_snprintf_hex(hash_hex, sizeof(hash_hex), pw_base_bin, hash_len - sec) >= 0)
        fprintf(stderr, "PasswordBase binary data for base32: %s", hash_hex);
