...
static int hostapd_get_global_ctrl_iface(struct hapd_interfaces *interfaces, const char *path)
{
...
static int hostapd_get_ctrl_iface_group(struct hapd_interfaces *interfaces, const char *group)
{
#ifndef CONFIG_NATIVE_WINDOWS
    struct group *grp;
    grp = getgrnam(group);
...
static int hostapd_get_interface_names(char ***if_names, size_t *if_names_size, char *arg)
{
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < interfaces.count; i++)
    {
        ...
        interfaces.iface[i] = hostapd_interface_init(&interfaces, if_name, argv[optind + i], debug);
