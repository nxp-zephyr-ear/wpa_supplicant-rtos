...
WpaGui::WpaGui(QApplication *_app, QWidget *parent, const char *, Qt::WindowFlags) : QMainWindow(parent), app(_app)
{
    setupUi(this);
    ...
    connect(fileStartServiceAction, SIGNAL(triggered()), this, SLOT(startService()));
    connect(fileStopServiceAction, SIGNAL(triggered()), this, SLOT(stopService()));
    ...
    connect(fileEventHistoryAction, SIGNAL(triggered()), this, SLOT(eventHistory()));
    connect(fileSaveConfigAction, SIGNAL(triggered()), this, SLOT(saveConfig()));
    ...
    connect(networkAddAction, SIGNAL(triggered()), this, SLOT(addNetwork()));
    connect(networkEditAction, SIGNAL(triggered()), this, SLOT(editSelectedNetwork()));
    connect(networkRemoveAction, SIGNAL(triggered()), this, SLOT(removeSelectedNetwork()));
    connect(networkEnableAllAction, SIGNAL(triggered()), this, SLOT(enableAllNetworks()));
    connect(networkDisableAllAction, SIGNAL(triggered()), this, SLOT(disableAllNetworks()));
    connect(networkRemoveAllAction, SIGNAL(triggered()), this, SLOT(removeAllNetworks()));
    ...
    connect(adapterSelect, SIGNAL(activated(const QString &)), this, SLOT(selectAdapter(const QString &)));
    connect(networkSelect, SIGNAL(activated(const QString &)), this, SLOT(selectNetwork(const QString &)));
    connect(addNetworkButton, SIGNAL(clicked()), this, SLOT(addNetwork()));
    connect(editNetworkButton, SIGNAL(clicked()), this, SLOT(editListedNetwork()));
    connect(removeNetworkButton, SIGNAL(clicked()), this, SLOT(removeListedNetwork()));
    connect(networkList, SIGNAL(itemSelectionChanged()), this, SLOT(updateNetworkDisabledStatus()));
    connect(enableRadioButton, SIGNAL(toggled(bool)), this, SLOT(enableListedNetwork(bool)));
    connect(disableRadioButton, SIGNAL(toggled(bool)), this, SLOT(disableListedNetwork(bool)));
    connect(scanNetworkButton, SIGNAL(clicked()), this, SLOT(scan()));
    connect(networkList, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this, SLOT(editListedNetwork()));
    connect(wpaguiTab, SIGNAL(currentChanged(int)), this, SLOT(tabChanged(int)));
    ...
    if (app->isSessionRestored())
    {
        ...
        if (app->sessionId().compare(settings.value("session_id").toString()) == 0)
...
void WpaGui::updateStatus()
{
...
#ifdef CONFIG_NATIVE_WINDOWS
        static bool first = true;
        if (first && connectedToService && (ctrl_iface == NULL || *ctrl_iface == '\0'))
        {
            first = false;
    ...
    {
        QString encr;
        if (pairwise_cipher && group_cipher && strcmp(pairwise_cipher, group_cipher) != 0)
        {
...
void WpaGui::processMsg(char *msg)
{
    ...
    if (str_match(pos, WPA_CTRL_REQ))
        ...
        showTrayMessage(QSystemTrayIcon::Information, 3, tr("Disconnected from network."));
    else if (str_match(pos, WPA_EVENT_CONNECTED))
    {
        showTrayMessage(QSystemTrayIcon::Information, 3, tr("Connection to network established."));
    ...
    else if (str_match(pos, WPS_EVENT_AP_AVAILABLE_PBC))
    {
        ...
        if (textStatus->text() == "INACTIVE" || textStatus->text() == "DISCONNECTED")
            wpaguiTab->setCurrentWidget(wpsTab);
        wpsInstructions->setText(
            tr("Press the PBC button on the "
               "screen to start registration"));
    }
    else if (str_match(pos, WPS_EVENT_AP_AVAILABLE_PIN))
    {
        wpsStatusText->setText(
            tr("WPS AP with recently selected "
               "registrar"));
        if (textStatus->text() == "INACTIVE" || textStatus->text() == "DISCONNECTED")
            wpaguiTab->setCurrentWidget(wpsTab);
    }
    else if (str_match(pos, WPS_EVENT_AP_AVAILABLE_AUTH))
    {
        ...
        wpsStatusText->setText(
            "WPS AP indicating this client is "
            "authorized");
        if (textStatus->text() == "INACTIVE" || textStatus->text() == "DISCONNECTED")
            wpaguiTab->setCurrentWidget(wpsTab);
    }
    ...
    else if (str_match(pos, WPS_EVENT_OVERLAP))
    {
        ...
        wpsInstructions->setText(
            tr("More than one AP is currently in "
...
void WpaGui::enableNetwork(const QString &sel)
{
    ...
    {
        debug("Invalid editNetwork '%s'", cmd.toLocal8Bit().constData());
        return;
    }
...
void WpaGui::editSelectedNetwork()
{
    if (networkSelect->count() < 1)
    {
        QMessageBox::information(this, tr("No Networks"), tr("There are no networks to edit.\n"));
        return;
    }
...
int WpaGui::getNetworkDisabled(const QString &sel)
{
    ...
    if (pos < 0)
    {
        debug("Invalid getNetworkDisabled '%s'", cmd.toLocal8Bit().constData());
        return -1;
    }
    cmd.truncate(pos);
    ...
    if (ctrlRequest(cmd.toLocal8Bit().constData(), reply, &reply_len) >= 0 && reply_len >= 1)
    {
        reply[reply_len] = '\0';
...
void WpaGui::saveConfig()
{
    ...
    if (str_match(buf, "FAIL"))
        QMessageBox::warning(this, tr("Failed to save configuration"),
        ...
        QMessageBox::information(this, tr("Saved configuration"),
...
void WpaGui::createTrayIcon(bool trayOnly)
{
    ...
    connect(tray_icon, SIGNAL(activated(QSystemTrayIcon::ActivationReason)), this,
    ...
    connect(reconnectAction, SIGNAL(triggered()), this, SLOT(connectB()));
...
void WpaGui::showTrayMessage(QSystemTrayIcon::MessageIcon type, int sec, const QString &msg)
{
...
void WpaGui::closeEvent(QCloseEvent *event)
{
    ...
    if (tray_icon && !ackTrayIcon)
    {
        ...
        if (QSystemTrayIcon::supportsMessages())
        {
            hide();
            showTrayMessage(QSystemTrayIcon::Information, 3,
                            qAppName() + tr(" will keep running in "
                                            "the system tray."));
        }
        else
        {
            QMessageBox::information(this, qAppName() + tr(" systray"),
...
void WpaGui::wpsPbc()
{
    ...
    {
        wpsInstructions->setText(
            tr("Press the push button on the AP to "
               "start the PBC mode."));
    }
    else
    {
        wpsInstructions->setText(
            tr("If you have not yet done so, press "
...
void WpaGui::wpsGeneratePin()
{
    ...
    wpsInstructions->setText(
        tr("Enter the generated PIN into the Registrar "
...
void WpaGui::setBssFromScan(const QString &bssid)
{
    ...
    wpsInstructions->setText(
        tr("If you want to use an AP device PIN, e.g., "
...
{
    if (wpsRunning)
        wpsStatusText->setText(success ? tr("Connected to the network") : tr("Stopped"));
    else
        wpsStatusText->setText("");
    wpsPinEdit->setEnabled(false);
    wpsInstructions->setText("");
...
ErrorMsg::ErrorMsg(QWidget *parent, DWORD last_err) : QMessageBox(parent), err(last_err)
{
...
void ErrorMsg::showMsg(QString msg)
{
    ...
    setText(msg);
    if (FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, err, 0, (LPTSTR)(void *)&buf,
                      0, NULL) > 0)
    {
...
void WpaGui::startService()
{
    ...
    {
        ErrorMsg(this).showMsg(
            tr("Failed to start wpa_supplicant "
               "service"));
    }
...
void WpaGui::stopService()
{
    ...
    {
        ErrorMsg(this).showMsg(
            tr("Failed to stop wpa_supplicant "
               "service"));
    }
