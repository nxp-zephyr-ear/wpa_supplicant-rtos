...
AddInterface::AddInterface(WpaGui *_wpagui, QWidget *parent) : QDialog(parent), wpagui(_wpagui)
{
    ...
    connect(interfaceWidget, SIGNAL(itemActivated(QTreeWidgetItem *, int)), this,
...
bool AddInterface::addRegistryInterface(const QString &ifname)
{
    ...
    ret = RegOpenKeyEx(WPA_KEY_ROOT, WPA_KEY_PREFIX TEXT("\\interfaces"), 0,
                       KEY_ENUMERATE_SUB_KEYS | KEY_CREATE_SUB_KEY, &hk);
...
#endif /* UNICODE */
    ret = RegCreateKeyEx(hk, name, 0, NULL, 0, KEY_WRITE, NULL, &ihk, NULL);
    RegCloseKey(hk);
    if (ret != ERROR_SUCCESS)
        return false;

#ifdef UNICODE
    RegSetValueEx(ihk, TEXT("adapter"), 0, REG_SZ, (LPBYTE)ifname.unicode(), (ifname.length() + 1) * sizeof(TCHAR));

#else  /* UNICODE */
    RegSetValueEx(ihk, TEXT("adapter"), 0, REG_SZ, (LPBYTE)ifname.toLocal8Bit(), ifname.length() + 1);
#endif /* UNICODE */
    RegSetValueEx(ihk, TEXT("config"), 0, REG_SZ, (LPBYTE)TEXT("default"), 8 * sizeof(TCHAR));
    RegSetValueEx(ihk, TEXT("ctrl_interface"), 0, REG_SZ, (LPBYTE)TEXT(""), 1 * sizeof(TCHAR));
    val = 1;
    RegSetValueEx(ihk, TEXT("skip_on_error"), 0, REG_DWORD, (LPBYTE)&val, sizeof(val));
...
void AddInterface::interfaceSelected(QTreeWidgetItem *sel)
{
     ...
     */
    snprintf(cmd, sizeof(cmd), "INTERFACE_ADD %s\t%s\t%s\t%s\t%s\t%s", sel->text(1).toLocal8Bit().constData(),
             "default", sel->text(0).toLocal8Bit().constData(), "yes", "", "");
    cmd[sizeof(cmd) - 1] = '\0';
    ...
    if (buf[0] != 'O' || buf[1] != 'K')
    {
        QMessageBox::warning(this, "wpa_gui", tr("Failed to add the interface."));
        return;
    }
