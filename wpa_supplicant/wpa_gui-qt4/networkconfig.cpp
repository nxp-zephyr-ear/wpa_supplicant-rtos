...
NetworkConfig::NetworkConfig(QWidget *parent, const char *, bool, Qt::WindowFlags) : QDialog(parent)
{
    setupUi(this);
...
{
    encrSelect->setEnabled(sel != AUTH_NONE_OPEN && sel != AUTH_NONE_WEP && sel != AUTH_NONE_WEP_SHARED);
    pskEdit->setEnabled(sel == AUTH_WPA_PSK || sel == AUTH_WPA2_PSK);
    bool eap = sel == AUTH_IEEE8021X || sel == AUTH_WPA_EAP || sel == AUTH_WPA2_EAP;
    ...
    if (sel == AUTH_NONE_OPEN || sel == AUTH_NONE_WEP || sel == AUTH_NONE_WEP_SHARED || sel == AUTH_IEEE8021X)
    {
    ...
    else
    {
        ...
        encrSelect->setCurrentIndex((sel == AUTH_WPA2_PSK || sel == AUTH_WPA2_EAP) ? 1 : 0);
    }
...
void NetworkConfig::addNetwork()
{
    ...
    if (auth == AUTH_WPA_PSK || auth == AUTH_WPA2_PSK)
    {
        ...
        {
            QMessageBox::warning(this, tr("WPA Pre-Shared Key Error"),
    ...
    if (idstrEdit->isEnabled() && !idstrEdit->text().isEmpty())
    {
        ...
        {
            QMessageBox::warning(this, tr("Network ID Error"),
    ...
    if (pskEdit->isEnabled() && strcmp(pskEdit->text().toLocal8Bit().constData(), WPA_GUI_KEY_DATA) != 0)
        setNetworkParam(id, "psk", pskEdit->text().toLocal8Bit().constData(), psklen != 64);
    if (eapSelect->isEnabled())
    {
        const char *eap = eapSelect->currentText().toLocal8Bit().constData();
    ...
    if (phase2Select->isEnabled())
    {
        ...
        if (eap.compare("PEAP") == 0)
        {
            if (inner.startsWith("EAP-"))
                snprintf(phase2, sizeof(phase2), "auth=%s", inner.right(inner.size() - 4).toLocal8Bit().constData());
        }
        else if (eap.compare("TTLS") == 0)
        {
            if (inner.startsWith("EAP-"))
                snprintf(phase2, sizeof(phase2), "autheap=%s", inner.right(inner.size() - 4).toLocal8Bit().constData());
            else
                snprintf(phase2, sizeof(phase2), "auth=%s", inner.toLocal8Bit().constData());
        }
        else if (eap.compare("FAST") == 0)
        {
            ...
            {
                snprintf(phase2, sizeof(phase2), "auth=GTC auth=MSCHAPV2");
            ...
            {
                char blob[32];
                setNetworkParam(id, "phase1", provisioning, true);
                snprintf(blob, sizeof(blob), "blob://fast-pac-%d", id);
    ...
    if (identityEdit->isEnabled() && identityEdit->text().length() > 0)
        setNetworkParam(id, "identity", identityEdit->text().toLocal8Bit().constData(), true);
    else
        setNetworkParam(id, "identity", "NULL", false);
    if (passwordEdit->isEnabled() && passwordEdit->text().length() > 0 &&
        strcmp(passwordEdit->text().toLocal8Bit().constData(), WPA_GUI_KEY_DATA) != 0)
        setNetworkParam(id, "password", passwordEdit->text().toLocal8Bit().constData(), true);
    else if (passwordEdit->text().length() == 0)
        setNetworkParam(id, "password", "NULL", false);
    if (cacertEdit->isEnabled() && cacertEdit->text().length() > 0)
        setNetworkParam(id, "ca_cert", cacertEdit->text().toLocal8Bit().constData(), true);
    else
    ...
    if (idstrEdit->isEnabled() && idstrEdit->text().length() > 0)
        setNetworkParam(id, "id_str", idstrEdit->text().toLocal8Bit().constData(), true);
    else
...
void NetworkConfig::paramsFromConfig(int network_id)
{
    ...
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest(cmd, reply, &reply_len) >= 0 && reply_len >= 2 && reply[0] == '"')
    {
        reply[reply_len] = '\0';
        pos              = strchr(reply + 1, '"');
        if (pos)
    ...
    {
        reply[reply_len] = '\0';
        if (strstr(reply, "CCMP") && auth != AUTH_NONE_OPEN && auth != AUTH_NONE_WEP && auth != AUTH_NONE_WEP_SHARED)
            encr = 1;
    ...
    if (eap != NO_INNER)
    {
        snprintf(cmd, sizeof(cmd), "GET_NETWORK %d phase2", network_id);
        reply_len = sizeof(reply) - 1;
        if (wpagui->ctrlRequest(cmd, reply, &reply_len) >= 0 && reply_len >= 1)
        {
            reply[reply_len] = '\0';
    ...
    {
        snprintf(cmd, sizeof(cmd), "GET_NETWORK %d auth_alg", network_id);
        reply_len = sizeof(reply) - 1;
        if (wpagui->ctrlRequest(cmd, reply, &reply_len) >= 0)
        {
            reply[reply_len] = '\0';
