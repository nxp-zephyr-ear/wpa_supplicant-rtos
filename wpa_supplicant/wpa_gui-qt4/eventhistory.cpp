...
EventHistory::EventHistory(QWidget *parent, const char *, bool, Qt::WindowFlags) : QDialog(parent)
{
    setupUi(this);
...
void EventHistory::addEvent(WpaMsg msg)
{
    ...
    if (eventListView->verticalScrollBar()->value() < eventListView->verticalScrollBar()->maximum())
        scroll = false;

    elm->addEvent(msg.getTimestamp().toString("yyyy-MM-dd hh:mm:ss.zzz"), msg.getMsg());
