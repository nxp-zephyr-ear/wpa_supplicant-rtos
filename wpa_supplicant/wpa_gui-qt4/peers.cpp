...
Peers::Peers(QWidget *parent, const char *, bool, Qt::WindowFlags) : QDialog(parent)
{
    setupUi(this);
    ...
    connect(peers, SIGNAL(customContextMenuRequested(const QPoint &)), this, SLOT(context_menu(const QPoint &)));
...
void Peers::context_menu(const QPoint &pos)
{
    ...
    if (idx.isValid())
    {
        ...
        if ((type == PEER_TYPE_ASSOCIATED_STATION || type == PEER_TYPE_AP_WPS || type == PEER_TYPE_WPS_PIN_NEEDED ||
             type == PEER_TYPE_WPS_ER_ENROLLEE || type == PEER_TYPE_WPS_ENROLLEE) &&
            (config_methods == -1 || (config_methods & 0x010c)))
        {
            menu->addAction(tr("Enter WPS PIN"), this, SLOT(enter_pin()));
        }

        if (type == PEER_TYPE_P2P || type == PEER_TYPE_P2P_CLIENT)
        {
            menu->addAction(tr("P2P Connect"), this, SLOT(ctx_p2p_connect()));
            if (method == SEL_METHOD_NONE && config_methods > -1 && config_methods & 0x0080 /* PBC */ &&
                config_methods != 0x0080)
                menu->addAction(tr("P2P Connect (PBC)"), this, SLOT(connect_pbc()));
            if (method == SEL_METHOD_NONE)
            {
                menu->addAction(tr("P2P Request PIN"), this, SLOT(ctx_p2p_req_pin()));
                menu->addAction(tr("P2P Show PIN"), this, SLOT(ctx_p2p_show_pin()));
            }

            if (config_methods > -1 && (config_methods & 0x0100))
            {
                /* Peer has Keypad */
                menu->addAction(tr("P2P Display PIN"), this, SLOT(ctx_p2p_display_pin()));
            }

            if (config_methods > -1 && (config_methods & 0x000c))
            {
                /* Peer has Label or Display */
                menu->addAction(tr("P2P Enter PIN"), this, SLOT(ctx_p2p_enter_pin()));
            }
        }

        if (type == PEER_TYPE_P2P_GROUP)
        {
            menu->addAction(tr("Show passphrase"), this, SLOT(ctx_p2p_show_passphrase()));
            menu->addAction(tr("Remove P2P Group"), this, SLOT(ctx_p2p_remove_group()));
        }

        if (type == PEER_TYPE_P2P_PERSISTENT_GROUP_GO || type == PEER_TYPE_P2P_PERSISTENT_GROUP_CLIENT ||
            type == PEER_TYPE_P2P_INVITATION)
        {
            menu->addAction(tr("Start group"), this, SLOT(ctx_p2p_start_persistent()));
        }

        if (type == PEER_TYPE_P2P_PERSISTENT_GROUP_GO || type == PEER_TYPE_P2P_PERSISTENT_GROUP_CLIENT)
        {
            menu->addAction(tr("Invite"), this, SLOT(ctx_p2p_invite()));
        }

        if (type == PEER_TYPE_P2P_INVITATION)
        {
            menu->addAction(tr("Ignore"), this, SLOT(ctx_p2p_delete()));
        }

        if (type == PEER_TYPE_AP_WPS)
        {
            menu->addAction(tr("Connect (PBC)"), this, SLOT(connect_pbc()));
        }

        if ((type == PEER_TYPE_ASSOCIATED_STATION || type == PEER_TYPE_WPS_ER_ENROLLEE ||
             type == PEER_TYPE_WPS_ENROLLEE) &&
            config_methods >= 0 && (config_methods & 0x0080))
        {
            menu->addAction(tr("Enroll (PBC)"), this, SLOT(connect_pbc()));
        }

        if (type == PEER_TYPE_WPS_ER_AP)
        {
            menu->addAction(tr("Learn Configuration"), this, SLOT(learn_ap_config()));
        }
    ...
    {
        ctx_item = NULL;
        menu->addAction(QString(tr("Refresh")), this, SLOT(ctx_refresh()));
        menu->addAction(tr("Start P2P discovery"), this, SLOT(ctx_p2p_start()));
        menu->addAction(tr("Stop P2P discovery"), this, SLOT(ctx_p2p_stop()));
        menu->addAction(tr("P2P listen only"), this, SLOT(ctx_p2p_listen()));
        menu->addAction(tr("Start P2P group"), this, SLOT(ctx_p2p_start_group()));
        if (hide_ap)
            menu->addAction(tr("Show AP entries"), this, SLOT(ctx_show_ap()));
        else
            menu->addAction(tr("Hide AP entries"), this, SLOT(ctx_hide_ap()));
    }
...
void Peers::enter_pin()
{
    ...
    if (peer_type == PEER_TYPE_WPS_ER_ENROLLEE)
    {
        snprintf(cmd, sizeof(cmd), "WPS_ER_PIN %s %s %s", uuid.toLocal8Bit().constData(),
                 input.get_string().toLocal8Bit().constData(), addr.toLocal8Bit().constData());
    }
    else
    {
        snprintf(cmd, sizeof(cmd), "WPS_PIN %s %s", addr.toLocal8Bit().constData(),
...
void Peers::ctx_p2p_start()
{
    ...
    size_t reply_len;
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest("P2P_FIND", reply, &reply_len) < 0 || memcmp(reply, "FAIL", 4) == 0)
    {
        QMessageBox msg;
        msg.setIcon(QMessageBox::Warning);
...
void Peers::ctx_p2p_listen()
{
    ...
    size_t reply_len;
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest("P2P_LISTEN 3600", reply, &reply_len) < 0 || memcmp(reply, "FAIL", 4) == 0)
    {
        QMessageBox msg;
        msg.setIcon(QMessageBox::Warning);
...
void Peers::ctx_p2p_start_group()
{
    ...
    size_t reply_len;
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest("P2P_GROUP_ADD", reply, &reply_len) < 0 || memcmp(reply, "FAIL", 4) == 0)
    {
        QMessageBox msg;
        msg.setIcon(QMessageBox::Warning);
...
void Peers::add_station(QString info)
{
    ...
    if (item)
    {
        ...
        if (model.rowCount() > 0)
        {
            QModelIndexList lst = model.match(model.index(0, 0), peer_role_address, lines[0]);
        ...
        item->setData(PEER_TYPE_ASSOCIATED_STATION, peer_role_type);
...
void Peers::add_stations()
{
    ...
    do
    {
        ...
        while (*txt != '\0' && *txt != '\n')
            txt++;
        *txt++ = '\0';
        if (strncmp(reply, "FAIL", 4) == 0 || strncmp(reply, "UNKNOWN", 7) == 0)
            break;
...
void Peers::add_p2p_group_client(QStandardItem * /*parent*/, QString params)
{
    ...
    QStringList items  = params.split(QRegExp(" (?=[^']*('[^']*'[^']*)*$)"));
    QString addr       = "";
    QString name       = "";
...
void Peers::remove_bss(int id)
{
    ...
    QModelIndexList lst = model.match(model.index(0, 0), peer_role_bss_id, id);
    if (lst.size() == 0)
        return;
    ...
    if (item)
    {
        ...
        for (QStringList::Iterator it = lines.begin(); it != lines.end(); it++)
        {
            if ((*it).startsWith("p2p_group_client:"))
                add_p2p_group_client(item, (*it).mid(18));
        }
    }
...
QStandardItem *Peers::find_addr(QString addr)
{
    ...
    QModelIndexList lst = model.match(model.index(0, 0), peer_role_address, addr);
    if (lst.size() == 0)
        return NULL;
...
QStandardItem *Peers::find_uuid(QString uuid)
{
    ...
    QModelIndexList lst = model.match(model.index(0, 0), peer_role_uuid, uuid);
    if (lst.size() == 0)
        return NULL;
    return model.itemFromIndex(lst[0]);
}
...
void Peers::event_notify(WpaMsg msg)
{
    ...
    if (text.startsWith(WPS_EVENT_PIN_NEEDED))
    {
        ...
        if (pos >= 0)
        {
            ...
            if (pos2 >= pos)
            {
                items = text.mid(pos + 1, pos2 - pos - 1).split('|');
                name  = items[0];
                items.append(addr);
            }
        }
        ...
        {
            item->setData(addr, peer_role_address);
            item->setData(PEER_TYPE_WPS_PIN_NEEDED, peer_role_type);
    ...
    if (text.startsWith(AP_STA_CONNECTED))
    {
        ...
        QStandardItem *item = find_addr(addr);
        if (item == NULL || item->data(peer_role_type).toInt() != PEER_TYPE_ASSOCIATED_STATION)
    ...
    if (text.startsWith(AP_STA_DISCONNECTED))
    {
        ...
        QModelIndexList lst = model.match(model.index(0, 0), peer_role_address, addr, -1);
        for (int i = 0; i < lst.size(); i++)
        {
            QStandardItem *item = model.itemFromIndex(lst[i]);
            if (item && item->data(peer_role_type).toInt() == PEER_TYPE_ASSOCIATED_STATION)
            {
                model.removeRow(lst[i].row());
                break;
            }
    ...
    if (text.startsWith(P2P_EVENT_DEVICE_FOUND))
    {
        ...
        item = find_addr_type(addr, PEER_TYPE_P2P_PERSISTENT_GROUP_CLIENT);
        if (item)
    ...
    if (text.startsWith(P2P_EVENT_GROUP_STARTED))
    {
        ...
        if (item)
        {
            ...
            if (items[2] == "GO")
            {
                details = tr("P2P GO for interface ") + items[1];
            }
            else
            {
                details = tr("P2P client for interface ") + items[1];
            }
    ...
    if (text.startsWith(P2P_EVENT_GROUP_REMOVED))
    {
        ...
        QModelIndexList lst = model.match(model.index(0, 0), peer_role_ifname, items[1]);
        for (int i = 0; i < lst.size(); i++)
            model.removeRow(lst[i].row());
        return;
    }
    ...
    if (text.startsWith(P2P_EVENT_PROV_DISC_SHOW_PIN))
    {
        ...
        if (item == NULL)
            return;
        item->setData(SEL_METHOD_PIN_LOCAL_DISPLAY, peer_role_selected_method);
        item->setData(pin, peer_role_selected_pin);
        QVariant var = item->data(peer_role_requested_method);
        if (var.isValid() && var.toInt() == SEL_METHOD_PIN_LOCAL_DISPLAY)
        {
            ctx_item = item;
    ...
    if (text.startsWith(P2P_EVENT_PROV_DISC_ENTER_PIN))
    {
        ...
        if (item == NULL)
            return;
        item->setData(SEL_METHOD_PIN_PEER_DISPLAY, peer_role_selected_method);
        QVariant var = item->data(peer_role_requested_method);
        if (var.isValid() && var.toInt() == SEL_METHOD_PIN_PEER_DISPLAY)
        {
            ctx_item = item;
            ctx_p2p_connect();
        }
    ...
    if (text.startsWith(P2P_EVENT_INVITATION_RECEIVED))
    {
        ...
        if (items.size() < 3)
            return;
        if (!items[1].startsWith("sa=") || !items[2].startsWith("persistent="))
            return;
    ...
    if (text.startsWith(WPS_EVENT_ER_AP_ADD))
    {
        ...
        if (item)
        {
            ...
            item->setData(addr, peer_role_address);
            int type = wps_state == 2 ? PEER_TYPE_WPS_ER_AP : PEER_TYPE_WPS_ER_AP_UNCONFIGURED;
            item->setData(type, peer_role_type);
            item->setToolTip(ItemType(type));
            item->setData(pri_dev_type, peer_role_pri_dev_type);
            item->setData(items.join(QString("\n")), peer_role_details);
    ...
    if (text.startsWith(WPS_EVENT_ER_AP_REMOVE))
    {
        ...
        QModelIndexList lst = model.match(model.index(0, 0), peer_role_uuid, items[1]);
        for (int i = 0; i < lst.size(); i++)
        {
            QStandardItem *item = model.itemFromIndex(lst[i]);
            if (item && (item->data(peer_role_type).toInt() == PEER_TYPE_WPS_ER_AP ||
                         item->data(peer_role_type).toInt() == PEER_TYPE_WPS_ER_AP_UNCONFIGURED))
                model.removeRow(lst[i].row());
        }
        return;
    }
    ...
    if (text.startsWith(WPS_EVENT_ER_ENROLLEE_ADD))
    {
        ...
        if (item)
        {
            item->setData(uuid, peer_role_uuid);
            item->setData(addr, peer_role_address);
            item->setData(PEER_TYPE_WPS_ER_ENROLLEE, peer_role_type);
    ...
    if (text.startsWith(WPS_EVENT_ENROLLEE_SEEN))
    {
        ...
        if (pos >= 0)
        {
            ...
            if (pos2 >= pos)
            {
                QStringList items2 = text.mid(pos + 1, pos2 - pos - 1).split('|');
                name               = items2[0];
            }
        }
        if (name.isEmpty())
        ...
        if (item)
        {
            ...
            if ((var.isValid() && config_methods != var.toInt()) || (var2.isValid() && dev_passwd_id != var2.toInt()))
                remove_enrollee_uuid(uuid);
            else
                return;
        }
        ...
        if (item)
        {
            item->setData(uuid, peer_role_uuid);
            item->setData(addr, peer_role_address);
            item->setData(PEER_TYPE_WPS_ENROLLEE, peer_role_type);
...
void Peers::ctx_p2p_connect()
{
    ...
    QString arg;
    int config_methods          = ctx_item->data(peer_role_config_methods).toInt();
    enum selected_method method = SEL_METHOD_NONE;
    ...
    if (method == SEL_METHOD_PIN_LOCAL_DISPLAY)
    {
        ...
        char reply[100];
        size_t reply_len;
        snprintf(cmd, sizeof(cmd), "P2P_CONNECT %s %s display", addr.toLocal8Bit().constData(),
        ...
        QMessageBox::information(this, tr("PIN for ") + ctx_item->text(),
    ...
    char reply[100];
    size_t reply_len;
    snprintf(cmd, sizeof(cmd), "P2P_CONNECT %s %s", addr.toLocal8Bit().constData(), arg.toLocal8Bit().constData());
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0)
    {
        QMessageBox msg;
...
void Peers::ctx_p2p_req_pin()
{
    if (ctx_item == NULL)
        return;
    QString addr = ctx_item->data(peer_role_address).toString();
    ctx_item->setData(SEL_METHOD_PIN_PEER_DISPLAY, peer_role_requested_method);

    char cmd[100];
    char reply[100];
    size_t reply_len;
    snprintf(cmd, sizeof(cmd), "P2P_PROV_DISC %s display", addr.toLocal8Bit().constData());
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0)
    {
        QMessageBox msg;
...
void Peers::ctx_p2p_show_pin()
{
    if (ctx_item == NULL)
        return;
    QString addr = ctx_item->data(peer_role_address).toString();
    ctx_item->setData(SEL_METHOD_PIN_LOCAL_DISPLAY, peer_role_requested_method);

    char cmd[100];
    char reply[100];
    size_t reply_len;
    snprintf(cmd, sizeof(cmd), "P2P_PROV_DISC %s keypad", addr.toLocal8Bit().constData());
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0)
    {
        QMessageBox msg;
...
void Peers::ctx_p2p_display_pin()
{
    ...
    char reply[100];
    size_t reply_len;
    snprintf(cmd, sizeof(cmd), "P2P_CONNECT %s pin", addr.toLocal8Bit().constData());
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0)
    {
        QMessageBox msg;
...
void Peers::ctx_p2p_enter_pin()
{
    ...
    char reply[100];
    size_t reply_len;
    snprintf(cmd, sizeof(cmd), "P2P_CONNECT %s %s keypad", addr.toLocal8Bit().constData(),
             arg.toLocal8Bit().constData());
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0)
    {
...
void Peers::ctx_p2p_remove_group()
{
    ...
    snprintf(cmd, sizeof(cmd), "P2P_GROUP_REMOVE %s",
             ctx_item->data(peer_role_ifname).toString().toLocal8Bit().constData());
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0)
    {
        QMessageBox msg;
...
void Peers::remove_enrollee_uuid(QString uuid)
{
    ...
    for (int i = 0; i < lst.size(); i++)
    {
        ...
        if (item == NULL)
            continue;
        int type = item->data(peer_role_type).toInt();
        if (type == PEER_TYPE_WPS_ER_ENROLLEE || type == PEER_TYPE_WPS_ENROLLEE)
            model.removeRow(lst[i].row());
    }
}
...
void Peers::properties()
{
    ...
    if (var.isValid())
        info += tr("Primary Device Type: ") + var.toString() + QString("\n");
    ...
    if (var.isValid())
    {
        enum selected_method method = (enum selected_method)var.toInt();
        switch (method)
        {
...
void Peers::connect_pbc()
{
    ...
    if (peer_type == PEER_TYPE_WPS_ER_ENROLLEE)
    {
        snprintf(cmd, sizeof(cmd), "WPS_ER_PBC %s",
                 ctx_item->data(peer_role_uuid).toString().toLocal8Bit().constData());
    }
    else if (peer_type == PEER_TYPE_P2P || peer_type == PEER_TYPE_P2P_CLIENT)
    {
        snprintf(cmd, sizeof(cmd), "P2P_CONNECT %s pbc",
                 ctx_item->data(peer_role_address).toString().toLocal8Bit().constData());
    }
    else
    {
...
void Peers::learn_ap_config()
{
    ...
    snprintf(cmd, sizeof(cmd), "WPS_ER_LEARN %s %s", uuid.toLocal8Bit().constData(),
             input.get_string().toLocal8Bit().constData());
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0)
    {
...
void Peers::ctx_hide_ap()
{
    ...
    {
        QModelIndexList lst;
        lst = model.match(model.index(0, 0), peer_role_type, PEER_TYPE_AP);
        if (lst.size() == 0)
        {
            lst = model.match(model.index(0, 0), peer_role_type, PEER_TYPE_AP_WPS);
            if (lst.size() == 0)
                break;
        }
...
void Peers::ctx_p2p_show_passphrase()
{
    ...
    reply_len = sizeof(reply) - 1;
    if (wpagui->ctrlRequest("P2P_GET_PASSPHRASE", reply, &reply_len) < 0 || memcmp(reply, "FAIL", 4) == 0)
    {
        QMessageBox msg;
        msg.setIcon(QMessageBox::Warning);
    ...
    {
        reply[reply_len] = '\0';
        QMessageBox::information(this, tr("Passphrase"), tr("P2P group passphrase:\n") + reply);
    }
}
...
void Peers::ctx_p2p_start_persistent()
{
    ...
    snprintf(cmd, sizeof(cmd), "P2P_GROUP_ADD persistent=%d", ctx_item->data(peer_role_network_id).toInt());
    if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0 || memcmp(reply, "FAIL", 4) == 0)
    {
        QMessageBox msg;
        msg.setIcon(QMessageBox::Warning);
    ...
    else if (ctx_item->data(peer_role_type).toInt() == PEER_TYPE_P2P_INVITATION)
...
void Peers::ctx_p2p_invite()
{
    ...
    snprintf(cmd, sizeof(cmd), "P2P_INVITE persistent=%d", ctx_item->data(peer_role_network_id).toInt());
    if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0 || memcmp(reply, "FAIL", 4) == 0)
    {
        QMessageBox msg;
        msg.setIcon(QMessageBox::Warning);
        msg.setText(
            tr("Failed to invite peer to start persistent "
               "P2P Group."));
        msg.exec();
    }
}
...
void Peers::enable_persistent(int id)
{
    ...
    QModelIndexList lst = model.match(model.index(0, 0), peer_role_network_id, id);
    for (int i = 0; i < lst.size(); i++)
    {
        QStandardItem *item = model.itemFromIndex(lst[i]);
        int type            = item->data(peer_role_type).toInt();
