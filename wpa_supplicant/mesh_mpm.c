...
static const char *const mplstate[] = {
    [0] = "UNINITIALIZED",         [PLINK_IDLE] = "IDLE",   [PLINK_OPN_SNT] = "OPN_SNT", [PLINK_OPN_RCVD] = "OPN_RCVD",
    [PLINK_CNF_RCVD] = "CNF_RCVD", [PLINK_ESTAB] = "ESTAB", [PLINK_HOLDING] = "HOLDING", [PLINK_BLOCKED] = "BLOCKED"};

static const char *const mplevent[] = {
    [PLINK_UNDEFINED] = "UNDEFINED", [OPN_ACPT] = "OPN_ACPT", [OPN_RJCT] = "OPN_RJCT", [CNF_ACPT] = "CNF_ACPT",
    [CNF_RJCT] = "CNF_RJCT",         [CLS_ACPT] = "CLS_ACPT", [REQ_RJCT] = "REQ_RJCT",
};

static int mesh_mpm_parse_peer_mgmt(
    struct wpa_supplicant *wpa_s, u8 action_field, const u8 *ie, size_t len, struct mesh_peer_mgmt_ie *mpm_ie)
{
    ...
    if ((action_field == PLINK_OPEN && len != 4) || (action_field == PLINK_CONFIRM && len != 6) ||
...
static u16 copy_supp_rates(struct wpa_supplicant *wpa_s, struct sta_info *sta, struct ieee802_11_elems *elems)
{
    if (!elems->supp_rates)
    {
        wpa_msg(wpa_s, MSG_ERROR, "no supported rates from " MACSTR, MAC2STR(sta->addr));
        return WLAN_STATUS_UNSPECIFIED_FAILURE;
    }

    if (elems->supp_rates_len + elems->ext_supp_rates_len > sizeof(sta->supported_rates))
    {
        wpa_msg(wpa_s, MSG_ERROR, "Invalid supported rates element length " MACSTR " %d+%d", MAC2STR(sta->addr),
                elems->supp_rates_len, elems->ext_supp_rates_len);
        return WLAN_STATUS_UNSPECIFIED_FAILURE;
    }

    sta->supported_rates_len =
        merge_byte_arrays(sta->supported_rates, sizeof(sta->supported_rates), elems->supp_rates, elems->supp_rates_len,
...
static bool matches_local(struct wpa_supplicant *wpa_s, struct ieee802_11_elems *elems)
{
    ...
    return (mconf->meshid_len == elems->mesh_id_len &&
            ...
            mconf->mesh_pp_id == elems->mesh_config[0] && mconf->mesh_pm_id == elems->mesh_config[1] &&
            mconf->mesh_cc_id == elems->mesh_config[2] && mconf->mesh_sp_id == elems->mesh_config[3] &&
...
static void mesh_mpm_init_link(struct wpa_supplicant *wpa_s, struct sta_info *sta)
{
    u16 llid;
                                       ...
                                       u16 close_reason)
{
    ...
    {
        buf_len += 3 + HE_MAX_MAC_CAPAB_SIZE + HE_MAX_PHY_CAPAB_SIZE + HE_MAX_MCS_CAPAB_SIZE + HE_MAX_PPET_CAPAB_SIZE;
        buf_len += 3 + sizeof(struct ieee80211_he_operation);
        if (is_6ghz_op_class(bss->iconf->op_class))
            buf_len += sizeof(struct ieee80211_he_6ghz_oper_info) + 3 + sizeof(struct ieee80211_he_6ghz_band_cap);
    }
    ...
    if (type != PLINK_CLOSE)
    {
         ...
         * plinks for now */
        wpabuf_put_u8(buf, MESH_CAP_ACCEPT_ADDITIONAL_PEER | (conf->mesh_fwding ? MESH_CAP_FORWARDING : 0));
    }
    else
    { /* Peer closing frame */
        /* IE: Mesh ID */
    ...
    if (type != PLINK_CLOSE && wpa_s->mesh_he_enabled)
    {
        u8 he_capa_oper[3 + HE_MAX_MAC_CAPAB_SIZE + HE_MAX_PHY_CAPAB_SIZE + HE_MAX_MCS_CAPAB_SIZE +
                        HE_MAX_PPET_CAPAB_SIZE + 3 + sizeof(struct ieee80211_he_operation) +
                        sizeof(struct ieee80211_he_6ghz_oper_info) + 3 + sizeof(struct ieee80211_he_6ghz_band_cap)];

        pos = hostapd_eid_he_capab(bss, he_capa_oper, IEEE80211_MODE_MESH);
    ...
    if (type != PLINK_CLOSE && conf->ocv)
    {
        ...
        {
            wpa_printf(MSG_WARNING, "Mesh MPM: Failed to get channel info for OCI element");
            goto fail;
        }
    ...
    wpa_msg(wpa_s, MSG_DEBUG, "Mesh MPM: Sending peering frame type %d to " MACSTR " (my_lid=0x%x peer_lid=0x%x)", type,
            MAC2STR(sta->addr), sta->my_lid, sta->peer_lid);
    ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, sta->addr, wpa_s->own_addr, wpa_s->own_addr,
...
void wpa_mesh_set_plink_state(struct wpa_supplicant *wpa_s, struct sta_info *sta, enum mesh_plink_state state)
{
    ...
    wpa_msg(wpa_s, MSG_DEBUG, "MPM set " MACSTR " from %s into %s", MAC2STR(sta->addr), mplstate[sta->plink_state],
            mplstate[state]);
    ...
    if (ret)
    {
        wpa_msg(wpa_s, MSG_ERROR, "Driver failed to set " MACSTR ": %d", MAC2STR(sta->addr), ret);
    }
}

static void mesh_mpm_fsm_restart(struct wpa_supplicant *wpa_s, struct sta_info *sta)
{
    struct hostapd_data *hapd = wpa_s->ifmsh->bss[0];
...
static void plink_timer(void *eloop_ctx, void *user_data)
{
    ...
    switch (sta->plink_state)
    {
        ...
        case PLINK_OPN_SNT:
            ...
            {
                eloop_register_timeout(conf->dot11MeshRetryTimeout / 1000, (conf->dot11MeshRetryTimeout % 1000) * 1000,
        ...
        case PLINK_CNF_RCVD:
            ...
            eloop_register_timeout(conf->dot11MeshHoldingTimeout / 1000, (conf->dot11MeshHoldingTimeout % 1000) * 1000,
                                   plink_timer, wpa_s, sta);
...
static void mesh_mpm_plink_open(struct wpa_supplicant *wpa_s, struct sta_info *sta, enum mesh_plink_state next_state)
{
    struct mesh_conf *conf = wpa_s->ifmsh->mconf;

    eloop_cancel_timeout(plink_timer, wpa_s, sta);
    eloop_register_timeout(conf->dot11MeshRetryTimeout / 1000, (conf->dot11MeshRetryTimeout % 1000) * 1000, plink_timer,
                           wpa_s, sta);
    mesh_mpm_send_plink_action(wpa_s, sta, PLINK_OPEN, 0);
    wpa_mesh_set_plink_state(wpa_s, sta, next_state);
}

static int mesh_mpm_plink_close(struct hostapd_data *hapd, struct sta_info *sta, void *ctx)
{
...
int mesh_mpm_connect_peer(struct wpa_supplicant *wpa_s, const u8 *addr, int duration)
{
    ...
    {
        wpa_msg(wpa_s, MSG_INFO, "This command is available only with no_auto_peer mesh network");
        return -1;
    }
    ...
    else
    {
        ...
        eloop_register_timeout(duration == -1 ? 10 : duration, 0, peer_add_timer, wpa_s, NULL);
    }
 ...
 */
static struct sta_info *mesh_mpm_add_peer(struct wpa_supplicant *wpa_s, const u8 *addr, struct ieee802_11_elems *elems)
{
    struct hostapd_sta_add_params params;
    struct mesh_conf *conf    = wpa_s->ifmsh->mconf;
    struct hostapd_data *data = wpa_s->ifmsh->bss[0];
    ...
    {
        wpa_msg(wpa_s, MSG_DEBUG, "mesh: Ignore a crowded peer " MACSTR, MAC2STR(addr));
        return NULL;
    }
    ...
    if (oper && !(oper->ht_param & HT_INFO_HT_PARAM_STA_CHNL_WIDTH) && sta->ht_capabilities)
    {
        wpa_msg(wpa_s, MSG_DEBUG, MACSTR " does not support 40 MHz bandwidth", MAC2STR(sta->addr));
...
#ifdef CONFIG_IEEE80211AX
    copy_sta_he_capab(data, sta, IEEE80211_MODE_MESH, elems->he_capabilities, elems->he_capabilities_len);
...
void wpa_mesh_new_mesh_peer(struct wpa_supplicant *wpa_s, const u8 *addr, struct ieee802_11_elems *elems)
{
    ...
    {
        wpa_msg(wpa_s, MSG_INFO, "will not initiate new peer link with " MACSTR " because of no_auto_peer",
                MAC2STR(addr));
        if (data->mesh_pending_auth)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "mesh: Process pending Authentication frame from %u.%06u seconds ago",
                           (unsigned int)age.sec, (unsigned int)age.usec);
                os_memset(&fi, 0, sizeof(fi));
                ieee802_11_mgmt(data, wpabuf_head(data->mesh_pending_auth), wpabuf_len(data->mesh_pending_auth), &fi);
            }
...
void mesh_mpm_mgmt_rx(struct wpa_supplicant *wpa_s, struct rx_mgmt *rx_mgmt)
{
    ...
    ieee802_11_mgmt(wpa_s->ifmsh->bss[0], rx_mgmt->frame, rx_mgmt->frame_len, &fi);
}

static void mesh_mpm_plink_estab(struct wpa_supplicant *wpa_s, struct sta_info *sta)
{
    struct hostapd_data *hapd = wpa_s->ifmsh->bss[0];
    struct mesh_conf *conf    = wpa_s->ifmsh->mconf;
    u8 seq[6]                 = {};

    wpa_msg(wpa_s, MSG_INFO, "mesh plink with " MACSTR " established", MAC2STR(sta->addr));

    if (conf->security & MESH_CONF_SEC_AMPE)
    {
        wpa_hexdump_key(MSG_DEBUG, "mesh: MTK", sta->mtk, sta->mtk_len);
        wpa_drv_set_key(wpa_s, wpa_cipher_to_alg(conf->pairwise_cipher), sta->addr, 0, 0, seq, sizeof(seq), sta->mtk,
                        sta->mtk_len, KEY_FLAG_PAIRWISE_RX_TX);

        wpa_hexdump_key(MSG_DEBUG, "mesh: RX MGTK Key RSC", sta->mgtk_rsc, sizeof(sta->mgtk_rsc));
        wpa_hexdump_key(MSG_DEBUG, "mesh: RX MGTK", sta->mgtk, sta->mgtk_len);
        wpa_drv_set_key(wpa_s, wpa_cipher_to_alg(conf->group_cipher), sta->addr, sta->mgtk_key_id, 0, sta->mgtk_rsc,
                        sizeof(sta->mgtk_rsc), sta->mgtk, sta->mgtk_len, KEY_FLAG_GROUP_RX);

        if (sta->igtk_len)
        {
            wpa_hexdump_key(MSG_DEBUG, "mesh: RX IGTK Key RSC", sta->igtk_rsc, sizeof(sta->igtk_rsc));
            wpa_hexdump_key(MSG_DEBUG, "mesh: RX IGTK", sta->igtk, sta->igtk_len);
            wpa_drv_set_key(wpa_s, wpa_cipher_to_alg(conf->mgmt_group_cipher), sta->addr, sta->igtk_key_id, 0,
                            sta->igtk_rsc, sizeof(sta->igtk_rsc), sta->igtk, sta->igtk_len, KEY_FLAG_GROUP_RX);
        }
    }
...
static void mesh_mpm_fsm(struct wpa_supplicant *wpa_s, struct sta_info *sta, enum plink_event event, u16 reason)
{
    struct hostapd_data *hapd = wpa_s->ifmsh->bss[0];
    struct mesh_conf *conf    = wpa_s->ifmsh->mconf;

    wpa_msg(wpa_s, MSG_DEBUG, "MPM " MACSTR " state %s event %s", MAC2STR(sta->addr), mplstate[sta->plink_state],
            mplevent[event]);
    ...
    switch (sta->plink_state)
    {
            ...
            switch (event)
            {
                ...
                case CLS_ACPT:
                    ...
                    eloop_register_timeout(conf->dot11MeshHoldingTimeout / 1000,
                                           (conf->dot11MeshHoldingTimeout % 1000) * 1000, plink_timer, wpa_s, sta);
                    mesh_mpm_send_plink_action(wpa_s, sta, PLINK_CLOSE, reason);
                    break;
                case OPN_ACPT:
                ...
                case CNF_ACPT:
                    ...
                    eloop_register_timeout(conf->dot11MeshConfirmTimeout / 1000,
                                           (conf->dot11MeshConfirmTimeout % 1000) * 1000, plink_timer, wpa_s, sta);
                    break;
                default:
                    break;
            }
        ...
        case PLINK_ESTAB:
            switch (event)
            {
                ...
                case CLS_ACPT:
                    ...
                    wpa_msg(wpa_s, MSG_INFO, "mesh plink with " MACSTR " closed with reason %d", MAC2STR(sta->addr),
                            reason);

                    wpa_msg(wpa_s, MSG_INFO, MESH_PEER_DISCONNECTED MACSTR, MAC2STR(sta->addr));
        ...
        case PLINK_HOLDING:
            ...
            break;
        default:
            wpa_msg(wpa_s, MSG_DEBUG, "Unsupported MPM event %s for state %s", mplevent[event],
...
void mesh_mpm_action_rx(struct wpa_supplicant *wpa_s, const struct ieee80211_mgmt *mgmt, size_t len)
{
    ...
    if (action_field != PLINK_OPEN && action_field != PLINK_CONFIRM && action_field != PLINK_CLOSE)
        return;
    ...
    if (ie_len < 2 + 2)
    {
        wpa_printf(MSG_DEBUG, "MPM: Ignore too short action frame %u ie_len %u", action_field, (unsigned int)ie_len);
        return;
    }
    ...
    if (action_field != PLINK_CLOSE)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "MPM: Mesh ID or Mesh Configuration element do not match local MBSS");
            return;
        }
    }

    ret = mesh_mpm_parse_peer_mgmt(wpa_s, action_field, elems.peer_mgmt, elems.peer_mgmt_len, &peer_mgmt_ie);
    if (ret)
    {
    ...
    if (action_field == PLINK_CLOSE)
        wpa_printf(MSG_DEBUG, "MPM: close reason=%u", WPA_GET_LE16(peer_mgmt_ie.reason));
    ...
    if (!sta && action_field == PLINK_OPEN &&
        (!(mconf->security & MESH_CONF_SEC_AMPE) || wpa_auth_pmksa_get(hapd->wpa_auth, mgmt->sa, NULL)))
    ...
    if (mconf->security & MESH_CONF_SEC_AMPE)
    {
        ...
        res = mesh_rsn_process_ampe(wpa_s, sta, &elems, &mgmt->u.action.category, peer_mgmt_ie.chosen_pmk, ies, ie_len);
        if (res)
        {
        ...
        if (action_field == PLINK_OPEN && elems.rsn_ie)
        {
            ...
            res = wpa_parse_wpa_ie_rsn(elems.rsn_ie - 2, elems.rsn_ie_len + 2, &data);
            if (res)
            {
                wpa_printf(MSG_DEBUG, "Failed to parse RSN IE (res=%d)", res);
                wpa_hexdump(MSG_DEBUG, "RSN IE", elems.rsn_ie, elems.rsn_ie_len);
                return;
            }

            wpa_auth_set_ocv(sm, mconf->ocv && (data.capabilities & WPA_CAPABILITY_OCVC));
        }

        if (action_field != PLINK_CLOSE && wpa_auth_uses_ocv(sta->wpa_sm))
        {
            struct wpa_channel_info ci;
            int tx_chanwidth;
            int tx_seg1_idx;
            ...
            if (wpa_drv_channel_info(wpa_s, &ci) != 0)
            {
                wpa_printf(MSG_WARNING, "MPM: Failed to get channel info to validate received OCI in MPM Confirm");
                return;
            }

            if (get_tx_parameters(sta, channel_width_to_int(ci.chanwidth), ci.seg1_idx, &tx_chanwidth, &tx_seg1_idx) <
                0)
                return;

            if (ocv_verify_tx_params(elems.oci, elems.oci_len, &ci, tx_chanwidth, tx_seg1_idx) != OCI_SUCCESS)
            {
                wpa_printf(MSG_WARNING, "MPM: OCV failed: %s", ocv_errorstr);
                return;
            }
        }
#endif /* CONFIG_OCV */
    ...
    switch (action_field)
    {
        case PLINK_OPEN:
            if (plink_free_count(hapd) == 0)
            {
                ...
                wpa_printf(MSG_INFO, "MPM: Peer link num over quota(%d)", hapd->max_plinks);
            }
            else if (sta->peer_lid && sta->peer_lid != plid)
            {
                wpa_printf(MSG_DEBUG, "MPM: peer_lid mismatch: 0x%x != 0x%x", sta->peer_lid, plid);
                return; /* no FSM event */
            }
            else
            {
        ...
        case PLINK_CONFIRM:
            ...
            else if (sta->my_lid != llid || (sta->peer_lid && sta->peer_lid != plid))
            {
                wpa_printf(MSG_DEBUG, "MPM: lid mismatch: my_lid: 0x%x != 0x%x or peer_lid: 0x%x != 0x%x", sta->my_lid,
                           llid, sta->peer_lid, plid);
                return; /* no FSM event */
            }
            else
            {
        ...
        case PLINK_CLOSE:
            ...
            {
                wpa_printf(MSG_DEBUG, "MPM: my_lid mismatch: 0x%x != 0x%x", sta->my_lid, llid);
                return; /* no FSM event */
            }
            else
            {
