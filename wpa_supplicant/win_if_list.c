...
static HANDLE ndisuio_open(void)
{
    ...
    h = CreateFile(TEXT("\\\\.\\\\Ndisuio"), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
                   FILE_ATTRIBUTE_NORMAL, INVALID_HANDLE_VALUE);
    if (h == INVALID_HANDLE_VALUE)
        return h;

#ifndef _WIN32_WCE
    if (!DeviceIoControl(h, IOCTL_NDISUIO_BIND_WAIT, NULL, 0, NULL, 0, &written, NULL))
    {
...
static void ndisuio_query_bindings(HANDLE ndisuio)
{
    ...
    for (i = 0;; i++)
    {
        memset(b, 0, blen);
        b->BindingIndex = i;
        if (!DeviceIoControl(ndisuio, IOCTL_NDISUIO_QUERY_BINDING, b, sizeof(NDISUIO_QUERY_BINDING), b, (DWORD)blen,
                             &written, NULL))
        {
