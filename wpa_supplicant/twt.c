                        ...
                        u8 control)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "TWT: No connection - cannot send TWT Setup frame");
        return -ENOTCONN;
    }
    ...
    if (!buf)
    {
        wpa_printf(MSG_DEBUG, "TWT: Failed to allocate TWT Setup frame (Request)");
        return -ENOMEM;
    }

    wpa_printf(MSG_DEBUG, "TWT: Setup request, dtok: %d  exponent: %d  mantissa: %d  min-twt: %d", dtok, exponent,
               mantissa, min_twt);
...
int wpas_twt_send_teardown(struct wpa_supplicant *wpa_s, u8 flags)
{
    ...
    if (wpa_s->wpa_state != WPA_COMPLETED || !wpa_s->current_ssid)
    {
        wpa_printf(MSG_DEBUG, "TWT: No connection - cannot send TWT Teardown frame");
        return -ENOTCONN;
    }
    ...
    if (!buf)
    {
        wpa_printf(MSG_DEBUG, "TWT: Failed to allocate TWT Teardown frame");
        return -ENOMEM;
    }
