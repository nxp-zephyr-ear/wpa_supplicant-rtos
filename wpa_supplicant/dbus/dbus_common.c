...
static dbus_bool_t add_watch(DBusWatch *watch, void *data)
{
    ...
    if (eloop_register_sock(fd, EVENT_TYPE_EXCEPTION, process_watch_exception, priv, watch) < 0)
        return FALSE;

    if ((flags & DBUS_WATCH_READABLE) && eloop_register_sock(fd, EVENT_TYPE_READ, process_watch_read, priv, watch) < 0)
        return FALSE;
    if ((flags & DBUS_WATCH_WRITABLE) &&
        eloop_register_sock(fd, EVENT_TYPE_WRITE, process_watch_write, priv, watch) < 0)
        return FALSE;
...
static dbus_bool_t add_timeout(DBusTimeout *timeout, void *data)
{
    ...
    eloop_register_timeout(0, dbus_timeout_get_interval(timeout) * 1000, process_timeout, priv, timeout);
...
static void process_wakeup_main(int sig, void *signal_ctx)
{
    ...
    if (dbus_connection_get_dispatch_status(priv->con) != DBUS_DISPATCH_DATA_REMAINS)
        return;
...
{
    if (!dbus_connection_set_watch_functions(priv->con, add_watch, remove_watch, watch_toggled, priv, NULL) ||
        !dbus_connection_set_timeout_functions(priv->con, add_timeout, remove_timeout, timeout_toggled, priv, NULL))
    {
...
static DBusHandlerResult disconnect_filter(DBusConnection *conn, DBusMessage *message, void *data)
{
    struct wpas_dbus_priv *priv = data;
...
static int wpas_dbus_init_common(struct wpas_dbus_priv *priv)
{
    ...
    {
        wpa_printf(MSG_ERROR, "dbus: Could not acquire the system bus: %s - %s", error.name, error.message);
        ret = -1;
    }
...
static void wpas_dbus_deinit_common(struct wpas_dbus_priv *priv)
{
    if (priv->con)
    {
        ...
        dbus_connection_set_watch_functions(priv->con, NULL, NULL, NULL, NULL, NULL);
        dbus_connection_set_timeout_functions(priv->con, NULL, NULL, NULL, NULL, NULL);
