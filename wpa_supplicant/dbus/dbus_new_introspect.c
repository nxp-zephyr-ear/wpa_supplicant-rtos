...
static void add_entry(
    struct wpabuf *xml, const char *type, const char *name, const struct wpa_dbus_argument *args, int include_dir)
{
    ...
    {
        add_arg(xml, arg->name, arg->type, include_dir ? (arg->dir == ARG_IN ? "in" : "out") : NULL);
    }
...
static void add_property(struct wpabuf *xml, const struct wpa_dbus_property_desc *dsc)
{
    wpabuf_printf(xml,
                  ...
                  "access=\"%s%s\"/>",
                  dsc->dbus_property, dsc->type, dsc->getter ? "read" : "", dsc->setter ? "write" : "");
}
...
static void add_interfaces(struct dl_list *list, struct wpabuf *xml)
{
    ...
    dl_list_for_each_safe(iface, n, list, struct interfaces, list)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "dbus: Not enough room for add_interfaces inspect data: tailroom %u, add %u",
                       (unsigned int)wpabuf_tailroom(xml), (unsigned int)wpabuf_len(iface->xml));
        }
...
static void add_introspectable_interface(struct wpabuf *xml)
{
    wpabuf_printf(xml,
                  ...
                  "</method>"
                  "</interface>",
                  WPA_DBUS_INTROSPECTION_INTERFACE, WPA_DBUS_INTROSPECTION_METHOD);
}
...
DBusMessage *wpa_dbus_introspect(DBusMessage *message, struct wpa_dbus_object_desc *obj_dsc)
{
    ...
    if (reply)
    {
        ...
        dbus_message_append_args(reply, DBUS_TYPE_STRING, &intro_str, DBUS_TYPE_INVALID);
    }
    wpabuf_free(xml);
