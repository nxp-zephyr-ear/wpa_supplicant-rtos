...
dbus_bool_t wpa_dbus_dict_open_write(DBusMessageIter *iter, DBusMessageIter *iter_dict)
{
    ...
    result = dbus_message_iter_open_container(iter, DBUS_TYPE_ARRAY,
                                                  ...
                                                  const int value_type)
{
    ...
    return dbus_message_iter_append_basic(iter_dict_entry, DBUS_TYPE_STRING, &key);
}
                                                  ...
                                                  const void *value)
{
    ...
    if (!_wpa_dbus_add_dict_entry_start(iter_dict, &iter_dict_entry, key, value_type) ||
        !dbus_message_iter_open_container(&iter_dict_entry, DBUS_TYPE_VARIANT, type_as_string, &iter_dict_val) ||
        !dbus_message_iter_append_basic(&iter_dict_val, value_type, value))
        return FALSE;

    return _wpa_dbus_add_dict_entry_end(iter_dict, &iter_dict_entry, &iter_dict_val);
}
                                                       ...
                                                       const dbus_uint32_t value_len)
{
    ...
    if (!_wpa_dbus_add_dict_entry_start(iter_dict, &iter_dict_entry, key, DBUS_TYPE_ARRAY) ||
        ...
        !dbus_message_iter_open_container(&iter_dict_val, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE_AS_STRING, &iter_array))
        return FALSE;
...
dbus_bool_t wpa_dbus_dict_append_string(DBusMessageIter *iter_dict, const char *key, const char *value)
{
    if (!value)
        return FALSE;
    return _wpa_dbus_add_dict_entry_basic(iter_dict, key, DBUS_TYPE_STRING, &value);
}
...
{
    return _wpa_dbus_add_dict_entry_basic(iter_dict, key, DBUS_TYPE_BOOLEAN, &value);
}
...
{
    return _wpa_dbus_add_dict_entry_basic(iter_dict, key, DBUS_TYPE_INT16, &value);
}
...
{
    return _wpa_dbus_add_dict_entry_basic(iter_dict, key, DBUS_TYPE_UINT16, &value);
}
...
{
    return _wpa_dbus_add_dict_entry_basic(iter_dict, key, DBUS_TYPE_INT32, &value);
}
...
{
    return _wpa_dbus_add_dict_entry_basic(iter_dict, key, DBUS_TYPE_UINT32, &value);
}
...
dbus_bool_t wpa_dbus_dict_append_object_path(DBusMessageIter *iter_dict, const char *key, const char *value)
{
    if (!value)
        return FALSE;
    return _wpa_dbus_add_dict_entry_basic(iter_dict, key, DBUS_TYPE_OBJECT_PATH, &value);
}
                                      ...
                                      DBusMessageIter *iter_array)
{
    ...
    err = os_snprintf(array_type, sizeof(array_type), DBUS_TYPE_ARRAY_AS_STRING "%s", type);
    if (os_snprintf_error(sizeof(array_type), err))
        return FALSE;

    if (!iter_dict || !iter_dict_entry || !iter_dict_val || !iter_array ||
        !_wpa_dbus_add_dict_entry_start(iter_dict, iter_dict_entry, key, DBUS_TYPE_ARRAY) ||
        !dbus_message_iter_open_container(iter_dict_entry, DBUS_TYPE_VARIANT, array_type, iter_dict_val))
        return FALSE;

    return dbus_message_iter_open_container(iter_dict_val, DBUS_TYPE_ARRAY, type, iter_array);
}
...
dbus_bool_t wpa_dbus_dict_begin_string_array(DBusMessageIter *iter_dict,
                                             ...
                                             DBusMessageIter *iter_array)
{
    return wpa_dbus_dict_begin_array(iter_dict, key, DBUS_TYPE_STRING_AS_STRING, iter_dict_entry, iter_dict_val,
                                     iter_array);
}
...
dbus_bool_t wpa_dbus_dict_bin_array_add_element(DBusMessageIter *iter_array, const u8 *value, size_t value_len)
{
    ...
    if (!iter_array || !value ||
        !dbus_message_iter_open_container(iter_array, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE_AS_STRING, &iter_bytes))
        return FALSE;

    for (i = 0; i < value_len; i++)
    {
        if (!dbus_message_iter_append_basic(&iter_bytes, DBUS_TYPE_BYTE, &(value[i])))
            return FALSE;
    }
                                              ...
                                              const dbus_uint32_t num_items)
{
    ...
    if (!key || (!items && num_items != 0) ||
        !wpa_dbus_dict_begin_string_array(iter_dict, key, &iter_dict_entry, &iter_dict_val, &iter_array))
        return FALSE;
    ...
    return wpa_dbus_dict_end_string_array(iter_dict, &iter_dict_entry, &iter_dict_val, &iter_array);
}
                                              ...
                                              const dbus_uint32_t num_items)
{
    ...
    if (!key || (!items && num_items != 0) ||
        !wpa_dbus_dict_begin_array(iter_dict, key, DBUS_TYPE_ARRAY_AS_STRING DBUS_TYPE_BYTE_AS_STRING, &iter_dict_entry,
                                   &iter_dict_val, &iter_array))
        return FALSE;

    for (i = 0; i < num_items; i++)
    {
        if (!wpa_dbus_dict_bin_array_add_element(&iter_array, wpabuf_head(items[i]), wpabuf_len(items[i])))
            return FALSE;
    }

    return wpa_dbus_dict_end_array(iter_dict, &iter_dict_entry, &iter_dict_val, &iter_array);
}
...
dbus_bool_t wpa_dbus_dict_open_read(DBusMessageIter *iter, DBusMessageIter *iter_dict, DBusError *error)
{
    ...
    if (!iter || !iter_dict)
    {
        dbus_set_error_const(error, DBUS_ERROR_FAILED, "[internal] missing message iterators");
        return FALSE;
    }

    type = dbus_message_iter_get_arg_type(iter);
    if (type != DBUS_TYPE_ARRAY || dbus_message_iter_get_element_type(iter) != DBUS_TYPE_DICT_ENTRY)
    {
        wpa_printf(MSG_DEBUG, "%s: unexpected message argument types (arg=%c element=%c)", __func__, type,
                   type != DBUS_TYPE_ARRAY ? '?' : dbus_message_iter_get_element_type(iter));
        dbus_set_error_const(error, DBUS_ERROR_INVALID_ARGS, "unexpected message argument types");
        return FALSE;
    }
...
static dbus_bool_t _wpa_dbus_dict_entry_get_byte_array(DBusMessageIter *iter, struct wpa_dbus_dict_entry *entry)
{
    ...
    while (dbus_message_iter_get_arg_type(iter) == DBUS_TYPE_BYTE)
    {
        ...
        {
            nbuffer = os_realloc_array(buffer, count + BYTE_ARRAY_CHUNK_SIZE, BYTE_ARRAY_ITEM_SIZE);
            if (nbuffer == NULL)
            {
                os_free(buffer);
                wpa_printf(MSG_ERROR, "dbus: %s out of memory trying to retrieve the string array", __func__);
                goto done;
            }
            buffer = nbuffer;
        }
    ...
    wpa_hexdump_key(MSG_MSGDUMP, "dbus: byte array contents", entry->bytearray_value, entry->array_len);
                                                         ...
                                                         struct wpa_dbus_dict_entry *entry)
{
    ...
    while (dbus_message_iter_get_arg_type(iter) == DBUS_TYPE_STRING)
    {
        ...
        {
            nbuffer = os_realloc_array(buffer, count + STR_ARRAY_CHUNK_SIZE, STR_ARRAY_ITEM_SIZE);
            if (nbuffer == NULL)
            {
                wpa_printf(MSG_ERROR, "dbus: %s out of memory trying to retrieve the string array", __func__);
                goto fail;
        ...
        wpa_printf(MSG_MSGDUMP, "%s: string_array value: %s", __func__, wpa_debug_show_keys ? value : "[omitted]");
        str = os_strdup(value);
        if (str == NULL)
        {
            wpa_printf(MSG_ERROR, "dbus: %s out of memory trying to duplicate the string array", __func__);
            goto fail;
        }
        buffer[count++] = str;
        dbus_message_iter_next(iter);
    ...
    entry->array_len      = count;
    wpa_printf(MSG_MSGDUMP, "%s: string_array length %u", __func__, entry->array_len);
...
static dbus_bool_t _wpa_dbus_dict_entry_get_binarray(DBusMessageIter *iter, struct wpa_dbus_dict_entry *entry)
{
    ...
    while (dbus_message_iter_get_arg_type(iter) == DBUS_TYPE_ARRAY)
    {
        ...
        if (entry->array_len == buflen)
        {
            ...
            newbuf = os_realloc_array(entry->binarray_value, buflen, BIN_ARRAY_ITEM_SIZE);
            if (!newbuf)
                goto cleanup;
        ...
        entry->binarray_value[entry->array_len] =
            wpabuf_alloc_ext_data((u8 *)tmpentry.bytearray_value, tmpentry.array_len);
...
static dbus_bool_t _wpa_dbus_dict_entry_get_array(DBusMessageIter *iter_dict_val, struct wpa_dbus_dict_entry *entry)
{
    ...
    switch (array_type)
    {
        case DBUS_TYPE_BYTE:
            ...
            break;
        case DBUS_TYPE_STRING:
            success = _wpa_dbus_dict_entry_get_string_array(&iter_array, array_type, entry);
            break;
        case DBUS_TYPE_ARRAY:
            success = _wpa_dbus_dict_entry_get_binarray(&iter_array, entry);
            break;
        default:
            wpa_printf(MSG_MSGDUMP, "%s: unsupported array type %c", __func__, array_type);
            break;
    }
...
static dbus_bool_t _wpa_dbus_dict_fill_value_from_variant(struct wpa_dbus_dict_entry *entry, DBusMessageIter *iter)
{
    ...
    switch (entry->type)
    {
        ...
        case DBUS_TYPE_STRING:
            dbus_message_iter_get_basic(iter, &v);
            wpa_printf(MSG_MSGDUMP, "%s: string value: %s", __func__, wpa_debug_show_keys ? v : "[omitted]");
            entry->str_value = os_strdup(v);
            if (entry->str_value == NULL)
                return FALSE;
            break;
        case DBUS_TYPE_BOOLEAN:
            ...
            wpa_printf(MSG_MSGDUMP, "%s: boolean value: %d", __func__, entry->bool_value);
            break;
        case DBUS_TYPE_BYTE:
        ...
        case DBUS_TYPE_INT16:
            ...
            wpa_printf(MSG_MSGDUMP, "%s: int16 value: %d", __func__, entry->int16_value);
            break;
        case DBUS_TYPE_UINT16:
        ...
        case DBUS_TYPE_INT32:
            ...
            wpa_printf(MSG_MSGDUMP, "%s: int32 value: %d", __func__, entry->int32_value);
            break;
        case DBUS_TYPE_UINT32:
            dbus_message_iter_get_basic(iter, &entry->uint32_value);
            wpa_printf(MSG_MSGDUMP, "%s: uint32 value: %d", __func__, entry->uint32_value);
            break;
        case DBUS_TYPE_INT64:
            dbus_message_iter_get_basic(iter, &entry->int64_value);
            wpa_printf(MSG_MSGDUMP, "%s: int64 value: %lld", __func__, (long long int)entry->int64_value);
            break;
        case DBUS_TYPE_UINT64:
            dbus_message_iter_get_basic(iter, &entry->uint64_value);
            wpa_printf(MSG_MSGDUMP, "%s: uint64 value: %llu", __func__, (unsigned long long int)entry->uint64_value);
            break;
        case DBUS_TYPE_DOUBLE:
            dbus_message_iter_get_basic(iter, &entry->double_value);
            wpa_printf(MSG_MSGDUMP, "%s: double value: %f", __func__, entry->double_value);
            break;
        case DBUS_TYPE_ARRAY:
...
dbus_bool_t wpa_dbus_dict_get_entry(DBusMessageIter *iter_dict, struct wpa_dbus_dict_entry *entry)
{
    ...
    if (!iter_dict || !entry || dbus_message_iter_get_arg_type(iter_dict) != DBUS_TYPE_DICT_ENTRY)
    {
    ...
    {
        wpa_printf(MSG_DEBUG, "%s: unexpected dict entry variant type: %c", __func__, type);
        goto error;
    }

    dbus_message_iter_recurse(&iter_dict_entry, &iter_dict_val);
    entry->type = dbus_message_iter_get_arg_type(&iter_dict_val);
    wpa_printf(MSG_MSGDUMP, "%s: dict entry variant content type: %c", __func__, entry->type);
    entry->array_type = DBUS_TYPE_INVALID;
    if (!_wpa_dbus_dict_fill_value_from_variant(entry, &iter_dict_val))
    {
        wpa_printf(MSG_DEBUG, "%s: failed to fetch dict values from variant", __func__);
        goto error;
    }
