                                             ...
                                             DBusError *error)
{
    ...
    for (dsc = props; dsc && dsc->dbus_property; dsc++)
    {
        ...
        if (os_strncmp(dsc->dbus_interface, interface, WPAS_DBUS_INTERFACE_MAX) != 0)
            continue;
        ...
        if (!dbus_message_iter_open_container(dict_iter, DBUS_TYPE_DICT_ENTRY, NULL, &entry_iter) ||
            !dbus_message_iter_append_basic(&entry_iter, DBUS_TYPE_STRING, &dsc->dbus_property))
            goto error;

        /* An error getting a property fails the request entirely */
        if (!dsc->getter(dsc, &entry_iter, error, user_data))
        {
            wpa_printf(MSG_INFO, "dbus: %s dbus_interface=%s dbus_property=%s getter failed", __func__,
                       dsc->dbus_interface, dsc->dbus_property);
            return FALSE;
        }
...
static DBusMessage *get_all_properties(DBusMessage *message, char *interface, struct wpa_dbus_object_desc *obj_dsc)
{
    ...
    if (!fill_dict_with_properties(&dict_iter, obj_dsc->properties, interface, obj_dsc->user_data, &error))
    {
...
static DBusMessage *properties_get(DBusMessage *message, const struct wpa_dbus_property_desc *dsc, void *user_data)
{
    ...
    if (dsc->getter == NULL)
    {
        return dbus_message_new_error(message, DBUS_ERROR_INVALID_ARGS, "Property is write-only");
    }
    ...
    {
        dbus_message_unref(reply);
        reply = wpas_dbus_reply_new_from_error(message, &error, DBUS_ERROR_FAILED, "Failed to read property");
        dbus_error_free(&error);
    }
...
static DBusMessage *properties_set(DBusMessage *message, const struct wpa_dbus_property_desc *dsc, void *user_data)
{
    ...
    if (dsc->setter == NULL)
    {
        return dbus_message_new_error(message, DBUS_ERROR_INVALID_ARGS, "Property is read-only");
    }
    ...
    {
        reply = wpas_dbus_reply_new_from_error(message, &error, DBUS_ERROR_FAILED, "Failed to set property");
        dbus_error_free(&error);
    }
                                          ...
                                          struct wpa_dbus_object_desc *obj_dsc)
{
    ...
    while (property_dsc && property_dsc->dbus_property)
    {
         ...
         * interfaces */
        if (!os_strncmp(property_dsc->dbus_property, property, WPAS_DBUS_METHOD_SIGNAL_PROP_MAX) &&
            !os_strncmp(property_dsc->dbus_interface, interface, WPAS_DBUS_INTERFACE_MAX))
            break;
    ...
    {
        wpa_printf(MSG_DEBUG, "no property handler for %s.%s on %s", interface, property,
                   dbus_message_get_path(message));
        return dbus_message_new_error(message, DBUS_ERROR_INVALID_ARGS, "No such property");
    }

    if (os_strncmp(WPA_DBUS_PROPERTIES_GET, method, WPAS_DBUS_METHOD_SIGNAL_PROP_MAX) == 0)
    {
...
static DBusMessage *properties_handler(DBusMessage *message, struct wpa_dbus_object_desc *obj_dsc)
{
    ...
    if (!os_strncmp(WPA_DBUS_PROPERTIES_GET, method, WPAS_DBUS_METHOD_SIGNAL_PROP_MAX) ||
        !os_strncmp(WPA_DBUS_PROPERTIES_SET, method, WPAS_DBUS_METHOD_SIGNAL_PROP_MAX) ||
        !os_strncmp(WPA_DBUS_PROPERTIES_GETALL, method, WPAS_DBUS_METHOD_SIGNAL_PROP_MAX))
    {
        ...
        if (!os_strncmp(WPA_DBUS_PROPERTIES_GETALL, method, WPAS_DBUS_METHOD_SIGNAL_PROP_MAX))
        {
            /* GetAll */
...
static DBusMessage *msg_method_handler(DBusMessage *message, struct wpa_dbus_object_desc *obj_dsc)
{
    ...
    while (method_dsc && method_dsc->dbus_method)
    {
        ...
        if (!os_strncmp(method_dsc->dbus_method, method, WPAS_DBUS_METHOD_SIGNAL_PROP_MAX) &&
            !os_strncmp(method_dsc->dbus_interface, msg_interface, WPAS_DBUS_INTERFACE_MAX))
            break;
    ...
    {
        wpa_printf(MSG_DEBUG, "no method handler for %s.%s on %s", msg_interface, method,
                   dbus_message_get_path(message));
        return dbus_message_new_error(message, DBUS_ERROR_UNKNOWN_METHOD, NULL);
    }
...
static DBusHandlerResult message_handler(DBusConnection *connection, DBusMessage *message, void *user_data)
{
    ...
    wpa_printf(MSG_MSGDUMP, "dbus: %s.%s (%s) [%s]", msg_interface, method, path, dbus_message_get_signature(message));

    /* if message is introspection method call */
    if (!os_strncmp(WPA_DBUS_INTROSPECTION_METHOD, method, WPAS_DBUS_METHOD_SIGNAL_PROP_MAX) &&
        !os_strncmp(WPA_DBUS_INTROSPECTION_INTERFACE, msg_interface, WPAS_DBUS_INTERFACE_MAX))
    {
    ...
    else if (!os_strncmp(WPA_DBUS_PROPERTIES_INTERFACE, msg_interface, WPAS_DBUS_INTERFACE_MAX))
    {
                             ...
                             struct wpa_dbus_object_desc *obj_desc)
{
    DBusError error;
    int ret                         = -1;
    DBusObjectPathVTable wpa_vtable = {&free_dbus_object_desc_cb, &message_handler, NULL, NULL, NULL, NULL};

    obj_desc->connection = iface->con;
    obj_desc->path       = os_strdup(dbus_path);

    /* Register the message handler for the global dbus interface */
    if (!dbus_connection_register_object_path(iface->con, dbus_path, &wpa_vtable, obj_desc))
    {
    ...
    switch (dbus_bus_request_name(iface->con, dbus_service, 0, &error))
    {
        ...
        case DBUS_REQUEST_NAME_REPLY_ALREADY_OWNER:
            wpa_printf(MSG_ERROR, "dbus: Could not request service name: already registered");
            break;
        default:
            wpa_printf(MSG_ERROR, "dbus: Could not request service name: %s %s", error.name, error.message);
            break;
    }
    dbus_error_free(&error);
                                       ...
                                       struct wpa_dbus_object_desc *obj_desc)
{
    DBusConnection *con;
    DBusError error;
    DBusObjectPathVTable vtable = {&free_dbus_object_desc_cb, &message_handler, NULL, NULL, NULL, NULL};
    ...
    if (!dbus_connection_try_register_object_path(con, path, &vtable, obj_desc, &error))
    {
...
int wpa_dbus_unregister_object_per_iface(struct wpas_dbus_priv *ctrl_iface, const char *path)
{
    ...
    if (!obj_desc)
    {
        wpa_printf(MSG_ERROR, "dbus: %s: Could not obtain object's private data: %s", __func__, path);
        return 0;
    }
...
static dbus_bool_t put_changed_properties(const struct wpa_dbus_object_desc *obj_dsc,
                                          const char *interface,
                                          ...
                                          int clear_changed)
{
    ...
    for (dsc = obj_dsc->properties, i = 0; dsc && dsc->dbus_property; dsc++, i++)
    {
        ...
        if (!dsc->getter(dsc, &entry_iter, &error, obj_dsc->user_data))
        {
            ...
            {
                wpa_printf(MSG_ERROR, "dbus: %s: Cannot get new value of property %s: (%s) %s", __func__,
                           dsc->dbus_property, error.name, error.message);
            }
            else
            {
                wpa_printf(MSG_ERROR, "dbus: %s: Cannot get new value of property %s", __func__, dsc->dbus_property);
            }
            dbus_error_free(&error);
            return FALSE;
        }
...
static void do_send_prop_changed_signal(DBusConnection *con,
                                        const char *path,
                                        const char *interface,
                                        const struct wpa_dbus_object_desc *obj_dsc)
{
    ...
    msg = dbus_message_new_signal(path, DBUS_INTERFACE_PROPERTIES, "PropertiesChanged");
    if (msg == NULL)
        return;

    dbus_message_iter_init_append(msg, &signal_iter);

    if (!dbus_message_iter_append_basic(&signal_iter, DBUS_TYPE_STRING, &interface) ||
        /* Changed properties dict */
        !dbus_message_iter_open_container(&signal_iter, DBUS_TYPE_ARRAY, "{sv}", &dict_iter) ||
...
static void do_send_deprecated_prop_changed_signal(DBusConnection *con,
                                                   const char *path,
                                                   const char *interface,
                                                   const struct wpa_dbus_object_desc *obj_dsc)
{
    ...
    if (!dbus_message_iter_open_container(&signal_iter, DBUS_TYPE_ARRAY, "{sv}", &dict_iter) ||
...
static void flush_object_timeout_handler(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_printf(MSG_MSGDUMP, "dbus: %s: Timeout - sending changed properties of object %s", __func__, obj_desc->path);
...
static void recursive_flush_changed_properties(DBusConnection *con, const char *path)
{
    ...
    for (i = 0; objects[i]; i++)
    {
        os_snprintf(subobj_path, WPAS_DBUS_OBJECT_PATH_MAX, "%s/%s", path, objects[i]);
...
void wpa_dbus_flush_object_changed_properties(DBusConnection *con, const char *path)
{
    ...
    for (dsc = obj_desc->properties, i = 0; dsc && dsc->dbus_property; dsc++, i++)
    {
        if (obj_desc->prop_changed_flags == NULL || !obj_desc->prop_changed_flags[i])
            continue;
                                    ...
                                    const char *property)
{
    ...
    if (!obj_desc)
    {
        wpa_printf(MSG_ERROR, "dbus: wpa_dbus_property_changed: could not obtain object's private data: %s", path);
        return;
    }

    for (dsc = obj_desc->properties; dsc && dsc->dbus_property; dsc++, i++)
        if (os_strcmp(property, dsc->dbus_property) == 0 && os_strcmp(interface, dsc->dbus_interface) == 0)
        {
    ...
    {
        wpa_printf(MSG_ERROR, "dbus: wpa_dbus_property_changed: no property %s in object %s", property, path);
        return;
    }

    if (!eloop_is_timeout_registered(flush_object_timeout_handler, iface->con, obj_desc))
    {
        eloop_register_timeout(0, WPA_DBUS_SEND_PROP_CHANGED_TIMEOUT, flush_object_timeout_handler, iface->con,
                               obj_desc);
    }
}
                                           ...
                                           DBusMessageIter *iter)
{
    ...
    {
        wpa_printf(MSG_ERROR, "dbus: %s: failed to get object properties: (%s) %s", __func__,
                   dbus_error_is_set(&error) ? error.name : "none", dbus_error_is_set(&error) ? error.message : "none");
        dbus_error_free(&error);
        wpa_dbus_dict_close_write(iter, &dict_iter);
        return FALSE;
    }
...
{
    const unsigned int dev_path_prefix_len = os_strlen(WPAS_DBUS_NEW_PATH_INTERFACES "/");
    char *obj_path_only;
    char *pos;
    size_t sep_len;
    ...
    *item = NULL;
    ...
    if (os_strncmp(path, WPAS_DBUS_NEW_PATH_INTERFACES "/", dev_path_prefix_len) != 0)
