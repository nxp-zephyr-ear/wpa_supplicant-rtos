                                       ...
                                       DBusMessage **reply)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Wrong Bssid type, byte array required");
        *reply = wpas_dbus_error_invalid_args(message, "Bssid must be a byte array");
        return -1;
    }
                                              ...
                                              DBusMessage **reply)
{
    ...
    if (dbus_message_iter_get_arg_type(&variant_iter) != DBUS_TYPE_ARRAY ||
        dbus_message_iter_get_element_type(&variant_iter) != DBUS_TYPE_BYTE)
    {
        wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Wrong P2PDeviceAddress type, byte array required");
        *reply = wpas_dbus_error_invalid_args(message, "P2PDeviceAddress must be a byte array");
        return -1;
    }
    dbus_message_iter_recurse(&variant_iter, &array_iter);
    ...
    if (len != ETH_ALEN)
    {
        wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Wrong P2PDeviceAddress length %d", len);
        *reply = wpas_dbus_error_invalid_args(message, "P2PDeviceAddress has wrong length");
        return -1;
    }
    return 0;
}
...
static int wpas_dbus_handler_wps_start_entry(
    DBusMessage *message, char *key, DBusMessageIter *entry_iter, struct wps_start_params *params, DBusMessage **reply)
{
    if (os_strcmp(key, "Role") == 0)
        ...
        return wpas_dbus_handler_wps_p2p_dev_addr(message, entry_iter, params, reply);
#endif /* CONFIG_P2P */
...
DBusMessage *wpas_dbus_handler_wps_start(DBusMessage *message, struct wpa_supplicant *wpa_s)
{
    ...
    while (dbus_message_iter_get_arg_type(&dict_iter) == DBUS_TYPE_DICT_ENTRY)
    {
        ...
        if (wpas_dbus_handler_wps_start_entry(message, key, &entry_iter, &params, &reply))
            return reply;
    ...
    if (wpa_s->ap_iface && params.type == 1)
    {
        if (params.pin == NULL)
        {
            wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Pin required for registrar role");
            return wpas_dbus_error_invalid_args(message, "Pin required for registrar role.");
        }
        ret = wpa_supplicant_ap_wps_pin(wpa_s, params.bssid, params.pin, npin, sizeof(npin), 0);
    }
    ...
    {
        ret = wpas_wps_start_pin(wpa_s, params.bssid, params.pin, 0, DEV_PW_DEFAULT);
        if (ret > 0)
        {
            ret = os_snprintf(npin, sizeof(npin), "%08d", ret);
            if (os_snprintf_error(sizeof(npin), ret))
                return wpas_dbus_error_unknown_error(message, "invalid PIN");
        }
    }
    else
    {
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG, "dbus: WPS.Start wpas_wps_failed in role %s and key %s",
    ...
    if (!wpa_dbus_dict_open_write(&iter, &dict_iter) ||
        (os_strlen(npin) > 0 && !wpa_dbus_dict_append_string(&dict_iter, "Pin", npin)) ||
 ...
 * if otherwise.
 */
dbus_bool_t wpas_dbus_getter_process_credentials(const struct wpa_dbus_property_desc *property_desc,
                                                 ...
                                                 void *user_data)
{
    ...
    return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_BOOLEAN, &process, error);
}
 ...
 */
dbus_bool_t wpas_dbus_setter_process_credentials(const struct wpa_dbus_property_desc *property_desc,
                                                 DBusMessageIter *iter,
                                                 DBusError *error,
                                                 void *user_data)
{
    ...
    if (!wpa_s->dbus_new_path)
        return FALSE;
    if (!wpas_dbus_simple_property_setter(iter, error, DBUS_TYPE_BOOLEAN, &process_credentials))
        return FALSE;
    ...
    if ((wpa_s->conf->wps_cred_processing != 1) != old_pc)
        wpa_dbus_mark_property_changed(wpa_s->global->dbus, wpa_s->dbus_new_path, WPAS_DBUS_NEW_IFACE_WPS,
                                       "ProcessCredentials");
 ...
 */
dbus_bool_t wpas_dbus_getter_config_methods(const struct wpa_dbus_property_desc *property_desc,
                                            DBusMessageIter *iter,
                                            DBusError *error,
                                            void *user_data)
{
    ...
    return wpas_dbus_string_property_getter(iter, wpa_s->conf->config_methods, error);
}
 ...
 */
dbus_bool_t wpas_dbus_setter_config_methods(const struct wpa_dbus_property_desc *property_desc,
                                            DBusMessageIter *iter,
                                            DBusError *error,
                                            void *user_data)
{
    ...
    if (!wpas_dbus_simple_property_setter(iter, error, DBUS_TYPE_STRING, &methods))
        return FALSE;
 ...
 */
dbus_bool_t wpas_dbus_getter_wps_device_name(const struct wpa_dbus_property_desc *property_desc,
                                             DBusMessageIter *iter,
                                             DBusError *error,
                                             void *user_data)
{
    ...
    return wpas_dbus_string_property_getter(iter, wpa_s->conf->device_name, error);
}
 ...
 */
dbus_bool_t wpas_dbus_setter_wps_device_name(const struct wpa_dbus_property_desc *property_desc,
                                             DBusMessageIter *iter,
                                             DBusError *error,
                                             void *user_data)
{
 ...
 */
dbus_bool_t wpas_dbus_getter_wps_manufacturer(const struct wpa_dbus_property_desc *property_desc,
                                              DBusMessageIter *iter,
                                              DBusError *error,
                                              void *user_data)
{
    ...
    return wpas_dbus_string_property_getter(iter, wpa_s->conf->manufacturer, error);
}
 ...
 */
dbus_bool_t wpas_dbus_setter_wps_manufacturer(const struct wpa_dbus_property_desc *property_desc,
                                              DBusMessageIter *iter,
                                              DBusError *error,
                                              void *user_data)
{
 ...
 */
dbus_bool_t wpas_dbus_getter_wps_device_model_name(const struct wpa_dbus_property_desc *property_desc,
                                                   DBusMessageIter *iter,
                                                   DBusError *error,
                                                   void *user_data)
{
    ...
    return wpas_dbus_string_property_getter(iter, wpa_s->conf->model_name, error);
}
 ...
 */
dbus_bool_t wpas_dbus_setter_wps_device_model_name(const struct wpa_dbus_property_desc *property_desc,
                                                   DBusMessageIter *iter,
                                                   DBusError *error,
                                                   void *user_data)
{
 ...
 */
dbus_bool_t wpas_dbus_getter_wps_device_model_number(const struct wpa_dbus_property_desc *property_desc,
                                                     DBusMessageIter *iter,
                                                     DBusError *error,
                                                     void *user_data)
{
    ...
    return wpas_dbus_string_property_getter(iter, wpa_s->conf->model_number, error);
}
 ...
 */
dbus_bool_t wpas_dbus_setter_wps_device_model_number(const struct wpa_dbus_property_desc *property_desc,
                                                     DBusMessageIter *iter,
                                                     DBusError *error,
                                                     void *user_data)
{
 ...
 */
dbus_bool_t wpas_dbus_getter_wps_device_serial_number(const struct wpa_dbus_property_desc *property_desc,
                                                      DBusMessageIter *iter,
                                                      DBusError *error,
                                                      void *user_data)
{
    ...
    return wpas_dbus_string_property_getter(iter, wpa_s->conf->serial_number, error);
}
 ...
 */
dbus_bool_t wpas_dbus_setter_wps_device_serial_number(const struct wpa_dbus_property_desc *property_desc,
                                                      DBusMessageIter *iter,
                                                      DBusError *error,
                                                      void *user_data)
{
 ...
 */
dbus_bool_t wpas_dbus_getter_wps_device_device_type(const struct wpa_dbus_property_desc *property_desc,
                                                    DBusMessageIter *iter,
                                                    DBusError *error,
                                                    void *user_data)
{
    ...
    if (!wpas_dbus_simple_array_property_getter(iter, DBUS_TYPE_BYTE, (char *)wpa_s->conf->device_type,
 ...
 */
dbus_bool_t wpas_dbus_setter_wps_device_device_type(const struct wpa_dbus_property_desc *property_desc,
                                                    DBusMessageIter *iter,
                                                    DBusError *error,
                                                    void *user_data)
{
