...
static int wpas_ctrl_iface_reinit(struct wpa_supplicant *wpa_s, struct ctrl_iface_priv *priv);
static int wpas_ctrl_iface_global_reinit(struct wpa_global *global, struct ctrl_iface_global_priv *priv);

static void wpas_ctrl_sock_debug(const char *title, int sock, const char *buf, size_t len)
{
#ifdef __linux__
    socklen_t optlen;
    int sndbuf, outq;
    ...
    wpa_printf(level, "CTRL-DEBUG: %s: sock=%d sndbuf=%d outq=%d send_len=%d", title, sock, sndbuf, outq, (int)len);
#endif /* __linux__ */
}
...
static void wpa_supplicant_ctrl_iface_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    if (!buf)
        return;
    res = recvfrom(sock, buf, CTRL_IFACE_MAX_LEN + 1, 0, (struct sockaddr *)&from, &fromlen);
    if (res < 0)
    {
...
static int wpas_ctrl_iface_throttle(int sock)
{
    ...
    optlen = sizeof(sndbuf);
    sndbuf = 0;
    if (getsockopt(sock, SOL_SOCKET, SO_SNDBUF, &sndbuf, &optlen) < 0 || ioctl(sock, TIOCOUTQ, &outq) < 0 ||
        sndbuf <= 0 || outq < 0)
        return 0;
...
static void wpas_ctrl_msg_send_pending_global(struct wpa_global *global)
{
    ...
    while (gpriv && !dl_list_empty(&gpriv->msg_queue) && !wpas_ctrl_iface_throttle(gpriv->sock))
    {
        msg = dl_list_first(&gpriv->msg_queue, struct ctrl_iface_msg, list);
        if (!msg)
            break;
        dl_list_del(&msg->list);
        wpa_supplicant_ctrl_iface_send(msg->wpa_s, msg->type != WPA_MSG_PER_INTERFACE ? NULL : msg->wpa_s->ifname,
                                       gpriv->sock, &gpriv->ctrl_dst, msg->level, msg->txt, msg->len, NULL, gpriv);
        os_free(msg);
    }
}
...
static void wpas_ctrl_msg_send_pending_iface(struct wpa_supplicant *wpa_s)
{
    ...
    while (priv && !dl_list_empty(&priv->msg_queue) && !wpas_ctrl_iface_throttle(priv->sock))
    {
        msg = dl_list_first(&priv->msg_queue, struct ctrl_iface_msg, list);
        if (!msg)
            break;
        dl_list_del(&msg->list);
        wpa_supplicant_ctrl_iface_send(wpa_s, NULL, priv->sock, &priv->ctrl_dst, msg->level, msg->txt, msg->len, priv,
                                       NULL);
        os_free(msg);
    }
}
...
static void wpas_ctrl_msg_queue_timeout(void *eloop_ctx, void *timeout_ctx)
{
    ...
    if (sock > -1 || gsock > -1)
    {
        ...
        wpa_printf(MSG_MSGDUMP, "CTRL: Had to throttle pending event message transmission for (sock %d gsock %d)", sock,
                   gsock);
        eloop_register_timeout(0, 20000, wpas_ctrl_msg_queue_timeout, wpa_s, NULL);
    }
}

static void wpas_ctrl_msg_queue(
    struct dl_list *queue, struct wpa_supplicant *wpa_s, int level, enum wpa_msg_type type, const char *txt, size_t len)
{
    struct ctrl_iface_msg *msg;
...
static void wpas_ctrl_msg_queue_limit(unsigned int throttle_count, struct dl_list *queue)
{
    struct ctrl_iface_msg *msg;
...
static void wpa_supplicant_ctrl_iface_msg_cb(void *ctx, int level, enum wpa_msg_type type, const char *txt, size_t len)
{
    ...
    if (type != WPA_MSG_NO_GLOBAL && gpriv && !dl_list_empty(&gpriv->ctrl_dst))
    {
        ...
        else
        {
            ...
            {
                wpa_printf(MSG_MSGDUMP, "CTRL: Had to throttle %u global event message(s) for sock %d",
                           gpriv->throttle_count, gpriv->sock);
            }
            gpriv->throttle_count = 0;
            wpa_supplicant_ctrl_iface_send(wpa_s, type != WPA_MSG_PER_INTERFACE ? NULL : wpa_s->ifname, gpriv->sock,
                                           &gpriv->ctrl_dst, level, txt, len, NULL, gpriv);
        }
    }
    ...
    if (type != WPA_MSG_ONLY_GLOBAL && priv)
    {
        ...
        else
        {
            ...
            {
                wpa_printf(MSG_MSGDUMP, "CTRL: Had to throttle %u event message(s) for sock %d", priv->throttle_count,
                           priv->sock);
            }
...
static int wpas_ctrl_iface_open_sock(struct wpa_supplicant *wpa_s, struct ctrl_iface_priv *priv)
{
    ...
    if (mkdir(dir, S_IRWXU | S_IRWXG) < 0)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "mkdir[ctrl_interface=%s]: %s", dir, strerror(errno));
            goto fail;
        }
    }
    ...
    {
        wpa_printf(MSG_ERROR, "lchown[ctrl_interface=%s,gid=%d]: %s", dir, (int)gid, strerror(errno));
        goto fail;
    }

    /* Make sure the group can enter and read the directory */
    if (gid_set && chmod(dir, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IXGRP) < 0)
    {
    ...
    if (bind(priv->sock, (struct sockaddr *)&addr, sizeof(addr)) < 0)
    {
        ...
        if (connect(priv->sock, (struct sockaddr *)&addr, sizeof(addr)) < 0)
        {
            ...
            if (bind(priv->sock, (struct sockaddr *)&addr, sizeof(addr)) < 0)
            {
                wpa_printf(MSG_ERROR, "supp-ctrl-iface-init: bind(PF_UNIX): %s", strerror(errno));
                goto fail;
            }
            wpa_printf(MSG_DEBUG,
    ...
    {
        wpa_printf(MSG_ERROR, "lchown[ctrl_interface=%s,gid=%d]: %s", fname, (int)gid, strerror(errno));
        goto fail;
    }

    if (chmod(fname, S_IRWXU | S_IRWXG) < 0)
    {
        wpa_printf(MSG_ERROR, "chmod[ctrl_interface=%s]: %s", fname, strerror(errno));
        goto fail;
    }
    os_free(fname);
...
struct ctrl_iface_priv *wpa_supplicant_ctrl_iface_init(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->global->params.ctrl_interface)
    {
        ...
        {
            if (os_strcmp(wpa_s->global->params.ctrl_interface, wpa_s->conf->ctrl_interface) == 0)
                same = 1;
        }
        else if (os_strncmp(wpa_s->global->params.ctrl_interface, "@android:", 9) == 0 ||
                 os_strncmp(wpa_s->global->params.ctrl_interface, "@abstract:", 10) == 0)
        {
            char *pos;
        ...
        if (same)
        {
            ...
            wpa_printf(MSG_ERROR,
                       ...
                       wpa_s->global->params.ctrl_interface, wpa_s->conf->ctrl_interface);
            return priv;
        }
    }
#endif /* ANDROID */
...
static void wpas_global_ctrl_iface_flush_queued_msg(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
    struct ctrl_iface_global_priv *gpriv;
    ...
    gpriv = global->ctrl_iface;
    dl_list_for_each_safe(msg, prev_msg, &gpriv->msg_queue, struct ctrl_iface_msg, list)
    {
        if (msg->wpa_s == wpa_s)
        {
            count++;
    ...
    if (count)
    {
        wpa_printf(MSG_DEBUG, "CTRL: Dropped %u pending message(s) for interface that is being removed", count);
    }
}
...
void wpa_supplicant_ctrl_iface_deinit(struct wpa_supplicant *wpa_s, struct ctrl_iface_priv *priv)
{
    ...
    if (priv->sock > -1)
    {
        ...
        if (rmdir(dir) < 0)
        {
            ...
            {
                wpa_printf(MSG_ERROR, "rmdir[ctrl_interface=%s]: %s", dir, strerror(errno));
            }
        }
        os_free(buf);
    }
...
free_dst:
    ...
    dl_list_for_each_safe(msg, prev_msg, &priv->msg_queue, struct ctrl_iface_msg, list)
    {
        dl_list_del(&msg->list);
        os_free(msg);
    }
                                           ...
                                           struct ctrl_iface_global_priv *gp)
{
    ...
    dl_list_for_each_safe(dst, next, ctrl_dst, struct wpa_ctrl_dst, list)
    {
        ...
        {
            sockaddr_print(MSG_MSGDUMP, "CTRL_IFACE monitor sent successfully to", &dst->addr, dst->addrlen);
            dst->errors = 0;
            continue;
        }

        _errno = errno;
        os_snprintf(txt, sizeof(txt), "CTRL_IFACE monitor: %d (%s) for", _errno, strerror(_errno));
        sockaddr_print(MSG_DEBUG, txt, &dst->addr, dst->addrlen);
        dst->errors++;

        if (dst->errors > 10 || _errno == ENOENT || _errno == EPERM)
        {
            sockaddr_print(MSG_INFO, "CTRL_IFACE: Detach monitor that cannot receive messages:", &dst->addr,
                           dst->addrlen);
...
void wpa_supplicant_ctrl_iface_wait(struct ctrl_iface_priv *priv)
{
    ...
    for (;;)
    {
        ...
        res = recvfrom(priv->sock, buf, sizeof(buf) - 1, 0, (struct sockaddr *)&from, &fromlen);
        if (res < 0)
        {
            wpa_printf(MSG_ERROR, "recvfrom(ctrl_iface): %s", strerror(errno));
            continue;
        ...
        if (os_strcmp(buf, "ATTACH") == 0)
        {
            ...
            if (!wpa_supplicant_ctrl_iface_attach(&priv->ctrl_dst, &from, fromlen, 0))
            {
                if (sendto(priv->sock, "OK\n", 3, 0, (struct sockaddr *)&from, fromlen) < 0)
                {
...
static int wpas_global_ctrl_iface_open_sock(struct wpa_global *global, struct ctrl_iface_global_priv *priv)
{
    ...
    if (bind(priv->sock, (struct sockaddr *)&addr, sizeof(addr)) < 0)
    {
        wpa_printf(MSG_INFO, "supp-global-ctrl-iface-init(%s) (will try fixup): bind(PF_UNIX): %s", ctrl,
                   strerror(errno));
        if (connect(priv->sock, (struct sockaddr *)&addr, sizeof(addr)) < 0)
        {
            wpa_printf(MSG_DEBUG,
            ...
            if (unlink(ctrl) < 0)
            {
                wpa_printf(MSG_ERROR, "Could not unlink existing ctrl_iface socket '%s': %s", ctrl, strerror(errno));
                goto fail;
            }
            if (bind(priv->sock, (struct sockaddr *)&addr, sizeof(addr)) < 0)
            {
                wpa_printf(MSG_ERROR, "supp-glb-iface-init: bind(PF_UNIX;%s): %s", ctrl, strerror(errno));
                goto fail;
            }
            wpa_printf(MSG_DEBUG,
                       "Successfully replaced leftover "
    ...
    if (global->params.ctrl_interface_group)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "lchown[global_ctrl_interface=%s,gid=%d]: %s", ctrl, (int)gid, strerror(errno));
            goto fail;
        }

        if (chmod(ctrl, S_IRWXU | S_IRWXG) < 0)
        {
            wpa_printf(MSG_ERROR, "chmod[global_ctrl_interface=%s]: %s", ctrl, strerror(errno));
            goto fail;
        }
    }
    else
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "chmod[global_ctrl_interface=%s](S_IRWXU): %s", ctrl, strerror(errno));
