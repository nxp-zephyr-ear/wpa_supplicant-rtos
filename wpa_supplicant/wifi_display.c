...
static int wifi_display_update_wfd_ie(struct wpa_global *global)
{
    ...
    p2p_set_wfd_dev_info(global->p2p, global->wfd_subelem[WFD_SUBELEM_DEVICE_INFO]);
    p2p_set_wfd_r2_dev_info(global->p2p, global->wfd_subelem[WFD_SUBELEM_R2_DEVICE_INFO]);
    p2p_set_wfd_assoc_bssid(global->p2p, global->wfd_subelem[WFD_SUBELEM_ASSOCIATED_BSSID]);
    p2p_set_wfd_coupled_sink_info(global->p2p, global->wfd_subelem[WFD_SUBELEM_COUPLED_SINK]);
    ...
    if (global->wfd_subelem[WFD_SUBELEM_R2_DEVICE_INFO])
        len += wpabuf_len(global->wfd_subelem[WFD_SUBELEM_R2_DEVICE_INFO]);
    ...
    if (global->wfd_subelem[WFD_SUBELEM_R2_DEVICE_INFO])
        wpabuf_put_buf(buf, global->wfd_subelem[WFD_SUBELEM_R2_DEVICE_INFO]);
...
{
    wpa_printf(MSG_DEBUG, "WFD: Wi-Fi Display %s", enabled ? "enabled" : "disabled");
...
int wifi_display_subelem_set_from_ies(struct wpa_global *global, struct wpabuf *ie)
{
    ...
    wpa_printf(MSG_DEBUG, "WFD IEs set: %p - %lu", ie, ie ? (unsigned long)wpabuf_len(ie) : 0);
...
int wifi_display_subelem_get(struct wpa_global *global, char *cmd, char *buf, size_t buflen)
{
    ...
    return wpa_snprintf_hex(buf, buflen, wpabuf_head_u8(global->wfd_subelem[subelem]) + 1,
...
char *wifi_display_subelem_hex(const struct wpabuf *wfd_subelems, u8 id)
{
    ...
    while (i + WIFI_DISPLAY_SUBELEM_HEADER_LEN < buflen)
    {
        ...
        if (buf[i] == id)
        {
            ...
            if (!subelem)
                return NULL;
            wpa_snprintf_hex(subelem, 2 * elen + 1, buf + i + WIFI_DISPLAY_SUBELEM_HEADER_LEN, elen);
            break;
        }
