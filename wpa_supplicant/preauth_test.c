...
static int wpa_supplicant_remove_pmkid(
    void *wpa_s, void *network_ctx, const u8 *bssid, const u8 *pmkid, const u8 *fils_cache_id)
{
    printf("%s - not implemented\n", __func__);
    return -1;
}
...
int main(int argc, char *argv[])
{
    ...
    {
        printf(
            "usage: preauth_test <conf> <target MAC address> "
            "<ifname>\n");
        return -1;
    }
    ...
    {
        ret = pmksa_cache_set_current(wpa_s.wpa, NULL, bssid, NULL, 0, NULL, 0) ? 0 : -3;
    }
