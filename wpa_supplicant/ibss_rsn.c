...
static int supp_ether_send(void *ctx, const u8 *dest, u16 proto, const u8 *buf, size_t len)
{
    ...
    wpa_printf(MSG_DEBUG, "SUPP: %s(dest=" MACSTR " proto=0x%04x len=%lu no_encrypt=%d)", __func__, MAC2STR(dest),
               proto, (unsigned long)len, !encrypt);
...
static void ibss_check_rsn_completed(struct ibss_rsn_peer *peer)
{
    ...
    if ((peer->authentication_status & (IBSS_RSN_SET_PTK_SUPP | IBSS_RSN_SET_PTK_AUTH)) !=
                        ...
                        enum key_flag key_flag)
{
    ...
    if (is_broadcast_ether_addr(addr))
        addr = peer->addr;
    return wpa_drv_set_key(peer->ibss_rsn->wpa_s, alg, addr, key_idx, set_tx, seq, seq_len, key, key_len, key_flag);
}
...
static int auth_send_eapol(void *ctx, const u8 *addr, const u8 *data, size_t data_len, int encrypt)
{
    ...
    if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_CONTROL_PORT)
        return wpa_drv_tx_control_port(wpa_s, addr, ETH_P_EAPOL, data, data_len, !encrypt);
    ...
    void *ctx, int vlan_id, enum wpa_alg alg, const u8 *addr, int idx, u8 *key, size_t key_len, enum key_flag key_flag)
{
    ...
    return wpa_drv_set_key(ibss_rsn->wpa_s, alg, addr, idx, 1, seq, 6, key, key_len, key_flag);
}
...
static int ibss_rsn_auth_init_group(struct ibss_rsn *ibss_rsn, const u8 *own_addr, struct wpa_ssid *ssid)
{
...
static int ibss_rsn_auth_init(struct ibss_rsn *ibss_rsn, struct ibss_rsn_peer *peer)
{
    ...
    if (wpa_validate_wpa_ie(ibss_rsn->auth_group, peer->auth, 0,
                            (u8 *)"\x30\x14\x01\x00"
                                  ...
                                  "\x01\x00\x00\x0f\xac\x02"
                                  "\x00\x00",
                            22, NULL, 0, NULL, 0, NULL, 0) != WPA_IE_OK)
    {
...
static void ibss_rsn_auth_timeout(void *eloop_ctx, void *timeout_ctx)
{
     ...
     */
    wpa_printf(MSG_DEBUG, "RSN: Timeout on waiting Authentication frame response from " MACSTR " - start authenticator",
               MAC2STR(peer->addr));
...
struct ibss_rsn *ibss_rsn_init(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
...
static int ibss_rsn_process_rx_eapol(struct ibss_rsn *ibss_rsn, struct ibss_rsn_peer *peer, const u8 *buf, size_t len)
{
    ...
    else
    {
        if (ibss_rsn_is_auth_started(peer) == 0)
        {
            wpa_printf(MSG_DEBUG,
                       "RSN: IBSS EAPOL for "
                       "Authenticator dropped as " MACSTR
                       " is not "
                       "authenticated",
                       MAC2STR(peer->addr));
            os_free(tmp);
...
static void ibss_rsn_handle_auth_1_of_2(struct ibss_rsn *ibss_rsn, struct ibss_rsn_peer *peer, const u8 *addr)
{
    ...
    if (peer && peer->authentication_status & (IBSS_RSN_SET_PTK_SUPP | IBSS_RSN_SET_PTK_AUTH))
    {
        ...
        wpa_printf(MSG_DEBUG, "RSN: Clear pairwise key for peer " MACSTR, MAC2STR(addr));
        wpa_drv_set_key(ibss_rsn->wpa_s, WPA_ALG_NONE, addr, 0, 0, NULL, 0, NULL, 0, KEY_FLAG_PAIRWISE);
    }
