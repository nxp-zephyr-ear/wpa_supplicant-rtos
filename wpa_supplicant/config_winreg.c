...
#define TNAMELEN 255
        ...
        namelen = TNAMELEN;
        datalen = sizeof(data);
        ret     = RegEnumValue(bhk, i, name, &namelen, NULL, &type, (LPBYTE)data, &datalen);
        ...
        wpa_printf(MSG_MSGDUMP, "blob %d: field='%s' len %d", (int)i, name, (int)datalen);
...
static int wpa_config_read_global_os_version(struct wpa_config *config, HKEY hk)
{
    char *str;
    int ret = 0;
...
static int wpa_config_read_global(struct wpa_config *config, HKEY hk)
{
    ...
    wpa_config_read_reg_dword(hk, TEXT("dot11RSNAConfigPMKLifetime"), (int *)&config->dot11RSNAConfigPMKLifetime);
    wpa_config_read_reg_dword(hk, TEXT("dot11RSNAConfigPMKReauthThreshold"),
                              (int *)&config->dot11RSNAConfigPMKReauthThreshold);
    wpa_config_read_reg_dword(hk, TEXT("dot11RSNAConfigSATimeout"), (int *)&config->dot11RSNAConfigSATimeout);
    wpa_config_read_reg_dword(hk, TEXT("update_config"), &config->update_config);

    if (wpa_config_read_reg_dword(hk, TEXT("eapol_version"), &config->eapol_version) == 0)
    {
        if (config->eapol_version < 1 || config->eapol_version > 2)
        {
            wpa_printf(MSG_ERROR, "Invalid EAPOL version (%d)", config->eapol_version);
            errors++;
        }
    }

    config->ctrl_interface = wpa_config_read_reg_string(hk, TEXT("ctrl_interface"));
    ...
    config->config_methods = wpa_config_read_reg_string(hk, TEXT("config_methods"));
    if (wpa_config_read_global_os_version(config, hk))
        errors++;
    wpa_config_read_reg_dword(hk, TEXT("wps_cred_processing"), &config->wps_cred_processing);
    wpa_config_read_reg_dword(hk, TEXT("wps_cred_add_sae"), &config->wps_cred_add_sae);
#endif /* CONFIG_WPS */
#ifdef CONFIG_P2P
    config->p2p_ssid_postfix = wpa_config_read_reg_string(hk, TEXT("p2p_ssid_postfix"));
    wpa_config_read_reg_dword(hk, TEXT("p2p_group_idle"), (int *)&config->p2p_group_idle);
#endif /* CONFIG_P2P */

    wpa_config_read_reg_dword(hk, TEXT("bss_max_count"), (int *)&config->bss_max_count);
    wpa_config_read_reg_dword(hk, TEXT("filter_ssids"), &config->filter_ssids);
    wpa_config_read_reg_dword(hk, TEXT("max_num_sta"), (int *)&config->max_num_sta);
    wpa_config_read_reg_dword(hk, TEXT("disassoc_low_ack"), (int *)&config->disassoc_low_ack);

    wpa_config_read_reg_dword(hk, TEXT("okc"), &config->okc);
    wpa_config_read_reg_dword(hk, TEXT("pmf"), &val);
    config->pmf = val;
    if (wpa_config_read_reg_dword(hk, TEXT("extended_key_id"), &val) == 0)
    {
        if (val < 0 || val > 1)
        {
            wpa_printf(MSG_ERROR, "Invalid Extended Key ID setting (%d)", val);
            errors++;
        }
...
static struct wpa_ssid *wpa_config_read_network(HKEY hk, const TCHAR *netw, int id)
{
    ...
    for (i = 0;; i++)
    {
        ...
        namelen = 255;
        datalen = sizeof(data);
        ret     = RegEnumValue(nhk, i, name, &namelen, NULL, &type, (LPBYTE)data, &datalen);
    ...
    if ((ssid->group_cipher & WPA_CIPHER_CCMP) && !(ssid->pairwise_cipher & WPA_CIPHER_CCMP) &&
...
static int wpa_config_write_reg_dword(HKEY hk, const TCHAR *name, int val, int def)
{
    ...
    if (ret != ERROR_SUCCESS)
    {
        wpa_printf(MSG_ERROR, "WINREG: Failed to set %s=%d: error %d", name, val, (int)GetLastError());
        return -1;
    }
...
static int wpa_config_write_reg_string(HKEY hk, const char *name, const char *val)
{
    ...
    ret = RegSetValueEx(hk, _name, 0, REG_SZ, (BYTE *)_val, (os_strlen(val) + 1) * sizeof(TCHAR));
    if (ret != ERROR_SUCCESS)
    {
        wpa_printf(MSG_ERROR,
...
static int wpa_config_write_global(struct wpa_config *config, HKEY hk)
{
    ...
    wpa_config_write_reg_dword(hk, TEXT("eapol_version"), config->eapol_version, DEFAULT_EAPOL_VERSION);
    wpa_config_write_reg_dword(hk, TEXT("ap_scan"), config->ap_scan, DEFAULT_AP_SCAN);
    wpa_config_write_reg_dword(hk, TEXT("fast_reauth"), config->fast_reauth, DEFAULT_FAST_REAUTH);
    wpa_config_write_reg_dword(hk, TEXT("dot11RSNAConfigPMKLifetime"), config->dot11RSNAConfigPMKLifetime, 0);
    wpa_config_write_reg_dword(hk, TEXT("dot11RSNAConfigPMKReauthThreshold"), config->dot11RSNAConfigPMKReauthThreshold,
                               0);
    wpa_config_write_reg_dword(hk, TEXT("dot11RSNAConfigSATimeout"), config->dot11RSNAConfigSATimeout, 0);
    wpa_config_write_reg_dword(hk, TEXT("update_config"), config->update_config, 0);
#ifdef CONFIG_WPS
    ...
    wpa_config_write_reg_dword(hk, TEXT("auto_uuid"), config->auto_uuid, 0);
    ...
    wpa_config_write_reg_dword(hk, TEXT("wps_cred_processing"), config->wps_cred_processing, 0);
    wpa_config_write_reg_dword(hk, TEXT("wps_cred_add_sae"), config->wps_cred_add_sae, 0);
#endif /* CONFIG_WPS */
#ifdef CONFIG_P2P
    wpa_config_write_reg_string(hk, "p2p_ssid_postfix", config->p2p_ssid_postfix);
    wpa_config_write_reg_dword(hk, TEXT("p2p_group_idle"), config->p2p_group_idle, 0);
#endif /* CONFIG_P2P */

    wpa_config_write_reg_dword(hk, TEXT("bss_max_count"), config->bss_max_count, DEFAULT_BSS_MAX_COUNT);
    wpa_config_write_reg_dword(hk, TEXT("filter_ssids"), config->filter_ssids, 0);
    wpa_config_write_reg_dword(hk, TEXT("max_num_sta"), config->max_num_sta, DEFAULT_MAX_NUM_STA);
    wpa_config_write_reg_dword(hk, TEXT("ap_isolate"), config->ap_isolate, DEFAULT_AP_ISOLATE);
    wpa_config_write_reg_dword(hk, TEXT("disassoc_low_ack"), config->disassoc_low_ack, 0);
...
static int wpa_config_delete_subkeys(HKEY hk, const TCHAR *key)
{
    ...
    if (ret != ERROR_SUCCESS)
    {
        wpa_printf(MSG_DEBUG, "WINREG: Could not open key '" TSTR "' for subkey deletion: error 0x%x (%d)", key,
    ...
    for (i = 0;; i++)
    {
        ...
        if (ret != ERROR_SUCCESS)
        {
            wpa_printf(MSG_DEBUG, "RegEnumKeyEx failed: 0x%x (%d)", (unsigned int)ret, (int)GetLastError());
            break;
        }
        ...
        if (ret != ERROR_SUCCESS)
        {
            wpa_printf(MSG_DEBUG, "RegDeleteKey failed: 0x%x (%d)", (unsigned int)ret, (int)GetLastError());
            errors++;
        }
    }
...
#define INT_DEFe(t, m, def) write_int(netw, #t, ssid->eap.m, def)
    ...
    INT(disabled);
    write_int(netw, "ieee80211w", ssid->ieee80211w, MGMT_FRAME_PROTECTION_DEFAULT);
    STR(id_str);
#ifdef CONFIG_HS20
...
static int wpa_config_write_blob(HKEY hk, struct wpa_config_blob *blob)
{
    ...
    ret = RegCreateKeyEx(hk, TEXT("blobs"), 0, NULL, 0, KEY_WRITE, NULL, &bhk, NULL);
    if (ret != ERROR_SUCCESS)
    {
        wpa_printf(MSG_DEBUG,
...
int wpa_config_write(const char *name, struct wpa_config *config)
{
    ...
    wpa_printf(MSG_DEBUG, "Configuration '%s' written %ssuccessfully", name, errors ? "un" : "");
    return errors ? -1 : 0;
}
