...
static int ctrl_open_pipe(struct ctrl_iface_priv *priv)
{
    ...
    eloop_register_event(dst->overlap.hEvent, sizeof(dst->overlap.hEvent), wpa_supplicant_ctrl_iface_receive, dst,
                         NULL);
    ...
    dst->pipe = CreateNamedPipe(name, PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,
    ...
    {
        wpa_printf(MSG_ERROR, "CTRL: CreateNamedPipe failed: %d", (int)GetLastError());
        goto fail;
    }

    if (ConnectNamedPipe(dst->pipe, &dst->overlap))
    {
        wpa_printf(MSG_ERROR, "CTRL: ConnectNamedPipe failed: %d", (int)GetLastError());
...
static VOID WINAPI ctrl_iface_write_completed(DWORD err, DWORD bytes, LPOVERLAPPED overlap)
{
    ...
    if (!ReadFileEx(dst->pipe, dst->req_buf, sizeof(dst->req_buf), &dst->overlap, ctrl_iface_read_completed))
    {
...
static void wpa_supplicant_ctrl_iface_rx(struct wpa_ctrl_dst *dst, size_t len)
{
    ...
    if (!WriteFileEx(dst->pipe, dst->rsp_buf, send_len, &dst->overlap, ctrl_iface_write_completed))
    {
...
static void wpa_supplicant_ctrl_iface_receive(void *eloop_data, void *user_ctx)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "CTRL: GetOverlappedResult failed: %d", (int)GetLastError());
        return;
    }
    wpa_printf(MSG_DEBUG,
...
static int ctrl_iface_parse(struct ctrl_iface_priv *priv, const char *params)
{
    ...
    if (!ConvertStringSecurityDescriptorToSecurityDescriptor(
            t_sddl, SDDL_REVISION_1, (PSECURITY_DESCRIPTOR *)(void *)&priv->attr.lpSecurityDescriptor, NULL))
    {
        os_free(t_sddl);
...
static int global_open_pipe(struct ctrl_iface_global_priv *priv)
{
    ...
    eloop_register_event(dst->overlap.hEvent, sizeof(dst->overlap.hEvent), wpa_supplicant_global_iface_receive, dst,
                         NULL);

    /* TODO: add support for configuring access list for the pipe */
    dst->pipe = CreateNamedPipe(NAMED_PIPE_PREFIX, PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,
...
static VOID WINAPI global_iface_write_completed(DWORD err, DWORD bytes, LPOVERLAPPED overlap)
{
    ...
    if (!ReadFileEx(dst->pipe, dst->req_buf, sizeof(dst->req_buf), &dst->overlap, global_iface_read_completed))
    {
        wpa_printf(MSG_DEBUG, "CTRL: ReadFileEx failed: %d", (int)GetLastError());
        global_close_pipe(dst);
...
static void wpa_supplicant_global_iface_rx(struct wpa_global_dst *dst, size_t len)
{
    ...
    if (!WriteFileEx(dst->pipe, dst->rsp_buf, send_len, &dst->overlap, global_iface_write_completed))
    {
        wpa_printf(MSG_DEBUG, "CTRL: WriteFileEx failed: %d", (int)GetLastError());
        global_close_pipe(dst);
    }
