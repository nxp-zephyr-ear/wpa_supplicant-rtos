...
static struct mesh_conf *mesh_config_create(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    if (cipher < 0 || cipher == WPA_CIPHER_TKIP || cipher == WPA_CIPHER_GTK_NOT_USED)
    {
    ...
    {
        if (ssid->group_mgmt_cipher == WPA_CIPHER_BIP_GMAC_128 || ssid->group_mgmt_cipher == WPA_CIPHER_BIP_GMAC_256 ||
...
static void wpas_mesh_copy_groups(struct hostapd_data *bss, struct wpa_supplicant *wpa_s)
{
    int num_groups;
    size_t groups_size;
...
static int wpas_mesh_init_rsn(struct wpa_supplicant *wpa_s)
{
    ...
    {
        bss->conf->sae_groups = os_memdup(default_groups, sizeof(default_groups));
        if (!bss->conf->sae_groups)
            return -1;
    }
...
static int wpas_mesh_update_freq_params(struct wpa_supplicant *wpa_s)
{
    ...
    if (hostapd_set_freq_params(&params->freq, ifmsh->conf->hw_mode, ifmsh->freq, ifmsh->conf->channel,
                                ifmsh->conf->enable_edmg, ifmsh->conf->edmg_channel, ifmsh->conf->ieee80211n,
                                ifmsh->conf->ieee80211ac, ifmsh->conf->ieee80211ax, ifmsh->conf->secondary_channel,
                                hostapd_get_oper_chwidth(ifmsh->conf),
                                hostapd_get_oper_centr_freq_seg0_idx(ifmsh->conf),
                                hostapd_get_oper_centr_freq_seg1_idx(ifmsh->conf), ifmsh->conf->vht_capab, he_capab))
    {
...
static int wpas_mesh_complete(struct wpa_supplicant *wpa_s)
{
    ...
    {
        wpa_printf(MSG_ERROR, "mesh: %s called without active mesh", __func__);
        return -1;
    }
    ...
    if (ifmsh->mconf->security != MESH_CONF_SEC_NONE && wpas_mesh_init_rsn(wpa_s))
    {
    ...
    wpa_msg(wpa_s, MSG_INFO, "joining mesh %s", wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
    ...
    if (!ret)
    {
        ...
        wpa_msg(wpa_s, MSG_INFO, MESH_GROUP_STARTED "ssid=\"%s\" id=%d", wpa_ssid_txt(ssid->ssid, ssid->ssid_len),
                ssid->id);
                                    ...
                                    struct hostapd_freq_params *freq)
{
    ...
    if (frequency != freq->freq && frequency == freq->freq + freq->sec_channel_offset * 20)
    {
    ...
    ifmsh->bss[0]->dot11RSNASAERetransPeriod = wpa_s->conf->dot11RSNASAERetransPeriod;
    ...
    {
        wpa_printf(MSG_ERROR, "Unsupported mesh mode frequency: %d MHz", frequency);
        goto out_free;
    }
    ...
    if (ssid->mesh_basic_rates == NULL)
    {
        ...
        {
            conf->basic_rates = os_memdup(basic_rates_erp, sizeof(basic_rates_erp));
            if (!conf->basic_rates)
                goto out_free;
        }
    }
    else
    {
        ...
        if (conf->basic_rates == NULL)
            goto out_free;
        os_memcpy(conf->basic_rates, ssid->mesh_basic_rates, rate_len * sizeof(int));
...
void wpa_mesh_notify_peer(struct wpa_supplicant *wpa_s, const u8 *addr, const u8 *ies, size_t ie_len)
{
    ...
    {
        wpa_msg(wpa_s, MSG_INFO, "Could not parse beacon from " MACSTR, MAC2STR(addr));
        return;
    }
...
static int mesh_attr_text(const u8 *ies, size_t ies_len, char *buf, char *end)
{
    ...
    if (elems.mesh_config_len > 6)
    {
        ret = os_snprintf(pos, end - pos,
                          ...
                          elems.mesh_config[0], elems.mesh_config[1], elems.mesh_config[2], elems.mesh_config[3],
                          elems.mesh_config[4], elems.mesh_config[5], elems.mesh_config[6]);
        if (os_snprintf_error(end - pos, ret))
            return pos - buf;
        pos += ret;
    }

    bss_basic_rate_set = os_malloc(elems.supp_rates_len + elems.ext_supp_rates_len);
    ...
    for (i = 0; i < elems.supp_rates_len; i++)
    {
        ...
        {
            bss_basic_rate_set[bss_basic_rate_set_len++] = (elems.supp_rates[i] & 0x7f) * 5;
        }
    }
    for (i = 0; i < elems.ext_supp_rates_len; i++)
    {
        ...
        {
            bss_basic_rate_set[bss_basic_rate_set_len++] = (elems.ext_supp_rates[i] & 0x7f) * 5;
        }
    }
    if (bss_basic_rate_set_len > 0)
    {
        ret = os_snprintf(pos, end - pos, "bss_basic_rate_set=%d", bss_basic_rate_set[0]);
        if (os_snprintf_error(end - pos, ret))
            goto fail;
        pos += ret;
...
static int wpas_mesh_get_ifname(struct wpa_supplicant *wpa_s, char *ifname, size_t len)
{
    char *ifname_ptr = wpa_s->ifname;
    int res;

    res = os_snprintf(ifname, len, "mesh-%s-%d", ifname_ptr, wpa_s->mesh_if_idx);
    if (os_snprintf_error(len, res) || (os_strlen(ifname) >= IFNAMSIZ && os_strlen(wpa_s->ifname) < IFNAMSIZ))
    {
...
int wpas_mesh_add_interface(struct wpa_supplicant *wpa_s, char *ifname, size_t len)
{
    ...
    if (wpa_drv_if_add(wpa_s, WPA_IF_MESH, ifname, NULL, NULL, NULL, addr, NULL) < 0)
    {
        wpa_printf(MSG_ERROR, "mesh: Failed to create new mesh interface");
        return -1;
    }
    wpa_printf(MSG_INFO, "mesh: Created virtual interface %s addr " MACSTR, ifname, MAC2STR(addr));
    ...
    if (!mesh_wpa_s)
    {
        wpa_printf(MSG_ERROR, "mesh: Failed to create new wpa_supplicant interface");
