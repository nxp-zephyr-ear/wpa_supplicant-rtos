...
static int wpas_create_receive_sc(void *wpa_s,
...
static int wpas_create_transmit_sc(void *wpa_s, struct transmit_sc *sc, enum confidentiality_offset co)
{
...
int ieee802_1x_alloc_kay_sm(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    res =
        ieee802_1x_kay_init(kay_ctx, policy, ssid->macsec_replay_protect, ssid->macsec_replay_window, ssid->macsec_port,
                            ssid->mka_priority, ssid->macsec_csindex, wpa_s->ifname, wpa_s->own_addr);
...
void *ieee802_1x_notify_create_actor(struct wpa_supplicant *wpa_s, const u8 *peer_addr)
{
    ...
    if (ieee802_1x_cak_aes_cmac(msk->key, msk->len, wpa_s->own_addr, peer_addr, cak->key, cak->len))
    {
    ...
    if (ieee802_1x_ckn_aes_cmac(msk->key, msk->len, wpa_s->own_addr, peer_addr, sid, sid_len, ckn->name))
    {
