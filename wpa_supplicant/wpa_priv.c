...
static void wpa_priv_cmd_register(struct wpa_priv_interface *iface, struct sockaddr_un *from, socklen_t fromlen)
{
    int i;
    ...
    if (iface->driver->init2)
    {
        ...
        iface->drv_priv = iface->driver->init2(iface, iface->ifname, iface->drv_global_priv);
    }
...
static void wpa_priv_cmd_scan(struct wpa_priv_interface *iface, void *buf, size_t len)
{
...
static void wpa_priv_get_scan_results2(struct wpa_priv_interface *iface, struct sockaddr_un *from, socklen_t fromlen)
{
...
static void wpa_priv_cmd_get_scan_results(struct wpa_priv_interface *iface, struct sockaddr_un *from, socklen_t fromlen)
{
    if (iface->drv_priv == NULL)
        return;
...
static void wpa_priv_cmd_authenticate(struct wpa_priv_interface *iface, void *buf, size_t len)
{
...
static void wpa_priv_cmd_get_bssid(struct wpa_priv_interface *iface, struct sockaddr_un *from, socklen_t fromlen)
{
    u8 bssid[ETH_ALEN];
    ...
    sendto(iface->fd, bssid, ETH_ALEN, 0, (struct sockaddr *)from, fromlen);
    return;
...
static void wpa_priv_cmd_get_ssid(struct wpa_priv_interface *iface, struct sockaddr_un *from, socklen_t fromlen)
{
    ...
    sendto(iface->fd, ssid, sizeof(ssid), 0, (struct sockaddr *)from, fromlen);
    return;
...
static void wpa_priv_cmd_get_capa(struct wpa_priv_interface *iface, struct sockaddr_un *from, socklen_t fromlen)
{
    ...
    sendto(iface->fd, &capa, sizeof(capa), 0, (struct sockaddr *)from, fromlen);
    return;
...
{
    return proto == ETH_P_EAPOL || proto == ETH_P_RSN_PREAUTH || proto == ETH_P_80211_ENCAP;
}

static void wpa_priv_cmd_l2_register(
    struct wpa_priv_interface *iface, struct sockaddr_un *from, socklen_t fromlen, void *buf, size_t len)
{
    int *reg_cmd = buf;
    ...
    iface->l2[idx] = l2_packet_init(iface->ifname, NULL, proto, wpa_priv_l2_rx, &iface->l2_ctx[idx], reg_cmd[1]);
    if (!iface->l2[idx])
    {
        wpa_printf(MSG_DEBUG,
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "Failed to get own address from "
                   "l2_packet");
    ...
    res = sendto(iface->fd, own_addr, ETH_ALEN, 0, (struct sockaddr *)from, fromlen);
    wpa_printf(MSG_DEBUG, "L2 registration[idx=%d]: res=%d", idx, res);
}

static void wpa_priv_cmd_l2_unregister(struct wpa_priv_interface *iface, struct sockaddr_un *from, socklen_t fromlen)
{
    int idx;

    for (idx = 0; idx < WPA_PRIV_MAX_L2; idx++)
    {
        if (iface->l2_addr_len[idx] == fromlen && os_memcmp(&iface->l2_addr[idx], from, fromlen) == 0)
            break;
    }
    if (idx == WPA_PRIV_MAX_L2)
    {
        wpa_printf(MSG_DEBUG, "No registered l2_packet socket found for unregister request");
        return;
    }
...
static void wpa_priv_cmd_l2_send(
    struct wpa_priv_interface *iface, struct sockaddr_un *from, socklen_t fromlen, void *buf, size_t len)
{
    u8 *dst_addr;
    u16 proto;
    ...
    if (idx == WPA_PRIV_MAX_L2)
    {
        wpa_printf(MSG_DEBUG, "No registered l2_packet socket found for send request");
        return;
    }
    ...
    res = l2_packet_send(iface->l2[idx], dst_addr, proto, (unsigned char *)buf + ETH_ALEN + 2, len - ETH_ALEN - 2);
...
static void wpa_priv_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    switch (cmd)
    {
        ...
        case PRIVSEP_CMD_L2_REGISTER:
            wpa_priv_cmd_l2_register(iface, &from, fromlen, cmd_buf, cmd_len);
            break;
...
static struct wpa_priv_interface *wpa_priv_interface_init(void *ctx, const char *dir, const char *params)
{
    ...
    if (bind(iface->fd, (struct sockaddr *)&addr, sizeof(addr)) < 0)
    {
        ...
        {
            wpa_printf(MSG_DEBUG,
                       "Socket exists, but does not "
                       "allow connections - assuming it was "
                       "leftover from forced program termination");
            if (unlink(iface->sock_name) < 0)
            {
                wpa_printf(MSG_ERROR, "Could not unlink existing ctrl_iface socket '%s': %s", iface->sock_name,
                           strerror(errno));
                goto fail;
            }
            if (bind(iface->fd, (struct sockaddr *)&addr, sizeof(addr)) < 0)
...
static void wpa_priv_send_auth(struct wpa_priv_interface *iface, union wpa_event_data *data)
{
...
void wpa_supplicant_event(void *ctx, enum wpa_event_type event, union wpa_event_data *data)
{
    ...
    switch (event)
    {
        ...
        case EVENT_MICHAEL_MIC_FAILURE:
            if (data == NULL)
                return;
            wpa_priv_send_event(iface, PRIVSEP_EVENT_MICHAEL_MIC_FAILURE, &data->michael_mic_failure.unicast,
                                sizeof(int));
            break;
        ...
        case EVENT_PMKID_CANDIDATE:
            if (data == NULL)
                return;
            wpa_priv_send_event(iface, PRIVSEP_EVENT_PMKID_CANDIDATE, &data->pmkid_candidate,
        ...
        case EVENT_AUTH:
            ...
            break;
        default:
            wpa_printf(MSG_DEBUG, "Unsupported driver event %d (%s) - TODO", event, event_to_string(event));
            break;
    }
}
...
void wpa_supplicant_event_global(void *ctx, enum wpa_event_type event, union wpa_event_data *data)
{
    ...
    if (iface && iface->driver->get_ifindex)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "%s: interface status ifindex %d mismatch (%d)", iface->ifname, ifindex,
