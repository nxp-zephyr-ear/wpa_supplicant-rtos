...
static void ieee802_1x_encapsulate_radius(struct eapol_test_data *e, const u8 *eap, size_t len)
{
    ...
    if (!find_extra_attr(e->extra_attrs, RADIUS_ATTR_NAS_IP_ADDRESS) &&
        !radius_msg_add_attr(msg, RADIUS_ATTR_NAS_IP_ADDRESS, (u8 *)&e->own_ip_addr, 4))
    {
    ...
    if (e->last_recv_radius && radius_msg_get_hdr(e->last_recv_radius)->code == RADIUS_CODE_ACCESS_CHALLENGE)
    {
        ...
        {
            printf(
                "Could not copy State attribute from previous "
                "Access-Challenge\n");
            goto fail;
        }
        if (res > 0)
...
static int eapol_test_compare_pmk(struct eapol_test_data *e)
{
        ...
        os_memcmp(e->authenticator_eap_key_name, sess_id, sess_id_len) != 0)
    {
        ...
        wpa_hexdump(MSG_DEBUG, "EAP-Key-Name from server", e->authenticator_eap_key_name,
...
static void eapol_test_cert_cb(void *ctx, struct tls_cert_data *cert, const char *cert_hash)
{
    struct eapol_test_data *e = ctx;
    int i;

    wpa_msg(e->wpa_s, MSG_INFO, WPA_EVENT_EAP_PEER_CERT "depth=%d subject='%s'%s%s", cert->depth, cert->subject,
    ...
    if (cert->cert)
    {
        ...
        if (cert_hex)
        {
            ...
            wpa_msg_ctrl(e->wpa_s, MSG_INFO, WPA_EVENT_EAP_PEER_CERT "depth=%d subject='%s' cert=%s", cert->depth,
                         cert->subject, cert_hex);
            os_free(cert_hex);
        }

        if (e->server_cert_file)
            eapol_test_write_cert(e->server_cert_file, cert->subject, cert->cert);
    }

    for (i = 0; i < cert->num_altsubject; i++)
        wpa_msg(e->wpa_s, MSG_INFO, WPA_EVENT_EAP_PEER_ALT "depth=%d %s", cert->depth, cert->altsubject[i]);
}
...
static void ieee802_1x_decapsulate_radius(struct eapol_test_data *e)
{
    ...
    switch (hdr->code)
    {
        case EAP_CODE_REQUEST:
            os_snprintf(buf, sizeof(buf), "EAP-Request-%s (%d)", eap_type >= 0 ? eap_type_text(eap_type) : "??",
                        eap_type);
            break;
        case EAP_CODE_RESPONSE:
            os_snprintf(buf, sizeof(buf), "EAP Response-%s (%d)", eap_type >= 0 ? eap_type_text(eap_type) : "??",
                        eap_type);
            break;
    ...
    struct radius_msg *msg, struct radius_msg *req, const u8 *shared_secret, size_t shared_secret_len, void *data)
{
    ...
    {
        printf(
            "Incoming RADIUS packet did not have correct "
    ...
    if ((hdr->code == RADIUS_CODE_ACCESS_ACCEPT && e->eapol_test_num_reauths < 0) ||
...
int main(int argc, char *argv[])
{
    ...
    wpa_init_conf(&eapol_test, &wpa_s, as_addr, as_port, as_secret, cli_addr, ifname);
