...
void mesh_auth_timer(void *eloop_ctx, void *user_data)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "AUTH: Re-authenticate with " MACSTR " (attempt %d) ", MAC2STR(sta->addr),
                   sta->sae_auth_retry);
        wpa_msg(wpa_s, MSG_INFO, MESH_SAE_AUTH_FAILURE "addr=" MACSTR, MAC2STR(sta->addr));
        ...
        else
        {
            ...
            wpa_msg(wpa_s, MSG_INFO, MESH_SAE_AUTH_BLOCKED "addr=" MACSTR " duration=%d", MAC2STR(sta->addr),
    ...
    void *ctx, int vlan_id, enum wpa_alg alg, const u8 *addr, int idx, u8 *key, size_t key_len, enum key_flag key_flag)
{
    ...
    return wpa_drv_set_key(mesh_rsn->wpa_s, alg, addr, idx, 1, seq, 6, key, key_len, key_flag);
}
...
static int __mesh_rsn_auth_init(struct mesh_rsn *rsn, const u8 *addr, enum mfp_options ieee80211w, int ocv)
{
    ...
    if (ieee80211w != NO_MGMT_FRAME_PROTECTION)
    {
        ...
        wpa_drv_set_key(rsn->wpa_s, wpa_cipher_to_alg(rsn->mgmt_group_cipher), broadcast_ether_addr, rsn->igtk_key_id,
                        1, seq, sizeof(seq), rsn->igtk, rsn->igtk_len, KEY_FLAG_GROUP_TX_DEFAULT);
    }

    /* group privacy / data frames */
    wpa_hexdump_key(MSG_DEBUG, "mesh: Own TX MGTK", rsn->mgtk, rsn->mgtk_len);
    wpa_drv_set_key(rsn->wpa_s, wpa_cipher_to_alg(rsn->group_cipher), broadcast_ether_addr, rsn->mgtk_key_id, 1, seq,
                    sizeof(seq), rsn->mgtk, rsn->mgtk_len, KEY_FLAG_GROUP_TX_DEFAULT);
...
struct mesh_rsn *mesh_rsn_auth_init(struct wpa_supplicant *wpa_s, struct mesh_conf *conf)
{
    ...
    if (__mesh_rsn_auth_init(mesh_rsn, wpa_s->own_addr, conf->ieee80211w, conf->ocv) < 0)
    {
    ...
    while ((entry = dl_list_last(&wpa_s->mesh_external_pmksa_cache, struct external_pmksa_cache, list)) != NULL)
    {
        ...
        ret = wpa_auth_pmksa_add_entry(bss->wpa_auth, entry->pmksa_cache);
...
static int mesh_rsn_sae_group(struct wpa_supplicant *wpa_s, struct sae_data *sae)
{
...
static int mesh_rsn_build_sae_commit(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, struct sta_info *sta)
{
    const char *password;
    ...
    return sae_prepare_commit(wpa_s->own_addr, sta->addr, (u8 *)password, os_strlen(password), sta->sae);
}
...
int mesh_rsn_auth_sae_sta(struct wpa_supplicant *wpa_s, struct sta_info *sta)
{
    ...
    if (!ssid)
    {
        wpa_msg(wpa_s, MSG_DEBUG, "AUTH: No current_ssid known to initiate new SAE");
        return -1;
    }
    ...
    if (pmksa)
    {
        ...
        wpa_printf(MSG_DEBUG,
                   ...
                   MAC2STR(sta->addr));
        wpa_auth_pmksa_set_to_sm(pmksa, sta->wpa_sm, hapd->wpa_auth, sta->sae->pmkid, sta->sae->pmk);
    ...
    wpa_msg(wpa_s, MSG_DEBUG, "AUTH: started authentication with SAE peer: " MACSTR, MAC2STR(sta->addr));
    ...
    eloop_register_timeout(MESH_AUTH_TIMEOUT + rnd, 0, mesh_auth_timer, wpa_s, sta);
    return 0;
}
...
static void mesh_rsn_derive_aek(struct mesh_rsn *rsn, struct sta_info *sta)
{
    ...
    sha256_prf(sta->sae->pmk, sizeof(sta->sae->pmk), "AEK Derivation", context, sizeof(context), sta->aek,
               sizeof(sta->aek));
}
...
int mesh_rsn_derive_mtk(struct wpa_supplicant *wpa_s, struct sta_info *sta)
{
    ...
    sha256_prf(sta->sae->pmk, SAE_PMK_LEN, "Temporal Key Derivation", context, sizeof(context), sta->mtk, sta->mtk_len);
    return 0;
}
...
skip_keys:
    ...
    if (aes_siv_encrypt(sta->aek, sizeof(sta->aek), ampe_ie, 2 + len, 3, aad, aad_len, mic_payload))
    {
                          ...
                          size_t elems_len)
{
    ...
    const size_t aad_len[] = {ETH_ALEN, ETH_ALEN, elems->mic ? (elems->mic - 2) - cat : 0};
    size_t key_len;
    ...
    if (chosen_pmk && (!sta->sae || os_memcmp(chosen_pmk, sta->sae->pmkid, PMKID_LEN) != 0))
    {
        wpa_msg(wpa_s, MSG_DEBUG, "Mesh RSN: Invalid PMKID (Chosen PMK did not match calculated PMKID)");
        return -1;
    }
    ...
    if (aes_siv_decrypt(sta->aek, sizeof(sta->aek), crypt, crypt_len, 3, aad, aad_len, ampe_buf))
    {
    ...
    pos += sta->mgtk_len;
    wpa_hexdump(MSG_DEBUG, "mesh: GTKdata - MGTK - Key RSC", pos, WPA_KEY_RSC_LEN);
    os_memcpy(sta->mgtk_rsc, pos, sizeof(sta->mgtk_rsc));
    pos += WPA_KEY_RSC_LEN;
    wpa_printf(MSG_DEBUG, "mesh: GTKdata - MGTK - GTKExpirationTime: %u seconds", WPA_GET_LE32(pos));
    pos += 4;
    ...
    if (end - pos >= (int)(2 + 6 + key_len))
    {
        ...
        wpa_hexdump(MSG_DEBUG, "mesh: IGTKdata - IPN", sta->igtk_rsc, sizeof(sta->igtk_rsc));
        pos += 6;
        os_memcpy(sta->igtk, pos, key_len);
        sta->igtk_len = key_len;
        wpa_hexdump_key(MSG_DEBUG, "mesh: IGTKdata - IGTK", sta->igtk, sta->igtk_len);
    }
