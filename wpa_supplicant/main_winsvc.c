...
#define TBUFLEN 255
    ...
    ret    = RegQueryValueEx(hk, TEXT("ctrl_interface"), NULL, NULL, (LPBYTE)ctrl_interface, &buflen);
    if (ret == ERROR_SUCCESS)
    {
    ...
    ret    = RegQueryValueEx(hk, TEXT("adapter"), NULL, NULL, (LPBYTE)adapter, &buflen);
    if (ret == ERROR_SUCCESS)
    {
    ...
    ret    = RegQueryValueEx(hk, TEXT("config"), NULL, NULL, (LPBYTE)config, &buflen);
    if (ret == ERROR_SUCCESS)
    {
    ...
    buflen = sizeof(val);
    ret    = RegQueryValueEx(hk, TEXT("skip_on_error"), NULL, NULL, (LPBYTE)&val, &buflen);
...
static int wpa_supplicant_thread(void)
{
    ...
    buflen = sizeof(val);
    ret    = RegQueryValueEx(hk, TEXT("debug_level"), NULL, NULL, (LPBYTE)&val, &buflen);
    if (ret == ERROR_SUCCESS && buflen == sizeof(val))
    {
    ...
    buflen = sizeof(val);
    ret    = RegQueryValueEx(hk, TEXT("debug_show_keys"), NULL, NULL, (LPBYTE)&val, &buflen);
    if (ret == ERROR_SUCCESS && buflen == sizeof(val))
    {
    ...
    buflen = sizeof(val);
    ret    = RegQueryValueEx(hk, TEXT("debug_timestamp"), NULL, NULL, (LPBYTE)&val, &buflen);
    if (ret == ERROR_SUCCESS && buflen == sizeof(val))
    {
    ...
    buflen = sizeof(val);
    ret    = RegQueryValueEx(hk, TEXT("debug_use_file"), NULL, NULL, (LPBYTE)&val, &buflen);
    ...
    {
        printf(
            "Could not open wpa_supplicant interfaces registry "
            "key\n");
        return -1;
    }
...
static int register_service(const TCHAR *exe)
{
    ...
    svc = CreateService(scm, WPASVC_NAME, WPASVC_DISPLAY_NAME, SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS,
                        SERVICE_DEMAND_START, SERVICE_ERROR_NORMAL, exe, NULL, NULL, NULL, NULL, NULL);
...
static int unregister_service(void)
{
    ...
    if (QueryServiceStatus(svc, &status))
    {
        ...
        {
            printf(
                "Service currently active - stopping "
                "service...\n");
...
static void WINAPI service_start(DWORD argc, LPTSTR *argv)
{
    ...
    svc_status_handle = RegisterServiceCtrlHandler(WPASVC_NAME, service_ctrl_handler);
