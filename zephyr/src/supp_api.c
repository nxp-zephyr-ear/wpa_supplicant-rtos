...
extern struct k_sem hostapd_ready_sem;
...
static K_WORK_DELAYABLE_DEFINE(wpa_supp_status_work, supp_shell_connect_status);
...
static void supp_shell_connect_status(struct k_work *work)
{
    ...
    if (ctrl->requested_op == CONNECT)
    {
        if (wpa_s->wpa_state != WPA_COMPLETED &&
            (ctrl->connection_timeout == SYS_FOREVER_MS || i++ <= ctrl->connection_timeout))
        {
