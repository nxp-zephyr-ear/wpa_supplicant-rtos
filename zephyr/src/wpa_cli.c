...
static int cmd_supplicant(const struct shell *shell, size_t argc, const char *argv[])
{
    struct wpa_supplicant *wpa_s = wpa_supplicant_get_iface(global, if_name);

    if (!wpa_s)
    {
        shell_fprintf(shell, SHELL_ERROR, "%s: wpa_supplicant is not initialized, dropping connect\n", __func__);
        return -1;
    }
...
SHELL_STATIC_SUBCMD_SET_CREATE(
    ...
    SHELL_CMD(get, NULL, "\"Get Global params\"", cmd_supplicant),
    ...
    SHELL_CMD(bssid, NULL, "\"Associate with this BSSID\"", cmd_supplicant),
    SHELL_CMD(sta_autoconnect, NULL, "\"\"", cmd_supplicant),
    SHELL_CMD(signal_poll, NULL, "\"\"", cmd_supplicant),
    SHELL_CMD(wnm_bss_query, NULL, "\"\"", cmd_supplicant),
    SHELL_SUBCMD_SET_END);

/* Persisting with "wpa_cli" naming for compatibility with Wi-Fi
 * certification applications and scripts.
 */
SHELL_CMD_REGISTER(wpa_cli, &wpa_cli_cmds, "wpa_supplicant commands (only for internal use)", NULL);
