...
{
    return oid->len == 7 && oid_is_rsadsi(oid) && oid->oid[4] == 1 /* pkcs */ && oid->oid[5] == 5 /* pkcs-5 */ &&
           oid->oid[6] == alg;
}

static int enc_alg_is_oid(struct asn1_oid *oid, unsigned long alg)
{
    return oid->len == 6 && oid_is_rsadsi(oid) && oid->oid[4] == 3 /* encryptionAlgorithm */ && oid->oid[5] == alg;
}

static int pkcs12_is_pbe_oid(struct asn1_oid *oid, unsigned long alg)
{
    return oid->len == 8 && oid_is_rsadsi(oid) && oid->oid[4] == 1 /* pkcs */ && oid->oid[5] == 12 /* pkcs-12 */ &&
...
static int pkcs5_get_params_pbes2(struct pkcs5_params *params, const u8 *pos, const u8 *enc_alg_end)
{
    struct asn1_hdr hdr;
    ...
    if (asn1_get_next(pos, enc_alg_end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #5: Expected SEQUENCE (PBES2-params)");
        return -1;
    }
    pos = hdr.payload;
    ...
    {
        asn1_unexpected(&hdr, "PKCS #5: Expected SEQUENCE (keyDerivationFunc)");
        return -1;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "PKCS #5: Failed to parse OID (keyDerivationFunc algorithm)");
        return -1;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "PKCS #5: Unsupported PBES2 keyDerivationFunc algorithm %s", obuf);
        return -1;
    }
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #5: Expected SEQUENCE (PBKDF2-params)");
        return -1;
    }
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_octetstring(&hdr) || hdr.length > sizeof(params->salt))
    {
        asn1_unexpected(&hdr, "PKCS #5: Expected OCTET STRING (salt.specified)");
        return -1;
    }
    ...
    if (asn1_get_next(pos, enc_alg_end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #5: Expected SEQUENCE (encryptionScheme)");
        return -1;
    }
    ...
    if (asn1_get_oid(pos, end - pos, &oid, &pos))
    {
        wpa_printf(MSG_DEBUG, "PKCS #5: Failed to parse OID (encryptionScheme algorithm)");
        return -1;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "PKCS #5: Unsupported PBES2 encryptionScheme algorithm %s", obuf);
        return -1;
    }
     ...
     */
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_octetstring(&hdr) || hdr.length != 8)
    {
        asn1_unexpected(&hdr, "PKCS #5: Expected OCTET STRING (SIZE(8)) (IV)");
        return -1;
    }
...
static int pkcs5_get_params(const u8 *enc_alg, size_t enc_alg_len, struct pkcs5_params *params)
{
    ...
    if (asn1_get_next(pos, enc_alg_end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #5: Expected SEQUENCE (PBEParameter)");
        return -1;
    }
    pos = hdr.payload;
    end = hdr.payload + hdr.length;
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_octetstring(&hdr) || hdr.length > sizeof(params->salt))
    {
        asn1_unexpected(&hdr, "PKCS #5: Expected OCTETSTRING SIZE(8) (salt)");
        return -1;
    }
    pos = hdr.payload + hdr.length;
    os_memcpy(params->salt, hdr.payload, hdr.length);
...
static struct crypto_cipher *pkcs5_crypto_init_pbes2(struct pkcs5_params *params, const char *passwd)
{
    u8 key[24];
    ...
    wpa_hexdump_ascii_key(MSG_DEBUG, "PKCS #5: PBES2 password for PBKDF2", passwd, os_strlen(passwd));
    wpa_hexdump(MSG_DEBUG, "PKCS #5: PBES2 salt for PBKDF2", params->salt, params->salt_len);
    wpa_printf(MSG_DEBUG, "PKCS #5: PBES2 PBKDF2 iterations: %u", params->iter_count);
    if (pbkdf2_sha1(passwd, params->salt, params->salt_len, params->iter_count, key, sizeof(key)) < 0)
        return NULL;
    wpa_hexdump_key(MSG_DEBUG, "PKCS #5: DES EDE3 key", key, sizeof(key));
    wpa_hexdump(MSG_DEBUG, "PKCS #5: DES IV", params->iv, params->iv_len);

    return crypto_cipher_init(CRYPTO_CIPHER_ALG_3DES, params->iv, key, sizeof(key));
}
...
static int pkcs12_key_gen(
    const u8 *pw, size_t pw_len, const u8 *salt, size_t salt_len, u8 id, unsigned int iter, size_t out_len, u8 *out)
{
...
static struct crypto_cipher *pkcs12_crypto_init_sha1(struct pkcs5_params *params, const char *passwd)
{
    unsigned int i;
    u8 *pw;
    size_t pw_len;
    ...
    if (pkcs12_key_gen(pw, pw_len, params->salt, params->salt_len, PKCS12_ID_ENC, params->iter_count, sizeof(key),
                       key) < 0 ||
        pkcs12_key_gen(pw, pw_len, params->salt, params->salt_len, PKCS12_ID_IV, params->iter_count, sizeof(iv), iv) <
            0)
    {
        os_free(pw);
        return NULL;
