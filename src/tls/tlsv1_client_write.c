...
u8 *tls_send_client_hello(struct tlsv1_client *conn, size_t *out_len)
{
    ...
    wpa_printf(MSG_DEBUG, "TLSv1: Send ClientHello (ver %s)", tls_version_str(tls_version));
    *out_len = 0;
    ...
    *pos++ = TLS_COMPRESSION_NULL;
    ...
    {
        wpa_printf(MSG_DEBUG, "TLSv1: Add status_request extension for OCSP stapling");
        ...
        *pos++ = 1; /* status_type = ocsp(1) */
        ...
        wpa_printf(MSG_DEBUG, "TLSv1: Add status_request_v2 extension for OCSP stapling");
    ...
    if (tlsv1_record_send(&conn->rl, TLS_CONTENT_TYPE_HANDSHAKE, rhdr, end - rhdr, hs_start, pos - hs_start, out_len) <
        0)
    {
...
static int tlsv1_key_x_dh(struct tlsv1_client *conn, u8 **pos, u8 *end)
{
    ...
    if (crypto_mod_exp(conn->dh_g, conn->dh_g_len, csecret_start, csecret_len, conn->dh_p, conn->dh_p_len, dh_yc,
                       &dh_yc_len))
    {
        tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
        os_free(csecret);
    ...
    *pos += dh_yc_len;
    ...
    if (crypto_mod_exp(conn->dh_ys, conn->dh_ys_len, csecret_start, csecret_len, conn->dh_p, conn->dh_p_len, shared,
                       &shared_len))
    {
        tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
        os_free(csecret);
...
static int tlsv1_key_x_rsa(struct tlsv1_client *conn, u8 **pos, u8 *end)
{
    ...
    *pos += 2;
    clen = end - *pos;
    res  = crypto_public_key_encrypt_pkcs1_v15(conn->server_rsa_key, pre_master_secret, TLS_PRE_MASTER_SECRET_LEN, *pos,
                                              &clen);
...
static u8 *tls_send_client_key_exchange(struct tlsv1_client *conn, size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    if (tls_write_client_key_exchange(conn, &pos, end) < 0 ||
        ...
        tls_write_client_change_cipher_spec(conn, &pos, end) < 0 || tls_write_client_finished(conn, &pos, end) < 0)
    {
        os_free(msg);
        return NULL;
    }
