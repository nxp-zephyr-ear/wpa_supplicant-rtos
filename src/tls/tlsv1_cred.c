    ...
    struct tlsv1_credentials *cred, const char *cert, const u8 *cert_blob, size_t cert_blob_len, const char *path)
{
    if (cert && os_strncmp(cert, "hash://", 7) == 0)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "TLSv1: Unexpected SHA256 hash length in ca_cert '%s'", cert);
            return -1;
        }
        if (hexstr2bin(pos, cred->srv_cert_hash, 32) < 0)
        {
            wpa_printf(MSG_DEBUG, "TLSv1: Invalid SHA256 hash value in ca_cert '%s'", cert);
            return -1;
        }
...
{
    return oid->len == 9 && oid_is_rsadsi(oid) && oid->oid[4] == 1 /* pkcs */ && oid->oid[5] == 12 /* pkcs-12 */ &&
           oid->oid[6] == 10 && oid->oid[7] == 1 /* bagtypes */ && oid->oid[8] == type;
}

static int is_oid_pkcs7(struct asn1_oid *oid)
{
    return oid->len == 7 && oid->oid[0] == 1 /* iso */ && oid->oid[1] == 2 /* member-body */ &&
           oid->oid[2] == 840 /* us */ && oid->oid[3] == 113549 /* rsadsi */ && oid->oid[4] == 1 /* pkcs */ &&
           oid->oid[5] == 7 /* pkcs-7 */;
}
...
{
    return oid->len >= 6 && oid->oid[0] == 1 /* iso */ && oid->oid[1] == 2 /* member-body */ &&
           oid->oid[2] == 840 /* us */ && oid->oid[3] == 113549 /* rsadsi */ && oid->oid[4] == 1 /* pkcs */ &&
           oid->oid[5] == 9 /* pkcs-9 */;
}
...
{
    return oid->len == 8 && is_oid_pkcs9(oid) && oid->oid[6] == 22 /* certTypes */ &&
           oid->oid[7] == 1 /* x509Certificate */;
}

static int pkcs12_keybag(struct tlsv1_credentials *cred, const u8 *buf, size_t len)
{
    /* TODO */
    return 0;
}

static int pkcs12_pkcs8_keybag(struct tlsv1_credentials *cred, const u8 *buf, size_t len, const char *passwd)
{
...
static int pkcs12_certbag(struct tlsv1_credentials *cred, const u8 *buf, size_t len)
{
    struct asn1_hdr hdr;
    struct asn1_oid oid;
    char obuf[80];
    ...
    {
        wpa_printf(MSG_DEBUG, "PKCS #12: Ignored unsupported certificate type (certId %s)", obuf);
    }

    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !hdr.constructed || !asn1_is_cs_tag(&hdr, 0))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected [0] EXPLICIT (certValue)");
        return -1;
    }

    if (asn1_get_next(hdr.payload, hdr.length, &hdr) < 0 || !asn1_is_octetstring(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected OCTET STRING (x509Certificate)");
        return -1;
    }
...
static int pkcs12_parse_attr_friendly_name(const u8 *pos, const u8 *end)
{
     ...
     * }
     */
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_bmpstring(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected BMPSTRING (friendlyName)");
        return 0;
    }
    wpa_hexdump_ascii(MSG_DEBUG, "PKCS #12: friendlyName", hdr.payload, hdr.length);
    return 0;
}
...
static int pkcs12_parse_attr_local_key_id(const u8 *pos, const u8 *end)
{
    ...
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected OCTET STRING (localKeyID)");
        return -1;
    }
...
static int pkcs12_safebag(struct tlsv1_credentials *cred, const u8 *buf, size_t len, const char *passwd)
{
    struct asn1_hdr hdr;
    struct asn1_oid oid;
    char obuf[80];
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !hdr.constructed || !asn1_is_cs_tag(&hdr, 0))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected [0] EXPLICIT (bagValue)");
        return 0;
    }
    value     = hdr.payload;
    ...
    if (pos < end)
    {
        ...
        if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_set(&hdr))
        {
            asn1_unexpected(&hdr, "PKCS #12: Expected SET (bagAttributes)");
            return -1;
        }
        wpa_hexdump_key(MSG_MSGDUMP, "PKCS #12: bagAttributes", hdr.payload, hdr.length);
        ...
        while (pos < end)
        {
            ...
            {
                asn1_unexpected(&hdr, "PKCS #12: Expected SEQUENCE (PKCS12Attribute)");
                return -1;
            }
...
static int pkcs12_safecontents(struct tlsv1_credentials *cred, const u8 *buf, size_t len, const char *passwd)
{
    struct asn1_hdr hdr;
    const u8 *pos, *end;
    ...
    if (asn1_get_next(buf, len, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected SEQUENCE (SafeContents)");
        return -1;
    }
    pos = hdr.payload;
    end = hdr.payload + hdr.length;
    ...
    while (pos < end)
    {
        if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
        {
            asn1_unexpected(&hdr, "PKCS #12: Expected SEQUENCE (SafeBag)");
            return -1;
        }
...
static int pkcs12_parse_content_data(struct tlsv1_credentials *cred, const u8 *pos, const u8 *end, const char *passwd)
{
    struct asn1_hdr hdr;
                                         ...
                                         const char *passwd)
{
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected SEQUENCE (EncryptedData)");
        return 0;
    }
    pos = hdr.payload;
    ...
    {
        asn1_unexpected(&hdr, "PKCS #12: No INTEGER tag found for version");
        return -1;
    }
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected SEQUENCE (EncryptedContentInfo)");
        return -1;
    }
    ...
    if (asn1_get_oid(pos, end - pos, &oid, &pos))
    {
        wpa_printf(MSG_DEBUG, "PKCS #12: Could not find OBJECT IDENTIFIER (contentType)");
        return -1;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "PKCS #12: Unsupported EncryptedContentInfo::contentType %s", buf);
        return 0;
    }
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected SEQUENCE (ContentEncryptionAlgorithmIdentifier)");
        return -1;
    }
    enc_alg     = hdr.payload;
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || hdr.constructed || !asn1_is_cs_tag(&hdr, 0))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected [0] IMPLICIT (encryptedContent)");
        return -1;
    }
    ...
    if (data)
    {
        wpa_hexdump_key(MSG_MSGDUMP, "PKCS #12: Decrypted encryptedContent", data, data_len);
...
static int pkcs12_parse_content(struct tlsv1_credentials *cred, const u8 *buf, size_t len, const char *passwd)
{
    const u8 *pos = buf;
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !hdr.constructed || !asn1_is_cs_tag(&hdr, 0))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected [0] EXPLICIT (content)");
        return 0;
    }
    pos = hdr.payload;
...
static int pkcs12_parse(struct tlsv1_credentials *cred, const u8 *key, size_t len, const char *passwd)
{
    struct asn1_hdr hdr;
    const u8 *pos, *end;
    struct asn1_oid oid;
    ...
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected SEQUENCE (PFX); assume PKCS #12 not used");
        return -1;
    }
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected SEQUENCE (authSafe); assume PKCS #12 not used");
        return -1;
    }
    ...
    if (asn1_get_oid(pos, end - pos, &oid, &pos))
    {
        wpa_printf(MSG_DEBUG, "PKCS #12: Could not find OBJECT IDENTIFIER (contentType); assume PKCS #12 not used");
        return -1;
    }
    asn1_oid_to_str(&oid, buf, sizeof(buf));
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !hdr.constructed || !asn1_is_cs_tag(&hdr, 0))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected [0] EXPLICIT (content); assume PKCS #12 not used");
        return -1;
    }
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_octetstring(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected OCTET STRING (Data); assume PKCS #12 not used");
        return -1;
    }
    ...
    {
        asn1_unexpected(&hdr, "PKCS #12: Expected SEQUENCE within Data content; assume PKCS #12 not used");
        return -1;
    }
    ...
    while (end > pos)
    {
        if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
        {
            asn1_unexpected(&hdr, "PKCS #12: Expected SEQUENCE (ContentInfo); assume PKCS #12 not used");
            return -1;
        }
...
static int tlsv1_set_dhparams_der(struct tlsv1_credentials *cred, const u8 *dh, size_t len)
{
    ...
    {
        asn1_unexpected(&hdr, "DH: DH parameters did not start with a valid SEQUENCE");
        return -1;
    }
    pos = hdr.payload;
