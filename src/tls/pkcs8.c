...
struct crypto_private_key *pkcs8_key_import(const u8 *buf, size_t len)
{
    ...
    {
        asn1_unexpected(&hdr, "PKCS #8: Does not start with PKCS #8 header (SEQUENCE)");
        return NULL;
    }
    pos = hdr.payload;
    end = pos + hdr.length;
    ...
    {
        asn1_unexpected(&hdr, "PKCS #8: Expected SEQUENCE (AlgorithmIdentifier); assume PKCS #8 not used");
        return NULL;
    }
    ...
    if (oid.len != 7 || oid.oid[0] != 1 /* iso */ || oid.oid[1] != 2 /* member-body */ || oid.oid[2] != 840 /* us */ ||
        oid.oid[3] != 113549 /* rsadsi */ || oid.oid[4] != 1 /* pkcs */ || oid.oid[5] != 1 /* pkcs-1 */ ||
    ...
    {
        asn1_unexpected(&hdr, "PKCS #8: Expected OCTETSTRING (privateKey)");
        return NULL;
    }
    wpa_printf(MSG_DEBUG, "PKCS #8: Try to parse RSAPrivateKey");

    return (struct crypto_private_key *)crypto_rsa_import_private_key(hdr.payload, hdr.length);
}
...
struct crypto_private_key *pkcs8_enc_key_import(const u8 *buf, size_t len, const char *passwd)
{
    ...
    {
        asn1_unexpected(&hdr, "PKCS #8: Expected SEQUENCE (AlgorithmIdentifier); assume encrypted PKCS #8 not used");
        return NULL;
    }
    enc_alg     = hdr.payload;
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_octetstring(&hdr))
    {
        asn1_unexpected(&hdr, "PKCS #8: Expected OCTETSTRING (encryptedData)");
        return NULL;
    }
