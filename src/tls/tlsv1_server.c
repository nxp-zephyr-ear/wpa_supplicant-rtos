...
int tlsv1_server_derive_keys(struct tlsv1_server *conn, const u8 *pre_master_secret, size_t pre_master_secret_len)
{
    ...
    if (pre_master_secret)
    {
        ...
        if (tls_prf(conn->rl.tls_version, pre_master_secret, pre_master_secret_len, "master secret", seed,
                    2 * TLS_RANDOM_LEN, conn->master_secret, TLS_MASTER_SECRET_LEN))
        {
    ...
    if (tls_prf(conn->rl.tls_version, conn->master_secret, TLS_MASTER_SECRET_LEN, "key expansion", seed,
                2 * TLS_RANDOM_LEN, key_block, key_block_len))
    {
...
int tlsv1_server_decrypt(struct tlsv1_server *conn, const u8 *in_data, size_t in_len, u8 *out_data, size_t out_len)
{
    ...
    while (pos < in_end)
    {
        ...
        if ((conn->test_flags & (TLS_BREAK_VERIFY_DATA | TLS_BREAK_SRV_KEY_X_HASH | TLS_BREAK_SRV_KEY_X_SIGNATURE)) &&
                     ...
                     size_t out_len)
{
    ...
    res =
        tls_prf(conn->rl.tls_version, conn->master_secret, TLS_MASTER_SECRET_LEN, label, seed, seed_len, out, out_len);
    os_free(seed);
    return res;
}
...
static const u8 test_tls_prime511b[64] = {0x50, 0xfb, 0xf1, 0xae, 0x01, 0xf1, 0xfe, 0xe6, 0xe1, 0xae, 0xdc, 0x1e, 0xbe,
                                          0xfb, 0x9e, 0x58, 0x9a, 0xd7, 0x54, 0x9d, 0x6b, 0xb3, 0x78, 0xe2, 0x39, 0x7f,
                                          0x30, 0x01, 0x25, 0xa1, 0xf9, 0x7c, 0x55, 0x0e, 0xa1, 0x15, 0xcc, 0x36, 0x34,
                                          0xbb, 0x6c, 0x8b, 0x64, 0x45, 0x15, 0x7f, 0xd3, 0xe7, 0x31, 0xc8, 0x8e, 0x56,
                                          0x8e, 0x95, 0xdc, 0xea, 0x9e, 0xdf, 0xf7, 0x56, 0xdd, 0xb0, 0x34, 0xdb};

static const u8 test_tls_prime767b[96] = {
    0x4c, 0xdc, 0xb8, 0x21, 0x20, 0x9d, 0xe8, 0xa3, 0x53, 0xd9, 0x1c, 0x18, 0xc1, 0x3a, 0x58, 0x67,
    0xa7, 0x85, 0xf9, 0x28, 0x9b, 0xce, 0xc0, 0xd1, 0x05, 0x84, 0x61, 0x97, 0xb2, 0x86, 0x1c, 0xd0,
    0xd1, 0x96, 0x23, 0x29, 0x8c, 0xc5, 0x30, 0x68, 0x3e, 0xf9, 0x05, 0xba, 0x60, 0xeb, 0xdb, 0xee,
    0x2d, 0xdf, 0x84, 0x65, 0x49, 0x87, 0x90, 0x2a, 0xc9, 0x8e, 0x34, 0x63, 0x6d, 0x9a, 0x2d, 0x32,
    0x1c, 0x46, 0xd5, 0x4e, 0x20, 0x20, 0x90, 0xac, 0xd5, 0x48, 0x79, 0x99, 0x0c, 0xe6, 0xed, 0xbf,
    0x79, 0xc2, 0x47, 0x50, 0x95, 0x38, 0x38, 0xbc, 0xde, 0xb0, 0xd2, 0xe8, 0x97, 0xcb, 0x22, 0xbb};
...
static const u8 test_tls_prime58[128] = {
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0xc1, 0xba, 0xc8, 0x25, 0xbe, 0x2d, 0xf3};
...
static const u8 test_tls_non_prime[] = {
     ...
     * 3
     */
    0x0C, 0x8C, 0x36, 0x9C, 0x6F, 0x71, 0x2E, 0xA7, 0xAB, 0x32, 0xD3, 0x0F, 0x68, 0x3D, 0xB2, 0x6D,
    0x81, 0xDD, 0xC4, 0x84, 0x0D, 0x9C, 0x6E, 0x36, 0x29, 0x70, 0xF3, 0x1E, 0x9A, 0x42, 0x0B, 0x67,
    0x82, 0x6B, 0xB1, 0xF2, 0xAF, 0x55, 0x28, 0xE7, 0xDB, 0x67, 0x6C, 0xF7, 0x6B, 0xAC, 0xAC, 0xE5,
    0xF7, 0x9F, 0xD4, 0x63, 0x55, 0x70, 0x32, 0x7C, 0x70, 0xFB, 0xAF, 0xB8, 0xEB, 0x37, 0xCF, 0x3F,
    0xFE, 0x94, 0x73, 0xF9, 0x7A, 0xC7, 0x12, 0x2E, 0x9B, 0xB4, 0x7D, 0x08, 0x60, 0x83, 0x43, 0x52,
    0x83, 0x1E, 0xA5, 0xFC, 0xFA, 0x87, 0x12, 0xF4, 0x64, 0xE2, 0xCE, 0x71, 0x17, 0x72, 0xB6, 0xAB};
