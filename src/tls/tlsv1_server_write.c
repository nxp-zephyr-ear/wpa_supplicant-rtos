...
static int tls_write_server_hello(struct tlsv1_server *conn, u8 **msgpos, u8 *end)
{
    ...
    *pos++ = TLS_COMPRESSION_NULL;
    ...
    {
        int res = conn->session_ticket_cb(conn->session_ticket_cb_ctx, conn->session_ticket, conn->session_ticket_len,
                                          conn->client_random, conn->server_random, conn->master_secret);
        if (res < 0)
        {
...
static int tls_write_server_certificate_status(
    struct tlsv1_server *conn, u8 **msgpos, u8 *end, int ocsp_multi, char *ocsp_resp, size_t ocsp_resp_len)
{
    ...
    pos = *msgpos;
    if (TLS_RECORD_HEADER_LEN + 1 + 3 + 1 + 3 + ocsp_resp_len > (unsigned int)(end - pos))
    {
        tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
        return -1;
    }
...
static int tls_write_server_key_exchange(struct tlsv1_server *conn, u8 **msgpos, u8 *end)
{
    ...
    if (crypto_mod_exp(conn->cred->dh_g, conn->cred->dh_g_len, conn->dh_secret, conn->dh_secret_len, dh_p, dh_p_len,
                       dh_ys, &dh_ys_len))
    {
        tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
        os_free(dh_ys);
    ...
    *pos++ = TLS_HANDSHAKE_TYPE_SERVER_KEY_EXCHANGE;
    ...
    if (keyx == TLS_KEY_X_DHE_RSA)
    {
        ...
        {
#ifdef CONFIG_TLSV12
            hlen = tlsv12_key_x_server_params_hash(conn->rl.tls_version, TLS_HASH_ALG_SHA256, conn->client_random,
...
static int tls_write_server_hello_done(struct tlsv1_server *conn, u8 **msgpos, u8 *end)
{
    ...
    *pos++ = TLS_HANDSHAKE_TYPE_SERVER_HELLO_DONE;
    ...
    if (tlsv1_record_send(&conn->rl, TLS_CONTENT_TYPE_HANDSHAKE, *msgpos, end - *msgpos, payload, pos - payload,
                          &rlen) < 0)
    {
        wpa_printf(MSG_DEBUG, "TLSv1: Failed to generate a record");
        tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
...
static int tls_write_server_change_cipher_spec(struct tlsv1_server *conn, u8 **msgpos, u8 *end)
{
    ...
    if (tlsv1_record_send(&conn->rl, TLS_CONTENT_TYPE_CHANGE_CIPHER_SPEC, *msgpos, end - *msgpos, payload,
                          sizeof(payload), &rlen) < 0)
    {
...
static int tls_write_server_finished(struct tlsv1_server *conn, u8 **msgpos, u8 *end)
{
    ...
    *pos++ = TLS_HANDSHAKE_TYPE_FINISHED;
    ...
    if (tlsv1_record_send(&conn->rl, TLS_CONTENT_TYPE_HANDSHAKE, *msgpos, end - *msgpos, hs_start, pos - hs_start,
                          &rlen) < 0)
    {
        wpa_printf(MSG_DEBUG, "TLSv1: Failed to create a record");
        tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
...
static u8 *tls_send_server_hello(struct tlsv1_server *conn, size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    {
        ocsp_resp  = os_readfile(conn->cred->ocsp_stapling_response_multi, &ocsp_resp_len);
        ocsp_multi = 1;
    }
    else if ((conn->status_request || conn->status_request_v2) && conn->cred->ocsp_stapling_response)
    {
    ...
    if (tls_write_server_certificate(conn, &pos, end) < 0 ||
        tls_write_server_certificate_status(conn, &pos, end, ocsp_multi, ocsp_resp, ocsp_resp_len) < 0 ||
        tls_write_server_key_exchange(conn, &pos, end) < 0 ||
        tls_write_server_certificate_request(conn, &pos, end) < 0 || tls_write_server_hello_done(conn, &pos, end) < 0)
    {
        os_free(msg);
        os_free(ocsp_resp);
        return NULL;
