...
static int testing_cipher_suite_filter(struct tlsv1_server *conn, u16 suite)
{
    ...
    if ((conn->test_flags & (TLS_BREAK_SRV_KEY_X_HASH | TLS_BREAK_SRV_KEY_X_SIGNATURE | TLS_DHE_PRIME_511B |
                             TLS_DHE_PRIME_767B | TLS_DHE_PRIME_15 | TLS_DHE_PRIME_58B | TLS_DHE_NON_PRIME)) &&
        suite != TLS_DHE_RSA_WITH_AES_256_CBC_SHA256 && suite != TLS_DHE_RSA_WITH_AES_256_CBC_SHA &&
        suite != TLS_DHE_RSA_WITH_AES_128_CBC_SHA256 && suite != TLS_DHE_RSA_WITH_AES_128_CBC_SHA &&
...
static void tls_process_status_request_item(struct tlsv1_server *conn, const u8 *req, size_t req_len)
{
    const u8 *pos, *end;
    u8 status_type;
...
static void tls_process_status_request_v2(struct tlsv1_server *conn, const u8 *ext, size_t ext_len)
{
    const u8 *pos, *end;
...
static int tls_process_client_hello(struct tlsv1_server *conn, u8 ct, const u8 *in_data, size_t *in_len)
{
    ...
    if (left < 4)
    {
        tlsv1_server_log(conn, "Truncated handshake message (expected ClientHello)");
        goto decode_error;
    }
    ...
    if (len > left)
    {
        tlsv1_server_log(conn, "Truncated ClientHello (len=%d left=%d)", (int)len, (int)left);
        goto decode_error;
    }
    ...
    {
        tlsv1_server_log(conn, "Unexpected protocol version in ClientHello %u.%u", conn->client_version >> 8,
    ...
    if (end - pos < 2)
    {
        tlsv1_server_log(conn, "Truncated ClientHello/cipher_suites len");
        goto decode_error;
    }
    ...
    if (end - pos < 1)
    {
        tlsv1_server_log(conn, "Truncated ClientHello/compression_methods len");
        goto decode_error;
    }
    num_suites = *pos++;
    if (end - pos < num_suites)
    {
        tlsv1_server_log(conn, "Truncated ClientHello/compression_methods");
        goto decode_error;
    }
    ...
    if (end - pos >= 2)
    {
        ...
        if (end - pos != ext_len)
        {
            tlsv1_server_log(conn, "Invalid ClientHello extension list length %u (expected %u)", ext_len,
...
static int tls_process_certificate(struct tlsv1_server *conn, u8 ct, const u8 *in_data, size_t *in_len)
{
    ...
    if (len > left)
    {
        tlsv1_server_log(conn, "Unexpected Certificate message length (len=%lu != left=%lu)", (unsigned long)len,
                         (unsigned long)left);
        tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
        return -1;
    }
    ...
    {
        tlsv1_server_log(conn, "Unexpected certificate_list length (len=%lu left=%lu)", (unsigned long)list_len,
    ...
    while (pos < end)
    {
        ...
        {
            tlsv1_server_log(conn, "Unexpected certificate length (len=%lu left=%lu)", (unsigned long)cert_len,
                             (unsigned long)(end - pos));
            tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
            x509_certificate_chain_free(chain);
            return -1;
    ...
    if (chain && (chain->extensions_present & X509_EXT_EXT_KEY_USAGE) &&
        !(chain->ext_key_usage & (X509_EXT_KEY_USAGE_ANY | X509_EXT_KEY_USAGE_CLIENT_AUTH)))
    {
        tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_BAD_CERTIFICATE);
        x509_certificate_chain_free(chain);
        return -1;
...
static int tls_process_client_key_exchange_rsa(struct tlsv1_server *conn, const u8 *pos, const u8 *end)
{
    ...
    if (pos + encr_len > end)
    {
        tlsv1_server_log(conn, "Invalid ClientKeyExchange format: encr_len=%u left=%u", encr_len,
                         (unsigned int)(end - pos));
        tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
        return -1;
    }
...
static int tls_process_client_key_exchange(struct tlsv1_server *conn, u8 ct, const u8 *in_data, size_t *in_len)
{
    ...
    if (len > left)
    {
        tlsv1_server_log(conn, "Mismatch in ClientKeyExchange length (len=%lu != left=%lu)", (unsigned long)len,
                         (unsigned long)left);
        tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
        return -1;
    }
...
static int tls_process_certificate_verify(struct tlsv1_server *conn, u8 ct, const u8 *in_data, size_t *in_len)
{
    ...
    if (len > left)
    {
        tlsv1_server_log(conn, "Unexpected CertificateVerify message length (len=%lu != left=%lu)", (unsigned long)len,
                         (unsigned long)left);
        tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
        return -1;
    }
...
static int tls_process_client_finished(struct tlsv1_server *conn, u8 ct, const u8 *in_data, size_t *in_len)
{
    ...
    if (len > left)
    {
        tlsv1_server_log(conn, "Too short buffer for Finished (len=%lu > left=%lu)", (unsigned long)len,
                         (unsigned long)left);
        tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
        return -1;
    }
    ...
    {
        tlsv1_server_log(conn, "Unexpected verify_data length in Finished: %lu (expected %d)", (unsigned long)len,
                         TLS_VERIFY_DATA_LEN);
        tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
        return -1;
    }
    ...
    {
        hlen = SHA256_MAC_LEN;
        if (conn->verify.sha256_client == NULL || crypto_hash_finish(conn->verify.sha256_client, hash, &hlen) < 0)
        {
            tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
    ...
    else
    {
        ...
        if (conn->verify.sha1_client == NULL ||
            crypto_hash_finish(conn->verify.sha1_client, hash + MD5_MAC_LEN, &hlen) < 0)
        {
            conn->verify.sha1_client = NULL;
            tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
            return -1;
    ...
    if (tls_prf(conn->rl.tls_version, conn->master_secret, TLS_MASTER_SECRET_LEN, "client finished", hash, hlen,
