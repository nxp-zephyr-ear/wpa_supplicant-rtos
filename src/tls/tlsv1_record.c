                      ...
                      size_t *out_len)
{
    ...
    *pos++   = content_type;
    ...
    cpayload = pos;
    explicit_iv =
        rl->write_cipher_suite != TLS_NULL_WITH_NULL_NULL && rl->iv_size && rl->tls_version >= TLS_VERSION_1_1;
    if (explicit_iv)
    {
    ...
    struct tlsv1_record_layer *rl, const u8 *in_data, size_t in_len, u8 *out_data, size_t *out_len, u8 *alert)
{
    ...
    if (ct != TLS_CONTENT_TYPE_HANDSHAKE && ct != TLS_CONTENT_TYPE_CHANGE_CIPHER_SPEC && ct != TLS_CONTENT_TYPE_ALERT &&
