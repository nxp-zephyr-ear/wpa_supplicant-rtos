...
{
    return (((conn->flags & TLS_CONN_DISABLE_TLSv1_0) && ver == TLS_VERSION_1) ||
            ((conn->flags & TLS_CONN_DISABLE_TLSv1_1) && ver == TLS_VERSION_1_1) ||
            ((conn->flags & TLS_CONN_DISABLE_TLSv1_2) && ver == TLS_VERSION_1_2));
}

static int tls_process_server_hello_extensions(struct tlsv1_client *conn, const u8 *pos, size_t len)
{
...
static int tls_process_server_hello(struct tlsv1_client *conn, u8 ct, const u8 *in_data, size_t *in_len)
{
    ...
    if (end - pos >= 2)
    {
        ...
        if (end - pos < ext_len)
        {
            wpa_printf(MSG_INFO, "TLSv1: Invalid ServerHello extension length: %u (left: %u)", ext_len,
    ...
    if (conn->session_ticket_included && conn->session_ticket_cb)
    {
         ...
         * ServerHello */
        int res = conn->session_ticket_cb(conn->session_ticket_cb_ctx, NULL, 0, conn->client_random,
                                          conn->server_random, conn->master_secret);
        if (res < 0)
        {
            wpa_printf(MSG_DEBUG,
...
static void tls_peer_cert_event(struct tlsv1_client *conn, int depth, struct x509_certificate *cert)
{
    ...
    if (cert->extensions_present & X509_EXT_CERTIFICATE_POLICY)
    {
        if (cert->certificate_policy & X509_EXT_CERT_POLICY_TOD_STRICT)
            ev.peer_cert.tod = 1;
        else if (cert->certificate_policy & X509_EXT_CERT_POLICY_TOD_TOFU)
            ev.peer_cert.tod = 2;
    }
...
static void tls_cert_chain_failure_event(struct tlsv1_client *conn,
                                         int depth,
...
static int tls_process_certificate(struct tlsv1_client *conn, u8 ct, const u8 *in_data, size_t *in_len)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "TLSv1: Reject server certificate on probe-only run");
        if (conn->event_cb)
        {
            union tls_event_data ev;
            char buf[128];
        ...
        return -1;
             ...
             x509_certificate_chain_validate(conn->cred->trusted_certs, chain, &reason,
                                             !!(conn->flags & TLS_CONN_DISABLE_TIME_CHECKS)) < 0)
    {
        ...
        switch (reason)
        {
            case X509_VALIDATE_BAD_CERTIFICATE:
                ...
                tls_cert_chain_failure_event(conn, 0, chain, TLS_FAIL_BAD_CERTIFICATE, "bad certificate");
                break;
            ...
            case X509_VALIDATE_CERTIFICATE_REVOKED:
                ...
                tls_cert_chain_failure_event(conn, 0, chain, TLS_FAIL_REVOKED, "certificate revoked");
                break;
            case X509_VALIDATE_CERTIFICATE_EXPIRED:
                tls_reason = TLS_ALERT_CERTIFICATE_EXPIRED;
                tls_cert_chain_failure_event(conn, 0, chain, TLS_FAIL_EXPIRED,
            ...
            case X509_VALIDATE_UNKNOWN_CA:
                ...
                tls_cert_chain_failure_event(conn, 0, chain, TLS_FAIL_UNTRUSTED, "unknown CA");
                break;
            default:
                tls_reason = TLS_ALERT_BAD_CERTIFICATE;
                break;
    ...
    if (conn->cred && !conn->cred->server_cert_only && chain && (chain->extensions_present & X509_EXT_EXT_KEY_USAGE) &&
        !(chain->ext_key_usage & (X509_EXT_KEY_USAGE_ANY | X509_EXT_KEY_USAGE_SERVER_AUTH)))
    {
                                        ...
                                        tls_key_exchange key_exchange)
{
    ...
    if (key_exchange == TLS_KEY_X_DHE_RSA)
    {
        ...
        if (conn->rl.tls_version == TLS_VERSION_1_2)
        {
            ...
            if (end - pos < 2)
                goto fail;
            if ((pos[0] != TLS_HASH_ALG_SHA256 && pos[0] != TLS_HASH_ALG_SHA384 && pos[0] != TLS_HASH_ALG_SHA512) ||
            ...
            hlen =
                tlsv12_key_x_server_params_hash(conn->rl.tls_version, pos[0], conn->client_random, conn->server_random,
        ...
        {
            hlen = tls_key_x_server_params_hash(conn->rl.tls_version, conn->client_random, conn->server_random,
                                                server_params, server_params_end - server_params, hash);
        }
...
static enum tls_ocsp_result tls_process_certificate_status_ocsp_response(struct tlsv1_client *conn,
                                                                         const u8 *pos,
                                                                         size_t len)
{
    const u8 *end = pos + len;
...
static int tls_process_certificate_status(struct tlsv1_client *conn, u8 ct, const u8 *in_data, size_t *in_len)
{
    const u8 *pos, *end;
    size_t left, len;
    u8 type, status_type;
    ...
    if (left < 4)
    {
        wpa_printf(MSG_DEBUG, "TLSv1: Too short CertificateStatus (left=%lu)", (unsigned long)left);
        tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
        return -1;
    }
    ...
    if (len > left)
    {
        wpa_printf(MSG_DEBUG, "TLSv1: Mismatch in CertificateStatus length (len=%lu != left=%lu)", (unsigned long)len,
                   (unsigned long)left);
        tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
        return -1;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "TLSv1: Received unexpected handshake message %d (expected CertificateStatus)", type);
        tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_UNEXPECTED_MESSAGE);
        return -1;
    }
    ...
    else if (status_type == 2 /* ocsp_multi */)
    {
        ...
        if (end - pos < resp_len)
        {
            wpa_printf(MSG_DEBUG, "TLSv1: Truncated OCSPResponseList(len=%u)", resp_len);
            res = TLS_OCSP_INVALID;
            goto done;
        }
        end = pos + resp_len;
        ...
        while (end - pos >= 3)
        {
            ...
            if (resp_len > end - pos)
            {
                wpa_printf(MSG_DEBUG, "TLSv1: Truncated OCSPResponse(len=%u; left=%d) in ocsp_multi", resp_len,
                           (int)(end - pos));
                res = TLS_OCSP_INVALID;
                break;
            }
            ...
            res = tls_process_certificate_status_ocsp_response(conn, pos - 3, resp_len + 3);
    ...
    if (res == TLS_OCSP_REVOKED)
    {
        ...
        for (cert = conn->server_cert, depth = 0; cert; cert = cert->next, depth++)
        {
            if (cert->ocsp_revoked)
            {
                tls_cert_chain_failure_event(conn, depth, cert, TLS_FAIL_REVOKED, "certificate revoked");
            }
        }
        return -1;
    }
    ...
    if (conn->flags & TLS_CONN_REQUIRE_OCSP_ALL)
    {
        ...
        for (cert = conn->server_cert, depth = 0; cert; cert = cert->next, depth++)
        {
            if (!cert->ocsp_good)
            {
                tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_BAD_CERTIFICATE_STATUS_RESPONSE);
    ...
    if ((conn->flags & TLS_CONN_REQUIRE_OCSP) && res != TLS_OCSP_GOOD)
    {
        tls_alert(conn, TLS_ALERT_LEVEL_FATAL,
                  res == TLS_OCSP_INVALID ? TLS_ALERT_DECODE_ERROR : TLS_ALERT_BAD_CERTIFICATE_STATUS_RESPONSE);
        if (conn->server_cert)
            tls_cert_chain_failure_event(conn, 0, conn->server_cert, TLS_FAIL_UNSPECIFIED,
                                         "bad certificate status response");
        return -1;
    }
...
static int tls_process_server_key_exchange(struct tlsv1_client *conn, u8 ct, const u8 *in_data, size_t *in_len)
{
    ...
    if ((conn->flags & TLS_CONN_REQUEST_OCSP) && type == TLS_HANDSHAKE_TYPE_CERTIFICATE_STATUS)
    ...
    if (type != TLS_HANDSHAKE_TYPE_SERVER_KEY_EXCHANGE)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   type, (conn->flags & TLS_CONN_REQUEST_OCSP) ? "/CertificateStatus" : "");
        tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_UNEXPECTED_MESSAGE);
        return -1;
    }
...
static int tls_process_server_finished(struct tlsv1_client *conn, u8 ct, const u8 *in_data, size_t *in_len)
{
    ...
    {
        hlen = SHA256_MAC_LEN;
        if (conn->verify.sha256_server == NULL || crypto_hash_finish(conn->verify.sha256_server, hash, &hlen) < 0)
        {
            tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
    ...
    else
    {
        ...
        if (conn->verify.sha1_server == NULL ||
            crypto_hash_finish(conn->verify.sha1_server, hash + MD5_MAC_LEN, &hlen) < 0)
        {
            conn->verify.sha1_server = NULL;
            tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
            return -1;
    ...
    if (tls_prf(conn->rl.tls_version, conn->master_secret, TLS_MASTER_SECRET_LEN, "server finished", hash, hlen,
...
static int tls_process_application_data(
    struct tlsv1_client *conn, u8 ct, const u8 *in_data, size_t *in_len, u8 **out_data, size_t *out_len)
{
    const u8 *pos;
    size_t left;
