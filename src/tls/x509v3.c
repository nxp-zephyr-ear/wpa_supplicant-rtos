...
int x509_parse_algorithm_identifier(const u8 *buf, size_t len, struct x509_algorithm_identifier *id, const u8 **next)
{
    ...
    {
        asn1_unexpected(&hdr, "X509: Expected SEQUENCE (AlgorithmIdentifier)");
        return -1;
    }
...
static int x509_parse_public_key(const u8 *buf, size_t len, struct x509_certificate *cert, const u8 **next)
{
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "X509: Expected SEQUENCE (SubjectPublicKeyInfo)");
        return -1;
    }
    pos = hdr.payload;
    ...
    *next = end;
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_bitstring(&hdr))
    {
        asn1_unexpected(&hdr, "X509: Expected BITSTRING (subjectPublicKey)");
        return -1;
    }
    if (hdr.length < 1)
        return -1;
    ...
    if (*pos)
    {
        wpa_printf(MSG_DEBUG, "X509: BITSTRING (subjectPublicKey) - %d unused bits", *pos);
        /*
...
int x509_parse_name(const u8 *buf, size_t len, struct x509_name *name, const u8 **next)
{
    ...
    if (asn1_get_next(buf, len, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "X509: Expected SEQUENCE (Name / RDNSequencer)");
        return -1;
    }
    pos = hdr.payload;
    ...
    while (pos < end)
    {
        ...
        {
            asn1_unexpected(&hdr, "X509: Expected SET (RelativeDistinguishedName)");
            x509_free_name(name);
            return -1;
        }
        ...
        if (asn1_get_next(set_pos, set_end - set_pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
        {
            asn1_unexpected(&hdr, "X509: Expected SEQUENCE (AttributeTypeAndValue)");
            x509_free_name(name);
            return -1;
        }
        ...
        {
            wpa_printf(MSG_DEBUG, "X509: Ignore non-string type attribute (tag 0x%x)", hdr.tag);
            continue;
        }
        ...
        else if (oid.len == 7 && oid.oid[0] == 1 && oid.oid[1] == 2 && oid.oid[2] == 840 && oid.oid[3] == 113549 &&
                 oid.oid[4] == 1 && oid.oid[5] == 9 && oid.oid[6] == 1)
        {
        ...
        else if (oid.len == 7 && oid.oid[0] == 0 && oid.oid[1] == 9 && oid.oid[2] == 2342 && oid.oid[3] == 19200300 &&
...
static int x509_parse_validity(const u8 *buf, size_t len, struct x509_certificate *cert, const u8 **next)
{
    ...
    *next = pos + plen;
    ...
    if (asn1_get_next(pos, plen, &hdr) < 0 || (!asn1_is_utctime(&hdr) && !asn1_is_generalizedtime(&hdr)) ||
        x509_parse_time(hdr.payload, hdr.length, hdr.tag, &cert->not_before) < 0)
    {
        wpa_hexdump_ascii(MSG_DEBUG,
    ...
    wpa_printf(MSG_MSGDUMP, "X509: Validity: notBefore: %lu notAfter: %lu", (unsigned long)cert->not_before,
...
static int x509_id_ce_oid(struct asn1_oid *oid)
{
    ...
    return oid->len >= 4 && oid->oid[0] == 2 /* joint-iso-ccitt */ && oid->oid[1] == 5 /* ds */ &&
           oid->oid[2] == 29 /* id-ce */;
}

static int x509_any_ext_key_usage_oid(struct asn1_oid *oid)
{
    return oid->len == 6 && x509_id_ce_oid(oid) && oid->oid[3] == 37 /* extKeyUsage */ &&
...
static int x509_parse_ext_key_usage(struct x509_certificate *cert, const u8 *pos, size_t len)
{
    ...
    if (asn1_get_next(pos, len, &hdr) < 0 || !asn1_is_bitstring(&hdr) || hdr.length < 1)
    {
...
static int x509_parse_ext_basic_constraints(struct x509_certificate *cert, const u8 *pos, size_t len)
{
    ...
    {
        asn1_unexpected(&hdr, "X509: Expected SEQUENCE in BasicConstraints");
        return -1;
    }
    ...
    {
        asn1_unexpected(&hdr, "X509: Expected INTEGER in BasicConstraints");
        return -1;
    }
...
static int x509_parse_ext_subject_alt_name(struct x509_certificate *cert, const u8 *pos, size_t len)
{
    ...
    if (asn1_get_next(pos, len, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "X509: Expected SEQUENCE in SubjectAltName");
        return -1;
    }
...
static int x509_parse_ext_issuer_alt_name(struct x509_certificate *cert, const u8 *pos, size_t len)
{
    ...
    if (asn1_get_next(pos, len, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "X509: Expected SEQUENCE in IssuerAltName");
        return -1;
    }
...
{
    return oid->len == 5 && oid->oid[0] == 2 /* iso/itu-t */ && oid->oid[1] == 5 /* X.500 Directory Services */ &&
           oid->oid[2] == 29 /* id-ce */ && oid->oid[3] == 32 /* id-ce-certificate-policies */ &&
           oid->oid[4] == 0 /* anyPolicy */;
}

static int x509_id_wfa_oid(struct asn1_oid *oid)
{
    return oid->len >= 7 && oid->oid[0] == 1 /* iso */ && oid->oid[1] == 3 /* identified-organization */ &&
           oid->oid[2] == 6 /* dod */ && oid->oid[3] == 1 /* internet */ && oid->oid[4] == 4 /* private */ &&
           oid->oid[5] == 1 /* enterprise */ && oid->oid[6] == 40808 /* WFA */;
}

static int x509_id_wfa_tod_oid(struct asn1_oid *oid)
{
    return oid->len >= 9 && x509_id_wfa_oid(oid) && oid->oid[7] == 1 && oid->oid[8] == 3;
}

static int x509_id_wfa_tod_strict_oid(struct asn1_oid *oid)
{
    return oid->len == 10 && x509_id_wfa_tod_oid(oid) && oid->oid[9] == 1;
}

static int x509_id_wfa_tod_tofu_oid(struct asn1_oid *oid)
{
    return oid->len == 10 && x509_id_wfa_tod_oid(oid) && oid->oid[9] == 2;
}

static int x509_parse_ext_certificate_policies(struct x509_certificate *cert, const u8 *pos, size_t len)
{
    struct asn1_hdr hdr;
    const u8 *end;
    ...
    if (asn1_get_next(pos, len, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "X509: Expected SEQUENCE (certificatePolicies)");
        return -1;
    }
    ...
    while (pos < end)
    {
        ...
        if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_sequence(&hdr))
        {
            asn1_unexpected(&hdr, "X509: Expected SEQUENCE (PolicyInformation)");
            return -1;
        }
        ...
        wpa_hexdump(MSG_MSGDUMP, "X509: PolicyInformation", pos, pol_end - pos);
        ...
        if (x509_id_cert_policy_any_oid(&oid))
        {
            ...
            cert->certificate_policy |= X509_EXT_CERT_POLICY_ANY;
        }
        else if (x509_id_wfa_tod_strict_oid(&oid))
        {
            os_strlcpy(buf, "TOD-STRICT", sizeof(buf));
            cert->certificate_policy |= X509_EXT_CERT_POLICY_TOD_STRICT;
        }
        else if (x509_id_wfa_tod_tofu_oid(&oid))
        {
            os_strlcpy(buf, "TOD-TOFU", sizeof(buf));
            cert->certificate_policy |= X509_EXT_CERT_POLICY_TOD_TOFU;
        }
        else
        {
...
{
    return oid->len >= 7 && oid->oid[0] == 1 /* iso */ && oid->oid[1] == 3 /* identified-organization */ &&
           oid->oid[2] == 6 /* dod */ && oid->oid[3] == 1 /* internet */ && oid->oid[4] == 5 /* security */ &&
           oid->oid[5] == 5 /* mechanisms */ && oid->oid[6] == 7 /* id-pkix */;
}
...
{
    /* id-kp */
    return oid->len == 9 && x509_id_kp_oid(oid) && oid->oid[8] == 1 /* id-kp-serverAuth */;
}

static int x509_id_kp_client_auth_oid(struct asn1_oid *oid)
{
    /* id-kp */
    return oid->len == 9 && x509_id_kp_oid(oid) && oid->oid[8] == 2 /* id-kp-clientAuth */;
}

static int x509_id_kp_ocsp_oid(struct asn1_oid *oid)
{
    /* id-kp */
    return oid->len == 9 && x509_id_kp_oid(oid) && oid->oid[8] == 9 /* id-kp-OCSPSigning */;
}

static int x509_parse_ext_ext_key_usage(struct x509_certificate *cert, const u8 *pos, size_t len)
{
    struct asn1_hdr hdr;
    const u8 *end;
    struct asn1_oid oid;
    ...
    if (asn1_get_next(pos, len, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "X509: Expected SEQUENCE (ExtKeyUsageSyntax)");
        return -1;
    }
    if (hdr.length > pos + len - hdr.payload)
        return -1;
...
static int x509_parse_extension(struct x509_certificate *cert, const u8 *pos, size_t len, const u8 **next)
{
    ...
    *next = end = pos + hdr.length;
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || (!asn1_is_boolean(&hdr) && !asn1_is_octetstring(&hdr)))
    {
        asn1_unexpected(&hdr, "X509: Expected BOOLEAN or OCTETSTRING in Extensions");
        return -1;
    }
    ...
    if (hdr.tag == ASN1_TAG_BOOLEAN)
    {
        ...
        if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
            (!asn1_is_octetstring(&hdr) && !(hdr.class == ASN1_CLASS_PRIVATE && hdr.tag == ASN1_TAG_OCTETSTRING)))
        {
            asn1_unexpected(&hdr, "X509: Expected OCTETSTRING in Extensions");
            return -1;
        }
    }
...
static int x509_parse_tbs_certificate(const u8 *buf, size_t len, struct x509_certificate *cert, const u8 **next)
{
    ...
    if (asn1_get_next(buf, len, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "X509: tbsCertificate did not start with a valid SEQUENCE");
        return -1;
    }
    pos = hdr.payload;
    end = *next = pos + hdr.length;
    ...
    if (asn1_is_cs_tag(&hdr, 0) && hdr.constructed)
    {
        ...
        {
            asn1_unexpected(&hdr, "X509: No INTEGER tag found for version field");
            return -1;
        }
        if (hdr.length != 1)
        {
    ...
    if (!asn1_is_integer(&hdr) || hdr.length < 1 || hdr.length > X509_MAX_SERIAL_NUM_LEN)
    {
        asn1_unexpected(&hdr, "X509: No INTEGER tag found for serialNumber");
        return -1;
    }
    ...
    wpa_hexdump(MSG_MSGDUMP, "X509: serialNumber", cert->serial_number, cert->serial_number_len);
    ...
    {
        asn1_unexpected(&hdr, "X509: Expected Context-Specific tag to parse optional tbsCertificate field(s)");
        return -1;
    }
...
{
    return oid->len >= 4 && oid->oid[0] == 1 /* iso */ && oid->oid[1] == 2 /* member-body */ &&
           oid->oid[2] == 840 /* us */ && oid->oid[3] == 113549 /* rsadsi */;
}
...
{
    return oid->len == 6 && oid->oid[0] == 1 /* iso */ && oid->oid[1] == 3 /* identified-organization */ &&
           oid->oid[2] == 14 /* oiw */ && oid->oid[3] == 3 /* secsig */ && oid->oid[4] == 2 /* algorithms */ &&
           oid->oid[5] == 26 /* id-sha1 */;
}

static int x509_sha2_oid(struct asn1_oid *oid)
{
    return oid->len == 9 && oid->oid[0] == 2 /* joint-iso-itu-t */ && oid->oid[1] == 16 /* country */ &&
           oid->oid[2] == 840 /* us */ && oid->oid[3] == 1 /* organization */ && oid->oid[4] == 101 /* gov */ &&
           oid->oid[5] == 3 /* csor */ && oid->oid[6] == 4 /* nistAlgorithm */ && oid->oid[7] == 2 /* hashAlgs */;
}
...
struct x509_certificate *x509_certificate_parse(const u8 *buf, size_t len)
{
    ...
    if (asn1_get_next(pos, len, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
        asn1_unexpected(&hdr, "X509: Certificate did not start with a valid SEQUENCE");
        x509_certificate_free(cert);
        return NULL;
    }
    pos = hdr.payload;
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_bitstring(&hdr))
    {
        asn1_unexpected(&hdr, "X509: Expected BITSTRING (signatureValue)");
        x509_certificate_free(cert);
        return NULL;
    }
    if (hdr.length < 1)
    ...
    if (*pos)
    {
        wpa_printf(MSG_DEBUG, "X509: BITSTRING (signatureValue) - %d unused bits", *pos);
        /* PKCS #1 v1.5 10.2.1:
...
{
    return x509_check_signature(issuer, &cert->signature, cert->sign_value, cert->sign_value_len, cert->tbs_cert_start,
                                cert->tbs_cert_len);
}
                         ...
                         size_t signed_data_len)
{
    ...
    if (!x509_pkcs_oid(&signature->oid) || signature->oid.len != 7 || signature->oid.oid[5] != 1 /* pkcs-1 */)
    {
        wpa_printf(MSG_DEBUG,
    ...
    if (crypto_public_key_decrypt_pkcs1(pk, sign_value, sign_value_len, data, &data_len) < 0)
    {
     ...
     *
     */
    if (asn1_get_next(data, data_len, &hdr) < 0 || !asn1_is_sequence(&hdr))
    {
    ...
    wpa_hexdump(MSG_MSGDUMP, "X509: DigestAlgorithmIdentifier", hdr.payload, hdr.length);
    ...
    wpa_hexdump(MSG_MSGDUMP, "X509: Digest algorithm parameters", next, da_end - next);
     ...
     */
    if (da_end > next &&
        (asn1_get_next(next, da_end - next, &hdr) < 0 || !asn1_is_null(&hdr) || hdr.payload + hdr.length != da_end))
    {
        wpa_printf(MSG_DEBUG, "X509: Unexpected digest algorithm parameters");
        os_free(data);
        return -1;
    }
...
skip_digest_oid:
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 || !asn1_is_octetstring(&hdr))
    {
    ...
    {
        wpa_hexdump(MSG_INFO, "X509: Extra data after certificate signature hash", hdr.payload + hdr.length,
                                    ...
                                    int disable_time_checks)
{
    ...
    *reason = X509_VALIDATE_OK;
    ...
    for (cert = chain, idx = 0; cert; cert = cert->next, idx++)
    {
        ...
        if (!disable_time_checks && ((unsigned long)now.sec < (unsigned long)cert->not_before ||
