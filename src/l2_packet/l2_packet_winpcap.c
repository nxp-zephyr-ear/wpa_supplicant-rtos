...
static DWORD WINAPI l2_packet_receive_thread(LPVOID arg)
{
    ...
    while (l2->running)
    {
        ...
        if (l2->rx_no_wait > 0)
            l2->rx_no_wait--;
        if (WaitForSingleObject(l2->rx_notify, l2->rx_no_wait ? 0 : 50) == WAIT_OBJECT_0)
        {
            l2->rx_no_wait = no_wait_count;
...
static int l2_packet_init_libpcap(struct l2_packet_data *l2, unsigned short protocol)
{
    ...
    os_snprintf(pcap_filter, sizeof(pcap_filter),
                ...
                " and "
                "( ether dst " MACSTR " or ether dst " MACSTR
                " ) and "
                "ether proto 0x%x",
                                      ...
                                      int l2_hdr)
{
    ...
    l2->running   = 1;
    l2->rx_thread = CreateThread(NULL, 0, l2_packet_receive_thread, l2, 0, &thread_id);
...
static void l2_packet_deinit_timeout(void *eloop_ctx, void *timeout_ctx)
{
    ...
    if (l2->rx_thread_done && WaitForSingleObject(l2->rx_thread_done, 2000) != WAIT_OBJECT_0)
    {
        wpa_printf(MSG_DEBUG,
                   "l2_packet_winpcap: RX thread did not "
                   "exit - kill it\n");
