...
static int wpa_priv_cmd(struct l2_packet_data *l2, int cmd, const void *data, size_t data_len)
{
    struct msghdr msg;
    struct iovec io[2];
...
static void l2_packet_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    if (from.sun_family != AF_UNIX || os_strncmp(from.sun_path, l2->priv_addr.sun_path, sizeof(from.sun_path)) != 0)
    {
                                      ...
                                      int l2_hdr)
{
    ...
    os_snprintf(l2->own_socket_path, len, "%s/wpa_privsep-l2-%d-%d", own_dir, getpid(), counter++);

    l2->priv_addr.sun_family = AF_UNIX;
    os_snprintf(l2->priv_addr.sun_path, sizeof(l2->priv_addr.sun_path), "%s/%s", priv_dir, ifname);
