    ...
    if (!res)
    {
        ...
        if (err == ERROR_IO_PENDING)
        {
            wpa_printf(MSG_DEBUG,
                       ...
                       "write to complete");
            res = GetOverlappedResult(driver_ndis_get_ndisuio_handle(), &overlapped, &written, TRUE);
            if (!res)
            {
                wpa_printf(MSG_DEBUG,
                           "L2(NDISUIO): "
...
#endif /* _WIN32_WCE */
        wpa_printf(MSG_DEBUG, "L2(NDISUIO): WriteFile failed: %d", (int)GetLastError());
        return -1;
    }
...
static void l2_packet_rx_thread_try_read(struct l2_packet_data *l2)
{
    ...
    if (!ReadFile(driver_ndis_get_ndisuio_handle(), l2->rx_buf, sizeof(l2->rx_buf), &l2->rx_written, NULL))
    {
        DWORD err = GetLastError();
        wpa_printf(MSG_DEBUG,
    ...
    wpa_printf(MSG_DEBUG, "l2_packet_rx_thread: Read %d byte packet", (int)l2->rx_written);
...
static int l2_ndisuio_start_read(struct l2_packet_data *l2, int recursive)
{
    ...
    if (!ReadFile(driver_ndis_get_ndisuio_handle(), l2->rx_buf, sizeof(l2->rx_buf), &l2->rx_written,
                  &l2->rx_overlapped))
    {
        DWORD err = GetLastError();
        if (err != ERROR_IO_PENDING)
...
#ifndef _WIN32_WCE
    if (!GetOverlappedResult(driver_ndis_get_ndisuio_handle(), &l2->rx_overlapped, &l2->rx_written, FALSE))
    {
        wpa_printf(MSG_DEBUG,
...
static int l2_ndisuio_set_ether_type(unsigned short protocol)
{
    ...
    if (!DeviceIoControl(driver_ndis_get_ndisuio_handle(), IOCTL_NDISUIO_SET_ETHER_TYPE, &proto, sizeof(proto), NULL, 0,
                         &written, NULL))
    {
                                      ...
                                      int l2_hdr)
{
    ...
    eloop_register_event(l2->rx_avail, sizeof(l2->rx_avail), l2_packet_rx_event, l2, NULL);
     ...
     */
    l2_ndisuio_global->ready_for_read = CreateEvent(NULL, TRUE, FALSE, TEXT("WpaSupplicantConnected"));
    l2_ndisuio_global->rx_processed   = CreateEvent(NULL, TRUE, FALSE, NULL);
    if (l2_ndisuio_global->stop_request == NULL || l2_ndisuio_global->ready_for_read == NULL ||
    ...
    l2_ndisuio_global->rx_thread = CreateThread(NULL, 0, l2_packet_rx_thread, l2, 0, NULL);
...
void l2_packet_deinit(struct l2_packet_data *l2)
{
    ...
    if (l2_ndisuio_global)
    {
         ...
         */
        if (!DeviceIoControl(driver_ndis_get_ndisuio_handle(), IOCTL_CANCEL_READ, NULL, 0, NULL, 0, NULL, NULL))
        {
            wpa_printf(MSG_DEBUG,
