...
static void http_request_cb(struct httpread *handle, void *cookie, enum httpread_event en)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "HTTP: Request from %s:%d received", inet_ntoa(req->cli.sin_addr),
...
void http_request_send(struct http_request *req, struct wpabuf *resp)
{
    ...
    wpa_printf(MSG_DEBUG, "HTTP: Send %lu byte response to %s:%d", (unsigned long)wpabuf_len(resp),
               inet_ntoa(req->cli.sin_addr), ntohs(req->cli.sin_port));
                                     ...
                                     void *cb_ctx)
{
    ...
    if (listen(srv->fd, 10 /* max backlog */) < 0 || fcntl(srv->fd, F_SETFL, O_NONBLOCK) < 0 ||
