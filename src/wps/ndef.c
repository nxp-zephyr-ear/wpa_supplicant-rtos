...
static int ndef_parse_record(const u8 *data, u32 size, struct ndef_record *record)
{
    ...
    if (record->total_length > size || record->total_length < record->payload_length)
        return -1;
    return 0;
}
...
static struct wpabuf *ndef_build_record(
    u8 flags, const void *type, u8 type_length, void *id, u8 id_length, const struct wpabuf *payload)
{
...
{
    return ndef_build_record(FLAG_MESSAGE_BEGIN | FLAG_MESSAGE_END | FLAG_TNF_RFC2046, wifi_handover_type,
...
{
    return ndef_build_record(FLAG_MESSAGE_BEGIN | FLAG_MESSAGE_END | FLAG_TNF_RFC2046, p2p_handover_type,
