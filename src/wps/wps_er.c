...
static struct wps_er_ap_settings *wps_er_ap_get_settings(struct wps_er *er, const u8 *uuid)
{
    ...
    dl_list_for_each(s, &er->ap_settings, struct wps_er_ap_settings,
                     list) if (os_memcmp(uuid, s->uuid, WPS_UUID_LEN) == 0) return s;
    return NULL;
}
...
static void wps_er_subscribe(struct wps_er_ap *ap)
{
    ...
    wpabuf_printf(req,
                  ...
                  "TIMEOUT: Second-%d\r\n"
                  "\r\n",
                  path, inet_ntoa(dst.sin_addr), ntohs(dst.sin_port), ap->er->ip_addr_text, ap->er->http_port,
                                      ...
                                      char **body_ptr)
{
    ...
    if (msg)
    {
        encoded = wpa_base64_encode(wpabuf_head(msg), wpabuf_len(msg), &encoded_len);
        if (encoded == NULL)
            return NULL;
    }
    else
...
static void wps_er_http_req(void *ctx, struct http_request *req)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "%s:%d",
               http_request_get_uri(req), type, inet_ntoa(cli->sin_addr), ntohs(cli->sin_port));
...
struct wps_er *wps_er_init(struct wps_context *wps, const char *ifname, const char *filter)
{
    ...
    if (get_netif_info(er->ifname, &er->ip_addr, &er->ip_addr_text, NULL, er->mac_addr))
    {
        wpa_printf(MSG_INFO,
...
void wps_er_set_sel_reg(struct wps_er *er, int sel_reg, u16 dev_passwd_id, u16 sel_reg_config_methods)
{
    ...
    if (wps_build_version(msg) || wps_er_build_selected_registrar(msg, sel_reg) ||
        ...
        wps_build_wfa_ext(msg, 0, auth_macs, count, 0) || wps_er_build_uuid_r(msg, er->wps->uuid))
    {
        wpabuf_free(msg);
        return;
    }
...
int wps_er_config(
    struct wps_er *er, const u8 *uuid, const u8 *addr, const u8 *pin, size_t pin_len, const struct wps_credential *cred)
{
    struct wps_er_ap *ap;
...
struct wpabuf *wps_er_config_token_from_cred(struct wps_context *wps, struct wps_credential *cred)
{
    ...
    data.wps      = wps;
    data.use_cred = cred;
    if (wps_build_cred(&data, ret) || wps_build_wfa_ext(ret, 0, NULL, 0, 0))
    {
        wpabuf_free(ret);
        return NULL;
    }
...
struct wpabuf *wps_er_nfc_config_token(struct wps_er *er, const u8 *uuid, const u8 *addr)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "WPS ER: No settings known for the "
                   "selected AP");
        return NULL;
    }
