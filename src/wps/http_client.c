            ...
            if (httpread_hdr_type_get(c->hread) == HTTPREAD_HDR_TYPE_REPLY)
            {
                ...
                else
                {
                    wpa_printf(MSG_DEBUG,
                               "HTTP: Error %d from "
                               "%s:%d",
                               reply_code, inet_ntoa(c->dst.sin_addr), ntohs(c->dst.sin_port));
...
static void http_client_tx_ready(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "bytes remaining)",
               inet_ntoa(c->dst.sin_addr), ntohs(c->dst.sin_port), (unsigned long)wpabuf_len(c->req),
    ...
    if ((size_t)res < send_len)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   "remaining",
                   res, (unsigned long)wpabuf_len(c->req), (unsigned long)send_len - res);
        c->req_pos += res;
        return;
    }

    wpa_printf(MSG_DEBUG, "HTTP: Full client request sent to %s:%d", inet_ntoa(c->dst.sin_addr),
                                     ...
                                     void *cb_ctx)
{
    ...
    if (eloop_register_sock(c->sd, EVENT_TYPE_WRITE, http_client_tx_ready, c, NULL) ||
        eloop_register_timeout(HTTP_CLIENT_TIMEOUT_SEC, 0, http_client_timeout, c, NULL))
        goto fail;
