...
static void httpread_read_handler(int sd, void *eloop_ctx, void *sock_ctx)
{
     ...
     */
    if (!os_strncasecmp(h->hdr, "SUBSCRIBE", 9) || !os_strncasecmp(h->hdr, "UNSUBSCRIBE", 11) ||
    ...
    if (!h->got_body)
    {
        ...
        if (h->body_alloc_nbytes < (h->body_nbytes + nread + 1))
        {
            ...
            if (new_alloc_nbytes < h->body_alloc_nbytes || new_alloc_nbytes > h->max_bytes + HTTPREAD_BODYBUF_DELTA)
            {
                wpa_printf(MSG_DEBUG, "httpread: Unacceptable body length %d (body_alloc_nbytes=%u max_bytes=%u)",
...
struct httpread *httpread_create(int sd, /* descriptor of TCP socket to read from */
                                 void (*cb)(struct httpread *handle,
                                            void *cookie,
