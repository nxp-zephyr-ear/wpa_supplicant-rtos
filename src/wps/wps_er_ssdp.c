...
static void wps_er_ssdp_rx(int sd, void *eloop_ctx, void *sock_ctx)
{
    ...
    for (start = buf; start && *start; start = pos)
    {
        ...
        if (os_strstr(start,
                      "schemas-wifialliance-org:device:"
                      "WFADevice:1"))
            wfa = 1;
        if (os_strstr(start,
                      "schemas-wifialliance-org:service:"
                      "WFAWLANConfig:1"))
            wfa = 1;
...
int wps_er_ssdp_init(struct wps_er *er)
{
    ...
    {
        wpa_printf(MSG_INFO,
                   "WPS ER: Failed to add routing entry for "
                   "SSDP");
        return -1;
    }
    ...
    {
        wpa_printf(MSG_INFO,
                   "WPS ER: Failed to open multicast socket "
                   "for SSDP");
        return -1;
    }
    ...
    {
        wpa_printf(MSG_INFO,
                   "WPS ER: Failed to open SSDP listener "
                   "socket");
        return -1;
    }

    if (eloop_register_sock(er->multicast_sd, EVENT_TYPE_READ, wps_er_ssdp_rx, er, NULL) ||
        eloop_register_sock(er->ssdp_sd, EVENT_TYPE_READ, wps_er_ssdp_rx, er, NULL))
        return -1;
