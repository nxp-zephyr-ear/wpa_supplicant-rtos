...
static const char *wps_device_xml_postfix =
    ...
    "\n"
    "<SCPDURL>" UPNP_WPS_SCPD_XML_FILE
    "</SCPDURL>\n"
    "<controlURL>" UPNP_WPS_DEVICE_CONTROL_FILE
    "</controlURL>\n"
    "<eventSubURL>" UPNP_WPS_DEVICE_EVENT_FILE
    "</eventSubURL>\n"
    "</service>\n"
    "</serviceList>\n"
    "</device>\n"
 ...
 */
static void web_connection_parse_get(struct upnp_wps_device_sm *sm, struct http_request *hreq, const char *filename)
{
                                                    ...
                                                    const char **replyname)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "WPS UPnP: Could not parse PutMessage - NewInMessage");
        wpabuf_free(msg);
    ...
    dl_list_for_each(tmp, &iface->peers, struct upnp_wps_peer, list)
    {
        if (!tmp->wps)
            continue;
        if (attr.enrollee_nonce && os_memcmp(tmp->wps->nonce_e, attr.enrollee_nonce, WPS_NONCE_LEN) != 0)
            continue; /* Enrollee nonce mismatch */
        if (attr.msg_type && *attr.msg_type != WPS_M2 && *attr.msg_type != WPS_M2D && attr.registrar_nonce &&
                                                          ...
                                                          const char **replyname)
{
    ...
    dl_list_for_each(iface, &sm->interfaces, struct upnp_wps_device_interface, list)
    {
        if (iface->ctx->rx_req_put_wlan_response &&
            iface->ctx->rx_req_put_wlan_response(iface->priv, ev_type, macaddr, msg, type) == 0)
            ok = 1;
    }
...
static struct subscription *find_er(struct upnp_wps_device_sm *sm, struct sockaddr_in *cli)
{
    ...
    dl_list_for_each(s, &sm->subscriptions, struct subscription, list) if (find_er_addr(s, cli)) return s;
    return NULL;
}

static enum http_reply_code web_process_set_selected_registrar(
    struct upnp_wps_device_sm *sm, struct sockaddr_in *cli, char *data, struct wpabuf **reply, const char **replyname)
{
    struct wpabuf *msg;
    enum http_reply_code ret;
    struct subscription *s;
                                      ...
                                      const char *replyname)
{
    ...
    {
        size_t len;
        replydata = wpa_base64_encode(wpabuf_head(reply), wpabuf_len(reply), &len);
    }
    else
        replydata = NULL;
                                             ...
                                             const char *filename)
{
    ...
    if (got_uuid)
    {
        ...
        {
            wpa_printf(MSG_INFO, "WPS UPnP: Could not find matching subscription to unsubscribe (SID %s)", str);
            ret = HTTP_PRECONDITION_FAILED;
            goto send_msg;
        }
    }
...
static void web_connection_check_data(void *ctx, struct http_request *req)
{
    ...
    wpa_printf(MSG_DEBUG, "WPS UPnP: Got HTTP request type %d from %s:%d", htype, inet_ntoa(cli->sin_addr),
               htons(cli->sin_port));
