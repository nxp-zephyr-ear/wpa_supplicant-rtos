...
static struct wpabuf *event_build_message(struct wps_event_ *e)
{
    ...
    buf = wpabuf_alloc(1000 + os_strlen(e->addr->path) + wpabuf_len(e->data));
    if (buf == NULL)
        return NULL;
...
static int event_send_start(struct subscription *s)
{
     ...
     */
    if (dl_list_empty(&s->addr_list) || s->current_event || dl_list_empty(&s->event_queue))
        return -1;
    ...
    itry = 0;
    dl_list_for_each(e->addr, &s->addr_list, struct subscr_addr, list) if (itry++ == e->retry) break;
    if (itry < e->retry)
        return -1;
...
static void event_send_all_later_handler(void *eloop_data, void *user_ctx)
{
    ...
    {
        if (s->current_event == NULL /* not busy */ && !dl_list_empty(&s->event_queue) /* more to do */)
        {
