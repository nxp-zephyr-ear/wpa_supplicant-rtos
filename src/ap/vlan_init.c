...
static int vlan_if_add(struct hostapd_data *hapd, struct hostapd_vlan *vlan, int existsok)
{
    int ret;
#ifdef CONFIG_WEP
    int i;
    ...
    for (i = 0; i < NUM_WEP_KEYS; i++)
    {
        if (!hapd->conf->ssid.wep.key[i])
            continue;
        wpa_printf(MSG_ERROR, "VLAN: Refusing to set up VLAN iface %s with WEP", vlan->ifname);
        return -1;
    }
#endif /* CONFIG_WEP */
    ...
    wpa_printf(MSG_ERROR, "WPA initialization for VLAN %d failed (%d)", vlan->vlan_id, ret);
...
int vlan_if_remove(struct hostapd_data *hapd, struct hostapd_vlan *vlan)
{
    ...
    if (ret)
        wpa_printf(MSG_ERROR, "WPA deinitialization for VLAN %d failed (%d)", vlan->vlan_id, ret);
...
int vlan_init(struct hostapd_data *hapd)
{
    ...
    if ((hapd->conf->ssid.dynamic_vlan != DYNAMIC_VLAN_DISABLED || hapd->conf->ssid.per_sta_vif) && !hapd->conf->vlan)
    {
        ...
        ret           = os_snprintf(vlan->ifname, sizeof(vlan->ifname), "%s.#", hapd->conf->iface);
        if (ret >= (int)sizeof(vlan->ifname))
        {
            wpa_printf(MSG_WARNING, "VLAN: Interface name was truncated to %s", vlan->ifname);
        }
        else if (ret < 0)
        {
            os_free(vlan);
                                      ...
                                      struct vlan_description *vlan_desc)
{
    ...
    *pos++ = '\0';
    ...
    ret = os_snprintf(n->ifname, sizeof(n->ifname), "%s%d%s", ifname, vlan_id, pos);
