    ...
    *fpos = '\0';
    ...
    while (ie_len >= 2)
    {
        ...
        if (id == WLAN_EID_VENDOR_SPECIFIC && elen >= 4)
        {
            ...
            ret = os_snprintf(fpos, fend - fpos, "%s%d(%02x%02x%02x,%d)", sep, id, ie[0], ie[1], ie[2], ie[3]);
        }
        else
        {
            ...
            if (id == WLAN_EID_HT_CAP && elen >= 7)
            {
                ...
                os_snprintf(htmcs, sizeof(htmcs), ",htmcs:%08hx", (u16)WPA_GET_LE32(ie + 3));
            }
            ...
            if (id == WLAN_EID_VHT_CAP && elen >= 8)
            {
                 ...
                 * information */
                os_snprintf(vhtrxmcs, sizeof(vhtrxmcs), ",vhtrxmcs:%08x", WPA_GET_LE32(ie + 4));
            }
            if (id == WLAN_EID_VHT_CAP && elen >= 12)
            {
                /* VHT Capabilities (802.11ac), TX MCS
                 * information */
                os_snprintf(vhttxmcs, sizeof(vhttxmcs), ",vhttxmcs:%08x", WPA_GET_LE32(ie + 8));
            }
    ...
    ret = os_snprintf(fpos, fend - fpos, "%s%s%s%s%s%s%s%s%s", htcap, htagg, htmcs, vhtcap, vhtrxmcs, vhttxmcs, txpow,
                      extcap, wps);
    if (os_snprintf_error(fend - fpos, ret))
    {
    fail:
