...
static int fils_dhcp_request(struct hostapd_data *hapd,
                             struct sta_info *sta,
                             struct dhcp_data *dhcpoffer,
                             u8 *dhcpofferend)
{
    u8 *pos, *end;
    ...
    {
        wpa_printf(MSG_DEBUG, "FILS: No pending HLP DHCPDISCOVER available");
        return -1;
    }
    ...
    if (wpabuf_resize(&sta->hlp_dhcp_discover, 6 + 1 + (server_id ? 2 + server_id[1] : 0)))
        return -1;
    if (server_id)
        wpabuf_put_data(sta->hlp_dhcp_discover, server_id, 2 + server_id[1]);
    ...
    res = sendto(hapd->dhcp_sock, wpabuf_head(sta->hlp_dhcp_discover), wpabuf_len(sta->hlp_dhcp_discover), 0,
                 (const struct sockaddr *)&addr, sizeof(addr));
    if (res < 0)
    {
        wpa_printf(MSG_ERROR, "FILS: DHCP sendto failed: %s", strerror(errno));
        return -1;
    }
    wpa_printf(MSG_DEBUG, "FILS: Acting as DHCP rapid commit proxy for %s:%d", inet_ntoa(addr.sin_addr),
               ntohs(addr.sin_port));
...
static void fils_dhcp_handler(int sd, void *eloop_ctx, void *sock_ctx)
{
    ...
    res      = recvfrom(sd, buf, sizeof(buf), 0, (struct sockaddr *)&addr, &addr_len);
    if (res < 0)
    {
        wpa_printf(MSG_DEBUG, "FILS: DHCP read failed: %s", strerror(errno));
        return;
    }
    wpa_printf(MSG_DEBUG, "FILS: DHCP response from server %s:%d (len=%d)", inet_ntoa(addr.sin_addr),
               ntohs(addr.sin_port), (int)res);
    ...
    {
        wpa_printf(MSG_DEBUG, "FILS: HLP - DHCP response to unknown relay address 0x%x", dhcp->relay_ip);
        return;
    }
    dhcp->relay_ip = 0;
    pos            = (u8 *)(dhcp + 1);
    ...
    wpa_hexdump(MSG_DEBUG, "FILS: HLP - DHCP options in response", pos, end - pos);
    while (pos < end && *pos != DHCP_OPT_END)
    {
        u8 opt, olen;
    ...
    wpa_printf(MSG_DEBUG, "FILS: HLP - DHCP message type %u (rapid_commit=%d hw_addr=" MACSTR ")", msgtype,
               rapid_commit, MAC2STR(dhcp->hw_addr));

    sta = ap_get_sta(hapd, dhcp->hw_addr);
    if (!sta || !sta->fils_pending_assoc_req)
    {
        wpa_printf(MSG_DEBUG, "FILS: No pending HLP DHCP exchange with hw_addr " MACSTR, MAC2STR(dhcp->hw_addr));
        return;
    }

    if (hapd->conf->dhcp_rapid_commit_proxy && msgtype == DHCPOFFER && !rapid_commit)
    {
    ...
    {
        wpa_printf(MSG_DEBUG, "FILS: No DHCPACK available from the server and cannot do rapid commit proxying");
    }

    pos  = buf;
    resp = wpabuf_alloc(2 * ETH_ALEN + 6 + 2 + sizeof(*iph) + sizeof(*udph) + (end - pos) + 2);
    if (!resp)
        return;
    ...
    if (hapd->conf->dhcp_rapid_commit_proxy && msgtype == DHCPACK && !rapid_commit &&
    ...
    if (wpabuf_resize(&sta->fils_hlp_resp, wpabuf_len(resp) + 2 * wpabuf_len(resp) / 255 + 100))
    {
        wpabuf_free(resp);
        return;
    }
...
static int fils_process_hlp_dhcp(struct hostapd_data *hapd, struct sta_info *sta, const u8 *msg, size_t len)
{
    ...
    wpa_printf(MSG_DEBUG, "FILS: HLP request DHCP: op=%u htype=%u hlen=%u hops=%u xid=0x%x", dhcp->op, dhcp->htype,
               dhcp->hlen, dhcp->hops, ntohl(dhcp->xid));
    pos += sizeof(*dhcp);
    if (dhcp->op != 1)
    ...
    if (hapd->conf->dhcp_server.af != AF_INET || hapd->conf->dhcp_server.u.v4.s_addr == 0)
    {
        wpa_printf(MSG_DEBUG, "FILS: HLP - no DHCPv4 server configured - drop request");
        return 0;
    }

    if (hapd->conf->own_ip_addr.af != AF_INET || hapd->conf->own_ip_addr.u.v4.s_addr == 0)
    {
        wpa_printf(MSG_DEBUG, "FILS: HLP - no IPv4 own_ip_addr configured - drop request");
        return 0;
    }
    ...
    if (hapd->dhcp_sock < 0)
    {
        ...
        if (s < 0)
        {
            wpa_printf(MSG_ERROR, "FILS: Failed to open DHCP socket: %s", strerror(errno));
            return 0;
        }
        ...
        if (hapd->conf->dhcp_relay_port)
        {
            os_memset(&addr, 0, sizeof(addr));
            addr.sin_family      = AF_INET;
            addr.sin_addr.s_addr = hapd->conf->own_ip_addr.u.v4.s_addr;
            addr.sin_port        = htons(hapd->conf->dhcp_relay_port);
            if (bind(s, (struct sockaddr *)&addr, sizeof(addr)))
            {
                wpa_printf(MSG_ERROR, "FILS: Failed to bind DHCP socket: %s", strerror(errno));
                close(s);
                return 0;
            }
        }
        if (eloop_register_sock(s, EVENT_TYPE_READ, fils_dhcp_handler, NULL, hapd))
        {
            close(s);
            return 0;
        }
    ...
    addr.sin_addr.s_addr = hapd->conf->dhcp_server.u.v4.s_addr;
    addr.sin_port        = htons(hapd->conf->dhcp_server_port);
    res                  = sendto(hapd->dhcp_sock, dhcp_msg, len, 0, (const struct sockaddr *)&addr, sizeof(addr));
    if (res < 0)
    {
        wpa_printf(MSG_ERROR, "FILS: DHCP sendto failed: %s", strerror(errno));
    ...
    wpa_printf(MSG_DEBUG, "FILS: HLP relayed DHCP request to server %s:%d (rapid_commit=%d)", inet_ntoa(addr.sin_addr),
               ntohs(addr.sin_port), rapid_commit);
    if (hapd->conf->dhcp_rapid_commit_proxy && rapid_commit)
    {
         ...
         * option. */
        wpa_printf(MSG_DEBUG, "FILS: Store DHCPDISCOVER for rapid commit proxy");
        wpabuf_free(sta->hlp_dhcp_discover);
...
static int fils_process_hlp_udp(
    struct hostapd_data *hapd, struct sta_info *sta, const u8 *dst, const u8 *pos, size_t len)
{
    const struct ip *iph;
    ...
    wpa_printf(MSG_DEBUG, "FILS: HLP request UDP: sport=%u dport=%u ulen=%u sum=0x%x", sport, dport, ulen,
               ntohs(udph->uh_sum));
    ...
    {
        return fils_process_hlp_dhcp(hapd, sta, (const u8 *)(udph + 1), ulen - sizeof(*udph));
    }

    return 0;
}

static int fils_process_hlp_ip(
    struct hostapd_data *hapd, struct sta_info *sta, const u8 *dst, const u8 *pos, size_t len)
{
    const struct ip *iph;
    uint16_t ip_len;
    ...
    {
        wpa_printf(MSG_DEBUG, "FILS: HLP request IPv4 packet had invalid header checksum - dropped");
        return 0;
    }
    ip_len = ntohs(iph->ip_len);
    if (ip_len > len)
        return 0;
    wpa_printf(MSG_DEBUG, "FILS: HLP request IPv4: saddr=%08x daddr=%08x protocol=%u", iph->ip_src.s_addr,
...
static int fils_process_hlp_req(struct hostapd_data *hapd, struct sta_info *sta, const u8 *pos, size_t len)
{
    const u8 *pkt, *end;

    wpa_printf(MSG_DEBUG, "FILS: HLP request from " MACSTR " (dst=" MACSTR " src=" MACSTR " len=%u)",
               MAC2STR(sta->addr), MAC2STR(pos), MAC2STR(pos + ETH_ALEN), (unsigned int)len);
    if (os_memcmp(sta->addr, pos + ETH_ALEN, ETH_ALEN) != 0)
    {
        wpa_printf(MSG_DEBUG, "FILS: Ignore HLP request with unexpected source address" MACSTR,
                   MAC2STR(pos + ETH_ALEN));
        return 0;
    }

    end = pos + len;
    pkt = pos + 2 * ETH_ALEN;
    if (end - pkt >= 6 && os_memcmp(pkt, "\xaa\xaa\x03\x00\x00\x00", 6) == 0)
    ...
    {
        case ETH_P_IP:
            return fils_process_hlp_ip(hapd, sta, pos, pkt + 2, end - pkt - 2);
    }
...
int fils_process_hlp(struct hostapd_data *hapd, struct sta_info *sta, const u8 *pos, int left)
{
    ...
    if (sta->fils_pending_assoc_req && eloop_is_timeout_registered(fils_hlp_timeout, hapd, sta))
    {
        ...
        wpa_printf(MSG_DEBUG,
                   "FILS: Do not relay another HLP request from " MACSTR
