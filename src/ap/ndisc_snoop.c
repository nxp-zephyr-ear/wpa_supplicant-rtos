...
static int sta_has_ip6addr(struct sta_info *sta, struct in6_addr *addr)
{
    ...
    {
        if (ip6addr->addr.s6_addr32[0] == addr->s6_addr32[0] && ip6addr->addr.s6_addr32[1] == addr->s6_addr32[1] &&
            ip6addr->addr.s6_addr32[2] == addr->s6_addr32[2] && ip6addr->addr.s6_addr32[3] == addr->s6_addr32[3])
            return 1;
    }
...
static void handle_ndisc(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    switch (msg->icmp6h.icmp6_type)
    {
        case NEIGHBOR_SOLICITATION:
            ...
            if (!(saddr.s6_addr32[0] == 0 && saddr.s6_addr32[1] == 0 && saddr.s6_addr32[2] == 0 &&
                  saddr.s6_addr32[3] == 0))
            {
                ...
                if (inet_ntop(AF_INET6, &saddr, addrtxt, sizeof(addrtxt)) == NULL)
                    addrtxt[0] = '\0';
                wpa_printf(MSG_DEBUG, "ndisc_snoop: Learned new IPv6 address %s for " MACSTR, addrtxt,
                           MAC2STR(sta->addr));
...
int ndisc_snoop_init(struct hostapd_data *hapd)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "ndisc_snoop: Failed to initialize L2 packet processing for NDISC packets: %s",
                   strerror(errno));
        return -1;
    }
