...
static const char *ipaddr_str(u32 addr)
{
    ...
    os_snprintf(buf, sizeof(buf), "%u.%u.%u.%u", (addr >> 24) & 0xff, (addr >> 16) & 0xff, (addr >> 8) & 0xff,
                addr & 0xff);
    return buf;
}
...
static void handle_dhcp(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    if (msgtype == DHCPACK)
    {
        ...
        wpa_printf(MSG_DEBUG, "dhcp_snoop: Found DHCPACK for " MACSTR " @ IPv4 address %s/%d", MAC2STR(sta->addr),
                   ipaddr_str(be_to_host32(b->your_ip)), prefixlen);
        ...
        if (sta->ipaddr != 0)
        {
            wpa_printf(MSG_DEBUG, "dhcp_snoop: Removing IPv4 address %s from the ip neigh table",
...
int dhcp_snoop_init(struct hostapd_data *hapd)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "dhcp_snoop: Failed to initialize L2 packet processing for DHCP packet: %s",
                   strerror(errno));
        return -1;
    }
