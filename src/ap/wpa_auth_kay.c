...
static int hapd_set_replay_protect(void *priv, bool enabled, u32 window)
{
    ...
    if (!hapd->driver->set_replay_protect)
        return -1;
    return hapd->driver->set_replay_protect(hapd->drv_priv, enabled, window);
}
...
static int hapd_create_receive_sc(void *priv,
                                  ...
                                  enum confidentiality_offset co)
{
    ...
    if (!hapd->driver->create_receive_sc)
        return -1;
    return hapd->driver->create_receive_sc(hapd->drv_priv, sc, conf_offset_val(co), vf);
}
...
static int hapd_create_transmit_sc(void *priv, struct transmit_sc *sc, enum confidentiality_offset co)
{
    struct hostapd_data *hapd = priv;

    if (!hapd->driver->create_transmit_sc)
        return -1;
    return hapd->driver->create_transmit_sc(hapd->drv_priv, sc, conf_offset_val(co));
}
...
int ieee802_1x_alloc_kay_sm_hapd(struct hostapd_data *hapd, struct sta_info *sta)
{
    ...
    res = ieee802_1x_kay_init(kay_ctx, policy, hapd->conf->macsec_replay_protect, hapd->conf->macsec_replay_window,
                              hapd->conf->macsec_port, hapd->conf->mka_priority, hapd->conf->macsec_csindex,
...
static int ieee802_1x_auth_get_session_id(struct hostapd_data *hapd, struct sta_info *sta, u8 *sid, size_t *len)
{
    const u8 *session_id;
    ...
    if (!session_id)
    {
        wpa_printf(MSG_DEBUG, "MACsec: Failed to get SessionID from EAPOL state machines");
        return -1;
    }
...
static int ieee802_1x_auth_get_msk(struct hostapd_data *hapd, struct sta_info *sta, u8 *msk, size_t *len)
{
    const u8 *key;
    size_t keylen;
    ...
    if (key == NULL)
    {
        wpa_printf(MSG_DEBUG, "MACsec: Failed to get MSK from EAPOL state machines");
        return -1;
    }
    wpa_printf(MSG_DEBUG, "MACsec: Successfully fetched key (len=%lu)", (unsigned long)keylen);
...
void *ieee802_1x_notify_create_actor_hapd(struct hostapd_data *hapd, struct sta_info *sta)
{
    u8 *sid;
    size_t sid_len = 128;
    ...
    wpa_printf(MSG_DEBUG, "IEEE 802.1X: External notification - Create MKA for " MACSTR, MAC2STR(sta->addr));
    ...
    if (ieee802_1x_cak_aes_cmac(msk->key, msk->len, hapd->own_addr, sta->addr, cak->key, cak->len))
    {
    ...
    if (ieee802_1x_ckn_aes_cmac(msk->key, msk->len, hapd->own_addr, sta->addr, sid, sid_len, ckn->name))
    {
    ...
    res = ieee802_1x_kay_create_mka(hapd->kay, ckn, cak, 0, EAP_EXCHANGE, true);
...
void *ieee802_1x_create_preshared_mka_hapd(struct hostapd_data *hapd, struct sta_info *sta)
{
    struct mka_key *cak;
    struct mka_key_name *ckn;
    void *res = NULL;
