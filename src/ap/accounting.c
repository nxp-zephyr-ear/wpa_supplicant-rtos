...
static struct radius_msg *accounting_msg(struct hostapd_data *hapd, struct sta_info *sta, int status_type)
{
    ...
    if (sta)
    {
        if (!hostapd_config_get_radius_attr(hapd->conf->radius_acct_req_attr, RADIUS_ATTR_ACCT_AUTHENTIC) &&
            !radius_msg_add_attr_int32(
                msg, RADIUS_ATTR_ACCT_AUTHENTIC,
                hapd->conf->ieee802_1x ? RADIUS_ACCT_AUTHENTIC_RADIUS : RADIUS_ACCT_AUTHENTIC_LOCAL))
        {
    ...
    if (sta)
    {
        ...
        if (!b && sta->radius_cui &&
            !radius_msg_add_attr(msg, RADIUS_ATTR_CHARGEABLE_USER_IDENTITY, (u8 *)sta->radius_cui,
        ...
        if (sta->ipaddr && !radius_msg_add_attr_int32(msg, RADIUS_ATTR_FRAMED_IP_ADDRESS, be_to_host32(sta->ipaddr)))
        {
    ...
    os_get_time(&now);
    if (now.sec > 1000000000 && !radius_msg_add_attr_int32(msg, RADIUS_ATTR_EVENT_TIMESTAMP, now.sec))
    {
                                       ...
                                       struct hostap_sta_driver_data *data)
{
    ...
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_DEBUG,
                   "updated TX/RX stats: rx_bytes=%llu [%u:%u] tx_bytes=%llu [%u:%u] bytes_64bit=%d", data->rx_bytes,
                   sta->last_rx_bytes_hi, sta->last_rx_bytes_lo, data->tx_bytes, sta->last_tx_bytes_hi,
                   sta->last_tx_bytes_lo, data->bytes_64bit);
...
void accounting_sta_start(struct hostapd_data *hapd, struct sta_info *sta)
{
    ...
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_INFO, "starting accounting session %016llX",
...
static void accounting_sta_report(struct hostapd_data *hapd, struct sta_info *sta, int stop)
{
    ...
    if (accounting_sta_update_stats(hapd, sta, &data) == 0)
    {
        ...
        if (!radius_msg_add_attr_int32(msg, RADIUS_ATTR_ACCT_INPUT_OCTETS, (u32)bytes))
        {
            wpa_printf(MSG_INFO, "Could not add Acct-Input-Octets");
            goto fail;
        }
        if (!radius_msg_add_attr_int32(msg, RADIUS_ATTR_ACCT_INPUT_GIGAWORDS, (u32)(bytes >> 32)))
        {
        ...
        if (!radius_msg_add_attr_int32(msg, RADIUS_ATTR_ACCT_OUTPUT_OCTETS, (u32)bytes))
        {
            wpa_printf(MSG_INFO, "Could not add Acct-Output-Octets");
            goto fail;
        }
        if (!radius_msg_add_attr_int32(msg, RADIUS_ATTR_ACCT_OUTPUT_GIGAWORDS, (u32)(bytes >> 32)))
        {
...
void accounting_sta_stop(struct hostapd_data *hapd, struct sta_info *sta)
{
    if (sta->acct_session_started)
    {
        ...
        hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_INFO, "stopped accounting session %016llX",
                       (unsigned long long)sta->acct_session_id);
...
{
    return radius_gen_session_id((u8 *)&sta->acct_session_id, sizeof(sta->acct_session_id));
}
 ...
 */
static RadiusRxResult accounting_receive(
...
static void accounting_report_state(struct hostapd_data *hapd, int on)
{
    ...
    if (hapd->acct_session_id)
    {
        ...
        os_snprintf(buf, sizeof(buf), "%016llX", (unsigned long long)hapd->acct_session_id);
...
static void accounting_interim_error_cb(const u8 *addr, void *ctx)
{
    ...
    res = eloop_deplete_timeout(wait_time, 0, accounting_interim_update, hapd, sta);
    if (res == 1)
        wpa_printf(MSG_DEBUG,
                   "Interim RADIUS accounting update failed for " MACSTR
        ...
        wpa_printf(MSG_DEBUG, "Interim RADIUS accounting update failed for " MACSTR " (error count: %u)", MAC2STR(addr),
                   sta->acct_interim_errors);
    else
        wpa_printf(MSG_DEBUG,
                   "Interim RADIUS accounting update failed for " MACSTR " (error count: %u) - no timer found",
...
{
    if (radius_gen_session_id((u8 *)&hapd->acct_session_id, sizeof(hapd->acct_session_id)) < 0)
        return -1;

    if (radius_client_register(hapd->radius, RADIUS_ACCT, accounting_receive, hapd))
        return -1;
    radius_client_set_interim_error_cb(hapd->radius, accounting_interim_error_cb, hapd);
