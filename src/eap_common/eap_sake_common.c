 ...
 */
static int eap_sake_kdf(const u8 *key,
                        size_t key_len,
                        const char *label,
                        const u8 *data,
                        size_t data_len,
                        ...
                        size_t buf_len)
{
    ...
    while (pos < buf_len)
    {
        ...
        {
            if (hmac_sha1_vector(key, key_len, 4, addr, len, &buf[pos]) < 0)
                return -1;
            pos += SHA1_MAC_LEN;
        }
        else
        {
            if (hmac_sha1_vector(key, key_len, 4, addr, len, hash) < 0)
                return -1;
    ...
    const u8 *root_secret_a, const u8 *root_secret_b, const u8 *rand_s, const u8 *rand_p, u8 *tek, u8 *msk, u8 *emsk)
{
    ...
    if (eap_sake_kdf(root_secret_a, EAP_SAKE_ROOT_SECRET_LEN, "SAKE Master Secret A", rand_p, EAP_SAKE_RAND_LEN, rand_s,
                     EAP_SAKE_RAND_LEN, sms_a, EAP_SAKE_SMS_LEN) < 0)
        return -1;
    wpa_hexdump_key(MSG_DEBUG, "EAP-SAKE: SMS-A", sms_a, EAP_SAKE_SMS_LEN);
    if (eap_sake_kdf(sms_a, EAP_SAKE_SMS_LEN, "Transient EAP Key", rand_s, EAP_SAKE_RAND_LEN, rand_p, EAP_SAKE_RAND_LEN,
    ...
    if (eap_sake_kdf(root_secret_b, EAP_SAKE_ROOT_SECRET_LEN, "SAKE Master Secret B", rand_p, EAP_SAKE_RAND_LEN, rand_s,
                     EAP_SAKE_RAND_LEN, sms_b, EAP_SAKE_SMS_LEN) < 0)
        return -1;
    wpa_hexdump_key(MSG_DEBUG, "EAP-SAKE: SMS-B", sms_b, EAP_SAKE_SMS_LEN);
    if (eap_sake_kdf(sms_b, EAP_SAKE_SMS_LEN, "Master Session Key", rand_s, EAP_SAKE_RAND_LEN, rand_p,
                     EAP_SAKE_RAND_LEN, key_buf, sizeof(key_buf)) < 0)
        return -1;
                         ...
                         u8 *mic)
{
    ...
    ret = eap_sake_kdf(tek_auth, EAP_SAKE_TEK_AUTH_LEN, peer ? "Peer MIC" : "Server MIC", _rand, 2 * EAP_SAKE_RAND_LEN,
