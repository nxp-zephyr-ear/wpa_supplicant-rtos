...
int eap_eke_dh_init(u8 group, u8 *ret_priv, u8 *ret_pub)
{
    ...
    if (crypto_dh_init(gen, dh->prime, dh->prime_len, ret_priv, ret_pub) < 0)
        return -1;
...
static int eap_eke_prf(
    u8 prf, const u8 *key, size_t key_len, const u8 *data, size_t data_len, const u8 *data2, size_t data2_len, u8 *res)
{
    const u8 *addr[2];
    size_t len[2];
    size_t num_elem = 1;
...
int eap_eke_shared_secret(struct eap_eke_session *sess, const u8 *key, const u8 *dhpriv, const u8 *peer_dhcomp)
{
    ...
    len = dh->prime_len;
    if (crypto_dh_derive_secret(*dh->generator, dh->prime, dh->prime_len, NULL, 0, dhpriv, dh->prime_len, peer_pub,
                       ...
                       u8 *emsk)
{
    ...
    pos += sess->nonce_len;
    os_memcpy(pos, nonce_s, sess->nonce_len);
    if (eap_eke_prfplus(sess->prf, sess->shared_secret, sess->prf_len, data, data_len, buf,
                        EAP_MSK_LEN + EAP_EMSK_LEN) < 0)
    {
        os_free(data);
        return -1;
    }
...
int eap_eke_auth(struct eap_eke_session *sess, const char *label, const struct wpabuf *msgs, u8 *auth)
{
    ...
    return eap_eke_prf(sess->prf, sess->ka, sess->auth_len, (const u8 *)label, os_strlen(label), wpabuf_head(msgs),
                       wpabuf_len(msgs), auth);
}
...
int eap_eke_session_init(struct eap_eke_session *sess, u8 dhgroup, u8 encr, u8 prf, u8 mac)
{
    ...
    if (sess->prf_len < 0 || sess->nonce_len < 0 || sess->auth_len < 0 || sess->dhcomp_len < 0 ||
        sess->pnonce_len < 0 || sess->pnonce_ps_len < 0)
        return -1;
