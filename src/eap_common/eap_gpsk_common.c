...
static int eap_gpsk_gkdf_cmac(
    const u8 *psk /* Y */, const u8 *data /* Z */, size_t data_len, u8 *buf, size_t len /* X */)
{
    u8 *opos;
...
#ifdef EAP_GPSK_SHA256
static int eap_gpsk_gkdf_sha256(
    const u8 *psk /* Y */, const u8 *data /* Z */, size_t data_len, u8 *buf, size_t len /* X */)
{
    u8 *opos;
    size_t i, n, hashlen, left, clen;
...
#define EAP_GPSK_PK_LEN_AES 16
    ...
    *pk_len = EAP_GPSK_PK_LEN_AES;
    ...
    return eap_gpsk_derive_keys_helper(EAP_GPSK_CIPHER_AES, kdf_out, sizeof(kdf_out), psk, psk_len, seed, seed_len, msk,
                                       emsk, sk, *sk_len, pk, *pk_len);
}

#ifdef EAP_GPSK_SHA256
static int eap_gpsk_derive_keys_sha256(
    const u8 *psk, size_t psk_len, const u8 *seed, size_t seed_len, u8 *msk, u8 *emsk, u8 *sk, size_t *sk_len)
{
...
#define EAP_GPSK_PK_LEN_SHA256 SHA256_MAC_LEN
    ...
    *sk_len = EAP_GPSK_SK_LEN_SHA256;
    ...
    return eap_gpsk_derive_keys_helper(EAP_GPSK_CIPHER_SHA256, kdf_out, sizeof(kdf_out), psk, psk_len, seed, seed_len,
                                       msk, emsk, sk, *sk_len, NULL, 0);
}
#endif /* EAP_GPSK_SHA256 */
                                      ...
                                      u8 method_type)
{
    ...
    switch (csuite_specifier)
    {
        ...
        default:
            wpa_printf(MSG_DEBUG,
                       "EAP-GPSK: Unknown cipher %ld used in "
