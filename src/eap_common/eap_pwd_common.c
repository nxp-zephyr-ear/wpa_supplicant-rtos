 ...
 */
int compute_password_element(EAP_PWD_group *grp,
                             u16 num,
                             const u8 *password,
                             size_t password_len,
                             const u8 *id_server,
                             ...
                             const u8 *token)
{
    ...
    if (crypto_bignum_to_bin(prime, prime_bin, sizeof(prime_bin), primebytelen) < 0)
        return -1;
    ...
    if (dragonfly_get_random_qr_qnr(prime, &qr, &qnr) < 0 ||
        crypto_bignum_to_bin(qr, qr_bin, sizeof(qr_bin), primebytelen) < 0 ||
        crypto_bignum_to_bin(qnr, qnr_bin, sizeof(qnr_bin), primebytelen) < 0)
        goto fail;
    ...
    while (ctr < 40)
    {
        ...
        is_odd = const_time_select_u8(found, is_odd, pwe_digest[SHA256_MAC_LEN - 1] & 0x01);
        if (eap_pwd_kdf(pwe_digest, SHA256_MAC_LEN, (u8 *)"EAP-pwd Hunting And Pecking",
                        os_strlen("EAP-pwd Hunting And Pecking"), prfbuf, primebitlen) < 0)
            goto fail;
        if (primebitlen % 8)
            buf_shift_right(prfbuf, primebytelen, 8 - primebitlen % 8);
        ...
        if (!x_candidate)
        {
            wpa_printf(MSG_INFO, "EAP-pwd: unable to create x_candidate");
            goto fail;
        }

        wpa_hexdump_key(MSG_DEBUG, "EAP-pwd: x_candidate", prfbuf, primebytelen);
        const_time_select_bin(found, x_bin, prfbuf, primebytelen, x_bin);
        ...
        res = dragonfly_is_quadratic_residue_blind(grp->group, qr_bin, qnr_bin, tmp2);
        if (res < 0)
            goto fail;
    ...
    if (!y || dragonfly_sqrt(grp->group, y, y) < 0 ||
        crypto_bignum_to_bin(y, x_y, MAX_ECC_PRIME_LEN, primebytelen) < 0 || crypto_bignum_sub(prime, y, y) < 0 ||
        crypto_bignum_to_bin(y, x_y + MAX_ECC_PRIME_LEN, MAX_ECC_PRIME_LEN, primebytelen) < 0)
    {
    ...
    const_time_select_bin(is_eq, x_y, x_y + MAX_ECC_PRIME_LEN, primebytelen, x_y + primebytelen);
...
int compute_keys(EAP_PWD_group *grp,
                 const struct crypto_bignum *k,
                 const struct crypto_bignum *peer_scalar,
                 const struct crypto_bignum *server_scalar,
                 const u8 *confirm_peer,
                 const u8 *confirm_server,
                 const u32 *ciphersuite,
                 u8 *msk,
                 u8 *emsk,
                 u8 *session_id)
{
...
static int eap_pwd_element_coord_ok(const struct crypto_bignum *prime, const u8 *buf, size_t len)
{
    struct crypto_bignum *val;
    int ok = 1;

    val = crypto_bignum_init_set(buf, len);
    if (!val || crypto_bignum_is_zero(val) || crypto_bignum_cmp(val, prime) >= 0)
        ok = 0;
    crypto_bignum_deinit(val, 0);
    return ok;
}

struct crypto_ec_point *eap_pwd_get_element(EAP_PWD_group *group, const u8 *buf)
{
    ...
    if (!crypto_ec_point_is_on_curve(group->group, element) || crypto_ec_point_is_at_infinity(group->group, element))
    {
...
struct crypto_bignum *eap_pwd_get_scalar(EAP_PWD_group *group, const u8 *buf)
{
    ...
    if (!scalar || crypto_bignum_is_zero(scalar) || crypto_bignum_is_one(scalar) ||
...
int eap_pwd_get_rand_mask(EAP_PWD_group *group,
                          struct crypto_bignum *_rand,
                          struct crypto_bignum *_mask,
                          struct crypto_bignum *scalar)
{
    return dragonfly_generate_scalar(crypto_ec_get_order(group->group), _rand, _mask, scalar);
}
