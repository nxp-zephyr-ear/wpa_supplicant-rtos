...
static struct wpabuf *eap_pax_process_std_1(
    struct eap_pax_data *data, struct eap_method_ret *ret, u8 id, const struct eap_pax_hdr *req, size_t req_plen)
{
    struct wpabuf *resp;
    const u8 *pos;
    u8 *rpos;
    ...
    if (eap_pax_initial_key_derivation(req->mac_id, data->ak, data->rand.e, data->mk, data->ck, data->ick, data->mid) <
        0)
    {
        ret->ignore = true;
        return NULL;
    ...
    eap_pax_mac(req->mac_id, data->ck, EAP_PAX_CK_LEN, data->rand.r.x, EAP_PAX_RAND_LEN, data->rand.r.y,
                EAP_PAX_RAND_LEN, (u8 *)data->cid, data->cid_len, rpos);
    wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: MAC_CK(A, B, CID)", rpos, EAP_PAX_MAC_LEN);

    /* Optional ADE could be added here, if needed */

    rpos = wpabuf_put(resp, EAP_PAX_ICV_LEN);
    eap_pax_mac(req->mac_id, data->ick, EAP_PAX_ICK_LEN, wpabuf_head(resp), wpabuf_len(resp) - EAP_PAX_ICV_LEN, NULL, 0,
                NULL, 0, rpos);
...
static struct wpabuf *eap_pax_process_std_3(
    struct eap_pax_data *data, struct eap_method_ret *ret, u8 id, const struct eap_pax_hdr *req, size_t req_plen)
{
    struct wpabuf *resp;
    ...
    rpos = wpabuf_put(resp, EAP_PAX_ICV_LEN);
    if (eap_pax_mac(data->mac_id, data->ick, EAP_PAX_ICK_LEN, wpabuf_head(resp), wpabuf_len(resp) - EAP_PAX_ICV_LEN,
                                      ...
                                      const struct wpabuf *reqData)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "public_key_id 0x%x",
               req->op_code, req->flags, req->mac_id, req->dh_group_id, req->public_key_id);
...
static u8 *eap_pax_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
    ...
    *len = EAP_MSK_LEN;
    if (eap_pax_kdf(data->mac_id, data->mk, EAP_PAX_MK_LEN, "Master Session Key", data->rand.e, 2 * EAP_PAX_RAND_LEN,
                    EAP_MSK_LEN, key) < 0)
    {
        os_free(key);
        return NULL;
...
static u8 *eap_pax_get_emsk(struct eap_sm *sm, void *priv, size_t *len)
{
    ...
    *len = EAP_EMSK_LEN;
    if (eap_pax_kdf(data->mac_id, data->mk, EAP_PAX_MK_LEN, "Extended Master Session Key", data->rand.e,
