                                                 ...
                                                 size_t payload_len)
{
    ...
    if (eap_sake_derive_keys(data->root_secret_a, data->root_secret_b, data->rand_s, data->rand_p, (u8 *)&data->tek,
    ...
    if (eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p, data->serverid, data->serverid_len,
                             data->peerid, data->peerid_len, 1, wpabuf_head(resp), wpabuf_len(resp), rpos, rpos))
    {
                                               ...
                                               size_t payload_len)
{
    ...
    if (eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p, data->serverid, data->serverid_len,
                             data->peerid, data->peerid_len, 0, wpabuf_head(reqData), wpabuf_len(reqData), attr.mic_s,
                             mic_s))
    {
        wpa_printf(MSG_INFO, "EAP-SAKE: Failed to compute MIC");
