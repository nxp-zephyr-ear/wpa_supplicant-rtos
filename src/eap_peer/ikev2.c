...
static int ikev2_process_sai1(struct ikev2_responder_data *data, const u8 *sai1, size_t sai1_len)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "INTEG:%d D-H:%d",
               data->proposal.proposal_num, data->proposal.encr, data->proposal.prf, data->proposal.integ,
               data->proposal.dh);
...
{
    if (ikev2_process_sai1(data, pl->sa, pl->sa_len) < 0 || ikev2_process_kei(data, pl->ke, pl->ke_len) < 0 ||
...
static int ikev2_process_auth_secret(struct ikev2_responder_data *data, u8 method, const u8 *auth, size_t auth_len)
{
    ...
    if (ikev2_derive_auth_data(data->proposal.prf, data->i_sign_msg, data->IDi, data->IDi_len, data->IDi_type,
                               &data->keys, 1, data->shared_secret, data->shared_secret_len, data->r_nonce,
                               data->r_nonce_len, data->key_pad, data->key_pad_len, auth_data) < 0)
    {
                                           ...
                                           size_t payload_len)
{
    ...
    if (ikev2_process_idi(data, pl.idi, pl.idi_len) < 0 || ikev2_process_cert(data, pl.cert, pl.cert_len) < 0 ||
                                 ...
                                 struct ikev2_payloads *pl)
{
    ...
    decrypted = ikev2_decrypt_payload(data->proposal.encr, data->proposal.integ, &data->keys, 1, hdr, pl->encrypted,
...
static int ikev2_validate_rx_state(struct ikev2_responder_data *data, u8 exchange_type, u32 message_id)
{
    switch (data->state)
    {
        case SA_INIT:
            ...
            {
                wpa_printf(MSG_INFO,
                           "IKEV2: Unexpected Message ID %lu "
                           "in SA_INIT state",
                           message_id);
                return -1;
            }
...
int ikev2_responder_process(struct ikev2_responder_data *data, const struct wpabuf *buf)
{
    ...
    wpa_printf(MSG_DEBUG, "IKEV2:   Message ID: %lu  Length: %lu", message_id, length);
...
static void ikev2_build_hdr(
    struct ikev2_responder_data *data, struct wpabuf *msg, u8 exchange_type, u8 next_payload, u32 message_id)
{
...
static int ikev2_build_auth(struct ikev2_responder_data *data, struct wpabuf *msg, u8 next_payload)
{
    ...
    if (ikev2_derive_auth_data(data->proposal.prf, data->r_sign_msg, data->IDr, data->IDr_len, ID_KEY_ID, &data->keys,
                               0, data->shared_secret, data->shared_secret_len, data->i_nonce, data->i_nonce_len,
                               data->key_pad, data->key_pad_len, wpabuf_put(msg, prf->hash_len)) < 0)
    {
        wpa_printf(MSG_INFO, "IKEV2: Could not derive AUTH data");
        return -1;
    }
...
static struct wpabuf *ikev2_build_sa_init(struct ikev2_responder_data *data)
{
    ...
    if (ikev2_build_sar1(data, msg, IKEV2_PAYLOAD_KEY_EXCHANGE) || ikev2_build_ker(data, msg, IKEV2_PAYLOAD_NONCE) ||
        ikev2_build_nr(data, msg,
                       data->peer_auth == PEER_AUTH_SECRET ? IKEV2_PAYLOAD_ENCRYPTED : IKEV2_PAYLOAD_NO_NEXT_PAYLOAD))
    {
        wpabuf_free(msg);
        return NULL;
    }
    ...
    if (data->peer_auth == PEER_AUTH_SECRET)
    {
        ...
        if (ikev2_build_idr(data, plain, IKEV2_PAYLOAD_NO_NEXT_PAYLOAD) ||
            ikev2_build_encrypted(data->proposal.encr, data->proposal.integ, &data->keys, 0, msg, plain,
                                  IKEV2_PAYLOAD_IDr))
        {
            wpabuf_free(plain);
            wpabuf_free(msg);
...
static struct wpabuf *ikev2_build_sa_auth(struct ikev2_responder_data *data)
{
    ...
    if (ikev2_build_idr(data, plain, IKEV2_PAYLOAD_AUTHENTICATION) ||
        ...
        ikev2_build_encrypted(data->proposal.encr, data->proposal.integ, &data->keys, 0, msg, plain, IKEV2_PAYLOAD_IDr))
    {
        wpabuf_free(plain);
        wpabuf_free(msg);
        return NULL;
