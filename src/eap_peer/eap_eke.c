...
{
    return dhgroup == EAP_EKE_DHGROUP_EKE_2 || dhgroup == EAP_EKE_DHGROUP_EKE_5 || dhgroup == EAP_EKE_DHGROUP_EKE_14 ||
...
static struct wpabuf *eap_eke_build_fail(struct eap_eke_data *data, struct eap_method_ret *ret, u8 id, u32 failure_code)
{
    ...
    wpa_printf(MSG_DEBUG, "EAP-EKE: Sending EAP-EKE-Failure/Response - code=0x%lx", failure_code);
                                             ...
                                             size_t payload_len)
{
     ...
     */
    if (eap_eke_derive_key(&data->sess, password, password_len, data->serverid, data->serverid_len, data->peerid,
                           data->peerid_len, key) < 0)
    {
                                              ...
                                              size_t payload_len)
{
    ...
    if (eap_eke_derive_ka(&data->sess, data->serverid, data->serverid_len, data->peerid, data->peerid_len,
    ...
    if (eap_eke_derive_msk(&data->sess, data->serverid, data->serverid_len, data->peerid, data->peerid_len,
                                              ...
                                              size_t payload_len)
{
    ...
    else
    {
        ...
        wpa_printf(MSG_INFO, "EAP-EKE: Failure-Code 0x%lx", code);
    }

    return eap_eke_build_fail(data, ret, eap_get_id(reqData), EAP_EKE_FAIL_NO_ERROR);
}

static struct wpabuf *eap_eke_process(struct eap_sm *sm,
                                      void *priv,
                                      struct eap_method_ret *ret,
                                      const struct wpabuf *reqData)
{
