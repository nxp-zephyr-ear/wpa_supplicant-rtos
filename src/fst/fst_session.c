...
#define fst_printf_session(s, level, format, ...)                                                    \
    fst_printf((level), "%u (0x%08x): [" MACSTR "," MACSTR "] :" format, (s)->id, (s)->data.fsts_id, \
               MAC2STR((s)->data.old_peer_addr), MAC2STR((s)->data.new_peer_addr), ##__VA_ARGS__)

#define fst_printf_siface(s, iface, level, format, ...) \
    fst_printf_session((s), (level), "%s: " format, fst_iface_get_name(iface), ##__VA_ARGS__)

#define fst_printf_sframe(s, is_old, level, format, ...) \
    fst_printf_siface((s), (is_old) ? (s)->data.old_iface : (s)->data.new_iface, (level), format, ##__VA_ARGS__)
...
static const char *const fst_action_names[] = {
    [FST_ACTION_SETUP_REQUEST] = "Setup Request", [FST_ACTION_SETUP_RESPONSE] = "Setup Response",
    [FST_ACTION_TEAR_DOWN] = "Tear Down",         [FST_ACTION_ACK_REQUEST] = "Ack Request",
    [FST_ACTION_ACK_RESPONSE] = "Ack Response",   [FST_ACTION_ON_CHANNEL_TUNNEL] = "On Channel Tunnel",
};
...
#define foreach_fst_session(s) dl_list_for_each((s), &global_sessions_list, struct fst_session, global_sessions_lentry)

#define foreach_fst_session_safe(s, temp) \
    dl_list_for_each_safe((s), (temp), &global_sessions_list, struct fst_session, global_sessions_lentry)
                                  ...
                                  union fst_session_state_switch_extra *extra)
{
    if (s->state != state)
    {
        ...
        fst_printf_session(s, MSG_INFO, "State: %s => %s", fst_session_state_name(s->state),
...
static void fst_session_stt_arm(struct fst_session *s)
{
    ...
    eloop_register_timeout(0, 2 * TU_TO_US(FST_DEFAULT_SESSION_TIMEOUT_TU), fst_session_timeout_handler, NULL, s);
    s->stt_armed = true;
}
...
{
    return s->state == FST_SESSION_STATE_SETUP_COMPLETION && fst_session_is_in_transition(s);
}
...
{
    return s->state == FST_SESSION_STATE_TRANSITION_DONE && fst_session_is_in_transition(s);
}

static struct fst_session *fst_find_session_in_progress(const u8 *peer_addr, struct fst_group *g)
{
    struct fst_session *s;
...
static int fst_session_send_action(
    struct fst_session *s, bool old_iface, const void *payload, size_t size, const struct wpabuf *extra_buf)
{
    size_t len;
    int res;
    struct wpabuf *buf;
    ...
    if (!iface)
    {
        fst_printf_session(s, MSG_ERROR, "no %s interface for FST Action '%s' sending", old_iface ? "old" : "new",
    ...
    if (!buf)
    {
        fst_printf_session(s, MSG_ERROR, "cannot allocate buffer of %zu bytes for FST Action '%s' sending", len,
                           fst_action_names[action]);
        return -1;
    }
    ...
    res = fst_iface_send_action(iface, old_iface ? s->data.old_peer_addr : s->data.new_peer_addr, buf);
    if (res < 0)
        fst_printf_siface(s, iface, MSG_ERROR, "failed to send FST Action '%s'", fst_action_names[action]);
    else
        fst_printf_siface(s, iface, MSG_DEBUG, "FST Action '%s' sent", fst_action_names[action]);
    wpabuf_free(buf);
                                             ...
                                             size_t frame_len)
{
    ...
    {
        fst_printf_iface(iface, MSG_WARNING, "FST Request dropped: too short (%zu < %zu)", frame_len,
                         IEEE80211_HDRLEN + 1 + sizeof(*req));
        return;
    }
    plen = frame_len - IEEE80211_HDRLEN - 1;
    req  = (const struct fst_setup_req *)(((const u8 *)mgmt) + IEEE80211_HDRLEN + 1);
    if (req->stie.element_id != WLAN_EID_SESSION_TRANSITION || req->stie.length < 11)
    {
    ...
    {
        fst_printf_iface(iface, MSG_WARNING, "FST Request dropped: new and old band IDs are the same");
        return;
    }
    ...
    if (plen > sizeof(*req))
    {
        fst_iface_update_mb_ie(iface, mgmt->sa, (const u8 *)(req + 1), plen - sizeof(*req));
        fst_printf_iface(iface, MSG_INFO, "FST Request: MB IEs updated for " MACSTR, MAC2STR(mgmt->sa));
    }

    new_iface = fst_group_get_peer_other_connection(iface, mgmt->sa, req->stie.new_band_id, new_iface_peer_addr);
    if (!new_iface)
    {
        fst_printf_iface(iface, MSG_WARNING, "FST Request dropped: new iface not found");
        return;
    }
    fst_printf_iface(iface, MSG_INFO, "FST Request: new iface (%s:" MACSTR ") found", fst_iface_get_name(new_iface),
    ...
    if (s)
    {
        ...
        {
            fst_printf_session(s, MSG_WARNING, "FST Request dropped due to MAC comparison (our MAC is " MACSTR ")",
                               MAC2STR(mgmt->da));
            return;
        }
    ...
    if (!s)
    {
        fst_printf(MSG_WARNING, "FST Request dropped: cannot create session for %s and %s", fst_iface_get_name(iface),
                                              ...
                                              size_t frame_len)
{
    ...
    {
        fst_printf_session(s, MSG_WARNING, "FST Response dropped: %s is not the old iface", fst_iface_get_name(iface));
        return;
    }

    if (!fst_session_is_ready_pending(s))
    {
        fst_printf_session(s, MSG_WARNING, "FST Response dropped due to wrong state: %s",
    ...
    res = (const struct fst_setup_res *)(((const u8 *)mgmt) + IEEE80211_HDRLEN + 1);
    if (res->stie.element_id != WLAN_EID_SESSION_TRANSITION || res->stie.length < 11)
    {
    ...
    {
        fst_printf_session(s, MSG_WARNING, "FST Response dropped due to wrong dialog token (%u != %u)",
                           s->data.pending_setup_req_dlgt, res->dialog_token);
        return;
    }

    if (res->status_code == WLAN_STATUS_SUCCESS && le_to_host32(res->stie.fsts_id) != s->data.fsts_id)
    {
        fst_printf_session(s, MSG_WARNING, "FST Response dropped due to wrong FST Session ID (%u)",
    ...
    if (res->status_code != WLAN_STATUS_SUCCESS)
    {
        ...
        fst_session_set_state(s, FST_SESSION_STATE_INITIAL, &evext);
        fst_printf_session(s, MSG_WARNING, "FST Setup rejected by remote side with status %u", res->status_code);
        return;
    }
    ...
    fst_printf_session(s, MSG_INFO, "%s: FST Setup established for %s (llt=%u)", fst_iface_get_name(s->data.old_iface),
                                         ...
                                         size_t frame_len)
{
    ...
    td = (const struct fst_tear_down *)(((const u8 *)mgmt) + IEEE80211_HDRLEN + 1);

    if (le_to_host32(td->fsts_id) != s->data.fsts_id)
    {
        fst_printf_siface(s, iface, MSG_WARNING, "tear down for wrong FST Setup ID (%u)", le_to_host32(td->fsts_id));
        return;
    }
                                           ...
                                           size_t frame_len)
{
    ...
    {
        fst_printf_siface(s, iface, MSG_ERROR, "cannot initiate switch due to wrong session state (%s)",
                          fst_session_state_name(s->state));
        return;
    }
    ...
    {
        fst_printf_siface(s, iface, MSG_WARNING, "Ack for wrong FST Setup ID (%u)", le_to_host32(req->fsts_id));
        return;
    }
                                            ...
                                            size_t frame_len)
{
    ...
    {
        fst_printf_siface(s, iface, MSG_ERROR, "Ack Response in inappropriate session state (%s)",
                          fst_session_state_name(s->state));
        return;
    }
    ...
    if (iface != s->data.new_iface)
    {
        fst_printf_siface(s, iface, MSG_ERROR, "Ack response received on wrong interface");
        return;
    }
    ...
    {
        fst_printf_siface(s, iface, MSG_ERROR, "Ack response for wrong FST Setup ID (%u)", le_to_host32(res->fsts_id));
        return;
    }
...
int fst_session_initiate_setup(struct fst_session *s)
{
    ...
    if (!fst_iface_is_connected(s->data.old_iface, s->data.old_peer_addr, false))
    {
        fst_printf_session(s, MSG_ERROR, "The preset old peer address is not connected");
        return -EINVAL;
    }

    if (!fst_iface_is_connected(s->data.new_iface, s->data.new_peer_addr, false))
    {
        fst_printf_session(s, MSG_ERROR, "The preset new peer address is not connected");
        return -EINVAL;
    }
    ...
    if (_s)
    {
        fst_printf_session(s, MSG_ERROR, "There is another session in progress (old): %u", _s->id);
        return -EINVAL;
    }
    ...
    if (_s)
    {
        fst_printf_session(s, MSG_ERROR, "There is another session in progress (new): %u", _s->id);
        return -EINVAL;
    }
    ...
    fst_printf_siface(s, s->data.old_iface, MSG_INFO, "initiating FST setup for %s (llt=%u ms)",
                      fst_iface_get_name(s->data.new_iface), s->data.llt_ms);
    ...
    res = fst_session_send_action(s, true, &req, sizeof(req), fst_iface_get_mbie(s->data.old_iface));
    if (!res)
    {
...
int fst_session_respond(struct fst_session *s, u8 status_code)
{
    ...
    if (!fst_session_is_ready_pending(s))
    {
        fst_printf_session(s, MSG_ERROR, "incorrect state: %s", fst_session_state_name(s->state));
        return -EINVAL;
    }
    ...
    if (!fst_iface_is_connected(s->data.old_iface, s->data.old_peer_addr, false))
    {
        fst_printf_session(s, MSG_ERROR, "The preset peer address is not in the peer list");
        return -EINVAL;
    }
    ...
    if (status_code == WLAN_STATUS_SUCCESS)
    {
        ...
        fst_printf_session(s, MSG_INFO, "%s: FST Setup Request accepted for %s (llt=%u)",
                           fst_iface_get_name(s->data.old_iface), fst_iface_get_name(s->data.new_iface),
                           s->data.llt_ms);
    }
    else
    {
        fst_printf_session(s, MSG_WARNING, "%s: FST Setup Request rejected with code %d",
                           fst_iface_get_name(s->data.old_iface), status_code);
    }

    if (fst_session_send_action(s, true, &res, sizeof(res), fst_iface_get_mbie(s->data.old_iface)))
    {
        fst_printf_sframe(s, true, MSG_ERROR, "cannot send FST Setup Response with code %d", status_code);
        return -EINVAL;
    }
...
int fst_session_initiate_switch(struct fst_session *s)
{
    ...
    {
        fst_printf_session(s, MSG_ERROR, "cannot initiate switch due to wrong setup state (%d)", s->state);
        return -1;
    }
    ...
    fst_printf_session(s, MSG_INFO, "initiating FST switch: %s => %s", fst_iface_get_name(s->data.old_iface),
...
void fst_session_on_action_rx(struct fst_iface *iface, const struct ieee80211_mgmt *mgmt, size_t len)
{
    ...
    if (len < IEEE80211_HDRLEN + 2 || mgmt->u.action.category != WLAN_ACTION_FST)
    {
    ...
    {
        fst_printf_iface(iface, MSG_WARNING, "unknown FST Action (%u) received!", mgmt->u.action.u.fst_action.action);
        return;
    }
    ...
    {
        fst_printf_iface(iface, MSG_WARNING, "FST Action '%s' dropped: no session in progress found",
                         fst_action_names[mgmt->u.action.u.fst_action.action]);
    }
}
...
int fst_session_set_str_ifname(struct fst_session *s, const char *ifname, bool is_old)
{
    ...
    if (!i)
    {
        fst_printf_session(s, MSG_WARNING, "Cannot set iface %s: no such iface within group '%s'", ifname,
                           fst_group_get_id(g));
        return -1;
    }
...
int fst_session_set_str_llt(struct fst_session *s, const char *llt_str)
{
    ...
    {
        fst_printf_session(s, MSG_WARNING, "Cannot set llt %s: Invalid llt value (1..%u expected)", llt_str,
                           FST_MAX_LLT_MS);
        return -1;
    }
...
void fst_session_global_on_iface_detached(struct fst_iface *iface)
{
    ...
    foreach_fst_session(s)
    {
        if (fst_session_is_in_progress(s) && (s->data.new_iface == iface || s->data.old_iface == iface))
...
static int get_group_fill_session(struct fst_group **g, struct fst_session *s)
{
    ...
    s->data.old_iface = dl_list_entry(s->data.new_iface->group_lentry.next, struct fst_iface, group_lentry);
    if (!s->data.old_iface)
        return -EINVAL;
...
int fst_test_req_send_fst_request(const char *params)
{
    ...
    if (!fst_read_next_text_param(endp, additional_param, sizeof(additional_param), &endp))
    {
        if (!os_strcasecmp(additional_param, FST_CTR_PVAL_BAD_NEW_BAND))
            req.stie.new_band_id = req.stie.old_band_id;
    }

    return fst_session_send_action(&s, true, &req, sizeof(req), s.data.old_iface->mb_ie);
}
...
int fst_test_req_send_fst_response(const char *params)
{
    ...
    res.dialog_token = (_s && fst_session_is_ready_pending(_s)) ? _s->data.pending_setup_req_dlgt : g->dialog_token;
    res.status_code  = status_code;
    ...
    return fst_session_send_action(&s, true, &res, sizeof(res), s.data.old_iface->mb_ie);
}
...
int fst_test_req_get_local_mbies(const char *request, char *buf, size_t buflen)
{
    ...
    if (request[0] != ' ')
        return -EINVAL;
    request++;
    if (fst_read_next_text_param(request, ifname, sizeof(ifname), &endp) || !*ifname)
        goto problem;
    g = dl_list_first(&fst_global_groups_list, struct fst_group, global_groups_lentry);
    if (!g)
        goto problem;
    iface = fst_group_get_iface_by_name(g, ifname);
    if (!iface || !iface->mb_ie)
        goto problem;
    return wpa_snprintf_hex(buf, buflen, wpabuf_head(iface->mb_ie), wpabuf_len(iface->mb_ie));
