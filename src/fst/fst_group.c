...
static void fst_dump_mb_ies(const char *group_id, const char *ifname, struct wpabuf *mbies)
{
    ...
    while (s >= 2)
    {
        ...
        fst_printf(MSG_WARNING,
                   ...
                   group_id, ifname, mbie->mb_ctrl, mbie->band_id, mbie->op_class, mbie->chan, MAC2STR(mbie->bssid),
                   mbie->beacon_int, mbie->tsf_offs[0], mbie->tsf_offs[1], mbie->tsf_offs[2], mbie->tsf_offs[3],
                   mbie->tsf_offs[4], mbie->tsf_offs[5], mbie->tsf_offs[6], mbie->tsf_offs[7],
...
static void fst_fill_mb_ie(struct wpabuf *buf, const u8 *bssid, const u8 *own_addr, enum mb_band_id band, u8 channel)
{
...
static unsigned fst_fill_iface_mb_ies(struct fst_iface *f, struct wpabuf *buf)
{
    ...
    if (bssid)
    {
        ...
        if (buf)
        {
            ...
            fst_fill_mb_ie(buf, bssid, fst_iface_get_addr(f), fst_hw_mode_to_band(hw_mode), channel);
        }
        return 1;
    }
    else
...
static struct wpabuf *fst_group_create_mb_ie(struct fst_group *g, struct fst_iface *i)
{
    struct wpabuf *buf;
    struct fst_iface *f;
    ...
    if (!buf)
    {
        fst_printf_iface(i, MSG_ERROR, "cannot allocate mem for %u MB IEs", nof_mbies);
        return NULL;
    }
...
static const u8 *fst_mbie_get_peer_addr(const struct multi_band_ie *mbie)
{
    ...
    switch (MB_CTRL_ROLE(mbie->mb_ctrl))
    {
        ...
        case MB_STA_ROLE_NON_PCP_NON_AP:
            if (mbie->mb_ctrl & MB_CTRL_STA_MAC_PRESENT && (size_t)2 + mbie->len >= sizeof(*mbie) + ETH_ALEN)
...
static const u8 *fst_mbie_get_peer_addr_for_band(const struct wpabuf *mbies, u8 band_id)
{
    const u8 *p = wpabuf_head(mbies);
    size_t s    = wpabuf_len(mbies);
...
struct fst_iface *fst_group_get_iface_by_name(struct fst_group *g, const char *ifname)
{
    struct fst_iface *f;
 ...
 */
static struct fst_iface *fst_group_get_peer_other_connection_1(struct fst_iface *iface,
                                                               const u8 *peer_addr,
                                                               u8 band_id,
                                                               u8 *other_peer_addr)
{
    ...
    fst_printf(MSG_DEBUG, "found other peer addr from mbies: " MACSTR, MAC2STR(tmp_peer_addr));
 ...
 */
static struct fst_iface *fst_group_get_peer_other_connection_2(struct fst_iface *iface,
                                                               const u8 *peer_addr,
                                                               u8 band_id,
                                                               u8 *other_peer_addr)
{
    ...
    foreach_fst_group_iface(fst_iface_get_group(iface), other_iface)
    {
        ...
        for (; cur_peer_addr; cur_peer_addr = fst_iface_get_peer_next(other_iface, &ctx, true))
        {
                                                      ...
                                                      u8 *other_peer_addr)
{
    ...
    fst_printf(MSG_DEBUG, "%s: %s:" MACSTR ", %d", __func__, fst_iface_get_name(iface), MAC2STR(peer_addr), band_id);
    ...
    other_iface = fst_group_get_peer_other_connection_1(iface, peer_addr, band_id, other_peer_addr);
    if (other_iface)
    {
        fst_printf(MSG_DEBUG, "found by method #1. %s:" MACSTR, fst_iface_get_name(other_iface),
                   MAC2STR(other_peer_addr));
        return other_iface;
    }

    other_iface = fst_group_get_peer_other_connection_2(iface, peer_addr, band_id, other_peer_addr);
    if (other_iface)
    {
        fst_printf(MSG_DEBUG, "found by method #2. %s:" MACSTR, fst_iface_get_name(other_iface),
                   MAC2STR(other_peer_addr));
        return other_iface;
    }
...
{
    bool is_empty = !fst_group_has_ifaces(group) && !fst_session_global_get_first_by_group(group);
