...
static void fst_ctrl_iface_notify_peer_state_change(struct fst_iface *iface, bool connected, const u8 *peer_addr)
{
    union fst_event_extra extra;

    extra.peer_state.connected = connected;
    os_strlcpy(extra.peer_state.ifname, fst_iface_get_name(iface), sizeof(extra.peer_state.ifname));
    os_memcpy(extra.peer_state.addr, peer_addr, ETH_ALEN);

    foreach_fst_ctrl_call(on_event, EVENT_PEER_STATE_CHANGED, iface, NULL, &extra);
}
                             ...
                             const struct fst_iface_cfg *cfg)
{
    ...
    fst_printf_iface(iface, MSG_DEBUG, "iface attached to group %s (prio=%d, llt=%d)", cfg->group_id, cfg->priority,
                     cfg->llt);
...
void fst_detach(struct fst_iface *iface)
{
    ...
    fst_printf_iface(iface, MSG_DEBUG, "iface detached from group %s", fst_group_get_id(group));
...
void fst_rx_action(struct fst_iface *iface, const struct ieee80211_mgmt *mgmt, size_t len)
{
    if (fst_iface_is_connected(iface, mgmt->sa, false))
        fst_session_on_action_rx(iface, mgmt, len);
    else
        wpa_printf(MSG_DEBUG, "FST: Ignore FST Action frame - no FST connection with " MACSTR, MAC2STR(mgmt->sa));
}
...
void fst_notify_peer_disconnected(struct fst_iface *iface, const u8 *addr)
{
    ...
    fst_printf_iface(iface, MSG_DEBUG, MACSTR " became disconnected", MAC2STR(addr));

    fst_ctrl_iface_notify_peer_state_change(iface, false, addr);
}

bool fst_are_ifaces_aggregated(struct fst_iface *iface1, struct fst_iface *iface2)
{
...
{
    fst_printf_iface(iface, MSG_DEBUG, "new MAC address " MACSTR, MAC2STR(addr));
