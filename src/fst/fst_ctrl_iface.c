...
/* notifications */
static bool format_session_state_extra(const union fst_event_extra *extra, char *buffer, size_t size)
{
    int len;
    ...
    switch (ss->extra.to_initial.reason)
    {
        case REASON_REJECT:
            if (ss->extra.to_initial.reject_code != WLAN_STATUS_SUCCESS)
                os_snprintf(reject_str, sizeof(reject_str), "%u", ss->extra.to_initial.reject_code);
            /* fall through */
    ...
    len = os_snprintf(buffer, size,
                      FST_CES_PNAME_REASON "=%s " FST_CES_PNAME_REJECT_CODE "=%s " FST_CES_PNAME_INITIATOR "=%s",
                      fst_reason_name(ss->extra.to_initial.reason), reject_str, initiator);

    return !os_snprintf_error(size, len);
}

static void fst_ctrl_iface_notify(struct fst_iface *f,
                                  u32 session_id,
    ...
    switch (event_type)
    {
        case EVENT_FST_IFACE_STATE_CHANGED:
            ...
            wpa_msg_global_only(f->iface_obj.ctx, MSG_INFO,
                                FST_CTRL_EVENT_IFACE " %s " FST_CEI_PNAME_IFNAME "=%s " FST_CEI_PNAME_GROUP "=%s",
                                is->attached ? FST_CEI_PNAME_ATTACHED : FST_CEI_PNAME_DETACHED, is->ifname,
                                is->group_id);
            break;
        case EVENT_PEER_STATE_CHANGED:
            ...
            wpa_msg_global_only(fst_iface_get_wpa_obj_ctx(f), MSG_INFO,
                                FST_CTRL_EVENT_PEER " %s " FST_CEP_PNAME_IFNAME "=%s " FST_CEP_PNAME_ADDR "=" MACSTR,
                                ps->connected ? FST_CEP_PNAME_CONNECTED : FST_CEP_PNAME_DISCONNECTED, ps->ifname,
                                MAC2STR(ps->addr));
            break;
        case EVENT_FST_SESSION_STATE_CHANGED:
            ...
            wpa_msg_global_only(
                ...
                session_id, fst_session_event_type_name(event_type), fst_session_state_name(ss->old_state),
                fst_session_state_name(ss->new_state), extra_str);
            break;
        case EVENT_FST_ESTABLISHED:
        case EVENT_FST_SETUP:
            wpa_msg_global_only(fst_iface_get_wpa_obj_ctx(f), MSG_INFO,
                                FST_CTRL_EVENT_SESSION " " FST_CES_PNAME_SESSION_ID "=%u " FST_CES_PNAME_EVT_TYPE "=%s",
...
static int session_get(const char *session_id, char *buf, size_t buflen)
{
    ...
    return os_snprintf(buf, buflen,
                       ...
                       old_iface ? fst_iface_get_name(old_iface) : FST_CTRL_PVAL_NONE, fst_session_get_llt(s),
...
static int session_add(const char *group_id, char *buf, size_t buflen)
{
    ...
    if (!s)
    {
        fst_printf(MSG_ERROR, "CTRL: Cannot create session for group '%s'", group_id);
        return os_snprintf(buf, buflen, "FAIL\n");
    }
...
static int session_respond(const char *session_id, char *buf, size_t buflen)
{
    ...
    {
        fst_printf(MSG_WARNING, "CTRL: session %u: unknown response status: %s", id, p);
        return os_snprintf(buf, buflen, "FAIL\n");
    }
...
static int test_request(const char *request, char *buf, size_t buflen)
{
    ...
    if (!os_strncasecmp(p, FST_CTR_SEND_SETUP_REQUEST, os_strlen(FST_CTR_SEND_SETUP_REQUEST)))
    {
        ret = fst_test_req_send_fst_request(p + os_strlen(FST_CTR_SEND_SETUP_REQUEST));
    }
    else if (!os_strncasecmp(p, FST_CTR_SEND_SETUP_RESPONSE, os_strlen(FST_CTR_SEND_SETUP_RESPONSE)))
    {
        ret = fst_test_req_send_fst_response(p + os_strlen(FST_CTR_SEND_SETUP_RESPONSE));
    }
    else if (!os_strncasecmp(p, FST_CTR_SEND_ACK_REQUEST, os_strlen(FST_CTR_SEND_ACK_REQUEST)))
    {
        ret = fst_test_req_send_ack_request(p + os_strlen(FST_CTR_SEND_ACK_REQUEST));
    }
    else if (!os_strncasecmp(p, FST_CTR_SEND_ACK_RESPONSE, os_strlen(FST_CTR_SEND_ACK_RESPONSE)))
    {
        ret = fst_test_req_send_ack_response(p + os_strlen(FST_CTR_SEND_ACK_RESPONSE));
    }
    else if (!os_strncasecmp(p, FST_CTR_SEND_TEAR_DOWN, os_strlen(FST_CTR_SEND_TEAR_DOWN)))
    {
        ret = fst_test_req_send_tear_down(p + os_strlen(FST_CTR_SEND_TEAR_DOWN));
    }
    else if (!os_strncasecmp(p, FST_CTR_GET_FSTS_ID, os_strlen(FST_CTR_GET_FSTS_ID)))
    {
    ...
    else if (!os_strncasecmp(p, FST_CTR_GET_LOCAL_MBIES, os_strlen(FST_CTR_GET_LOCAL_MBIES)))
    {
        return fst_test_req_get_local_mbies(p + os_strlen(FST_CTR_GET_LOCAL_MBIES), buf, buflen);
    }
    else if (!os_strncasecmp(p, FST_CTR_IS_SUPPORTED, os_strlen(FST_CTR_IS_SUPPORTED)))
    {
        ret = 0;
    }
    else
...
static void list_session_enum_cb(struct fst_group *g, struct fst_session *s, void *ctx)
{
...
static int get_peer_mbies(const char *params, char *buf, size_t buflen)
{
    ...
    if (fst_read_next_text_param(params, ifname, sizeof(ifname), &endp) || !*ifname)
        goto problem;
...
static int print_band(unsigned num, struct fst_iface *iface, const u8 *addr, char *buf, size_t buflen)
{
    ...
    if (wpabuf)
    {
        ...
        ret += wpa_snprintf_hex(buf + ret, buflen - ret, wpabuf_head(wpabuf), wpabuf_len(wpabuf));
...
static void fst_ctrl_iface_on_iface_state_changed(struct fst_iface *i, bool attached)
{
    union fst_event_extra extra;

    os_memset(&extra, 0, sizeof(extra));
    extra.iface_state.attached = attached;
    os_strlcpy(extra.iface_state.ifname, fst_iface_get_name(i), sizeof(extra.iface_state.ifname));
    os_strlcpy(extra.iface_state.group_id, fst_iface_get_group_id(i), sizeof(extra.iface_state.group_id));

    fst_ctrl_iface_notify(i, FST_INVALID_SESSION_ID, EVENT_FST_IFACE_STATE_CHANGED, &extra);
}
...
int fst_parse_attach_command(const char *cmd, char *ifname, size_t ifname_size, struct fst_iface_cfg *cfg)
{
    ...
    if (fst_read_next_text_param(cmd, ifname, ifname_size, &endp) ||
        fst_read_next_text_param(endp, cfg->group_id, sizeof(cfg->group_id), &endp))
        return -EINVAL;
