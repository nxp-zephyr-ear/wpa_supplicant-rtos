...
static int wpa_priv_reg_cmd(struct wpa_driver_privsep_data *drv, int cmd)
{
    ...
    res = sendto(drv->priv_socket, &cmd, sizeof(cmd), 0, (struct sockaddr *)&drv->priv_addr, sizeof(drv->priv_addr));
    if (res < 0)
        wpa_printf(MSG_ERROR, "sendto: %s", strerror(errno));
    return res < 0 ? -1 : 0;
}

static int wpa_priv_cmd(
    struct wpa_driver_privsep_data *drv, int cmd, const void *data, size_t data_len, void *reply, size_t *reply_len)
{
    struct msghdr msg;
    struct iovec io[2];
...
static int wpa_driver_privsep_scan(void *priv, struct wpa_driver_scan_params *params)
{
    ...
    for (i = 0; i < params->num_ssids; i++)
    {
        ...
        os_memcpy(scan.ssids[i], params->ssids[i].ssid, scan.ssid_lens[i]);
    }

    for (i = 0; i < PRIVSEP_MAX_SCAN_FREQS && params->freqs && params->freqs[i]; i++)
        scan.freqs[i] = params->freqs[i];
    scan.num_freqs = i;

    return wpa_priv_cmd(drv, PRIVSEP_CMD_SCAN, &scan, sizeof(scan), NULL, NULL);
}
...
static struct wpa_scan_results *wpa_driver_privsep_get_scan_results2(void *priv)
{
    ...
    if (buf == NULL)
        return NULL;
    res = wpa_priv_cmd(drv, PRIVSEP_CMD_GET_SCAN_RESULTS, NULL, 0, buf, &reply_len);
    if (res < 0)
    {
        os_free(buf);
        return NULL;
    ...
    wpa_printf(MSG_DEBUG, "privsep: Received %lu bytes of scan results", (unsigned long)reply_len);
    if (reply_len < sizeof(int))
    {
        wpa_printf(MSG_DEBUG, "privsep: Invalid scan result len %lu", (unsigned long)reply_len);
        os_free(buf);
        return NULL;
    }
    ...
    while (results->num < (size_t)num && end - pos > (int)sizeof(int))
    {
        ...
        {
            wpa_printf(MSG_ERROR, "privsep: Invalid scan result len (%d + %d + %d > %d)", (int)sizeof(*r),
                       (int)r->ie_len, (int)r->beacon_ie_len, len);
            os_free(r);
            break;
        }
...
static int wpa_driver_privsep_set_key(void *priv, struct wpa_driver_set_key_params *params)
{
    struct wpa_driver_privsep_data *drv = priv;
    ...
    wpa_printf(MSG_DEBUG, "%s: priv=%p alg=%d key_idx=%d set_tx=%d", __func__, priv, alg, key_idx, set_tx);
...
static int wpa_driver_privsep_authenticate(void *priv, struct wpa_driver_auth_params *params)
{
    struct wpa_driver_privsep_data *drv = priv;
    ...
    wpa_printf(MSG_DEBUG, "%s: priv=%p freq=%d bssid=" MACSTR " auth_alg=%d local_state_change=%d p2p=%d", __func__,
               priv, params->freq, MAC2STR(params->bssid), params->auth_alg, params->local_state_change, params->p2p);
...
static int wpa_driver_privsep_associate(void *priv, struct wpa_driver_associate_params *params)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               __func__, priv, params->freq.freq, params->pairwise_suite, params->group_suite, params->key_mgmt_suite,
...
static int wpa_driver_privsep_deauthenticate(void *priv, const u8 *addr, u16 reason_code)
{
    // struct wpa_driver_privsep_data *drv = priv;
...
static void wpa_driver_privsep_event_interface_status(void *ctx, u8 *buf, size_t len)
{
    ...
    if (len < sizeof(int) || len - sizeof(int) > sizeof(data.interface_status.ifname))
        return;
...
static void wpa_driver_privsep_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "privsep: Event %d received (len=%lu)", event, (unsigned long)event_len);
    ...
    switch (e)
    {
        ...
        case PRIVSEP_EVENT_ASSOCINFO:
            wpa_driver_privsep_event_assoc(drv->ctx, EVENT_ASSOCINFO, event_buf, event_len);
            break;
        case PRIVSEP_EVENT_MICHAEL_MIC_FAILURE:
            wpa_driver_privsep_event_michael_mic_failure(drv->ctx, event_buf, event_len);
            break;
...
static int wpa_driver_privsep_set_param(void *priv, const char *param)
{
    ...
    os_snprintf(drv->own_socket_path, len, "%s/wpa_privsep-%d-%d", own_dir, getpid(), counter++);
    ...
    os_snprintf(drv->own_cmd_path, len, "%s/wpa_privsep-%d-%d", own_dir, getpid(), counter++);

    os_free(own_dir);

    drv->priv_addr.sun_family = AF_UNIX;
    os_snprintf(drv->priv_addr.sun_path, sizeof(drv->priv_addr.sun_path), "%s/%s", priv_dir, drv->ifname);
    os_free(priv_dir);
    ...
    eloop_register_read_sock(drv->priv_socket, wpa_driver_privsep_receive, drv, NULL);
...
static int wpa_driver_privsep_set_country(void *priv, const char *alpha2)
{
    ...
    return wpa_priv_cmd(drv, PRIVSEP_CMD_SET_COUNTRY, alpha2, os_strlen(alpha2), NULL, NULL);
}
...
const struct wpa_driver_ops *const wpa_drivers[] = {&wpa_driver_privsep_ops, NULL};
