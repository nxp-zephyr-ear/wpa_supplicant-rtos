...
static void wpa_driver_wext_event_wireless(struct wpa_driver_wext_data *drv, char *data, unsigned int len)
{
    ...
    while ((size_t)(end - pos) >= IW_EV_LCP_LEN)
    {
        ...
        if (drv->we_version_compiled > 18 &&
            (iwe->cmd == IWEVMICHAELMICFAILURE || iwe->cmd == IWEVCUSTOM || iwe->cmd == IWEVASSOCREQIE ||
             iwe->cmd == IWEVASSOCRESPIE || iwe->cmd == IWEVPMKIDCAND))
        {
        ...
        switch (iwe->cmd)
        {
            ...
            case IWEVMICHAELMICFAILURE:
                ...
                wpa_driver_wext_event_wireless_michaelmicfailure(drv->ctx, custom, iwe->u.data.length);
                break;
            case IWEVCUSTOM:
                if (iwe->u.data.length > end - custom)
                {
            ...
            case IWEVASSOCREQIE:
                ...
                wpa_driver_wext_event_wireless_assocreqie(drv, custom, iwe->u.data.length);
                break;
            case IWEVASSOCRESPIE:
                ...
                wpa_driver_wext_event_wireless_assocrespie(drv, custom, iwe->u.data.length);
                break;
            case IWEVPMKIDCAND:
                if (iwe->u.data.length > end - custom)
                {
...
static void wpa_driver_wext_event_link(struct wpa_driver_wext_data *drv, char *buf, size_t len, int del)
{
    ...
    event.interface_status.ievent = del ? EVENT_INTERFACE_REMOVED : EVENT_INTERFACE_ADDED;

    wpa_printf(MSG_DEBUG, "RTM_%sLINK, IFLA_IFNAME: Interface '%s' %s", del ? "DEL" : "NEW",
               event.interface_status.ifname, del ? "removed" : "added");
...
static void wpa_driver_wext_event_rtm_newlink(void *ctx, struct ifinfomsg *ifi, u8 *buf, size_t len)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "Ignore event for foreign ifindex %d", ifi->ifi_index);
        return;
    }
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "(%s%s%s%s)",
               drv->operstate, ifi->ifi_flags, (ifi->ifi_flags & IFF_UP) ? "[UP]" : "",
    ...
    if (drv->if_disabled && (ifi->ifi_flags & IFF_UP))
    {
        if (if_indextoname(ifi->ifi_index, namebuf) && linux_iface_up(drv->ioctl_sock, drv->ifname) == 0)
        {
            wpa_printf(MSG_DEBUG,
    ...
    while (RTA_OK(attr, attrlen))
    {
        ...
        {
            wpa_driver_wext_event_wireless(drv, ((char *)attr) + rta_len, attr->rta_len - rta_len);
        }
        else if (attr->rta_type == IFLA_IFNAME)
        {
            wpa_driver_wext_event_link(drv, ((char *)attr) + rta_len, attr->rta_len - rta_len, 0);
        }
        attr = RTA_NEXT(attr, attrlen);
    }
}
...
static void wpa_driver_wext_event_rtm_dellink(void *ctx, struct ifinfomsg *ifi, u8 *buf, size_t len)
{
    ...
    while (RTA_OK(attr, attrlen))
    {
        if (attr->rta_type == IFLA_IFNAME)
        {
            wpa_driver_wext_event_link(drv, ((char *)attr) + rta_len, attr->rta_len - rta_len, 1);
        }
        attr = RTA_NEXT(attr, attrlen);
    }
}
...
static void wext_get_phy_name(struct wpa_driver_wext_data *drv)
{
    ...
    drv->phyname[0] = '\0';
    snprintf(buf, sizeof(buf) - 1, "/sys/class/net/%s/phy80211/name", drv->ifname);
    f = open(buf, O_RDONLY);
    if (f < 0)
    {
    ...
    if (pos)
        *pos = '\0';
    wpa_printf(MSG_DEBUG, "wext: interface %s phy: %s", drv->ifname, drv->phyname);
}
...
static int wext_hostap_ifname(struct wpa_driver_wext_data *drv, const char *ifname)
{
    char buf[200], *res;
    int type, ret;
    FILE *f;
    ...
    ret = snprintf(buf, sizeof(buf), "/sys/class/net/%s/device/net/%s/type", drv->ifname, ifname);
    ...
    {
        wpa_printf(MSG_DEBUG, "WEXT: Found hostap driver wifi# interface (%s)", ifname);
...
static void wext_check_hostap(struct wpa_driver_wext_data *drv)
{
    ...
    snprintf(path, sizeof(path), "/sys/class/net/%s/device/driver", drv->ifname);
...
int wpa_driver_wext_scan(void *priv, struct wpa_driver_scan_params *params)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "%s: too long SSID (%lu)", __FUNCTION__, (unsigned long)ssid_len);
        return -1;
    }
    ...
    wpa_printf(MSG_DEBUG,
               ...
               ret, timeout);
    eloop_cancel_timeout(wpa_driver_wext_scan_timeout, drv, drv->ctx);
    eloop_register_timeout(timeout, 0, wpa_driver_wext_scan_timeout, drv, drv->ctx);
...
static void wext_get_scan_freq(struct iw_event *iwe, struct wext_scan_data *res)
{
    ...
    if (iwe->u.freq.e > 6)
    {
        wpa_printf(MSG_DEBUG, "Invalid freq in scan results (BSSID=" MACSTR " m=%d e=%d)", MAC2STR(res->res.bssid),
...
static void wext_get_scan_qual(struct wpa_driver_wext_data *drv, struct iw_event *iwe, struct wext_scan_data *res)
{
    ...
    if ((iwe->u.qual.updated & IW_QUAL_DBM) || ((iwe->u.qual.level != 0) && (iwe->u.qual.level > drv->max_level)))
    {
...
static int wext_19_iw_point(struct wpa_driver_wext_data *drv, u16 cmd)
{
    return drv->we_version_compiled > 18 &&
           (cmd == SIOCGIWESSID || cmd == SIOCGIWENCODE || cmd == IWEVGENIE || cmd == IWEVCUSTOM);
}
...
struct wpa_scan_results *wpa_driver_wext_get_scan_results(void *priv)
{
    ...
    wpa_printf(MSG_DEBUG, "Received %lu bytes of scan results (%lu BSSes)", (unsigned long)len,
 ...
 */
static int wpa_driver_wext_set_key(void *priv, struct wpa_driver_set_key_params *params)
{
    struct wpa_driver_wext_data *drv = priv;
    struct iwreq iwr;
    int ret                = 0;
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "key_len=%lu",
               __FUNCTION__, alg, key_idx, set_tx, (unsigned long)seq_len, (unsigned long)key_len);

    ret = wpa_driver_wext_set_key_ext(drv, alg, addr, key_idx, set_tx, seq, seq_len, key, key_len, key_flag);
    if (ret == 0)
        return 0;
    ...
    if (set_tx && alg != WPA_ALG_NONE)
    {
        ...
        if (ioctl(drv->ioctl_sock, SIOCSIWENCODE, &iwr) < 0)
        {
            wpa_printf(MSG_ERROR, "ioctl[SIOCSIWENCODE] (set_tx): %s", strerror(errno));
            ret = -1;
        }
    }
...
static int wpa_driver_wext_set_countermeasures(void *priv, int enabled)
{
    struct wpa_driver_wext_data *drv = priv;
    wpa_printf(MSG_DEBUG, "%s", __FUNCTION__);
    return wpa_driver_wext_set_auth_param(drv, IW_AUTH_TKIP_COUNTERMEASURES, enabled);
}
...
static int wpa_driver_wext_mlme(struct wpa_driver_wext_data *drv, const u8 *addr, int cmd, u16 reason_code)
{
    struct iwreq iwr;
    struct iw_mlme mlme;
    int ret = 0;
...
static int wpa_driver_wext_deauthenticate(void *priv, const u8 *addr, u16 reason_code)
{
    struct wpa_driver_wext_data *drv = priv;
    int ret;
    wpa_printf(MSG_DEBUG, "%s", __FUNCTION__);
...
int wpa_driver_wext_associate(void *priv, struct wpa_driver_associate_params *params)
{
    ...
    if (drv->cfg80211)
    {
        ...
        if (wpa_driver_wext_set_ssid(drv, (u8 *)"", 0) < 0)
        {
            wpa_printf(MSG_DEBUG, "WEXT: Failed to clear SSID to stop pending cfg80211 association attempts (if any)");
            /* continue anyway */
        }
    }
     ...
     * SIOCSIWENCODE here.
     */
    if (drv->auth_alg_fallback && wpa_driver_wext_auth_alg_fallback(drv, params) < 0)
        ret = -1;
    ...
    if (wpa_driver_wext_set_auth_param(drv, IW_AUTH_KEY_MGMT, value) < 0)
        ret = -1;
    value = params->key_mgmt_suite != WPA_KEY_MGMT_NONE || params->pairwise_suite != WPA_CIPHER_NONE ||
            params->group_suite != WPA_CIPHER_NONE || (params->wpa_proto & (WPA_PROTO_RSN | WPA_PROTO_WPA));
    ...
    if (wpa_driver_wext_set_psk(drv, params->psk) < 0)
        ret = -1;
    if (wpa_driver_wext_set_auth_param(drv, IW_AUTH_RX_UNENCRYPTED_EAPOL, allow_unencrypted_eapol) < 0)
        ret = -1;
    ...
    if (params->freq.freq && wpa_driver_wext_set_freq(drv, params->freq.freq) < 0)
        ret = -1;
    if (!drv->cfg80211 && wpa_driver_wext_set_ssid(drv, params->ssid, params->ssid_len) < 0)
        ret = -1;
...
static int wpa_driver_wext_add_pmkid(void *priv, struct wpa_pmkid_params *params)
{
    struct wpa_driver_wext_data *drv = priv;
    return wpa_driver_wext_pmksa(drv, IW_PMKSA_ADD, params->bssid, params->pmkid);
}

static int wpa_driver_wext_remove_pmkid(void *priv, struct wpa_pmkid_params *params)
{
    struct wpa_driver_wext_data *drv = priv;
    return wpa_driver_wext_pmksa(drv, IW_PMKSA_REMOVE, params->bssid, params->pmkid);
}
...
int wpa_driver_wext_set_operstate(void *priv, int state)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: operstate %d->%d (%s)", __func__, drv->operstate, state, state ? "UP" : "DORMANT");
    drv->operstate = state;
    return netlink_send_oper_ifla(drv->netlink, drv->ifindex, -1, state ? IF_OPER_UP : IF_OPER_DORMANT);
}
...
static int wpa_driver_wext_signal_poll(void *priv, struct wpa_signal_info *si)
{
    ...
    si->current_signal = stats.qual.level - ((stats.qual.updated & IW_QUAL_DBM) ? 0x100 : 0);
    si->current_noise  = stats.qual.noise - ((stats.qual.updated & IW_QUAL_DBM) ? 0x100 : 0);
    return 0;
}
