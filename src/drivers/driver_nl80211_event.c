...
static void nl80211_control_port_frame_tx_status(
    struct wpa_driver_nl80211_data *drv, const u8 *frame, size_t len, struct nlattr *ack, struct nlattr *cookie);
...
static void nl80211_parse_wmm_params(struct nlattr *wmm_attr, struct wmm_params *wmm_params)
{
    ...
    if (!wmm_attr || nla_parse_nested(wmm_info, NL80211_STA_WME_MAX, wmm_attr, wme_policy) ||
...
static void mlme_event_assoc(
    struct wpa_driver_nl80211_data *drv, const u8 *frame, size_t len, struct nlattr *wmm, struct nlattr *req_ie)
{
    const struct ieee80211_mgmt *mgmt;
    union wpa_event_data event;
    u16 status;
    ...
    if (ssid_len > 0)
    {
        ...
        wpa_printf(MSG_DEBUG, "nl80211: Set drv->ssid based on scan res info to '%s'",
...
static int qca_drv_connect_fail_reason_code_handler(struct nl_msg *msg, void *arg)
{
    ...
    *reason_code = 0;
    ...
    nla_parse(tb_sta_info, QCA_WLAN_VENDOR_ATTR_GET_STA_INFO_MAX, nla_data(tb[NL80211_ATTR_VENDOR_DATA]),
...
static enum qca_sta_connect_fail_reason_codes drv_get_connect_fail_reason_code(struct wpa_driver_nl80211_data *drv)
{
    ...
    if (!msg || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_GET_STA_INFO))
    {
        nlmsg_free(msg);
        return 0;
    }

    ret = send_and_recv_msgs(drv, msg, qca_drv_connect_fail_reason_code_handler, &reason_code, NULL, NULL);
    if (ret)
        wpa_printf(MSG_DEBUG, "nl80211: Get connect fail reason_code failed: ret=%d (%s)", ret, strerror(-ret));

    return reason_code;
}

static enum sta_connect_fail_reason_codes convert_connect_fail_reason_codes(
    enum qca_sta_connect_fail_reason_codes reason_code)
{
    switch (reason_code)
    {
                               ...
                               struct nlattr *fils_pmkid)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "nl80211: Connect event (status=%u ignore_next_local_disconnect=%d)", status_code,
    ...
    if (cmd == NL80211_CMD_CONNECT && status_code != WLAN_STATUS_SUCCESS)
    {
        ...
        if (drv->ignore_next_local_disconnect)
        {
            ...
            if (!event.assoc_reject.bssid ||
                (os_memcmp(event.assoc_reject.bssid, drv->auth_attempt_bssid, ETH_ALEN) != 0))
            {
                /*
        ...
        if (fils_erp_next_seq_num)
            event.assoc_reject.fils_erp_next_seq_num = nla_get_u16(fils_erp_next_seq_num);
        ...
        if (drv->get_sta_info_vendor_cmd_avail)
        {
            ...
            event.assoc_reject.reason_code = convert_connect_fail_reason_codes(reason_code);
        }
#endif /* CONFIG_DRIVER_NL80211_QCA */
    ...
    if (req_ie)
    {
        ...
        {
            ssid = get_ie(event.assoc_info.req_ies, event.assoc_info.req_ies_len, WLAN_EID_SSID);
            if (ssid && ssid[1] > 0 && ssid[1] <= 32)
            {
                drv->ssid_len = ssid[1];
                os_memcpy(drv->ssid, ssid + 2, ssid[1]);
                wpa_printf(MSG_DEBUG, "nl80211: Set drv->ssid based on req_ie to '%s'",
                           wpa_ssid_txt(drv->ssid, drv->ssid_len));
            }
        }
    }
    ...
    if ((!ssid || ssid[1] == 0 || ssid[1] > 32) && (ssid_len = nl80211_get_assoc_ssid(drv, drv->ssid)) > 0)
    {
    ...
    if (fils_erp_next_seq_num)
        event.assoc_info.fils_erp_next_seq_num = nla_get_u16(fils_erp_next_seq_num);
                                 ...
                                 int finished)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Channel switch%s event", finished ? "" : " started");
    ...
    else if (bw && cf1)
    {
        ...
        chan_offset =
            calculate_chan_offset(nla_get_u32(bw), nla_get_u32(freq), nla_get_u32(cf1), cf2 ? nla_get_u32(cf2) : 0);
        wpa_printf(MSG_DEBUG, "nl80211: Calculated channel offset: %d", chan_offset);
    }
    else
    {
        wpa_printf(
    ...
    wpa_supplicant_event(bss->ctx, finished ? EVENT_CH_SWITCH : EVENT_CH_SWITCH_STARTED, &data);
}
...
static void mlme_event_mgmt_tx_status(
    struct wpa_driver_nl80211_data *drv, struct nlattr *cookie, const u8 *frame, size_t len, struct nlattr *ack)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Frame TX status event A1=" MACSTR " %sstype=%d cookie=0x%llx%s ack=%d",
               MAC2STR(hdr->addr1), WLAN_FC_GET_TYPE(fc) != WLAN_FC_TYPE_MGMT ? "not-mgmt " : "", WLAN_FC_GET_STYPE(fc),
               (long long unsigned int)cookie_val, cookie ? "" : "(N/A)", ack != NULL);

    if (cookie_val && cookie_val == drv->eapol_tx_cookie && len >= ETH_HLEN &&
        WPA_GET_BE16(frame + 2 * ETH_ALEN) == ETH_P_PAE)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Work around misdelivered control port TX status for EAPOL");
        nl80211_control_port_frame_tx_status(drv, frame, len, ack, cookie);
        return;
    }
    ...
    if (!is_ap_interface(drv->nlmode) && WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_ACTION)
    {
        if (!cookie)
            return;

        wpa_printf(MSG_DEBUG, "nl80211: Frame TX status: cookie=0x%llx%s (ack=%d)", (long long unsigned int)cookie_val,
                   cookie_val == drv->send_frame_cookie ? " (match)" : " (unknown)", ack != NULL);
        if (cookie_val != drv->send_frame_cookie)
            return;
    }
    else if (!is_ap_interface(drv->nlmode) && WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_AUTH)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Authentication frame TX status: ack=%d", !!ack);
    }
                                       ...
                                       size_t len)
{
    ...
    if (len >= 24)
    {
        ...
        if (!(drv->capa.flags & WPA_DRIVER_FLAGS_SME) && drv->connect_reassoc && drv->associated &&
            os_memcmp(bssid, drv->prev_bssid, ETH_ALEN) == 0 &&
            os_memcmp(bssid, drv->auth_attempt_bssid, ETH_ALEN) != 0)
        {
            /*
...
static void mlme_event_unprot_beacon(struct wpa_driver_nl80211_data *drv, const u8 *frame, size_t len)
{
    const struct ieee80211_mgmt *mgmt;
    union wpa_event_data event;
                       ...
                       struct nlattr *req_ie)
{
    ...
    if (len < 4 + 2 * ETH_ALEN)
    {
        wpa_printf(MSG_MSGDUMP, "nl80211: MLME event %d (%s) on %s(" MACSTR ") - too short", cmd,
                   nl80211_command_to_string(cmd), bss->ifname, MAC2STR(bss->addr));
        return;
    }
    wpa_printf(MSG_MSGDUMP, "nl80211: MLME event %d (%s) on %s(" MACSTR ") A1=" MACSTR " A2=" MACSTR, cmd,
               nl80211_command_to_string(cmd), bss->ifname, MAC2STR(bss->addr), MAC2STR(data + 4),
               MAC2STR(data + 4 + ETH_ALEN));
    if (cmd != NL80211_CMD_FRAME_TX_STATUS && !(data[4] & 0x01) && os_memcmp(bss->addr, data + 4, ETH_ALEN) != 0 &&
        (is_zero_ether_addr(bss->rand_addr) || os_memcmp(bss->rand_addr, data + 4, ETH_ALEN) != 0) &&
    ...
    switch (cmd)
    {
        ...
        case NL80211_CMD_ASSOCIATE:
            mlme_event_assoc(drv, nla_data(frame), nla_len(frame), wmm, req_ie);
            break;
...
static void mlme_event_michael_mic_failure(struct i802_bss *bss, struct nlattr *tb[])
{
    ...
    {
        wpa_hexdump(MSG_DEBUG, "nl80211: Source MAC address", nla_data(tb[NL80211_ATTR_MAC]),
...
static void mlme_event_remain_on_channel(struct wpa_driver_nl80211_data *drv, int cancel_event, struct nlattr *tb[])
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               cancel_event, freq, chan_type, duration, (long long unsigned int)cookie,
...
static void mlme_event_dh_event(struct wpa_driver_nl80211_data *drv, struct i802_bss *bss, struct nlattr *tb[])
{
    union wpa_event_data data;
    ...
    wpa_printf(MSG_DEBUG, "nl80211: DH event - peer " MACSTR, MAC2STR(data.update_dh.peer));

    wpa_supplicant_event(bss->ctx, EVENT_UPDATE_DH, &data);
}

static void send_scan_event(struct wpa_driver_nl80211_data *drv, int aborted, struct nlattr *tb[], int external_scan)
{
    union wpa_event_data event;
    struct nlattr *nl;
    int rem;
...
#define MAX_REPORT_FREQS 100
    ...
    if (tb[NL80211_ATTR_SCAN_START_TIME_TSF] && tb[NL80211_ATTR_SCAN_START_TIME_TSF_BSSID])
    {
        info->scan_start_tsf = nla_get_u64(tb[NL80211_ATTR_SCAN_START_TIME_TSF]);
        os_memcpy(info->scan_start_tsf_bssid, nla_data(tb[NL80211_ATTR_SCAN_START_TIME_TSF_BSSID]), ETH_ALEN);
    }
...
static void nl80211_cqm_event(struct wpa_driver_nl80211_data *drv, struct nlattr *tb[])
{
    ...
    if (cqm[NL80211_ATTR_CQM_PKT_LOSS_EVENT])
    {
        ...
        ed.low_ack.num_packets = nla_get_u32(cqm[NL80211_ATTR_CQM_PKT_LOSS_EVENT]);
        wpa_printf(MSG_DEBUG, "nl80211: Packet loss event for " MACSTR " (num_packets %u)", MAC2STR(ed.low_ack.addr),
    ...
    if (cqm[NL80211_ATTR_CQM_TXE_RATE] && cqm[NL80211_ATTR_CQM_TXE_PKTS] && cqm[NL80211_ATTR_CQM_TXE_INTVL] &&
        cqm[NL80211_ATTR_MAC])
    {
        wpa_printf(MSG_DEBUG, "nl80211: CQM TXE event for " MACSTR " (rate: %u pkts: %u interval: %u)",
                   MAC2STR((u8 *)nla_data(cqm[NL80211_ATTR_MAC])), nla_get_u32(cqm[NL80211_ATTR_CQM_TXE_RATE]),
                   nla_get_u32(cqm[NL80211_ATTR_CQM_TXE_PKTS]), nla_get_u32(cqm[NL80211_ATTR_CQM_TXE_INTVL]));
        return;
    }

    if (cqm[NL80211_ATTR_CQM_RSSI_THRESHOLD_EVENT] == NULL)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Not a CQM RSSI threshold event");
        return;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "nl80211: Unknown CQM RSSI threshold event: %d", event);
        return;
    }
    ...
    if (res == 0)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Signal: %d dBm  txrate: %d", ed.signal_change.current_signal,
                   ed.signal_change.current_txrate);
    }
    else
    {
        wpa_printf(MSG_DEBUG, "nl80211: Querying the driver for signal info failed");
    }

    res = nl80211_get_link_noise(drv, &ed.signal_change);
    if (res == 0)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Noise: %d dBm", ed.signal_change.current_noise);
    }
    else
    {
        wpa_printf(MSG_DEBUG, "nl80211: Querying the driver for noise info failed");
    }
...
static void nl80211_del_station_event(struct wpa_driver_nl80211_data *drv, struct i802_bss *bss, struct nlattr **tb)
{
    u8 *addr;
    union wpa_event_data data;
...
static void nl80211_rekey_offload_event(struct wpa_driver_nl80211_data *drv, struct nlattr **tb)
{
    ...
    if (!tb[NL80211_ATTR_MAC] || !tb[NL80211_ATTR_REKEY_DATA] ||
        nla_parse_nested(rekey_info, MAX_NL80211_REKEY_DATA, tb[NL80211_ATTR_REKEY_DATA], rekey_policy) ||
    ...
    wpa_hexdump(MSG_DEBUG, "nl80211: Rekey offload - Replay Counter", data.driver_gtk_rekey.replay_ctr,
...
static void nl80211_pmksa_candidate_event(struct wpa_driver_nl80211_data *drv, struct nlattr **tb)
{
    ...
    if (!tb[NL80211_ATTR_PMKSA_CANDIDATE] ||
        nla_parse_nested(cand, MAX_NL80211_PMKSA_CANDIDATE, tb[NL80211_ATTR_PMKSA_CANDIDATE], cand_policy) ||
        !cand[NL80211_PMKSA_CANDIDATE_INDEX] || !cand[NL80211_PMKSA_CANDIDATE_BSSID])
        return;
...
static void nl80211_client_probe_event(struct wpa_driver_nl80211_data *drv, struct nlattr **tb)
{
    ...
    if (tb[NL80211_ATTR_COOKIE])
        cookie = nla_get_u64(tb[NL80211_ATTR_COOKIE]);
    wpa_printf(MSG_DEBUG, "nl80211: Probe client event (addr=" MACSTR " ack=%d cookie=%llu)", MAC2STR(addr),
               tb[NL80211_ATTR_ACK] != NULL, (long long unsigned int)cookie);
...
static void nl80211_radar_event(struct wpa_driver_nl80211_data *drv, struct nlattr **tb)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: DFS event on freq %d MHz, ht: %d, offset: %d, width: %d, cf1: %dMHz, cf2: %dMHz",
               data.dfs_event.freq, data.dfs_event.ht_enabled, data.dfs_event.chan_offset, data.dfs_event.chan_width,
    ...
    switch (event_type)
    {
        ...
        case NL80211_RADAR_PRE_CAC_EXPIRED:
            wpa_supplicant_event(drv->ctx, EVENT_DFS_PRE_CAC_EXPIRED, &data);
            break;
...
static unsigned int chan_to_freq(struct wpa_driver_nl80211_data *drv, u8 chan, enum hostapd_hw_mode hw_mode)
{
    if (hw_mode == NUM_HOSTAPD_MODES)
    {
        ...
        modes = nl80211_get_hw_feature_data(drv->first_bss, &num_modes, &flags, &dfs_domain);
        if (!modes)
        {
            wpa_printf(MSG_DEBUG, "nl80211: Fetching hardware mode failed");
            goto try_2_4_or_5;
        }
        if (num_modes == 1)
...
static void qca_nl80211_acs_select_ch(struct wpa_driver_nl80211_data *drv, const u8 *data, size_t len)
{
    ...
    if (nla_parse(tb, QCA_WLAN_VENDOR_ATTR_ACS_MAX, (struct nlattr *)data, len, NULL) ||
        (!tb[QCA_WLAN_VENDOR_ATTR_ACS_PRIMARY_FREQUENCY] && !tb[QCA_WLAN_VENDOR_ATTR_ACS_PRIMARY_CHANNEL]) ||
        (!tb[QCA_WLAN_VENDOR_ATTR_ACS_SECONDARY_FREQUENCY] && !tb[QCA_WLAN_VENDOR_ATTR_ACS_SECONDARY_CHANNEL]))
        return;
    ...
    {
        event.acs_selected_channels.pri_freq = nla_get_u32(tb[QCA_WLAN_VENDOR_ATTR_ACS_PRIMARY_FREQUENCY]);
    }
    else
    {
        chan                                 = nla_get_u8(tb[QCA_WLAN_VENDOR_ATTR_ACS_PRIMARY_CHANNEL]);
        event.acs_selected_channels.pri_freq = chan_to_freq(drv, chan, event.acs_selected_channels.hw_mode);
    }

    if (tb[QCA_WLAN_VENDOR_ATTR_ACS_SECONDARY_FREQUENCY])
    {
        event.acs_selected_channels.sec_freq = nla_get_u32(tb[QCA_WLAN_VENDOR_ATTR_ACS_SECONDARY_FREQUENCY]);
    }
    else
    {
        chan                                 = nla_get_u8(tb[QCA_WLAN_VENDOR_ATTR_ACS_SECONDARY_CHANNEL]);
        event.acs_selected_channels.sec_freq = chan_to_freq(drv, chan, event.acs_selected_channels.hw_mode);
    }
    ...
    wpa_printf(MSG_INFO, "nl80211: ACS Results: PFreq: %d SFreq: %d BW: %d VHT0: %d VHT1: %d HW_MODE: %d EDMGCH: %d",
               event.acs_selected_channels.pri_freq, event.acs_selected_channels.sec_freq,
               event.acs_selected_channels.ch_width, event.acs_selected_channels.vht_seg0_center_ch,
...
static void qca_nl80211_key_mgmt_auth(struct wpa_driver_nl80211_data *drv, const u8 *data, size_t len)
{
    ...
    if (nla_parse(tb, QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_MAX, (struct nlattr *)data, len, NULL) ||
        !tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_BSSID] || nla_len(tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_BSSID]) != ETH_ALEN ||
        !tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_REQ_IE] || !tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_RESP_IE] ||
    ...
    mlme_event_connect(drv, NL80211_CMD_ROAM, NULL, tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_BSSID],
                       tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_REQ_IE], tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_RESP_IE], NULL,
                       NULL, tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_AUTHORIZED],
                       tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_KEY_REPLAY_CTR], tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_PTK_KCK],
                       tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_PTK_KEK], tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_SUBNET_STATUS],
                       tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_FILS_ERP_NEXT_SEQ_NUM], tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_PMK],
                       tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_PMKID]);
}

static void qca_nl80211_key_mgmt_auth_handler(struct wpa_driver_nl80211_data *drv, const u8 *data, size_t len)
{
    if (!drv->roam_indication_done)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Pending roam indication, delay processing roam+auth vendor event");
...
static void qca_nl80211_scan_trigger_event(struct wpa_driver_nl80211_data *drv, u8 *data, size_t len)
{
    ...
    if (nla_parse(tb, QCA_WLAN_VENDOR_ATTR_SCAN_MAX, (struct nlattr *)data, len, NULL) ||
...
static void qca_nl80211_scan_done_event(struct wpa_driver_nl80211_data *drv, u8 *data, size_t len)
{
    struct nlattr *tb[QCA_WLAN_VENDOR_ATTR_SCAN_MAX + 1];
    u64 cookie = 0;
    ...
    if (nla_parse(tb, QCA_WLAN_VENDOR_ATTR_SCAN_MAX, (struct nlattr *)data, len, NULL) ||
        !tb[QCA_WLAN_VENDOR_ATTR_SCAN_STATUS] || !tb[QCA_WLAN_VENDOR_ATTR_SCAN_COOKIE])
        return;
    ...
    send_vendor_scan_event(drv, (status == VENDOR_SCAN_STATUS_ABORTED), tb, external_scan);
}

static void qca_nl80211_p2p_lo_stop_event(struct wpa_driver_nl80211_data *drv, u8 *data, size_t len)
{
    struct nlattr *tb[QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_MAX + 1];
    union wpa_event_data event;

    wpa_printf(MSG_DEBUG, "nl80211: P2P listen offload stop vendor event received");

    if (nla_parse(tb, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_MAX, (struct nlattr *)data, len, NULL) ||
        !tb[QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_STOP_REASON])
        return;

    os_memset(&event, 0, sizeof(event));
    event.p2p_lo_stop.reason_code = nla_get_u8(tb[QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_STOP_REASON]);

    wpa_printf(MSG_DEBUG, "nl80211: P2P Listen offload stop reason: %d", event.p2p_lo_stop.reason_code);
...
static void brcm_nl80211_acs_select_ch(struct wpa_driver_nl80211_data *drv, const u8 *data, size_t len)
{
    struct nlattr *tb[BRCM_VENDOR_ATTR_ACS_LAST + 1];
    union wpa_event_data event;

    wpa_printf(MSG_DEBUG, "nl80211: BRCM ACS channel selection vendor event received");

    if (nla_parse(tb, BRCM_VENDOR_ATTR_ACS_LAST, (struct nlattr *)data, len, NULL) ||
        !tb[BRCM_VENDOR_ATTR_ACS_PRIMARY_FREQ] || !tb[BRCM_VENDOR_ATTR_ACS_SECONDARY_FREQ])
        return;
    ...
    if (tb[BRCM_VENDOR_ATTR_ACS_SECONDARY_FREQ])
        event.acs_selected_channels.sec_freq = nla_get_u32(tb[BRCM_VENDOR_ATTR_ACS_SECONDARY_FREQ]);
    if (tb[BRCM_VENDOR_ATTR_ACS_VHT_SEG0_CENTER_CHANNEL])
        event.acs_selected_channels.vht_seg0_center_ch = nla_get_u8(tb[BRCM_VENDOR_ATTR_ACS_VHT_SEG0_CENTER_CHANNEL]);
    if (tb[BRCM_VENDOR_ATTR_ACS_VHT_SEG0_CENTER_CHANNEL])
        event.acs_selected_channels.vht_seg1_center_ch = nla_get_u8(tb[BRCM_VENDOR_ATTR_ACS_VHT_SEG1_CENTER_CHANNEL]);
    if (tb[BRCM_VENDOR_ATTR_ACS_CHWIDTH])
        event.acs_selected_channels.ch_width = nla_get_u16(tb[BRCM_VENDOR_ATTR_ACS_CHWIDTH]);
    if (tb[BRCM_VENDOR_ATTR_ACS_HW_MODE])
    {
        ...
        if (event.acs_selected_channels.hw_mode == NUM_HOSTAPD_MODES ||
            event.acs_selected_channels.hw_mode == HOSTAPD_MODE_IEEE80211ANY)
        {
            wpa_printf(MSG_DEBUG, "nl80211: Invalid hw_mode %d in ACS selection event",
                       event.acs_selected_channels.hw_mode);
            return;
        }
    }

    wpa_printf(MSG_DEBUG, "nl80211: ACS Results: PCH: %d SCH: %d BW: %d VHT0: %d VHT1: %d HW_MODE: %d",
               event.acs_selected_channels.pri_freq, event.acs_selected_channels.sec_freq,
               event.acs_selected_channels.ch_width, event.acs_selected_channels.vht_seg0_center_ch,
               event.acs_selected_channels.vht_seg1_center_ch, event.acs_selected_channels.hw_mode);
    wpa_supplicant_event(drv->ctx, EVENT_ACS_CHANNEL_SELECTED, &event);
...
static void nl80211_vendor_event_brcm(struct wpa_driver_nl80211_data *drv, u32 subcmd, u8 *data, size_t len)
{
    ...
    switch (subcmd)
    {
        ...
        case BRCM_VENDOR_EVENT_ACS:
            ...
            break;
        default:
            wpa_printf(MSG_DEBUG, "%s: Ignore unsupported BRCM vendor event %u", __func__, subcmd);
            break;
    }
}
...
static void nl80211_reg_change_event(struct wpa_driver_nl80211_data *drv, struct nlattr *tb[])
{
    ...
    {
        os_strlcpy(data.channel_list_changed.alpha2, nla_get_string(tb[NL80211_ATTR_REG_ALPHA2]),
...
static void nl80211_dump_freq(const char *title, struct nlattr *nl_freq)
{
    ...
    nla_parse(tb, NL80211_FREQUENCY_ATTR_MAX, nla_data(nl_freq), nla_len(nl_freq), freq_policy);
    ...
    if (tb[NL80211_FREQUENCY_ATTR_MAX_TX_POWER])
        max_tx_power = nla_get_u32(tb[NL80211_FREQUENCY_ATTR_MAX_TX_POWER]);

    wpa_printf(MSG_DEBUG, "nl80211: Channel (%s): freq=%u max_tx_power=%u%s%s%s", title, freq, max_tx_power,
               tb[NL80211_FREQUENCY_ATTR_DISABLED] ? " disabled" : "", tb[NL80211_FREQUENCY_ATTR_NO_IR] ? " no-IR" : "",
               tb[NL80211_FREQUENCY_ATTR_RADAR] ? " radar" : "");
}

static void nl80211_reg_beacon_hint_event(struct wpa_driver_nl80211_data *drv, struct nlattr *tb[])
{
    union wpa_event_data data;
...
static void nl80211_external_auth(struct wpa_driver_nl80211_data *drv, struct nlattr **tb)
{
    union wpa_event_data event;
    enum nl80211_external_auth_action act;

    if (!tb[NL80211_ATTR_AKM_SUITES] || !tb[NL80211_ATTR_EXTERNAL_AUTH_ACTION] || !tb[NL80211_ATTR_BSSID] ||
    ...
    event.external_auth.key_mgmt_suite = nla_get_u32(tb[NL80211_ATTR_AKM_SUITES]);
    ...
    wpa_printf(MSG_DEBUG, "nl80211: External auth action: %u, AKM: 0x%x", event.external_auth.action,
               event.external_auth.key_mgmt_suite);
    wpa_supplicant_event(drv->ctx, EVENT_EXTERNAL_AUTH, &event);
}

static void nl80211_port_authorized(struct wpa_driver_nl80211_data *drv, struct nlattr **tb)
{
    const u8 *addr;

    if (!tb[NL80211_ATTR_MAC] || nla_len(tb[NL80211_ATTR_MAC]) != ETH_ALEN)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Ignore port authorized event without BSSID");
        return;
    }
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "nl80211: Ignore port authorized event for " MACSTR " (not the currently connected BSSID " MACSTR
                   ")",
...
static void nl80211_sta_opmode_change_event(struct wpa_driver_nl80211_data *drv, struct nlattr **tb)
{
    union wpa_event_data ed;
    u8 smps_mode, max_bw;

    if (!tb[NL80211_ATTR_MAC] ||
        (!tb[NL80211_ATTR_CHANNEL_WIDTH] && !tb[NL80211_ATTR_SMPS_MODE] && !tb[NL80211_ATTR_NSS]))
        return;
...
static void nl80211_control_port_frame(struct wpa_driver_nl80211_data *drv, struct nlattr **tb)
{
    u8 *src_addr;
    u16 ethertype;

    if (!tb[NL80211_ATTR_MAC] || !tb[NL80211_ATTR_FRAME] || !tb[NL80211_ATTR_CONTROL_PORT_ETHERTYPE])
        return;
    ...
    switch (ethertype)
    {
        case ETH_P_RSN_PREAUTH:
            wpa_printf(MSG_INFO, "nl80211: Got pre-auth frame from " MACSTR " over control port unexpectedly",
                       MAC2STR(src_addr));
            break;
        case ETH_P_PAE:
            drv_event_eapol_rx(drv->ctx, src_addr, nla_data(tb[NL80211_ATTR_FRAME]), nla_len(tb[NL80211_ATTR_FRAME]));
            break;
        default:
            wpa_printf(MSG_INFO, "nl80211: Unexpected ethertype 0x%04x from " MACSTR " over control port", ethertype,
                       MAC2STR(src_addr));
            break;
    }
}
    ...
    struct wpa_driver_nl80211_data *drv, const u8 *frame, size_t len, struct nlattr *ack, struct nlattr *cookie)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Control port TX status (ack=%d), cookie=%llu", ack != NULL,
               (long long unsigned int)nla_get_u64(cookie));
...
static void nl80211_frame_wait_cancel(struct wpa_driver_nl80211_data *drv, struct nlattr *cookie_attr)
{
    unsigned int i;
    u64 cookie;
    bool match = false;
    ...
    wpa_printf(MSG_DEBUG, "nl80211: TX frame wait expired for cookie 0x%llx%s%s", (long long unsigned int)cookie,
               match ? " (match)" : "", drv->send_frame_cookie == cookie ? " (match-saved)" : "");
    ...
    if (i < drv->num_send_frame_cookies - 1)
        os_memmove(&drv->send_frame_cookies[i], &drv->send_frame_cookies[i + 1],
...
static void nl80211_assoc_comeback(struct wpa_driver_nl80211_data *drv, struct nlattr *mac, struct nlattr *timeout)
{
    if (!mac || !timeout)
        return;
    wpa_printf(MSG_DEBUG, "nl80211: Association comeback requested by " MACSTR " (timeout: %u ms)",
...
static void do_process_drv_event(struct i802_bss *bss, int cmd, struct nlattr **tb)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Drv Event %d (%s) received for %s", cmd, nl80211_command_to_string(cmd),
               bss->ifname);
    ...
    if (cmd == NL80211_CMD_ROAM && (drv->capa.flags & WPA_DRIVER_FLAGS_KEY_MGMT_OFFLOAD))
    {
        if (drv->pending_roam_data)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "nl80211: Process pending roam+auth vendor event");
                qca_nl80211_key_mgmt_auth(drv, drv->pending_roam_data, drv->pending_roam_data_len);
            }
            os_free(drv->pending_roam_data);
    ...
    switch (cmd)
    {
        ...
        case NL80211_CMD_UNPROT_DEAUTHENTICATE:
        case NL80211_CMD_UNPROT_DISASSOCIATE:
            mlme_event(bss, cmd, tb[NL80211_ATTR_FRAME], tb[NL80211_ATTR_MAC], tb[NL80211_ATTR_TIMED_OUT],
                       tb[NL80211_ATTR_WIPHY_FREQ], tb[NL80211_ATTR_ACK], tb[NL80211_ATTR_COOKIE],
                       tb[NL80211_ATTR_RX_SIGNAL_DBM], tb[NL80211_ATTR_STA_WME], tb[NL80211_ATTR_REQ_IE]);
            break;
        case NL80211_CMD_CONNECT:
        case NL80211_CMD_ROAM:
            mlme_event_connect(drv, cmd, tb[NL80211_ATTR_STATUS_CODE], tb[NL80211_ATTR_MAC], tb[NL80211_ATTR_REQ_IE],
                               tb[NL80211_ATTR_RESP_IE], tb[NL80211_ATTR_TIMED_OUT], tb[NL80211_ATTR_TIMEOUT_REASON],
                               NULL, NULL, NULL, tb[NL80211_ATTR_FILS_KEK], NULL,
                               tb[NL80211_ATTR_FILS_ERP_NEXT_SEQ_NUM], tb[NL80211_ATTR_PMK], tb[NL80211_ATTR_PMKID]);
            break;
        case NL80211_CMD_CH_SWITCH_STARTED_NOTIFY:
            mlme_event_ch_switch(drv, tb[NL80211_ATTR_IFINDEX], tb[NL80211_ATTR_WIPHY_FREQ],
                                 tb[NL80211_ATTR_WIPHY_CHANNEL_TYPE], tb[NL80211_ATTR_CHANNEL_WIDTH],
                                 tb[NL80211_ATTR_CENTER_FREQ1], tb[NL80211_ATTR_CENTER_FREQ2], 0);
            break;
        case NL80211_CMD_CH_SWITCH_NOTIFY:
            mlme_event_ch_switch(drv, tb[NL80211_ATTR_IFINDEX], tb[NL80211_ATTR_WIPHY_FREQ],
                                 tb[NL80211_ATTR_WIPHY_CHANNEL_TYPE], tb[NL80211_ATTR_CHANNEL_WIDTH],
                                 tb[NL80211_ATTR_CENTER_FREQ1], tb[NL80211_ATTR_CENTER_FREQ2], 1);
            break;
        case NL80211_CMD_DISCONNECT:
            mlme_event_disconnect(drv, tb[NL80211_ATTR_REASON_CODE], tb[NL80211_ATTR_MAC],
        ...
        case NL80211_CMD_CONTROL_PORT_FRAME_TX_STATUS:
            if (!frame)
                break;
            nl80211_control_port_frame_tx_status(drv, nla_data(frame), nla_len(frame), tb[NL80211_ATTR_ACK],
        ...
        case NL80211_CMD_ASSOC_COMEBACK:
            nl80211_assoc_comeback(drv, tb[NL80211_ATTR_MAC], tb[NL80211_ATTR_TIMEOUT]);
            break;
        default:
...
int process_global_event(struct nl_msg *msg, void *arg)
{
    ...
    dl_list_for_each_safe(drv, tmp, &global->interfaces, struct wpa_driver_nl80211_data, list)
    {
        for (bss = drv->first_bss; bss; bss = bss->next)
        {
            ...
            if ((ifidx == -1 && !wiphy_idx_set && !wdev_id_set) || ifidx == bss->ifindex ||
        ...
        wpa_printf(MSG_DEBUG, "nl80211: Ignored event %d (%s) for foreign interface (ifindex %d wdev 0x%llx)",
                   gnlh->cmd, nl80211_command_to_string(gnlh->cmd), ifidx, (long long unsigned int)wdev_id);
    }
...
int process_bss_event(struct nl_msg *msg, void *arg)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: BSS Event %d (%s) received for %s", gnlh->cmd, nl80211_command_to_string(gnlh->cmd),
               bss->ifname);
    ...
    switch (gnlh->cmd)
    {
        case NL80211_CMD_FRAME:
        case NL80211_CMD_FRAME_TX_STATUS:
            mlme_event(bss, gnlh->cmd, tb[NL80211_ATTR_FRAME], tb[NL80211_ATTR_MAC], tb[NL80211_ATTR_TIMED_OUT],
                       tb[NL80211_ATTR_WIPHY_FREQ], tb[NL80211_ATTR_ACK], tb[NL80211_ATTR_COOKIE],
                       tb[NL80211_ATTR_RX_SIGNAL_DBM], tb[NL80211_ATTR_STA_WME], NULL);
            break;
