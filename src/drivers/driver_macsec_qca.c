...
static void macsec_qca_handle_data(void *ctx, unsigned char *buf, size_t len)
{
    ...
    if (len < 14)
    {
        wpa_printf(MSG_MSGDUMP, "macsec_qca_handle_data: too short (%lu)", (unsigned long)len);
        return;
    }
...
static int macsec_qca_init_sockets(struct macsec_qca_data *drv, u8 *own_addr)
{
    ...
    if (eloop_register_read_sock(drv->common.sock, macsec_qca_handle_read, drv->common.ctx, NULL))
    {
    ...
    {
        wpa_printf(MSG_ERROR, "macsec_qca_init_sockets: Failed to add multicast group membership");
        return -1;
    }
...
static int macsec_qca_secy_id_get(const char *ifname, u32 *secy_id)
{
    ...
    else if (os_strcmp(ifname, "eth4") == 0 || os_strcmp(ifname, "eth0") == 0)
    {
        *secy_id = 0;
    }
    else if (os_strcmp(ifname, "eth5") == 0 || os_strcmp(ifname, "eth1") == 0)
    {
        *secy_id = 1;
    }
    else
...
static void *macsec_qca_init(void *ctx, const char *ifname)
{
    ...
    {
        wpa_printf(MSG_ERROR, "macsec_qca: Failed to get secy_id for %s", ifname);
        os_free(drv);
        return NULL;
    }
...
static void *macsec_qca_hapd_init(struct hostapd_data *hapd, struct wpa_init_params *params)
{
    struct macsec_qca_data *drv;
    ...
    if (!drv)
    {
        wpa_printf(MSG_INFO, "Could not allocate memory for macsec_qca driver data");
        return NULL;
    }

    if (macsec_qca_secy_id_get(params->ifname, &drv->secy_id))
    {
        wpa_printf(MSG_ERROR, "macsec_qca: Failed to get secy_id for %s", params->ifname);
        os_free(drv);
        return NULL;
    }
...
static int macsec_qca_send_eapol(
    void *priv, const u8 *addr, const u8 *data, size_t data_len, int encrypt, const u8 *own_addr, u32 flags)
{
    ...
    if (!hdr)
    {
        wpa_printf(MSG_INFO, "malloc() failed for macsec_qca_send_eapol(len=%lu)", (unsigned long)len);
        return -1;
    }
    ...
    if (res < 0)
    {
        wpa_printf(MSG_ERROR, "macsec_qca_send_eapol - packet len: %lu - failed: send: %s", (unsigned long)len,
                   strerror(errno));
    }
...
static int macsec_qca_macsec_init(void *priv, struct macsec_init_params *params)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: es=%d, scb=%d, sci=%d", __func__, drv->use_es, drv->use_scb, drv->always_include_sci);
...
static int macsec_qca_set_replay_protect(void *priv, bool enabled, unsigned int window)
{
    struct macsec_qca_data *drv = priv;
    int ret                     = 0;
...
static int macsec_qca_lookup_channel(struct channel_map *map, struct ieee802_1x_mka_sci *sci, u32 *channel)
{
    ...
    for (i = 0; i < MAXSC; i++)
    {
        if (os_memcmp(&map[i].sci, sci, sizeof(struct ieee802_1x_mka_sci)) == 0)
        {
            *channel = i;
            return 0;
        }
...
static int macsec_qca_get_receive_lowest_pn(void *priv, struct receive_sa *sa)
{
    ...
    ret += nss_macsec_secy_rx_sa_next_pn_get(drv->secy_id, channel, sa->an, &next_pn);
    ret += nss_macsec_secy_rx_sc_replay_protect_get(drv->secy_id, channel, &enabled);
    ret += nss_macsec_secy_rx_sc_anti_replay_window_get(drv->secy_id, channel, &win);
...
static int macsec_qca_get_transmit_next_pn(void *priv, struct transmit_sa *sa)
{
    ...
    ret += nss_macsec_secy_tx_sa_next_pn_get(drv->secy_id, channel, sa->an, &sa->next_pn);
...
static int macsec_qca_set_transmit_next_pn(void *priv, struct transmit_sa *sa)
{
    ...
    ret += nss_macsec_secy_tx_sa_next_pn_set(drv->secy_id, channel, sa->an, sa->next_pn);
...
static int macsec_qca_create_receive_sc(void *priv, struct receive_sc *sc, unsigned int conf_offset, int validation)
{
    struct macsec_qca_data *drv = priv;
    int ret                     = 0;
    ...
    ret += nss_macsec_secy_rx_sc_replay_protect_set(drv->secy_id, channel, drv->replay_protect);
    ret += nss_macsec_secy_rx_sc_anti_replay_window_set(drv->secy_id, channel, drv->replay_window);
...
static int macsec_qca_create_receive_sa(void *priv, struct receive_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, "%s, channel=%d, an=%d, lpn=0x%x", __func__, channel, sa->an, sa->lowest_pn);
...
static int macsec_qca_get_available_transmit_sc(void *priv, u32 *channel)
{
    ...
    for (sc_ch = 0; sc_ch < MAXSC; sc_ch++)
    {
        if (nss_macsec_secy_tx_sc_in_used_get(drv->secy_id, sc_ch, &in_use))
            continue;
...
static int macsec_qca_create_transmit_sc(void *priv, struct transmit_sc *sc, unsigned int conf_offset)
{
    struct macsec_qca_data *drv = priv;
    int ret;
    ...
    ret += nss_macsec_secy_tx_sc_protect_set(drv->secy_id, channel, drv->protect_frames);
    ret += nss_macsec_secy_tx_sc_confidentiality_offset_set(drv->secy_id, channel, conf_offset);
...
static int macsec_qca_create_transmit_sa(void *priv, struct transmit_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: channel=%d, an=%d, next_pn=0x%x, confidentiality=%d", __func__, channel, sa->an,
               sa->next_pn, sa->confidentiality);
    ...
    if (sa->pkey->confidentiality_offset == CONFIDENTIALITY_OFFSET_0)
        ...
        return -1;
    ret += nss_macsec_secy_tx_sc_confidentiality_offset_set(drv->secy_id, channel, offset);
    ret += nss_macsec_secy_tx_sa_next_pn_set(drv->secy_id, channel, sa->an, sa->next_pn);
