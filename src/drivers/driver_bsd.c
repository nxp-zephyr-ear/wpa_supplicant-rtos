...
static struct bsd_driver_data *bsd_get_drvindex(void *priv, unsigned int ifindex)
{
...
static int bsd_get_ssid(void *priv, u8 *ssid, int len)
{
    ...
    if (ioctl(drv->global->sock, SIOCG80211NWID, &ifr) < 0 || nwid.i_len > IEEE80211_NWID_LEN)
        return -1;
...
static int bsd_get_iface_flags(struct bsd_driver_data *drv)
{
    struct ifreq ifr;
...
static int bsd_set_key(void *priv, struct wpa_driver_set_key_params *params)
{
...
static int bsd_configure_wpa(void *priv, struct wpa_bss_params *params)
{
    ...
    static const char *ciphernames[] = {"WEP", "TKIP", "AES-OCB", "AES-CCM", "CKIP", "NONE"};
    int v;
    ...
    wpa_printf(MSG_DEBUG, "%s: group key cipher=%s (%u)", __func__, ciphernames[v], v);
    if (set80211param(priv, IEEE80211_IOC_MCASTCIPHER, v))
    {
        wpa_printf(MSG_INFO, "Unable to set group key cipher to %u (%s)", v, ciphernames[v]);
        return -1;
    }
    ...
    if (params->rsn_preauth)
        v |= BIT(0);
    wpa_printf(MSG_DEBUG, "%s: rsn capabilities=0x%x", __func__, params->rsn_preauth);
...
static int bsd_set_ieee8021x(void *priv, struct wpa_bss_params *params)
{
    ...
    if (set80211param(priv, IEEE80211_IOC_AUTHMODE, (params->wpa ? IEEE80211_AUTH_WPA : IEEE80211_AUTH_8021X)))
    {
...
static int bsd_set_freq(void *priv, struct hostapd_freq_params *freq)
{
    ...
    if (channel < 14)
    {
        mode = freq->ht_enabled ? IFM_IEEE80211_11NG : IFM_IEEE80211_11G;
    }
    else if (channel == 14)
    {
    ...
    {
        mode = freq->ht_enabled ? IFM_IEEE80211_11NA : IFM_IEEE80211_11A;
    }
...
static int bsd_set_opt_ie(void *priv, const u8 *ie, size_t ie_len)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: set WPA+RSN ie (len %lu)", __func__, (unsigned long)ie_len);
    return bsd_set80211(priv, IEEE80211_IOC_APPIE, IEEE80211_APPIE_WPA, ie, ie_len);
#endif /* IEEE80211_IOC_APPIE */
    return 0;
}

#ifdef SO_RERROR
static void bsd_route_overflow(int sock, void *ctx, struct bsd_driver_global *global)
{
    ...
    {
        wpa_printf(MSG_ERROR, "%s getifaddrs() failed: %s", __func__, strerror(errno));
        return;
    }
    ...
    for (ifa = ifaddrs; ifa != NULL; ifa = ifa->ifa_next)
    {
        ...
        if (drv != NULL && (drv->ifindex != sdl->sdl_index || drv->if_removed))
        {
            wpa_printf(MSG_DEBUG, "RTM_IFANNOUNCE: Interface '%s' added", drv->ifname);
            drv->ifindex                  = sdl->sdl_index;
            drv->if_removed               = 0;
            event.interface_status.ievent = EVENT_INTERFACE_ADDED;
            os_strlcpy(event.interface_status.ifname, ifa->ifa_name, sizeof(event.interface_status.ifname));
            wpa_supplicant_event(drv->ctx, EVENT_INTERFACE_STATUS, &event);
        }
        if (!drv && (drv = bsd_get_drvindex(global, sdl->sdl_index)) != NULL)
        {
            /* Driver name is invalid */
            wpa_printf(MSG_DEBUG, "RTM_IFANNOUNCE: Interface '%s' removed", drv->ifname);
            drv->if_removed               = 1;
    ...
    dl_list_for_each(drv, &global->ifaces, struct bsd_driver_data, list)
    {
        ...
        if ((ifa->ifa_flags & IFF_UP) == 0 && (drv->flags & IFF_UP) != 0)
        {
            wpa_printf(MSG_DEBUG, "RTM_IFINFO: Interface '%s' DOWN", drv->ifname);
            wpa_supplicant_event(drv->ctx, EVENT_INTERFACE_DISABLED, NULL);
        }
        else if ((ifa->ifa_flags & IFF_UP) != 0 && (drv->flags & IFF_UP) == 0)
        {
...
static void bsd_wireless_event_receive(int sock, void *ctx, void *sock_ctx)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "Invalid routing message version=%d", rtm->rtm_version);
        return;
    }
    ...
    switch (rtm->rtm_type)
    {
        ...
        case RTM_IFANNOUNCE:
            ...
            os_strlcpy(event.interface_status.ifname, ifan->ifan_name, sizeof(event.interface_status.ifname));
            if (drv)
            {
                wpa_supplicant_event(drv->ctx, EVENT_INTERFACE_STATUS, &event);
                /*
...
static int bsd_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr, u16 reason_code);
...
static int bsd_get_seqnum(const char *ifname, void *priv, const u8 *addr, int idx, u8 *seq)
{
    struct ieee80211req_key wk;
...
static int bsd_read_sta_driver_data(void *priv, struct hostap_sta_driver_data *data, const u8 *addr)
{
...
static int bsd_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr, u16 reason_code)
{
...
static void *bsd_init(struct hostapd_data *hapd, struct wpa_init_params *params)
{
    struct bsd_driver_data *drv;
    ...
    if (drv->ifindex == 0)
    {
        wpa_printf(MSG_DEBUG, "%s: interface %s does not exist", __func__, params->ifname);
        goto bad;
    }
    ...
    drv->sock_xmit = l2_packet_init(drv->ifname, NULL, ETH_P_EAPOL, handle_read, drv, 0);
...
static int bsd_set_sta_authorized(
...
static int wpa_driver_bsd_get_bssid(void *priv, u8 *bssid)
{
    ...
    return 0;
#else
    return get80211var(drv, IEEE80211_IOC_BSSID, bssid, IEEE80211_ADDR_LEN) < 0 ? -1 : 0;
#endif
}
...
static int wpa_driver_bsd_deauthenticate(void *priv, const u8 *addr, u16 reason_code)
{
    return bsd_send_mlme_param(priv, IEEE80211_MLME_DEAUTH, reason_code, addr);
}
...
static int wpa_driver_bsd_associate(void *priv, struct wpa_driver_associate_params *params)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: ssid '%.*s' wpa ie len %u pairwise %u group %u key mgmt %u", __func__,
               (unsigned int)params->ssid_len, params->ssid, (unsigned int)params->wpa_ie_len, params->pairwise_suite,
    ...
    privacy = !(params->pairwise_suite == WPA_CIPHER_NONE && params->group_suite == WPA_CIPHER_NONE &&
    ...
    if (params->wpa_ie_len && set80211param(drv, IEEE80211_IOC_WPA, params->wpa_ie[0] == WLAN_EID_RSN ? 2 : 1) < 0)
        return -1;
...
static int wpa_driver_bsd_scan(void *priv, struct wpa_driver_scan_params *params)
{
    ...
    {
        wpa_printf(MSG_ERROR, "%s: failed to set wpa: %s", __func__, strerror(errno));
        return -1;
    }
    ...
    if (!(drv->flags & IFF_UP))
    {
        wpa_printf(MSG_DEBUG, "%s: interface is not up, cannot scan", __func__);
        return -1;
    }
...
static void wpa_driver_bsd_add_scan_entry(struct wpa_scan_results *res, struct ieee80211req_scan_result *sr)
{
    ...
    *pos++ = sr->isr_erp;
    ...
    os_memcpy(pos, (u8 *)(sr + 1) + sr->isr_ssid_len + sr->isr_meshid_len, sr->isr_ie_len);
#else
...
struct wpa_scan_results *wpa_driver_bsd_get_scan_results2(void *priv)
{
    ...
    wpa_printf(MSG_DEBUG, "Received %d bytes of scan results (%lu BSSes)", len, (unsigned long)res->num);
...
#define IEEE80211_C_WPA2         0x01000000 /* CAPABILITY: WPA2 avail */
    ...
    {
        wpa_printf(MSG_ERROR, "failed to IEEE80211_IOC_DEVCAPS: %s", strerror(errno));
        return -1;
    }

    wpa_printf(MSG_DEBUG, "%s: drivercaps=0x%08x,cryptocaps=0x%08x", __func__, devcaps.dc_drivercaps,
               devcaps.dc_cryptocaps);
...
#undef IEEE80211_C_WPA2
    ...
    drv->capa.enc =
        WPA_DRIVER_CAPA_ENC_WEP40 | WPA_DRIVER_CAPA_ENC_WEP104 | WPA_DRIVER_CAPA_ENC_TKIP | WPA_DRIVER_CAPA_ENC_CCMP;
    drv->capa.flags |= WPA_DRIVER_FLAGS_AP;
...
#define GETPARAM(drv, param, v) (((v) = get80211param(drv, param)) != -1)
    ...
    {
        wpa_printf(MSG_DEBUG, "%s: failed to get roaming state: %s", __func__, strerror(errno));
        goto fail;
    }
    if (!GETPARAM(drv, IEEE80211_IOC_PRIVACY, drv->prev_privacy))
    {
        wpa_printf(MSG_DEBUG, "%s: failed to get privacy state: %s", __func__, strerror(errno));
        goto fail;
    }
    if (!GETPARAM(drv, IEEE80211_IOC_WPA, drv->prev_wpa))
    {
        wpa_printf(MSG_DEBUG, "%s: failed to get wpa state: %s", __func__, strerror(errno));
        goto fail;
    }
...
static void *bsd_global_init(void *ctx)
{
    ...
    if (setsockopt(global->route, PF_ROUTE, RO_MSGFILTER, &msgfilter, sizeof(msgfilter)) < 0)
        wpa_printf(MSG_ERROR, "socket[PF_ROUTE,RO_MSGFILTER]: %s", strerror(errno));
#elif defined(ROUTE_MSGFILTER)
    msgfilter_mask = 0;
    for (i = 0; i < (sizeof(msgfilter) / sizeof(msgfilter[0])); i++)
        msgfilter_mask |= ROUTE_FILTER(msgfilter[i]);
    if (setsockopt(global->route, PF_ROUTE, ROUTE_MSGFILTER, &msgfilter_mask, sizeof(msgfilter_mask)) < 0)
        wpa_printf(MSG_ERROR, "socket[PF_ROUTE,ROUTE_MSGFILTER]: %s", strerror(errno));
#endif

    eloop_register_read_sock(global->route, bsd_wireless_event_receive, NULL, global);
