...
#define IOCTL_NDISUIO_OPEN_DEVICE _NDISUIO_CTL_CODE(0x200, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)

#define IOCTL_NDISUIO_QUERY_OID_VALUE _NDISUIO_CTL_CODE(0x201, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)

#define IOCTL_NDISUIO_SET_OID_VALUE _NDISUIO_CTL_CODE(0x205, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)

#define IOCTL_NDISUIO_SET_ETHER_TYPE _NDISUIO_CTL_CODE(0x202, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)

#define IOCTL_NDISUIO_QUERY_BINDING _NDISUIO_CTL_CODE(0x203, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)

#define IOCTL_NDISUIO_BIND_WAIT _NDISUIO_CTL_CODE(0x204, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
static int ndis_get_oid(struct wpa_driver_ndis_data *drv, unsigned int oid, char *data, size_t len)
{
...
#endif /* _WIN32_WCE */
    if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_QUERY_OID_VALUE, o, sizeof(NDISUIO_QUERY_OID), o, buflen, &written,
                         NULL))
    {
    ...
    if (o->Length > len)
    {
        wpa_printf(MSG_DEBUG, "%s: oid=0x%x Length (%d) > len (%d)", __func__, oid, (unsigned int)o->Length, len);
        os_free(buf);
        return -1;
    }
...
static int ndis_set_oid(struct wpa_driver_ndis_data *drv, unsigned int oid, const char *data, size_t len)
{
    ...
    if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_SET_OID_VALUE, o, reallen, NULL, 0, &written, NULL))
    {
        wpa_printf(MSG_DEBUG,
...
{
    u32 auth_mode = mode;
    if (ndis_set_oid(drv, OID_802_11_AUTHENTICATION_MODE, (char *)&auth_mode, sizeof(auth_mode)) < 0)
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: Failed to set "
...
static int ndis_get_auth_mode(struct wpa_driver_ndis_data *drv)
{
    u32 auth_mode;
    int res;
    res = ndis_get_oid(drv, OID_802_11_AUTHENTICATION_MODE, (char *)&auth_mode, sizeof(auth_mode));
...
static int ndis_set_encr_status(struct wpa_driver_ndis_data *drv, int encr)
{
    ...
    if (ndis_set_oid(drv, OID_802_11_ENCRYPTION_STATUS, (char *)&encr_status, sizeof(encr_status)) < 0)
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: Failed to set "
...
static int ndis_get_encr_status(struct wpa_driver_ndis_data *drv)
{
    u32 encr;
    int res;
    res = ndis_get_oid(drv, OID_802_11_ENCRYPTION_STATUS, (char *)&encr, sizeof(encr));
    if (res != sizeof(encr))
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: Failed to get "
...
static int wpa_driver_ndis_get_bssid(void *priv, u8 *bssid)
{
    ...
    return ndis_get_oid(drv, OID_802_11_BSSID, (char *)bssid, ETH_ALEN) < 0 ? -1 : 0;
}
...
static int wpa_driver_ndis_deauthenticate(void *priv, const u8 *addr, u16 reason_code)
{
    struct wpa_driver_ndis_data *drv = priv;
...
static int wpa_driver_ndis_scan(void *priv, struct wpa_driver_scan_params *params)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: turning radio on before the first"
                   " scan");
...
static struct wpa_scan_results *wpa_driver_ndis_get_scan_results(void *priv)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        os_memcpy(r + 1, bss->IEs + sizeof(NDIS_802_11_FIXED_IEs), bss->IELength - sizeof(NDIS_802_11_FIXED_IEs));
...
static int wpa_driver_ndis_remove_key(
    struct wpa_driver_ndis_data *drv, int key_idx, const u8 *addr, const u8 *bssid, int pairwise)
{
    ...
    {
        index = key_idx;
        res2  = ndis_set_oid(drv, OID_802_11_REMOVE_WEP, (char *)&index, sizeof(index));
    }
    else
        res2 = 0;
...
static int wpa_driver_ndis_add_wep(
    struct wpa_driver_ndis_data *drv, int pairwise, int key_idx, int set_tx, const u8 *key, size_t key_len)
{
    NDIS_802_11_WEP *wep;
    size_t len;
    int res;
                                   ...
                                   size_t key_len)
{
    ...
    if (alg == WPA_ALG_WEP)
    {
        return wpa_driver_ndis_add_wep(drv, pairwise, key_idx, set_tx, key, key_len);
    }
...
static int wpa_driver_ndis_set_key_wrapper(void *priv, struct wpa_driver_set_key_params *params)
{
    return wpa_driver_ndis_set_key(params->ifname, priv, params->alg, params->addr, params->key_idx, params->set_tx,
                                   params->seq, params->seq_len, params->key, params->key_len);
}
...
static int wpa_driver_ndis_associate(void *priv, struct wpa_driver_associate_params *params)
{
    ...
    if (ndis_set_oid(drv, OID_802_11_INFRASTRUCTURE_MODE, (char *)&mode, sizeof(mode)) < 0)
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: Failed to set "
    ...
    if (params->key_mgmt_suite == WPA_KEY_MGMT_NONE || params->key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X_NO_WPA)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            wpa_printf(MSG_DEBUG,
                       ...
                       "key %d",
                       i);
            wpa_driver_ndis_set_key(drv->ifname, drv, WPA_ALG_WEP, bcast, i, i == params->wep_tx_keyidx, NULL, 0,
    ...
    else if (params->key_mgmt_suite == WPA_KEY_MGMT_WPS)
    {
        ...
        if (params->wps == WPS_MODE_PRIVACY)
        {
             ...
             */
            wpa_printf(MSG_DEBUG,
                       "NDIS: Set stub WEP key as a "
                       "workaround to allow driver to associate "
                       "for WPS");
            wpa_driver_ndis_set_key(drv->ifname, drv, WPA_ALG_WEP, bcast, 0, 1, NULL, 0, stub_key, sizeof(stub_key));
        }
#endif /* CONFIG_WPS */
    }
    else
    ...
    if (ndis_set_oid(drv, OID_802_11_PRIVACY_FILTER, (char *)&priv_mode, sizeof(priv_mode)) < 0)
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: Failed to set "
...
static int wpa_driver_ndis_add_pmkid(void *priv, struct wpa_pmkid_params *params)
{
    struct wpa_driver_ndis_data *drv = priv;
...
static int wpa_driver_ndis_remove_pmkid(void *priv, struct wpa_pmkid_params *params)
{
    struct wpa_driver_ndis_data *drv = priv;
    struct ndis_pmkid_entry *entry, *prev;
    const u8 *bssid = params->bssid;
    ...
    while (entry)
    {
        if (os_memcmp(entry->bssid, bssid, ETH_ALEN) == 0 && os_memcmp(entry->pmkid, pmkid, 16) == 0)
        {
            if (prev)
                prev->next = entry->next;
            else
...
static int wpa_driver_ndis_get_associnfo(struct wpa_driver_ndis_data *drv)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: failed to get association "
                   "information");
        return -1;
    }
    if (len > sizeof(buf))
    ...
    wpa_printf(MSG_DEBUG,
               ...
               ai->AvailableRequestFixedIEs, ai->AvailableResponseFixedIEs, (int)ai->OffsetRequestIEs,
               (int)ai->OffsetResponseIEs, (int)ai->RequestIELength, (int)ai->ResponseIELength);

    if (ai->OffsetRequestIEs + ai->RequestIELength > (unsigned)len ||
        ai->OffsetResponseIEs + ai->ResponseIELength > (unsigned)len)
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: association information - "
                   "IE overflow");
        return -1;
    }

    wpa_hexdump(MSG_MSGDUMP, "NDIS: Request IEs", (u8 *)buf + ai->OffsetRequestIEs, ai->RequestIELength);
    wpa_hexdump(MSG_MSGDUMP, "NDIS: Response IEs", (u8 *)buf + ai->OffsetResponseIEs, ai->ResponseIELength);
    ...
    wpa_printf(MSG_DEBUG, "NDIS: %d BSSID items to process for AssocInfo", (unsigned int)b->NumberOfItems);

    pos = (char *)&b->Bssid[0];
    for (i = 0; i < b->NumberOfItems; i++)
    {
        NDIS_WLAN_BSSID_EX *bss = (NDIS_WLAN_BSSID_EX *)pos;
        if (os_memcmp(drv->bssid, bss->MacAddress, ETH_ALEN) == 0 && bss->IELength > sizeof(NDIS_802_11_FIXED_IEs))
        {
            data.assoc_info.beacon_ies     = ((u8 *)bss->IEs) + sizeof(NDIS_802_11_FIXED_IEs);
            data.assoc_info.beacon_ies_len = bss->IELength - sizeof(NDIS_802_11_FIXED_IEs);
            wpa_hexdump(MSG_MSGDUMP, "NDIS: Beacon IEs", data.assoc_info.beacon_ies, data.assoc_info.beacon_ies_len);
            break;
        }
        pos += bss->Length;
        if (pos > (char *)b + blen)
...
static void wpa_driver_ndis_event_auth(struct wpa_driver_ndis_data *drv, const u8 *data, size_t data_len)
{
    ...
    if ((req->Flags & NDIS_802_11_AUTH_REQUEST_PAIRWISE_ERROR) == NDIS_802_11_AUTH_REQUEST_PAIRWISE_ERROR)
        pairwise = 1;
    else if ((req->Flags & NDIS_802_11_AUTH_REQUEST_GROUP_ERROR) == NDIS_802_11_AUTH_REQUEST_GROUP_ERROR)
        group = 1;
...
static void wpa_driver_ndis_event_pmkid(struct wpa_driver_ndis_data *drv, const u8 *data, size_t data_len)
{
    ...
    for (i = 0; i < pmkid->NumCandidates; i++)
    {
        ...
        wpa_printf(MSG_DEBUG, "NDIS: %d: " MACSTR " Flags 0x%x", i, MAC2STR(p->BSSID), (int)p->Flags);
        os_memcpy(event.pmkid_candidate.bssid, p->BSSID, ETH_ALEN);
        event.pmkid_candidate.index   = i;
        event.pmkid_candidate.preauth = p->Flags & NDIS_802_11_PMKID_CANDIDATE_PREAUTH_ENABLED;
...
void wpa_driver_ndis_event_media_specific(struct wpa_driver_ndis_data *drv, const u8 *data, size_t data_len)
{
    ...
    switch (status->StatusType)
    {
        ...
        case Ndis802_11StatusType_PMKID_CandidateList:
            ...
            break;
        default:
            wpa_printf(MSG_DEBUG, "NDIS: Unknown StatusType %d", (int)status->StatusType);
            break;
    }
}
...
void wpa_driver_ndis_event_adapter_arrival(struct wpa_driver_ndis_data *drv)
{
    ...
    for (i = 0; i < 30; i++)
    {
        ...
        if (wpa_driver_ndis_adapter_init(drv) < 0 || wpa_driver_ndis_adapter_open(drv) < 0)
        {
            wpa_printf(MSG_DEBUG,
    ...
    os_memset(&event, 0, sizeof(event));
    os_strlcpy(event.interface_status.ifname, drv->ifname, sizeof(event.interface_status.ifname));
...
static void wpa_driver_ndis_get_wpa_capability(struct wpa_driver_ndis_data *drv)
{
    ...
    if (ndis_set_auth_mode(drv, Ndis802_11AuthModeWPA) == 0 && ndis_get_auth_mode(drv) == Ndis802_11AuthModeWPA)
    {
        wpa_printf(MSG_DEBUG, "NDIS: WPA key management supported");
        drv->capa.key_mgmt |= WPA_DRIVER_CAPA_KEY_MGMT_WPA;
    }

    if (ndis_set_auth_mode(drv, Ndis802_11AuthModeWPAPSK) == 0 && ndis_get_auth_mode(drv) == Ndis802_11AuthModeWPAPSK)
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: WPA-PSK key management "
                   "supported");
    ...
    if (ndis_set_auth_mode(drv, Ndis802_11AuthModeShared) == 0 && ndis_get_auth_mode(drv) == Ndis802_11AuthModeShared)
    {
        drv->capa.auth |= WPA_DRIVER_AUTH_SHARED;
    }

    if (ndis_set_auth_mode(drv, Ndis802_11AuthModeOpen) == 0 && ndis_get_auth_mode(drv) == Ndis802_11AuthModeOpen)
    {
...
static void wpa_driver_ndis_get_capability(struct wpa_driver_ndis_data *drv)
{
    ...
    for (i = 0; i < c->NoOfAuthEncryptPairsSupported; i++)
    {
        ...
        {
            wpa_printf(MSG_DEBUG,
                       "NDIS: auth/encr pair list "
                       "overflow");
            break;
        }
        wpa_printf(MSG_MSGDUMP, "NDIS: %d - auth %d encr %d", i, (int)ae->AuthModeSupported,
...
static void ndisuio_notification_receive(void *eloop_data, void *user_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "NDIS: Notification received: len=%d type=0x%x", (int)len, hdr->dwNotificationType);
    ...
    switch (hdr->dwNotificationType)
    {
        ...
        case NDISUIO_NOTIFICATION_MEDIA_SPECIFIC_NOTIFICATION:
            ...
            wpa_driver_ndis_event_media_specific(drv, hdr->pvStatusBuffer, hdr->uiStatusBufferSize);
#else
            wpa_driver_ndis_event_media_specific(drv, ((const u8 *)hdr) + hdr->uiOffsetToStatusBuffer,
                                                 (size_t)hdr->uiStatusBufferSize);
#endif
            break;
        default:
            wpa_printf(MSG_DEBUG, "NDIS: Unknown notification type 0x%x", hdr->dwNotificationType);
            break;
    }
}
...
static void ndisuio_notification_deinit(struct wpa_driver_ndis_data *drv)
{
    ...
    if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_REQUEST_NOTIFICATION, &req, sizeof(req), NULL, 0, NULL, NULL))
    {
        wpa_printf(MSG_INFO,
    ...
    if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_CANCEL_NOTIFICATION, NULL, 0, NULL, 0, NULL, NULL))
    {
        wpa_printf(MSG_INFO,
                   "ndisuio_notification_deinit: "
...
static int ndisuio_notification_init(struct wpa_driver_ndis_data *drv)
{
    ...
    drv->connected_event = CreateEvent(NULL, TRUE, FALSE, TEXT("WpaSupplicantConnected"));
...
#endif
        NDISUIO_NOTIFICATION_MEDIA_CONNECT | NDISUIO_NOTIFICATION_MEDIA_DISCONNECT |
    ...
    eloop_register_event(drv->event_queue, sizeof(drv->event_queue), ndisuio_notification_receive, drv, NULL);
...
static int wpa_driver_ndis_get_names(struct wpa_driver_ndis_data *drv)
{
    ...
    for (i = 0;; i++)
    {
        os_memset(b, 0, blen);
        b->BindingIndex = i;
        if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_QUERY_BINDING, b, sizeof(NDISUIO_QUERY_BINDING), b, blen,
                             &written, NULL))
        {
    ...
    os_strlcpy(drv->ifname, os_strncmp(name, "\\DEVICE\\", 8) == 0 ? name + 8 : name, sizeof(drv->ifname));
#ifdef _WIN32_WCE
    drv->adapter_name = wpa_strdup_tchar(drv->ifname);
    if (drv->adapter_name == NULL)
    {
        wpa_printf(MSG_ERROR,
                   "NDIS: Failed to allocate memory for "
                   "adapter name");
        os_free(b);
        return -1;
    }
    ...
    wpa_printf(MSG_DEBUG, "NDIS: Adapter description prefix '%s'", drv->adapter_desc);
...
#define MAX_ADAPTERS 32
    ...
    {
        wpa_printf(MSG_ERROR,
                   "NDIS: Failed to get adapter list "
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: Looks like adapter names are in "
                   "UNICODE");
        /* Convert to ASCII */
        pos2 = pos = names;
        while (pos2 < names + len)
        {
            if (pos2[0] == '\0' && pos2[1] == '\0' && pos2[2] == '\0' && pos2[3] == '\0')
            {
                pos2 += 4;
                break;
            }
    ...
    for (i = 0; i < num_name; i++)
    {
        ...
        if (found_name == -1 && os_strstr(name[i], drv->ifname))
            found_name = i;
        if (found_desc == -1 && os_strncmp(desc[i], drv->ifname, os_strlen(drv->ifname)) == 0)
            found_desc = i;
    }
    ...
    if (found_name < 0 && found_desc >= 0)
    {
        ...
        os_strlcpy(drv->ifname,
                   os_strncmp(name[found_desc], "\\Device\\NPF_", 12) == 0 ? name[found_desc] + 12 : name[found_desc],
                   sizeof(drv->ifname));
    }
...
static int wpa_driver_ndis_rebind_adapter(struct wpa_driver_ndis_data *drv)
{
    ...
    ndis = CreateFile(DD_NDIS_DEVICE_NAME, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
    ...
    if (!DeviceIoControl(ndis, IOCTL_NDIS_REBIND_ADAPTER, multi, len * sizeof(TCHAR), NULL, 0, NULL, NULL))
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: IOCTL_NDIS_REBIND_ADAPTER "
                   "failed: 0x%x",
                   (int)GetLastError());
        wpa_hexdump_ascii(MSG_DEBUG, "NDIS: rebind multi_sz", (u8 *)multi, len * sizeof(TCHAR));
        CloseHandle(ndis);
        return -1;
    }
    ...
    wpa_printf(MSG_DEBUG,
               "NDIS: Requested NDIS rebind of NDISUIO "
               "protocol");
...
#define WZC_DRIVER TEXT("Drivers\\BuiltIn\\ZeroConfig")
    ...
    for (i = 0;; i++)
    {
        ...
        len = sizeof(devname);
        ret = RegQueryValueEx(hk2, DEVLOAD_DEVKEY_VALNAME, NULL, NULL, (LPBYTE)devname, &len);
        if (ret != ERROR_SUCCESS)
        {
            wpa_printf(MSG_DEBUG,
    ...
    len = sizeof(hnd);
    ret = RegQueryValueEx(hk2, DEVLOAD_HANDLE_VALNAME, NULL, NULL, (PUCHAR)&hnd, &len);
    if (ret != ERROR_SUCCESS)
    {
        wpa_printf(MSG_DEBUG,
    ...
    {
        wpa_printf(MSG_DEBUG, "NDIS: DeactivateDevice failed: %d", (int)GetLastError());
        return -1;
    }
    ...
    DWORD(WINAPI * wzc_query_interf)(LPWSTR pSrvAddr, DWORD dwInFlags, PINTF_ENTRY pIntf, LPDWORD pdwOutFlags);
    DWORD(WINAPI * wzc_set_interf)(LPWSTR pSrvAddr, DWORD dwInFlags, PINTF_ENTRY pIntf, LPDWORD pdwOutFlags);
    int ret = -1, j;
    DWORD res;
    ...
    if (wzc_enum_interf == NULL || wzc_query_interf == NULL || wzc_set_interf == NULL)
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: WZCEnumInterfaces, "
    ...
    wpa_printf(MSG_DEBUG, "NDIS: WZCEnumInterfaces: %d interfaces", (int)guids.dwNumIntfs);
    ...
    for (i = 0; i < guids.dwNumIntfs; i++)
    {
        ...
        wpa_printf(MSG_DEBUG,
                   "NDIS: Current interface found from "
                   "WZC");
        break;
    }
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: Current interface not found from "
                   "WZC");
        goto fail;
    }
    ...
    if (res != 0)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   (int)res, (int)res);
        wpa_printf(MSG_DEBUG, "NDIS: GetLastError: %u", (unsigned int)GetLastError());
        goto fail;
    }

    wpa_printf(MSG_DEBUG, "NDIS: WZC interface flags 0x%x dwCtlFlags 0x%x", (int)flags, (int)intf.dwCtlFlags);

    if (intf.dwCtlFlags == -1)
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: Looks like wzcsapi has changed "
...
#define NDISUIO_DEVICE_NAME TEXT("\\\\.\\\\Ndisuio")
    DWORD written;
#endif /* _WIN32_WCE */
    drv->ndisuio = CreateFile(NDISUIO_DEVICE_NAME, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
...
#define MAX_NDIS_DEVICE_NAME_LEN 256
    ...
    if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_OPEN_DEVICE, ifname, len * sizeof(WCHAR), NULL, 0, &written, NULL))
    {
        wpa_printf(MSG_ERROR,
                   "NDIS: IOCTL_NDISUIO_OPEN_DEVICE "
                   "failed: %d",
                   (int)GetLastError());
        wpa_hexdump_ascii(MSG_DEBUG, "NDIS: ifname", (const u8 *)ifname, len * sizeof(WCHAR));
        CloseHandle(drv->ndisuio);
        drv->ndisuio = INVALID_HANDLE_VALUE;
        return -1;
    }
...
{
    if (ndis_set_oid(drv, OID_802_3_MULTICAST_LIST, (const char *)pae_group_addr, ETH_ALEN) < 0)
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: Failed to add PAE group address "
...
static void *wpa_driver_ndis_init(void *ctx, const char *ifname)
{
    ...
    if (ndis_get_oid(drv, OID_802_3_CURRENT_ADDRESS, (char *)drv->own_addr, ETH_ALEN) < 0)
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: Get OID_802_3_CURRENT_ADDRESS "
                   "failed");
        wpa_driver_ndis_adapter_close(drv);
        os_free(drv);
        return NULL;
    ...
    eloop_register_event(drv->event_avail, sizeof(drv->event_avail), wpa_driver_ndis_event_pipe_cb, drv, NULL);
#endif /* CONFIG_NDIS_EVENTS_INTEGRATED */
...
static void wpa_driver_ndis_deinit(void *priv)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "NDIS: failed to disassociate and turn "
                   "radio off");
    }
...
static struct wpa_interface_info *wpa_driver_ndis_get_interfaces(void *global_priv)
{
    ...
    ndisuio = CreateFile(NDISUIO_DEVICE_NAME, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
                         FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, INVALID_HANDLE_VALUE);
...
#ifndef _WIN32_WCE
    if (!DeviceIoControl(ndisuio, IOCTL_NDISUIO_BIND_WAIT, NULL, 0, NULL, 0, &written, NULL))
    {
        wpa_printf(MSG_ERROR,
                   "NDIS: IOCTL_NDISUIO_BIND_WAIT failed: "
                   "%d",
    ...
    for (i = 0;; i++)
    {
        os_memset(b, 0, blen);
        b->BindingIndex = i;
        if (!DeviceIoControl(ndisuio, IOCTL_NDISUIO_QUERY_BINDING, b, sizeof(NDISUIO_QUERY_BINDING), b, blen, &written,
                             NULL))
        {
            error = (int)GetLastError();
            if (error == ERROR_NO_MORE_ITEMS)
