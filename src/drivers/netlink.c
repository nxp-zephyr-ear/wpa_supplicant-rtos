                                 ...
                                 struct nlmsghdr *h)
{
    if (cb == NULL || NLMSG_PAYLOAD(h, 0) < sizeof(struct ifinfomsg))
        return;
    cb(netlink->cfg->ctx, NLMSG_DATA(h), (u8 *)NLMSG_DATA(h) + NLMSG_ALIGN(sizeof(struct ifinfomsg)),
...
int netlink_send_oper_ifla(struct netlink_data *netlink, int ifindex, int linkmode, int operstate)
{
    ...
    wpa_printf(MSG_DEBUG, "netlink: Operstate: ifindex=%d linkmode=%d (%s), operstate=%d (%s)", ifindex, linkmode,
               linkmode_str(linkmode), operstate, operstate_str(operstate));
