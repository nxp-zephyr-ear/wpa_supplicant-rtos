...
static int try_commit(struct macsec_drv_data *drv)
{
    ...
    if (drv->controlled_port_enabled_set)
    {
        ...
        wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: try_commit controlled_port_enabled=%d", drv->ifname,
                   drv->controlled_port_enabled);
        if (!change)
            return -1;
    ...
    {
        wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: try_commit protect_frames=%d", drv->ifname, drv->protect_frames);
        rtnl_link_macsec_set_protect(drv->link, drv->protect_frames);
    }

    if (drv->encrypt_set)
    {
        wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: try_commit encrypt=%d", drv->ifname, drv->encrypt);
        rtnl_link_macsec_set_encrypt(drv->link, drv->encrypt);
    }

    if (drv->replay_protect_set)
    {
        wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: try_commit replay_protect=%d replay_window=%d", drv->ifname,
                   drv->replay_protect, drv->replay_window);
    ...
    {
        wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: try_commit encoding_sa=%d", drv->ifname, drv->encoding_sa);
...
static int macsec_drv_macsec_init(void *priv, struct macsec_init_params *params)
{
    ...
    if (err < 0)
    {
        wpa_printf(MSG_ERROR, DRV_PREFIX "Unable to connect NETLINK_ROUTE socket: %s", nl_geterror(err));
        goto sock;
    }
    ...
    if (err < 0)
    {
        wpa_printf(MSG_ERROR, DRV_PREFIX "Unable to get link cache: %s", nl_geterror(err));
        goto sock;
    }
    ...
    if (drv->parent_ifi == 0)
    {
        wpa_printf(MSG_ERROR, DRV_PREFIX "couldn't find ifindex for interface %s", drv->common.ifname);
        goto cache;
    }
    wpa_printf(MSG_DEBUG, DRV_PREFIX "ifname=%s parent_ifi=%d", drv->common.ifname, drv->parent_ifi);
 ...
 * Returns: 0 on success, -1 on failure (or if not supported)
 */
static int macsec_drv_set_replay_protect(void *priv, bool enabled, u32 window)
{
    struct macsec_drv_data *drv = priv;

    wpa_printf(MSG_DEBUG, "%s -> %s, %u", __func__, enabled ? "TRUE" : "FALSE", window);
...
static int dump_callback(struct nl_msg *msg, void *argp)
{
    ...
    err = nla_parse(tb_msg, MACSEC_ATTR_MAX, genlmsg_attrdata(gnlh, 0), genlmsg_attrlen(gnlh, 0), main_policy);
    if (err < 0)
        return 0;
...
static int nl_send_recv(struct nl_sock *sk, struct nl_msg *msg)
{
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_ERROR, DRV_PREFIX "%s: failed to send: %d (%s)", __func__, ret, nl_geterror(-ret));
        return ret;
    }
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_ERROR, DRV_PREFIX "%s: failed to recv: %d (%s)", __func__, ret, nl_geterror(-ret));
    }
...
static int do_dump(struct macsec_drv_data *drv, u8 txsa, u64 rxsci, u8 rxsa, u32 *pn)
{
    ...
    if (!msg)
    {
        wpa_printf(MSG_ERROR, DRV_PREFIX "%s: failed to alloc message", __func__);
        return 1;
    }

    if (!genlmsg_put(msg, NL_AUTO_PORT, NL_AUTO_SEQ, ctx->macsec_genl_id, 0, NLM_F_DUMP, MACSEC_CMD_GET_TXSC, 0))
    {
        wpa_printf(MSG_ERROR, DRV_PREFIX "%s: failed to put header", __func__);
        goto out_free_msg;
    }

    ret = nl_send_recv(ctx->sk, msg);
    if (ret < 0)
        wpa_printf(MSG_ERROR, DRV_PREFIX "failed to communicate: %d (%s)", ret, nl_geterror(-ret));
...
static int macsec_drv_get_receive_lowest_pn(void *priv, struct receive_sa *sa)
{
    ...
    err = do_dump(drv, 0xff, mka_sci_u64(&sa->sc->sci), sa->an, &sa->lowest_pn);
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: result %d", __func__, sa->lowest_pn);
...
static int macsec_drv_set_receive_lowest_pn(void *priv, struct receive_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: set_receive_lowest_pn -> %d: %d", drv->ifname, sa->an, sa->next_pn);
...
static int macsec_drv_get_transmit_next_pn(void *priv, struct transmit_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: err %d result %d", __func__, err, sa->next_pn);
    return err;
}
 ...
 * Returns: 0 on success, -1 on failure (or if not supported)
 */
static int macsec_drv_create_receive_sc(void *priv, struct receive_sc *sc, unsigned int conf_offset, int validation)
{
    struct macsec_drv_data *drv = priv;
    struct macsec_genl_ctx *ctx = &drv->ctx;
    struct nl_msg *msg;
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: create_receive_sc -> " SCISTR " (conf_offset=%u validation=%d)", drv->ifname,
               SCI2STR(sc->sci.addr, sc->sci.port), conf_offset, validation);
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_ERROR, DRV_PREFIX "%s: failed to communicate: %d (%s)", __func__, ret, nl_geterror(-ret));
    }
...
static int macsec_drv_delete_receive_sc(void *priv, struct receive_sc *sc)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: delete_receive_sc -> " SCISTR, drv->ifname,
...
static int macsec_drv_create_receive_sa(void *priv, struct receive_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: create_receive_sa -> %d on " SCISTR " (enable_receive=%d next_pn=%u)",
               drv->ifname, sa->an, SCI2STR(sa->sc->sci.addr, sa->sc->sci.port), sa->enable_receive, sa->next_pn);
    wpa_hexdump(MSG_DEBUG, DRV_PREFIX "SA keyid", &sa->pkey->key_identifier, sizeof(sa->pkey->key_identifier));
    wpa_hexdump_key(MSG_DEBUG, DRV_PREFIX "SA key", sa->pkey->key, sa->pkey->key_len);
...
static int macsec_drv_delete_receive_sa(void *priv, struct receive_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: delete_receive_sa -> %d on " SCISTR, drv->ifname, sa->an,
...
static int set_active_rx_sa(const struct macsec_genl_ctx *ctx, int ifindex, u64 sci, unsigned char an, bool state)
{
    struct nl_msg *msg;
    struct nlattr *nest;
    int ret = -1;
...
static int macsec_drv_enable_receive_sa(void *priv, struct receive_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: enable_receive_sa -> %d on " SCISTR, drv->ifname, sa->an,
               SCI2STR(sa->sc->sci.addr, sa->sc->sci.port));
...
static int macsec_drv_disable_receive_sa(void *priv, struct receive_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: disable_receive_sa -> %d on " SCISTR, drv->ifname, sa->an,
               SCI2STR(sa->sc->sci.addr, sa->sc->sci.port));

    return set_active_rx_sa(ctx, drv->ifi, mka_sci_u64(&sa->sc->sci), sa->an, false);
}

static struct rtnl_link *lookup_sc(struct nl_cache *cache, int parent, u64 sci, u64 cs)
{
 ...
 * Returns: 0 on success, -1 on failure
 */
static int macsec_drv_create_transmit_sc(void *priv, struct transmit_sc *sc, unsigned int conf_offset)
{
    struct macsec_drv_data *drv = priv;
    struct rtnl_link *link;
    char *ifname;
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: create_transmit_sc -> " SCISTR " (conf_offset=%d)", drv->common.ifname,
               SCI2STR(sc->sci.addr, sc->sci.port), conf_offset);
    ...
    if (err == -NLE_BUSY)
    {
        wpa_printf(MSG_INFO, DRV_PREFIX "link already exists, using it");
        drv->created_link = false;
    }
    else if (err < 0)
    {
        rtnl_link_put(link);
        wpa_printf(MSG_ERROR, DRV_PREFIX "couldn't create link: err %d", err);
        return err;
    }
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: create_transmit_sc: ifi=%d ifname=%s", drv->common.ifname, drv->ifi, ifname);
...
static int macsec_drv_delete_transmit_sc(void *priv, struct transmit_sc *sc)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: delete_transmit_sc -> " SCISTR, drv->ifname,
               SCI2STR(sc->sci.addr, sc->sci.port));
    ...
    if (!drv->created_link)
    {
        ...
        drv->link = NULL;
        wpa_printf(MSG_DEBUG, DRV_PREFIX "we didn't create the link, leave it alone");
        return 0;
    }
...
static int macsec_drv_create_transmit_sa(void *priv, struct transmit_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: create_transmit_sa -> %d on " SCISTR " (enable_transmit=%d next_pn=%u)",
               drv->ifname, sa->an, SCI2STR(sa->sc->sci.addr, sa->sc->sci.port), sa->enable_transmit, sa->next_pn);
    wpa_hexdump(MSG_DEBUG, DRV_PREFIX "SA keyid", &sa->pkey->key_identifier, sizeof(sa->pkey->key_identifier));
    wpa_hexdump_key(MSG_DEBUG, DRV_PREFIX "SA key", sa->pkey->key, sa->pkey->key_len);
...
static int macsec_drv_delete_transmit_sa(void *priv, struct transmit_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: delete_transmit_sa -> %d on " SCISTR, drv->ifname, sa->an,
               SCI2STR(sa->sc->sci.addr, sa->sc->sci.port));
...
static int set_active_tx_sa(const struct macsec_genl_ctx *ctx, int ifindex, unsigned char an, bool state)
{
    struct nl_msg *msg;
    struct nlattr *nest;
    int ret = -1;
...
static int macsec_drv_enable_transmit_sa(void *priv, struct transmit_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: enable_transmit_sa -> %d on " SCISTR, drv->ifname, sa->an,
               SCI2STR(sa->sc->sci.addr, sa->sc->sci.port));
...
static int macsec_drv_disable_transmit_sa(void *priv, struct transmit_sa *sa)
{
    ...
    wpa_printf(MSG_DEBUG, DRV_PREFIX "%s: disable_transmit_sa -> %d on " SCISTR, drv->ifname, sa->an,
               SCI2STR(sa->sc->sci.addr, sa->sc->sci.port));
...
static int macsec_drv_status(void *priv, char *buf, size_t buflen)
{
    ...
    res = os_snprintf(pos, end - pos,
                      ...
                      "parent_ifname=%s\n"
                      "parent_ifi=%d\n",
                      drv->common.ifname, drv->ifi, drv->ifname, drv->parent_ifi);
...
static void macsec_drv_handle_data(void *ctx, unsigned char *buf, size_t len)
{
    ...
    if (len < 14)
    {
        wpa_printf(MSG_MSGDUMP, "%s: too short (%lu)", __func__, (unsigned long)len);
        return;
    }
    ...
    switch (ntohs(hdr->ethertype))
    {
        ...
        default:
            wpa_printf(MSG_DEBUG, "Unknown ethertype 0x%04x in data frame", ntohs(hdr->ethertype));
            break;
    }
#endif /* HOSTAPD */
}
...
static void macsec_drv_handle_read(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    if (len < 0)
    {
        wpa_printf(MSG_ERROR, "macsec_linux: recv: %s", strerror(errno));
        return;
    }
...
static int macsec_drv_init_sockets(struct macsec_drv_data *drv, u8 *own_addr)
{
    ...
    if (eloop_register_read_sock(drv->common.sock, macsec_drv_handle_read, drv->common.ctx, NULL))
    {
    ...
    if (wired_multicast_membership(drv->common.sock, ifr.ifr_ifindex, pae_group_addr, 1) < 0)
    {
    ...
    {
        wpa_printf(MSG_INFO, "Invalid HW-addr family 0x%04x", ifr.ifr_hwaddr.sa_family);
        return -1;
    }
...
static void *macsec_drv_hapd_init(struct hostapd_data *hapd, struct wpa_init_params *params)
{
    struct macsec_drv_data *drv;
...
static int macsec_drv_send_eapol(
    void *priv, const u8 *addr, const u8 *data, size_t data_len, int encrypt, const u8 *own_addr, u32 flags)
{
    ...
    if (hdr == NULL)
    {
        wpa_printf(MSG_INFO, "%s: malloc() failed (len=%lu)", __func__, (unsigned long)len);
        return -1;
    }

    os_memcpy(hdr->dest, drv->use_pae_group_addr ? pae_group_addr : addr, ETH_ALEN);
    ...
    if (res < 0)
    {
        wpa_printf(MSG_ERROR, "%s: packet len: %lu - failed: send: %s", __func__, (unsigned long)len, strerror(errno));
    }
