...
static int hostap_init_sockets(struct hostap_driver_data *drv, u8 *own_addr)
{
    ...
    if (os_snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%sap", drv->iface) >= (int)sizeof(ifr.ifr_name))
    {
...
static int hostap_send_mlme(void *priv,
                            const u8 *msg,
                            size_t len,
                            int noack,
                            unsigned int freq,
...
static int hostap_sta_set_flags(
...
static int hostap_set_iface_flags(void *priv, int dev_up)
{
    ...
    if (dev_up)
    {
        ...
        {
            wpa_printf(MSG_INFO, "Setting MTU failed - trying to survive with current value: ioctl[SIOCSIFMTU]: %s",
                       strerror(errno));
        }
    }
...
static int hostap_set_ieee8021x(void *priv, struct wpa_bss_params *params)
{
    ...
    {
        printf(
            "Could not setup host-based encryption in kernel "
            "driver.\n");
        return -1;
    }
...
static int hostap_set_ap_wps_ie(void *priv,
                                ...
                                const struct wpabuf *assocresp)
{
    ...
    if (proberesp)
    {
        drv->wps_ie = os_memdup(wpabuf_head(proberesp), wpabuf_len(proberesp));
        if (drv->wps_ie == NULL)
            return -1;
...
static void hostapd_wireless_event_wireless(struct hostap_driver_data *drv, char *data, unsigned int len)
{
...
static int hostap_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr, u16 reason)
{
    struct hostap_driver_data *drv = priv;
    ...
    return hostap_send_mlme(drv, (u8 *)&mgmt, IEEE80211_HDRLEN + sizeof(mgmt.u.deauth), 0, 0, NULL, 0, 0, 0);
}
...
static int hostap_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr, u16 reason)
{
    struct hostap_driver_data *drv = priv;
    struct ieee80211_mgmt mgmt;
    ...
    return hostap_send_mlme(drv, (u8 *)&mgmt, IEEE80211_HDRLEN + sizeof(mgmt.u.disassoc), 0, 0, NULL, 0, 0, 0);
}

static struct hostapd_hw_modes *hostap_get_hw_feature_data(void *priv, u16 *num_modes, u16 *flags, u8 *dfs)
{
    struct hostapd_hw_modes *mode;
    int i, clen, rlen;
    const short chan2freq[14] = {2412, 2417, 2422, 2427, 2432, 2437, 2442, 2447, 2452, 2457, 2462, 2467, 2472, 2484};
