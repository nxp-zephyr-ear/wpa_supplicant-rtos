...
static int atheros_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr, u16 reason_code);
...
#ifdef CONFIG_FILS
static int get80211param(struct atheros_driver_data *drv, int op, int *data)
{
    struct iwreq iwr;
...
static int set80211priv(struct atheros_driver_data *drv, int op, void *data, int len)
{
    struct iwreq iwr;
    ...
    if (ioctl(drv->ioctl_sock, op, &iwr) < 0)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   "(%s) len=%d failed: %d (%s)",
                   __func__, drv->iface, op, athr_get_ioctl_name(op), len, errno, strerror(errno));
        return -1;
    }
    return 0;
}
...
static int set80211param(struct atheros_driver_data *drv, int op, int arg)
{
    ...
    {
        wpa_printf(MSG_INFO, "%s: %s: Failed to set parameter (op %d (%s) arg %d): ioctl[IEEE80211_IOCTL_SETPARAM]: %s",
                   __func__, drv->iface, op, athr_get_param_name(op), arg, strerror(errno));
        return -1;
    }
    return 0;
}
 ...
 */
static int atheros_configure_wpa(struct atheros_driver_data *drv, struct wpa_bss_params *params)
{
    int v;
    ...
    wpa_printf(MSG_DEBUG, "%s: key management algorithms=0x%x", __func__, params->wpa_key_mgmt);
    if (set80211param(drv, IEEE80211_PARAM_KEYMGTALGS, params->wpa_key_mgmt))
    {
        wpa_printf(MSG_INFO, "Unable to set key management algorithms to 0x%x", params->wpa_key_mgmt);
        return -1;
    }
...
static int atheros_set_ieee8021x(void *priv, struct wpa_bss_params *params)
{
    ...
    if (set80211param(priv, IEEE80211_PARAM_AUTHMODE, (params->wpa ? IEEE80211_AUTH_WPA : IEEE80211_AUTH_8021X)))
    {
...
static int atheros_set_sta_authorized(void *priv, const u8 *addr, int authorized)
{
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG, "%s: Failed to %sauthorize STA " MACSTR, __func__, authorized ? "" : "un", MAC2STR(addr));
    }

    return ret;
}

static int atheros_sta_set_flags(
...
static int atheros_del_key(void *priv, const u8 *addr, int key_idx)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: addr=%s key_idx=%d", __func__, ether_sprintf(addr), key_idx);
...
static int atheros_set_key(void *priv, struct wpa_driver_set_key_params *params)
{
    struct atheros_driver_data *drv = priv;
    ...
    {
        wpa_printf(MSG_INFO, "%s: key length %lu too big", __func__, (unsigned long)key_len);
        return -3;
    }
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   __func__, ether_sprintf(wk.ik_macaddr), key_idx, alg, (unsigned long)key_len, set_tx);
    }

    return ret;
}

static int atheros_get_seqnum(const char *ifname, void *priv, const u8 *addr, int idx, u8 *seq)
{
    struct atheros_driver_data *drv = priv;
    struct ieee80211req_key wk;

    wpa_printf(MSG_DEBUG, "%s: addr=%s idx=%d", __func__, ether_sprintf(addr), idx);
...
static int atheros_read_sta_driver_data(void *priv, struct hostap_sta_driver_data *data, const u8 *addr)
{
    struct atheros_driver_data *drv = priv;
    ...
    {
        wpa_printf(MSG_DEBUG, "%s: Failed to fetch STA stats (addr " MACSTR ")", __func__, MAC2STR(addr));
...
static int atheros_sta_clear_stats(void *priv, const u8 *addr)
{
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG, "%s: Failed to clear STA stats (addr " MACSTR ")", __func__, MAC2STR(addr));
    }
...
static int atheros_set_opt_ie(void *priv, const u8 *ie, size_t ie_len)
{
    ...
    {
        os_memcpy(&(app_ie->app_buf[ie_len]), wpabuf_head(drv->wps_beacon_ie), wpabuf_len(drv->wps_beacon_ie));
        app_ie->app_buflen = ie_len + wpabuf_len(drv->wps_beacon_ie);
    }
    wpa_hexdump(MSG_DEBUG, "atheros: SET_APPIEBUF(Beacon)", app_ie->app_buf, app_ie->app_buflen);
    set80211priv(drv, IEEE80211_IOCTL_SET_APPIEBUF, app_ie,
                 sizeof(struct ieee80211req_getset_appiebuf) + app_ie->app_buflen);

    /* append WPS IE for Probe Response */
    app_ie->app_frmtype = IEEE80211_APPIE_FRAME_PROBE_RESP;
    if (drv->wps_probe_resp_ie != NULL)
    {
        os_memcpy(&(app_ie->app_buf[ie_len]), wpabuf_head(drv->wps_probe_resp_ie), wpabuf_len(drv->wps_probe_resp_ie));
        app_ie->app_buflen = ie_len + wpabuf_len(drv->wps_probe_resp_ie);
    }
    else
        app_ie->app_buflen = ie_len;
    wpa_hexdump(MSG_DEBUG, "atheros: SET_APPIEBUF(ProbeResp)", app_ie->app_buf, app_ie->app_buflen);
    set80211priv(drv, IEEE80211_IOCTL_SET_APPIEBUF, app_ie,
                 sizeof(struct ieee80211req_getset_appiebuf) + app_ie->app_buflen);
    return 0;
}
...
static int atheros_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr, u16 reason_code)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: addr=%s reason_code=%d", __func__, ether_sprintf(addr), reason_code);
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG, "%s: Failed to deauth STA (addr " MACSTR " reason %d)", __func__, MAC2STR(addr),
                   reason_code);
    }
...
static int atheros_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr, u16 reason_code)
{
    struct atheros_driver_data *drv = priv;
    struct ieee80211req_mlme mlme;
    int ret;
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG, "%s: Failed to disassoc STA (addr " MACSTR " reason %d)", __func__, MAC2STR(addr),
                   reason_code);
    }
...
static int atheros_set_qos_map(void *ctx, const u8 *qos_map_set, u8 qos_map_set_len)
{
    ...
    {
        wpa_printf(MSG_ERROR, "%s: %s: Failed to set QoS Map: ioctl[IEEE80211_IOCTL_DBGREQ]: %s", __func__, drv->iface,
                   strerror(errno));
        return -1;
    }
...
static void atheros_raw_receive(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    if (stype == WLAN_FC_STYPE_ACTION &&
        (os_memcmp(drv->own_addr, mgmt->bssid, ETH_ALEN) == 0 || is_broadcast_ether_addr(mgmt->bssid)))
    {
        os_memset(&event, 0, sizeof(event));
...
static int atheros_receive_pkt(struct atheros_driver_data *drv)
{
...
#endif /* CONFIG_WPS */
    filt.app_filterype |= (IEEE80211_FILTER_TYPE_ASSOC_REQ | IEEE80211_FILTER_TYPE_AUTH | IEEE80211_FILTER_TYPE_ACTION);
#ifdef CONFIG_WNM
    ...
    {
        ret = set80211priv(drv, IEEE80211_IOCTL_FILTERFRAME, &filt, sizeof(struct ieee80211req_set_filter));
        if (ret)
            return ret;
    }

#if defined(CONFIG_WPS) || defined(CONFIG_IEEE80211R) || defined(CONFIG_FILS)
    drv->sock_raw = l2_packet_init(drv->iface, NULL, ETH_P_80211_RAW, atheros_raw_receive, drv, 1);
...
static int atheros_reset_appfilter(struct atheros_driver_data *drv)
{
    struct ieee80211req_set_filter filt;
    filt.app_filterype = 0;
    return set80211priv(drv, IEEE80211_IOCTL_FILTERFRAME, &filt, sizeof(struct ieee80211req_set_filter));
}

#ifdef CONFIG_WPS
static int atheros_set_wps_ie(void *priv, const u8 *ie, size_t len, u32 frametype)
{
    struct atheros_driver_data *drv = priv;
    u8 buf[512];
    struct ieee80211req_getset_appiebuf *beac_ie;

    wpa_printf(MSG_DEBUG, "%s buflen = %lu frametype=%u", __func__, (unsigned long)len, frametype);
    ...
    if (((frametype == IEEE80211_APPIE_FRAME_BEACON) || (frametype == IEEE80211_APPIE_FRAME_PROBE_RESP)) &&
        (drv->wpa_ie != NULL))
    {
        wpa_hexdump_buf(MSG_DEBUG, "atheros: Append WPA/RSN IE", drv->wpa_ie);
        os_memcpy(&(beac_ie->app_buf[len]), wpabuf_head(drv->wpa_ie), wpabuf_len(drv->wpa_ie));
        beac_ie->app_buflen += wpabuf_len(drv->wpa_ie);
    }

    wpa_hexdump(MSG_DEBUG, "atheros: SET_APPIEBUF", beac_ie->app_buf, beac_ie->app_buflen);
    return set80211priv(drv, IEEE80211_IOCTL_SET_APPIEBUF, beac_ie,
                        sizeof(struct ieee80211req_getset_appiebuf) + beac_ie->app_buflen);
}

static int atheros_set_ap_wps_ie(void *priv,
                                 ...
                                 const struct wpabuf *assocresp)
{
    ...
    atheros_set_wps_ie(priv, assocresp ? wpabuf_head(assocresp) : NULL, assocresp ? wpabuf_len(assocresp) : 0,
                       IEEE80211_APPIE_FRAME_ASSOC_RESP);
    if (atheros_set_wps_ie(priv, beacon ? wpabuf_head(beacon) : NULL, beacon ? wpabuf_len(beacon) : 0,
                           IEEE80211_APPIE_FRAME_BEACON))
        return -1;
    return atheros_set_wps_ie(priv, proberesp ? wpabuf_head(proberesp) : NULL, proberesp ? wpabuf_len(proberesp) : 0,
...
static int atheros_sta_auth(void *priv, struct wpa_driver_sta_auth_params *params)
{
    struct atheros_driver_data *drv = priv;
    struct ieee80211req_mlme mlme;
    int ret;

    wpa_printf(MSG_DEBUG, "%s: addr=%s status_code=%d", __func__, ether_sprintf(params->addr), params->status);

#ifdef CONFIG_FILS
    /* Copy FILS AAD parameters if the driver supports FILS */
    if (params->fils_auth && drv->fils_en)
    {
        wpa_printf(MSG_DEBUG, "%s: im_op IEEE80211_MLME_AUTH_FILS", __func__);
        os_memcpy(mlme.fils_aad.ANonce, params->fils_anonce, IEEE80211_FILS_NONCE_LEN);
        os_memcpy(mlme.fils_aad.SNonce, params->fils_snonce, IEEE80211_FILS_NONCE_LEN);
        os_memcpy(mlme.fils_aad.kek, params->fils_kek, IEEE80211_MAX_WPA_KEK_LEN);
        mlme.fils_aad.kek_len = params->fils_kek_len;
        mlme.im_op            = IEEE80211_MLME_AUTH_FILS;
        wpa_hexdump(MSG_DEBUG, "FILS: ANonce", mlme.fils_aad.ANonce, FILS_NONCE_LEN);
        wpa_hexdump(MSG_DEBUG, "FILS: SNonce", mlme.fils_aad.SNonce, FILS_NONCE_LEN);
        wpa_hexdump_key(MSG_DEBUG, "FILS: KEK", mlme.fils_aad.kek, mlme.fils_aad.kek_len);
    }
    else
    {
    ...
    if (params->len)
    {
        ...
        else
        {
            wpa_printf(MSG_DEBUG,
                       ...
                       "opt_ie STA (addr " MACSTR
                       " reason %d, "
                       "ie_len %d)",
                       __func__, MAC2STR(params->addr), params->status, (int)params->len);
            return -1;
        }
    }
    ret = set80211priv(drv, IEEE80211_IOCTL_SETMLME, &mlme, sizeof(mlme));
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG, "%s: Failed to auth STA (addr " MACSTR " reason %d)", __func__, MAC2STR(params->addr),
                   params->status);
    }
    return ret;
}

static int atheros_sta_assoc(
    void *priv, const u8 *own_addr, const u8 *addr, int reassoc, u16 status_code, const u8 *ie, size_t len)
{
    struct atheros_driver_data *drv = priv;
    struct ieee80211req_mlme mlme;
    int ret;

    wpa_printf(MSG_DEBUG, "%s: addr=%s status_code=%d reassoc %d", __func__, ether_sprintf(addr), status_code, reassoc);
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG, "%s: Failed to assoc STA (addr " MACSTR " reason %d)", __func__, MAC2STR(addr),
                   status_code);
    }
    return ret;
}

static void atheros_new_sta(struct atheros_driver_data *drv, u8 addr[IEEE80211_ADDR_LEN])
{
    ...
    if (set80211priv(drv, IEEE80211_IOCTL_GETWPAIE, &ie, sizeof(ie)))
    {
         ...
         */
        wpa_printf(MSG_DEBUG, "%s: Failed to get WPA/RSN IE: %s", __func__, strerror(errno));
        goto no_ie;
    }
    wpa_hexdump(MSG_MSGDUMP, "atheros req WPA IE", ie.wpa_ie, IEEE80211_MAX_OPT_IE);
    wpa_hexdump(MSG_MSGDUMP, "atheros req RSN IE", ie.rsn_ie, IEEE80211_MAX_OPT_IE);
#ifdef ATH_WPS_IE
    wpa_hexdump(MSG_MSGDUMP, "atheros req WPS IE", ie.wps_ie, IEEE80211_MAX_OPT_IE);
#endif /* ATH_WPS_IE */
    iebuf = ie.wpa_ie;
...
static void atheros_wireless_event_wireless_custom(struct atheros_driver_data *drv, char *custom, char *end)
{
...
static void send_action_cb_event(struct atheros_driver_data *drv, char *data, size_t data_len)
{
    union wpa_event_data event;
    ...
    {
        wpa_printf(MSG_DEBUG, "athr: Too short event message (data_len=%d sizeof(*sa)=%d)", (int)data_len,
                   (int)sizeof(*sa));
        wpa_hexdump(MSG_DEBUG, "athr: Short event message", data, data_len);
        return;
    }
...
static void fetch_pending_big_events(struct atheros_driver_data *drv)
{
    ...
    while (1)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "%s: ACTION_CB frame_type=%u len=%zu", __func__, frame_type, data_len);
...
static void atheros_wireless_event_atheros_custom(struct atheros_driver_data *drv, int opcode, char *buf, int len)
{
    switch (opcode)
    {
...
static void atheros_wireless_event_wireless(struct atheros_driver_data *drv, char *data, unsigned int len)
{
    ...
    while ((size_t)(end - pos) >= IW_EV_LCP_LEN)
    {
        ...
        if (drv->we_version > 18 &&
            (iwe->cmd == IWEVMICHAELMICFAILURE || iwe->cmd == IWEVASSOCREQIE || iwe->cmd == IWEVCUSTOM))
        {
...
static void atheros_wireless_event_rtm_newlink(void *ctx, struct ifinfomsg *ifi, u8 *buf, size_t len)
{
    struct atheros_driver_data *drv = ctx;
    int attrlen, rta_len;
    struct rtattr *attr;
...
static int atheros_get_we_version(struct atheros_driver_data *drv)
{
    ...
    else if (iwr.u.data.length >= minlen && range->we_version_compiled >= 18)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   range->we_version_compiled, range->we_version_source, range->enc_capa);
    ...
    void *priv, const u8 *addr, const u8 *data, size_t data_len, int encrypt, const u8 *own_addr, u32 flags)
{
    ...
    if (len > sizeof(buf))
    {
        ...
        if (bp == NULL)
        {
            wpa_printf(MSG_INFO, "EAPOL frame discarded, cannot malloc temp buffer of size %lu!", (unsigned long)len);
            return -1;
        }
    }
...
{
    struct atheros_driver_data *drv = ctx;
    drv_event_eapol_rx(drv->hapd, src_addr, buf + sizeof(struct l2_ethhdr), len - sizeof(struct l2_ethhdr));
}
...
static void atheros_read_fils_cap(struct atheros_driver_data *drv)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "%s: Failed to get FILS capability from driver", __func__);
...
static void *atheros_init(struct hostapd_data *hapd, struct wpa_init_params *params)
{
    ...
    if (params->bridge[0])
    {
        wpa_printf(MSG_DEBUG, "Configure bridge %s for EAPOL traffic.", params->bridge[0]);
        drv->sock_recv = l2_packet_init(params->bridge[0], NULL, ETH_P_EAPOL, handle_read, drv, 1);
    ...
    else if (linux_br_get(brname, drv->iface) == 0)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   "EAPOL receive",
                   brname);
        drv->sock_recv = l2_packet_init(brname, NULL, ETH_P_EAPOL, handle_read, drv, 1);
        if (drv->sock_recv == NULL)
            goto bad;
    }
    else
...
static int atheros_set_ap(void *priv, struct wpa_driver_ap_params *params)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               params->pairwise_ciphers, params->group_cipher, params->key_mgmt_suites, params->auth_algs,
    ...
    if (params->osen)
    {
        ...
        if (atheros_set_privacy(priv, 1) || set80211param(priv, IEEE80211_PARAM_OSEN, 1))
            return -1;
...
static int atheros_send_mgmt(void *priv,
                             const u8 *frm,
                             size_t data_len,
                             int noack,
                             unsigned int freq,
                             ...
                             unsigned int wait)
{
    ...
    wpa_printf(MSG_DEBUG, "%s frmlen = %lu " MACSTR, __func__, (unsigned long)data_len, MAC2STR(mgmt->da));
    ...
    return set80211priv(drv, IEEE80211_IOCTL_SEND_MGMT, mgmt_frm, sizeof(struct ieee80211req_mgmtbuf) + data_len);
}

#ifdef CONFIG_IEEE80211R

static int atheros_add_tspec(void *priv, const u8 *addr, u8 *tspec_ie, size_t tspec_ielen)
{
    struct atheros_driver_data *drv = priv;
    int retv;
    ...
    retv = set80211priv(drv, IEEE80211_IOCTL_RES_REQ, &req, sizeof(struct ieee80211req_res));
    if (retv < 0)
    {
        wpa_printf(MSG_DEBUG,
...
static int atheros_add_sta_node(void *priv, const u8 *addr, u16 auth_alg)
{
    ...
    return set80211priv(drv, IEEE80211_IOCTL_RES_REQ, &req, sizeof(struct ieee80211req_res));
}

#endif /* CONFIG_IEEE80211R */

/* Use only to set a big param, get will not work. */
static int set80211big(struct atheros_driver_data *drv, int op, const void *data, int len)
{
    struct iwreq iwr;
    ...
    iwr.u.data.length  = len;
    iwr.u.data.flags   = op;
    wpa_printf(MSG_DEBUG, "%s: op=0x%x=%d (%s) len=0x%x", __func__, op, op, athr_get_param_name(op), len);
    ...
    if (ioctl(drv->ioctl_sock, IEEE80211_IOCTL_P2P_BIG_PARAM, &iwr) < 0)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   __func__, op, athr_get_ioctl_name(op), iwr.u.mode, iwr.u.mode, iwr.u.data.length, iwr.u.data.flags,
...
static int atheros_send_action(void *priv,
                               unsigned int freq,
                               unsigned int wait,
                               const u8 *dst,
                               const u8 *src,
                               ...
                               int no_cck)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: freq=%d, wait=%u, dst=" MACSTR ", src=" MACSTR ", bssid=" MACSTR, __func__, act->freq,
               wait, MAC2STR(act->dst_addr), MAC2STR(act->src_addr), MAC2STR(act->bssid));
    wpa_hexdump(MSG_MSGDUMP, "athr: act", (u8 *)act, sizeof(*act));
    wpa_hexdump(MSG_MSGDUMP, "athr: data", data, data_len);

    res = set80211big(drv, IEEE80211_IOC_P2P_SEND_ACTION, act, sizeof(*act) + data_len);
    os_free(act);
    return res;
}

#if defined(CONFIG_WNM) && defined(IEEE80211_APPIE_FRAME_WNM)
static int athr_wnm_tfs(struct atheros_driver_data *drv, const u8 *peer, u8 *ie, u16 *len, enum wnm_oper oper)
{
#define IEEE80211_APPIE_MAX 1024 /* max appie buffer size */
    ...
    wpa_printf(MSG_DEBUG, "atheros: ifname=%s, WNM TFS IE oper=%d " MACSTR, drv->iface, oper, MAC2STR(peer));

    switch (oper)
    {
        case WNM_SLEEP_TFS_REQ_IE_SET:
            if (*len > IEEE80211_APPIE_MAX - sizeof(struct ieee80211req_getset_appiebuf))
            {
            ...
            if (set80211priv(drv, IEEE80211_IOCTL_SET_APPIEBUF, tfs_ie, IEEE80211_APPIE_MAX))
            {
                wpa_printf(MSG_DEBUG,
        ...
        case WNM_SLEEP_TFS_RESP_IE_ADD:
            tfs_ie              = (struct ieee80211req_getset_appiebuf *)buf;
            tfs_ie->app_frmtype = IEEE80211_APPIE_FRAME_WNM;
            tfs_ie->app_buflen  = IEEE80211_APPIE_MAX - sizeof(struct ieee80211req_getset_appiebuf);
            /* Command header for driver */
            os_memcpy(&(tfs_ie->app_buf[0]), peer, ETH_ALEN);
            val = oper;
            os_memcpy(&(tfs_ie->app_buf[0]) + ETH_ALEN, &val, 2);
            ...
            if (set80211priv(drv, IEEE80211_IOCTL_GET_APPIEBUF, tfs_ie, IEEE80211_APPIE_MAX))
            {
                wpa_printf(MSG_DEBUG,
...
static int atheros_wnm_sleep(struct atheros_driver_data *drv, const u8 *peer, enum wnm_oper oper)
{
    u8 *data, *pos;
    size_t dlen;
    int ret;
    ...
    wpa_printf(MSG_DEBUG, "atheros: WNM-Sleep Oper %d, " MACSTR, oper, MAC2STR(peer));
...
static int atheros_wnm_oper(void *priv, enum wnm_oper oper, const u8 *peer, u8 *buf, u16 *buf_len)
{
    struct atheros_driver_data *drv = priv;
