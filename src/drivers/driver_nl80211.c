                                        ...
                                        int persist)
{
    ...
    if (err < 0)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Could not set nl_socket RX buffer size: %s", nl_geterror(err));
...
#ifdef CONFIG_MESH
static int nl80211_put_mesh_config(struct nl_msg *msg, struct wpa_driver_mesh_bss_params *params);
#endif /* CONFIG_MESH */
static int i802_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr, u16 reason);
...
static int ack_handler_cookie(struct nl_msg *msg, void *arg)
{
    ...
    *ext_arg->err = 0;
    ...
    attrs         = (struct nlattr *)((u8 *)nlmsg_data(nlmsg_hdr(msg)) + sizeof(struct nlmsghdr) + sizeof(int));
...
static int error_handler(struct sockaddr_nl *nla, struct nlmsgerr *err, void *arg)
{
    ...
    *ret = err->error;
    ...
    if (tb[NLMSGERR_ATTR_MSG])
    {
        ...
        wpa_printf(MSG_ERROR, "nl80211: kernel reports: %*s", len, (char *)nla_data(tb[NLMSGERR_ATTR_MSG]));
    }
                         ...
                         void *ack_data)
{
    ...
    opt = 1;
    setsockopt(nl_socket_get_fd(nl_handle), SOL_NETLINK, NETLINK_CAP_ACK, &opt, sizeof(opt));

    err = nl_send_auto_complete(nl_handle, msg);
    if (err < 0)
    {
        wpa_printf(MSG_INFO, "nl80211: nl_send_auto_complete() failed: %s", nl_geterror(err));
    ...
    if (ack_handler_custom)
    {
        ...
        ext_arg->err = &err;
        nl_cb_set(cb, NL_CB_ACK, NL_CB_CUSTOM, ack_handler_custom, ack_data);
    }
    else
    {
    ...
    while (err > 0)
    {
        ...
        if (res == -NLE_DUMP_INTR)
        {
            ...
            wpa_printf(MSG_DEBUG, "nl80211: %s; convert to -EAGAIN", nl_geterror(res));
            err = -EAGAIN;
        }
        else if (res < 0)
        {
            wpa_printf(MSG_INFO, "nl80211: %s->nl_recvmsgs failed: %d (%s)", __func__, res, nl_geterror(res));
        }
    }
out:
    nl_cb_put(cb);
...
int send_and_recv_msgs(struct wpa_driver_nl80211_data *drv,
                       ...
                       void *ack_data)
{
    return send_and_recv(drv->global, drv->global->nl, msg, valid_handler, valid_data, ack_handler_custom, ack_data);
}
                                    ...
                                    void *ack_data)
{
     ...
     */
    if ((drv->capa.flags2 & WPA_DRIVER_FLAGS2_CONTROL_PORT_RX) && handle && set_owner &&
        (nla_put_flag(msg, NL80211_ATTR_CONTROL_PORT_OVER_NL80211) || nla_put_flag(msg, NL80211_ATTR_SOCKET_OWNER) ||
         nla_put_u16(msg, NL80211_ATTR_CONTROL_PORT_ETHERTYPE, ETH_P_PAE) ||
         nla_put_flag(msg, NL80211_ATTR_CONTROL_PORT_NO_PREAUTH)))
        return -1;

    return send_and_recv(drv->global, handle ? handle : drv->global->nl, msg, valid_handler, valid_data,
                                             ...
                                             int set_owner)
{
    ...
    if (nl_connect)
        return send_and_recv_msgs_owner(drv, msg, nl_connect, set_owner, process_bss_event, bss, NULL, NULL);
    else
...
{
    if ((bss->drv->capa.flags2 & WPA_DRIVER_FLAGS2_CONTROL_PORT_RX) || bss->use_nl_connect)
...
static int family_handler(struct nl_msg *msg, void *arg)
{
    ...
    nla_for_each_nested(mcgrp, tb[CTRL_ATTR_MCAST_GROUPS], i)
    {
        ...
        if (!tb2[CTRL_ATTR_MCAST_GRP_NAME] || !tb2[CTRL_ATTR_MCAST_GRP_ID] ||
            os_strncmp(nla_data(tb2[CTRL_ATTR_MCAST_GRP_NAME]), res->group, nla_len(tb2[CTRL_ATTR_MCAST_GRP_NAME])) !=
                0)
            continue;
...
static int nl_get_multicast_id(struct nl80211_global *global, const char *family, const char *group)
{
    ...
    if (!msg)
        return -ENOMEM;
    if (!genlmsg_put(msg, 0, 0, global->nlctrl_id, 0, 0, CTRL_CMD_GETFAMILY, 0) ||
    ...
    ret = send_and_recv(global, global->nl, msg, family_handler, &res, NULL, NULL);
    if (ret == 0)
        ret = res.id;
    return ret;
}
...
static struct nl_msg *nl80211_ifindex_msg_build(
    struct wpa_driver_nl80211_data *drv, struct nl_msg *msg, int ifindex, int flags, uint8_t cmd)
{
    if (!msg)
        return NULL;
...
{
    return nl80211_ifindex_msg_build(drv, nlmsg_alloc(), ifindex, flags, cmd);
}
...
int nl80211_get_wiphy_index(struct i802_bss *bss)
{
    ...
    if (send_and_recv_msgs(bss->drv, msg, netdev_info_handler, &data, NULL, NULL) == 0)
...
static int nl80211_get_macaddr(struct i802_bss *bss)
{
    ...
    return send_and_recv_msgs(bss->drv, msg, netdev_info_handler, &data, NULL, NULL);
}
...
static int nl80211_get_4addr(struct i802_bss *bss)
{
    ...
    if (!(msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_GET_INTERFACE)) ||
        send_and_recv_msgs(bss->drv, msg, netdev_info_handler, &data, NULL, NULL))
        return -1;
...
static int nl80211_register_beacons(struct wpa_driver_nl80211_data *drv, struct nl80211_wiphy_data *w)
{
    ...
    ret = send_and_recv(drv->global, w->nl_beacons, msg, NULL, NULL, NULL, NULL);
    if (ret)
    {
...
static struct nl80211_wiphy_data *nl80211_get_wiphy_data_ap(struct i802_bss *bss)
{
    ...
    if (ieee80211_freq_to_chan(bss->freq, &channel) != HOSTAPD_MODE_IEEE80211AD)
    {
        ...
        nl80211_register_eloop_read(&w->nl_beacons, nl80211_recv_beacons, w, 0);
    }
...
static void wpa_driver_nl80211_event_newlink(struct nl80211_global *global,
                                             struct wpa_driver_nl80211_data *drv,
                                             int ifindex,
                                             const char *ifname)
{
...
static void wpa_driver_nl80211_event_dellink(struct nl80211_global *global,
                                             struct wpa_driver_nl80211_data *drv,
                                             int ifindex,
                                             const char *ifname)
{
...
static struct wpa_driver_nl80211_data *nl80211_find_drv(
    struct nl80211_global *global, int idx, u8 *buf, size_t len, int *init_failed)
{
    struct wpa_driver_nl80211_data *drv;
    int res;
    ...
    dl_list_for_each(drv, &global->interfaces, struct wpa_driver_nl80211_data, list)
    {
        ...
        if (res < 0)
        {
            wpa_printf(MSG_DEBUG, "nl80211: Found matching own interface, but failed to complete reinitialization");
            if (init_failed)
                *init_failed = 1;
            return drv;
        }
...
static void nl80211_refresh_mac(struct wpa_driver_nl80211_data *drv, int ifindex, int notify)
{
    struct i802_bss *bss;
    u8 addr[ETH_ALEN];
    ...
    {
        wpa_printf(MSG_DEBUG, "nl80211: Own MAC address on ifindex %d (%s) changed from " MACSTR " to " MACSTR, ifindex,
                   bss->ifname, MAC2STR(bss->addr), MAC2STR(addr));
        os_memcpy(bss->addr, addr, ETH_ALEN);
        if (notify)
            wpa_supplicant_event(drv->ctx, EVENT_INTERFACE_MAC_CHANGED, NULL);
    }
}
...
static void wpa_driver_nl80211_event_rtm_newlink(void *ctx, struct ifinfomsg *ifi, u8 *buf, size_t len)
{
    ...
    wpa_printf(MSG_DEBUG, "RTM_NEWLINK: ifi_index=%d ifname=%s%s ifi_family=%d ifi_flags=0x%x (%s%s%s%s)",
               ifi->ifi_index, ifname, extra, ifi->ifi_family, ifi->ifi_flags, (ifi->ifi_flags & IFF_UP) ? "[UP]" : "",
               (ifi->ifi_flags & IFF_RUNNING) ? "[RUNNING]" : "", (ifi->ifi_flags & IFF_LOWER_UP) ? "[LOWER_UP]" : "",
    ...
    if (!drv->if_disabled && !(ifi->ifi_flags & IFF_UP))
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "nl80211: Not the main interface (%s) - do not indicate interface down",
    ...
    if (drv->if_disabled && (ifi->ifi_flags & IFF_UP))
    {
        ...
        wpa_printf(MSG_DEBUG, "nl80211: Interface up (%s/%s)", namebuf, ifname);
        if (os_strcmp(drv->first_bss->ifname, ifname) != 0)
        {
            wpa_printf(MSG_DEBUG, "nl80211: Not the main interface (%s) - do not indicate interface up",
                       drv->first_bss->ifname);
        }
     ...
     */
    if (drv->operstate == 1 && (ifi->ifi_flags & (IFF_LOWER_UP | IFF_DORMANT)) == IFF_LOWER_UP &&
...
static int nl80211_get_assoc_freq_handler(struct nl_msg *msg, void *arg)
{
    ...
    nla_parse(tb, NL80211_ATTR_MAX, genlmsg_attrdata(gnlh, 0), genlmsg_attrlen(gnlh, 0), NULL);
    if (!tb[NL80211_ATTR_BSS] || nla_parse_nested(bss, NL80211_BSS_MAX, tb[NL80211_ATTR_BSS], bss_policy) ||
    ...
    if (status == NL80211_BSS_STATUS_ASSOCIATED && bss[NL80211_BSS_FREQUENCY])
    {
        ...
        wpa_printf(MSG_DEBUG, "nl80211: Associated on %u MHz", ctx->assoc_freq);
    }
    if (status == NL80211_BSS_STATUS_IBSS_JOINED && bss[NL80211_BSS_FREQUENCY])
    {
        ctx->ibss_freq = nla_get_u32(bss[NL80211_BSS_FREQUENCY]);
        wpa_printf(MSG_DEBUG, "nl80211: IBSS-joined on %u MHz", ctx->ibss_freq);
    }
    if (status == NL80211_BSS_STATUS_ASSOCIATED && bss[NL80211_BSS_BSSID])
    {
        os_memcpy(ctx->assoc_bssid, nla_data(bss[NL80211_BSS_BSSID]), ETH_ALEN);
        wpa_printf(MSG_DEBUG, "nl80211: Associated with " MACSTR, MAC2STR(ctx->assoc_bssid));
    }

    if (status == NL80211_BSS_STATUS_ASSOCIATED && bss[NL80211_BSS_INFORMATION_ELEMENTS])
    {
        const u8 *ie, *ssid;
        size_t ie_len;
...
try_again:
    ...
    arg.drv = drv;
    ret     = send_and_recv_msgs(drv, msg, nl80211_get_assoc_freq_handler, &arg, NULL, NULL);
    if (ret == -EAGAIN)
    {
        count++;
        if (count >= 10)
        {
            wpa_printf(MSG_INFO, "nl80211: Failed to receive consistent scan result dump for get_assoc_ssid");
        }
        else
        {
            wpa_printf(MSG_DEBUG,
...
try_again:
    ...
    if (ret == -EAGAIN)
    {
        ...
        if (count >= 10)
        {
            wpa_printf(MSG_INFO, "nl80211: Failed to receive consistent scan result dump for get_assoc_freq");
        }
        else
        {
            wpa_printf(MSG_DEBUG,
...
int nl80211_get_link_noise(struct wpa_driver_nl80211_data *drv, struct wpa_signal_info *sig_change)
{
    ...
    return send_and_recv_msgs(drv, msg, get_link_noise, sig_change, NULL, NULL);
}
...
static int get_channel_info(struct nl_msg *msg, void *arg)
{
    ...
    if (tb[NL80211_ATTR_WIPHY_FREQ])
        chan_info->frequency = nla_get_u32(tb[NL80211_ATTR_WIPHY_FREQ]);
    ...
    if (tb[NL80211_ATTR_CENTER_FREQ1])
        chan_info->center_frq1 = nla_get_u32(tb[NL80211_ATTR_CENTER_FREQ1]);
    ...
    if (chan_info->center_frq2)
    {
        ...
        if (ieee80211_freq_to_chan(chan_info->center_frq2, &seg1_idx) != NUM_HOSTAPD_MODES)
...
static int wpa_driver_nl80211_get_country(void *priv, char *alpha2)
{
    ...
    alpha2[0] = '\0';
    ret       = send_and_recv_msgs(drv, msg, nl80211_get_country, alpha2, NULL, NULL);
    if (!alpha2[0])
        ret = -1;
...
static void nl80211_check_global(struct nl80211_global *global)
{
    ...
    for (i = 0; groups[i]; i++)
    {
        ...
        if (ret < 0)
        {
            wpa_printf(MSG_INFO, "nl80211: Could not re-add multicast membership for %s events: %d (%s)", groups[i],
                       ret, nl_geterror(ret));
        }
    }
}
...
static int nl80211_init_connect_handle(struct i802_bss *bss)
{
    if (bss->nl_connect)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Connect handle already created (nl_connect=%p)", bss->nl_connect);
        return -1;
    }
    ...
    if (!bss->nl_connect)
        return -1;
    nl80211_register_eloop_read(&bss->nl_connect, wpa_driver_nl80211_event_receive, bss->nl_cb, 1);
    return 0;
}
...
static void wpa_driver_nl80211_drv_init_rfkill(struct wpa_driver_nl80211_data *drv)
{
    ...
    if (drv->nlmode == NL80211_IFTYPE_P2P_DEVICE)
    {
        ...
        dl_list_for_each(tmp1, &global->interfaces, struct wpa_driver_nl80211_data, list)
        {
...
static void *wpa_driver_nl80211_drv_init(
    void *ctx, const char *ifname, void *global_priv, int hostapd, const u8 *set_addr, const char *driver_params)
{
    struct wpa_driver_nl80211_data *drv;
    struct i802_bss *bss;
    ...
    if (drv->data_tx_status)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "nl80211: wifi status sockopt failed: %s", strerror(errno));
...
static int nl80211_register_frame(
    struct i802_bss *bss, struct nl_sock *nl_handle, u16 type, const u8 *match, size_t match_len, bool multicast)
{
    struct wpa_driver_nl80211_data *drv = bss->drv;
    struct nl_msg *msg;
    int ret;
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Register frame type=0x%x (%s) nl_handle=%p match=%s multicast=%d", type,
               fc2str(type), nl_handle, buf, multicast);

    if (!(msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_REGISTER_FRAME)) ||
        (multicast && nla_put_flag(msg, NL80211_ATTR_RECEIVE_MULTICAST)) ||
        nla_put_u16(msg, NL80211_ATTR_FRAME_TYPE, type) || nla_put(msg, NL80211_ATTR_FRAME_MATCH, match_len, match))
    {
        nlmsg_free(msg);
        return -1;
    }

    ret = send_and_recv(drv->global, nl_handle, msg, NULL, NULL, NULL, NULL);
    if (ret)
    {
        wpa_printf(MSG_DEBUG,
...
{
    nl80211_register_eloop_read(&bss->nl_mgmt, wpa_driver_nl80211_event_receive, bss->nl_cb, 0);
}

static int nl80211_register_action_frame(struct i802_bss *bss, const u8 *match, size_t match_len)
{
    u16 type = (WLAN_FC_TYPE_MGMT << 2) | (WLAN_FC_STYPE_ACTION << 4);
    return nl80211_register_frame(bss, bss->nl_mgmt, type, match, match_len, false);
}
...
static int nl80211_mgmt_subscribe_non_ap(struct i802_bss *bss)
{
    ...
    else if ((drv->capa.flags & WPA_DRIVER_FLAGS_SAE) && !(drv->capa.flags & WPA_DRIVER_FLAGS_SME))
    {
        /* register for SAE Authentication frames */
        nl80211_register_frame(bss, bss->nl_mgmt, type, (u8 *)"\x03\x00", 2, false);
    }

#ifdef CONFIG_PASN
    /* register for PASN Authentication frames */
    if ((drv->capa.flags & WPA_DRIVER_FLAGS_SME) &&
        nl80211_register_frame(bss, bss->nl_mgmt, type, (u8 *)"\x07\x00", 2, false))
        ret = -1;
#endif /* CONFIG_PASN */
...
#ifdef CONFIG_DPP
    /* DPP Public Action */
    if (nl80211_register_action_frame(bss, (u8 *)"\x04\x09\x50\x6f\x9a\x1a", 6) < 0)
        ret = -1;
#endif /* CONFIG_DPP */
#ifdef CONFIG_OCV
    /* SA Query Request */
    ...
    if (!drv->no_rrm && nl80211_register_action_frame(bss, (u8 *)"\x05\x00", 2) < 0)
        ret = -1;
    ...
    if (nl80211_register_action_frame(bss, (u8 *)"\x7e\x50\x6f\x9a\x1a", 5) < 0)
        ret = -1;
...
static int nl80211_mgmt_subscribe_mesh(struct i802_bss *bss)
{
    ...
    if (nl80211_register_frame(bss, bss->nl_mgmt, (WLAN_FC_TYPE_MGMT << 2) | (WLAN_FC_STYPE_AUTH << 4), NULL, 0,
                               false) < 0)
        ret = -1;
...
static int nl80211_register_spurious_class3(struct i802_bss *bss)
{
    ...
    ret = send_and_recv(bss->drv->global, bss->nl_mgmt, msg, NULL, NULL, NULL, NULL);
    if (ret)
    {
        wpa_printf(MSG_DEBUG,
...
static int nl80211_mgmt_subscribe_ap(struct i802_bss *bss)
{
    static const int stypes[] = {
        WLAN_FC_STYPE_AUTH,     WLAN_FC_STYPE_ASSOC_REQ, WLAN_FC_STYPE_REASSOC_REQ,
        WLAN_FC_STYPE_DISASSOC, WLAN_FC_STYPE_DEAUTH,    WLAN_FC_STYPE_PROBE_REQ,
    ...
    {
        if (nl80211_register_frame(bss, bss->nl_mgmt, (WLAN_FC_TYPE_MGMT << 2) | (stypes[i] << 4), NULL, 0, false) < 0)
        {
            goto out_err;
        }
    }
...
static int nl80211_mgmt_subscribe_ap_dev_sme(struct i802_bss *bss)
{
    ...
    if (bss->drv->device_ap_sme)
    {
        ...
        if (nl80211_register_frame(bss, bss->nl_mgmt, type, NULL, 0, false) < 0)
            wpa_printf(MSG_DEBUG,
...
static void nl80211_del_p2pdev(struct i802_bss *bss)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Delete P2P Device %s (0x%llx): %s", bss->ifname,
...
static int nl80211_set_p2pdev(struct i802_bss *bss, int start)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: %s P2P Device %s (0x%llx): %s", start ? "Start" : "Stop", bss->ifname,
               (long long unsigned int)bss->wdev_id, strerror(-ret));
    return ret;
}
...
static int qca_vendor_test_cmd_handler(struct nl_msg *msg, void *arg)
{
    ...
    wpa_hexdump(MSG_DEBUG, "nl80211: Received QCA vendor test command response", nla_data(tb[NL80211_ATTR_VENDOR_DATA]),
...
static void qca_vendor_test(struct wpa_driver_nl80211_data *drv)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_TEST) ||
        !(params = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) || nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_TEST, 123))
    {
        nlmsg_free(msg);
        return;
    }
    ...
    ret = send_and_recv_msgs(drv, msg, qca_vendor_test_cmd_handler, drv, NULL, NULL);
                                              ...
                                              const char *driver_params)
{
    ...
    if (set_addr && (linux_set_iface_flags(drv->global->ioctl_sock, bss->ifname, 0) ||
...
static int wpa_key_mgmt_to_suites(unsigned int key_mgmt_suites, u32 suites[], int max_suites)
{
    int num_suites = 0;

#define __AKM(a, b)                                                        \
    if (num_suites < max_suites && (key_mgmt_suites & (WPA_KEY_MGMT_##a))) \
...
static int issue_key_mgmt_set_key(struct wpa_driver_nl80211_data *drv, const u8 *key, size_t key_len)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_KEY_MGMT_SET_KEY) ||
...
static int nl80211_set_pmk(struct wpa_driver_nl80211_data *drv, const u8 *key, size_t key_len, const u8 *addr)
{
    ...
    if (!msg || nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, addr) || nla_put(msg, NL80211_ATTR_PMK, key_len, key))
    {
        nl80211_nlmsg_clear(msg);
        nlmsg_free(msg);
        return -ENOBUFS;
    ...
    if (ret)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Set PMK failed: ret=%d (%s)", ret, strerror(-ret));
    }
...
static int wpa_driver_nl80211_set_key(struct i802_bss *bss, struct wpa_driver_set_key_params *params)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               __func__, ifindex, ifname, alg, addr, key_idx, set_tx, (unsigned long)seq_len, (unsigned long)key_len,
               key_flag);
...
#ifdef CONFIG_DRIVER_NL80211_QCA
    if ((key_flag & KEY_FLAG_PMK) && (drv->capa.flags & WPA_DRIVER_FLAGS_KEY_MGMT_OFFLOAD))
    {
    ...
    else
    {
        ...
        if (!msg)
            goto fail2;
        if (nla_put(key_msg, NL80211_KEY_DATA, key_len, key) || nla_put_u32(key_msg, NL80211_KEY_CIPHER, suite))
            goto fail;
    ...
    if (nla_put_u8(key_msg, NL80211_KEY_IDX, key_idx) || nla_put_nested(msg, NL80211_ATTR_KEY, key_msg))
        goto fail;
    ...
    if (!msg)
        goto fail2;
    if (!key_msg || nla_put_u8(key_msg, NL80211_KEY_IDX, key_idx) ||
        nla_put_flag(key_msg, wpa_alg_bip(alg) ? (key_idx == 6 || key_idx == 7 ? NL80211_KEY_DEFAULT_BEACON :
    ...
    if (vlan_id && (drv->capa.flags & WPA_DRIVER_FLAGS_VLAN_OFFLOAD))
    {
        wpa_printf(MSG_DEBUG, "nl80211: set_key default - VLAN ID %d", vlan_id);
        if (nla_put_u16(msg, NL80211_ATTR_VLAN_ID, vlan_id))
            goto fail;
    }
                      ...
                      size_t key_len)
{
    ...
    if (nla_put_u8(msg, NL80211_KEY_IDX, key_idx) || nla_put_u32(msg, NL80211_KEY_CIPHER, suite) ||
        (seq && seq_len && nla_put(msg, NL80211_KEY_SEQ, seq_len, seq)) || nla_put(msg, NL80211_KEY_DATA, key_len, key))
        return -1;
...
static int nl80211_set_conn_keys(struct wpa_driver_associate_params *params, struct nl_msg *msg)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (!nl_key || nla_put(msg, NL80211_KEY_DATA, params->wep_key_len[i], params->wep_key[i]) ||
            nla_put_u32(msg, NL80211_KEY_CIPHER,
                        params->wep_key_len[i] == 5 ? RSN_CIPHER_SUITE_WEP40 : RSN_CIPHER_SUITE_WEP104) ||
                            ...
                            struct i802_bss *bss)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, cmd)) || nla_put_u16(msg, NL80211_ATTR_REASON_CODE, reason_code) ||
        (addr && nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, addr)) ||
        (local_state_change && nla_put_flag(msg, NL80211_ATTR_LOCAL_STATE_CHANGE)))
    {
        nlmsg_free(msg);
        return -1;
    }
    ...
    if (nl_connect)
        ret = send_and_recv(drv->global, nl_connect, msg, process_bss_event, bss, NULL, NULL);
    else
        ret = send_and_recv_msgs(drv, msg, NULL, NULL, NULL, NULL);
    if (ret)
    {
        wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: MLME command failed: reason=%u ret=%d (%s)", reason_code, ret,
                strerror(-ret));
    }
    return ret;
}

static int wpa_driver_nl80211_disconnect(struct wpa_driver_nl80211_data *drv, u16 reason_code, struct i802_bss *bss)
{
    int ret;
    ...
    ret = wpa_driver_nl80211_mlme(drv, NULL, NL80211_CMD_DISCONNECT, reason_code, 0, bss);
    /*
...
static int wpa_driver_nl80211_deauthenticate(struct i802_bss *bss, const u8 *addr, u16 reason_code)
{
    struct wpa_driver_nl80211_data *drv = bss->drv;
    int ret;
    int drv_associated = drv->associated;
    ...
    nl80211_mark_disconnected(drv);
    ret = wpa_driver_nl80211_mlme(drv, addr, NL80211_CMD_DEAUTHENTICATE, reason_code, 0, bss);
    /*
...
static void nl80211_copy_auth_params(struct wpa_driver_nl80211_data *drv, struct wpa_driver_auth_params *params)
{
    ...
    if (params->auth_data)
    {
        drv->auth_data = os_memdup(params->auth_data, params->auth_data_len);
        if (drv->auth_data)
...
static void nl80211_unmask_11b_rates(struct i802_bss *bss)
{
     ...
     */
    wpa_printf(MSG_DEBUG, "nl80211: Interface %s mode is for non-P2P, but 11b rates were disabled - re-enable them",
               bss->ifname);
    nl80211_disable_11b_rates(drv, drv->ifindex, 0);
}
...
retry:
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        p.key_flag = i == params->wep_tx_keyidx ? KEY_FLAG_GROUP_RX_TX_DEFAULT : KEY_FLAG_GROUP_RX_TX;
    ...
    if (params->ssid)
    {
        wpa_printf(MSG_DEBUG, "  * SSID=%s", wpa_ssid_txt(params->ssid, params->ssid_len));
    ...
    if (params->auth_data)
    {
        wpa_hexdump(MSG_DEBUG, "  * auth_data", params->auth_data, params->auth_data_len);
        if (nla_put(msg, NL80211_ATTR_SAE_DATA, params->auth_data_len, params->auth_data))
            goto fail;
    }
    type = get_nl_auth_type(params->auth_alg);
    wpa_printf(MSG_DEBUG, "  * Auth Type %d", type);
    if (type == NL80211_AUTHTYPE_MAX || nla_put_u32(msg, NL80211_ATTR_AUTH_TYPE, type))
        goto fail;
    ...
    if (ret)
    {
        wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: MLME command failed (auth): count=%d ret=%d (%s)", count, ret,
                strerror(-ret));
        count++;
        if ((ret == -EALREADY || ret == -EEXIST) && count == 1 && params->bssid && !params->local_state_change)
        {
            /*
...
static int wpa_driver_nl80211_send_mlme(struct i802_bss *bss,
                                        const u8 *data,
                                        size_t data_len,
                                        int noack,
                                        unsigned int freq,
                                        ...
                                        int no_encrypt)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               MAC2STR(mgmt->da), noack, freq, no_cck, offchanok, wait_time, no_encrypt, fc, fc2str(fc), drv->nlmode);

    if ((is_sta_interface(drv->nlmode) || drv->nlmode == NL80211_IFTYPE_P2P_DEVICE) &&
    ...
    if (is_sta_interface(drv->nlmode) && WLAN_FC_GET_TYPE(fc) == WLAN_FC_TYPE_MGMT &&
        WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_AUTH)
    {
        if (freq == 0 && (drv->capa.flags & WPA_DRIVER_FLAGS_SAE) && !(drv->capa.flags & WPA_DRIVER_FLAGS_SME))
        {
            freq = nl80211_get_assoc_freq(drv);
            wpa_printf(MSG_DEBUG, "nl80211: send_mlme - Use assoc_freq=%u for external auth", freq);
        }

        /* Allow off channel for PASN authentication */
        if (data_len >= IEEE80211_HDRLEN + 2 && WPA_GET_LE16(data + IEEE80211_HDRLEN) == WLAN_AUTH_PASN && !offchanok)
        {
            wpa_printf(MSG_DEBUG, "nl80211: send_mlme: allow off channel for PASN");
            offchanok = 1;
        }
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "nl80211: send_mlme: allow Deauthentication frame for PASN");
    ...
    {
        freq = nl80211_get_assoc_freq(drv);
        wpa_printf(MSG_DEBUG, "nl80211: send_mlme - Use assoc_freq=%u for IBSS", freq);
    }
    if (freq == 0)
    {
        wpa_printf(MSG_DEBUG, "nl80211: send_mlme - Use bss->freq=%u", bss->freq);
        freq = bss->freq;
    }
    ...
    if (drv->use_monitor && is_ap_interface(drv->nlmode))
    {
        ...
        return nl80211_send_monitor(drv, data, data_len, encrypt, noack);
    }

    if (noack || WLAN_FC_GET_TYPE(fc) != WLAN_FC_TYPE_MGMT || WLAN_FC_GET_STYPE(fc) != WLAN_FC_STYPE_ACTION)
        use_cookie = 0;
send_frame_cmd:
#ifdef CONFIG_TESTING_OPTIONS
    if (no_encrypt && !encrypt && !drv->use_monitor)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Request to send an unencrypted frame - use a monitor interface for this");
        if (nl80211_create_monitor_interface(drv) < 0)
            return -1;
        res = nl80211_send_monitor(drv, data, data_len, encrypt, noack);
        nl80211_remove_monitor_interface(drv);
        return res;
    }
#endif /* CONFIG_TESTING_OPTIONS */
    ...
    res = nl80211_send_frame_cmd(bss, freq, wait_time, data, data_len, use_cookie, no_cck, noack, offchanok, csa_offs,
                                 csa_offs_len);
...
static int nl80211_set_bss(
    struct i802_bss *bss, int cts, int preamble, int slot, int ht_opmode, int ap_isolate, const int *basic_rates)
{
    struct wpa_driver_nl80211_data *drv = bss->drv;
    struct nl_msg *msg;
...
static int wpa_driver_nl80211_set_acl(void *priv, struct hostapd_acl_params *params)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Set %s ACL (num_mac_acl=%u)", params->acl_policy ? "Accept" : "Deny",
               params->num_mac_acl);
    ...
    if (!(msg = nl80211_ifindex_msg_build(drv, nlmsg_alloc_size(nlmsg_sz), drv->ifindex, 0, NL80211_CMD_SET_MAC_ACL)) ||
        nla_put_u32(
            msg, NL80211_ATTR_ACL_POLICY,
            params->acl_policy ? NL80211_ACL_POLICY_DENY_UNLESS_LISTED : NL80211_ACL_POLICY_ACCEPT_UNLESS_LISTED) ||
...
#ifdef CONFIG_MESH
static int nl80211_set_mesh_config(void *priv, struct wpa_driver_mesh_bss_params *params)
{
    struct i802_bss *bss                = priv;
    struct wpa_driver_nl80211_data *drv = bss->drv;
    struct nl_msg *msg;
    ...
    if (ret)
    {
        wpa_printf(MSG_ERROR, "nl80211: Mesh config set failed: %d (%s)", ret, strerror(-ret));
        return ret;
    }
    return 0;
}
...
static int nl80211_put_beacon_rate(struct nl_msg *msg, u64 flags, u64 flags2, struct wpa_driver_ap_params *params)
{
    ...
    switch (params->rate_type)
    {
        case BEACON_RATE_LEGACY:
            ...
            {
                wpa_printf(MSG_INFO, "nl80211: Driver does not support setting Beacon frame rate (legacy)");
                return -1;
            }

            if (nla_put_u8(msg, NL80211_TXRATE_LEGACY, (u8)params->beacon_rate / 5) ||
                nla_put(msg, NL80211_TXRATE_HT, 0, NULL) ||
                (params->freq->vht_enabled && nla_put(msg, NL80211_TXRATE_VHT, sizeof(vht_rate), &vht_rate)))
                return -1;

            wpa_printf(MSG_DEBUG, " * beacon_rate = legacy:%u (* 100 kbps)", params->beacon_rate);
            break;
        case BEACON_RATE_HT:
            if (!(flags & WPA_DRIVER_FLAGS_BEACON_RATE_HT))
            {
                wpa_printf(MSG_INFO, "nl80211: Driver does not support setting Beacon frame rate (HT)");
                return -1;
            }
            if (nla_put(msg, NL80211_TXRATE_LEGACY, 0, NULL) ||
                ...
                (params->freq->vht_enabled && nla_put(msg, NL80211_TXRATE_VHT, sizeof(vht_rate), &vht_rate)))
                return -1;
            wpa_printf(MSG_DEBUG, " * beacon_rate = HT-MCS %u", params->beacon_rate);
            break;
        case BEACON_RATE_VHT:
            if (!(flags & WPA_DRIVER_FLAGS_BEACON_RATE_VHT))
            {
                wpa_printf(MSG_INFO, "nl80211: Driver does not support setting Beacon frame rate (VHT)");
                return -1;
            }
            ...
            if (nla_put(msg, NL80211_TXRATE_VHT, sizeof(vht_rate), &vht_rate))
                return -1;
            wpa_printf(MSG_DEBUG, " * beacon_rate = VHT-MCS %u", params->beacon_rate);
            break;
        case BEACON_RATE_HE:
            if (!(flags2 & WPA_DRIVER_FLAGS2_BEACON_RATE_HE))
            {
                wpa_printf(MSG_INFO, "nl80211: Driver does not support setting Beacon frame rate (HE)");
                return -1;
            }
            he_rate.mcs[0] = BIT(params->beacon_rate);
            if (nla_put(msg, NL80211_TXRATE_LEGACY, 0, NULL) || nla_put(msg, NL80211_TXRATE_HT, 0, NULL) ||
                nla_put(msg, NL80211_TXRATE_VHT, sizeof(vht_rate), &vht_rate) ||
                nla_put(msg, NL80211_TXRATE_HE, sizeof(he_rate), &he_rate))
                return -1;
            wpa_printf(MSG_DEBUG, " * beacon_rate = HE-MCS %u", params->beacon_rate);
            break;
    }
...
static int nl80211_set_multicast_to_unicast(struct i802_bss *bss, int multicast_to_unicast)
{
    struct wpa_driver_nl80211_data *drv = bss->drv;
    struct nl_msg *msg;
    int ret;
    ...
    {
        wpa_printf(MSG_ERROR, "nl80211: Failed to build NL80211_CMD_SET_MULTICAST_TO_UNICAST msg for %s", bss->ifname);
        nlmsg_free(msg);
        return -ENOBUFS;
    }
    ...
    {
        case 0:
            wpa_printf(MSG_DEBUG, "nl80211: multicast to unicast %s on interface %s",
                       multicast_to_unicast ? "enabled" : "disabled", bss->ifname);
            break;
        case -EOPNOTSUPP:
            if (!multicast_to_unicast)
                break;
            wpa_printf(MSG_INFO, "nl80211: multicast to unicast not supported on interface %s", bss->ifname);
            break;
        default:
            wpa_printf(MSG_ERROR, "nl80211: %s multicast to unicast failed with %d (%s) on interface %s",
                       multicast_to_unicast ? "enabling" : "disabling", ret, strerror(-ret), bss->ifname);
            break;
    }
...
#ifdef CONFIG_FILS
static int nl80211_fils_discovery(struct i802_bss *bss, struct nl_msg *msg, struct wpa_driver_ap_params *params)
{
    struct nlattr *attr;

    if (!bss->drv->fils_discovery)
    {
        wpa_printf(MSG_ERROR, "nl80211: Driver does not support FILS Discovery frame transmission for %s", bss->ifname);
        return -1;
    }

    attr = nla_nest_start(msg, NL80211_ATTR_FILS_DISCOVERY);
    if (!attr || nla_put_u32(msg, NL80211_FILS_DISCOVERY_ATTR_INT_MIN, params->fd_min_int) ||
        nla_put_u32(msg, NL80211_FILS_DISCOVERY_ATTR_INT_MAX, params->fd_max_int) ||
        (params->fd_frame_tmpl &&
         nla_put(msg, NL80211_FILS_DISCOVERY_ATTR_TMPL, params->fd_frame_tmpl_len, params->fd_frame_tmpl)))
        return -1;
...
static int nl80211_unsol_bcast_probe_resp(struct i802_bss *bss, struct nl_msg *msg, struct wpa_driver_ap_params *params)
{
    struct nlattr *attr;
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Unsolicited broadcast Probe Response frame interval: %u",
               params->unsol_bcast_probe_resp_interval);
    attr = nla_nest_start(msg, NL80211_ATTR_UNSOL_BCAST_PROBE_RESP);
    if (!attr || nla_put_u32(msg, NL80211_UNSOL_BCAST_PROBE_RESP_ATTR_INT, params->unsol_bcast_probe_resp_interval) ||
        (params->unsol_bcast_probe_resp_tmpl &&
         nla_put(msg, NL80211_UNSOL_BCAST_PROBE_RESP_ATTR_TMPL, params->unsol_bcast_probe_resp_tmpl_len,
                 params->unsol_bcast_probe_resp_tmpl)))
        return -1;
...
static int wpa_driver_nl80211_set_ap(void *priv, struct wpa_driver_ap_params *params)
{
    ...
    else if (!drv->device_ap_sme && !drv->use_monitor && !nl80211_get_wiphy_data_ap(bss))
        return -ENOBUFS;
    ...
    wpa_printf(MSG_DEBUG, "nl80211: ssid=%s", wpa_ssid_txt(params->ssid, params->ssid_len));
    ...
    if (num_suites > NL80211_MAX_NR_AKM_SUITES)
        wpa_printf(MSG_DEBUG, "nl80211: Not enough room for all AKM suites (num_suites=%d > NL80211_MAX_NR_AKM_SUITES)",
                   num_suites);
    else if (num_suites && nla_put(msg, NL80211_ATTR_AKM_SUITES, num_suites * sizeof(u32), suites))
        goto fail;

    if (params->key_mgmt_suites & WPA_KEY_MGMT_IEEE8021X_NO_WPA &&
        (!params->pairwise_ciphers || params->pairwise_ciphers & (WPA_CIPHER_WEP104 | WPA_CIPHER_WEP40)) &&
        (nla_put_u16(msg, NL80211_ATTR_CONTROL_PORT_ETHERTYPE, ETH_P_PAE) ||
    ...
    if (params->ftm_responder)
    {
        ...
        if (!ftm || nla_put_flag(msg, NL80211_FTM_RESP_ATTR_ENABLED) ||
            (params->lci &&
             nla_put(msg, NL80211_FTM_RESP_ATTR_LCI, wpabuf_len(params->lci), wpabuf_head(params->lci))) ||
            (params->civic &&
             nla_put(msg, NL80211_FTM_RESP_ATTR_CIVICLOC, wpabuf_len(params->civic), wpabuf_head(params->civic))))
            goto fail;
    ...
    if (params->he_spr_ctrl)
    {
        ...
        wpa_printf(MSG_DEBUG, "nl80211: he_spr_ctrl=0x%x", params->he_spr_ctrl);

        if (!spr || nla_put_u8(msg, NL80211_HE_OBSS_PD_ATTR_SR_CTRL, params->he_spr_ctrl) ||
            ((params->he_spr_ctrl & SPATIAL_REUSE_NON_SRG_OFFSET_PRESENT) &&
             nla_put_u8(msg, NL80211_HE_OBSS_PD_ATTR_NON_SRG_MAX_OFFSET, params->he_spr_non_srg_obss_pd_max_offset)))
            goto fail;

        if ((params->he_spr_ctrl & SPATIAL_REUSE_SRG_INFORMATION_PRESENT) &&
            (nla_put_u8(msg, NL80211_HE_OBSS_PD_ATTR_MIN_OFFSET, params->he_spr_srg_obss_pd_min_offset) ||
             nla_put_u8(msg, NL80211_HE_OBSS_PD_ATTR_MAX_OFFSET, params->he_spr_srg_obss_pd_max_offset) ||
             nla_put(msg, NL80211_HE_OBSS_PD_ATTR_BSS_COLOR_BITMAP, sizeof(params->he_spr_bss_color_bitmap),
                     params->he_spr_bss_color_bitmap) ||
             nla_put(msg, NL80211_HE_OBSS_PD_ATTR_PARTIAL_BSSID_BITMAP, sizeof(params->he_spr_partial_bssid_bitmap),
    ...
    if (params->freq && params->freq->he_enabled)
    {
        ...
        if (!bss_color || (params->he_bss_color_disabled && nla_put_flag(msg, NL80211_HE_BSS_COLOR_ATTR_DISABLED)) ||
            (params->he_bss_color_partial && nla_put_flag(msg, NL80211_HE_BSS_COLOR_ATTR_PARTIAL)) ||
            nla_put_u8(msg, NL80211_HE_BSS_COLOR_ATTR_COLOR, params->he_bss_color))
            goto fail;
    ...
    {
        wpa_printf(MSG_DEBUG, "nl80211: twt_responder=%d", params->twt_responder);
        if (nla_put_flag(msg, NL80211_ATTR_TWT_RESPONDER))
            goto fail;
    }

    if (params->unsol_bcast_probe_resp_interval && nl80211_unsol_bcast_probe_resp(bss, msg, params) < 0)
        goto fail;
#endif /* CONFIG_IEEE80211AX */

#ifdef CONFIG_SAE
    if (((params->key_mgmt_suites & WPA_KEY_MGMT_SAE) || (params->key_mgmt_suites & WPA_KEY_MGMT_FT_SAE)) &&
    ...
    {
        bss->beacon_set = 1;
        nl80211_set_bss(bss, params->cts_protect, params->preamble, params->short_slot_time, params->ht_opmode,
                        params->isolate, params->basic_rates);
        nl80211_set_multicast_to_unicast(bss, params->multicast_to_unicast);
        if (beacon_set && params->freq && params->freq->bandwidth != bss->bandwidth)
        {
            wpa_printf(MSG_DEBUG, "nl80211: Update BSS %s bandwidth: %d -> %d", bss->ifname, bss->bandwidth,
...
static int nl80211_put_freq_params(struct nl_msg *msg, const struct hostapd_freq_params *freq)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "  * EDMG configuration: channels=0x%x bw_config=%d", freq->edmg.channels,
                   freq->edmg.bw_config);
        if (nla_put_u8(msg, NL80211_ATTR_WIPHY_EDMG_CHANNELS, freq->edmg.channels) ||
            nla_put_u8(msg, NL80211_ATTR_WIPHY_EDMG_BW_CONFIG, freq->edmg.bw_config))
            return -1;
    }
    else
    {
...
static int nl80211_set_channel(struct i802_bss *bss, struct hostapd_freq_params *freq, int set_chan)
{
    ...
    wpa_printf(
        ...
        freq->freq, freq->ht_enabled, freq->vht_enabled, freq->he_enabled, freq->bandwidth, freq->center_freq1,
        freq->center_freq2);
...
static int wpa_driver_nl80211_sta_add(void *priv, struct hostapd_sta_add_params *params)
{
    ...
    if (!params->set || (params->flags & WPA_STA_TDLS_PEER) ||
        (params->set && FULL_AP_CLIENT_STATE_SUPP(drv->capa.flags) && (params->flags & WPA_STA_ASSOCIATED)))
    {
        ...
        {
            wpa_hexdump(MSG_DEBUG, "  * he_6ghz_capab", params->he_6ghz_capab, sizeof(*params->he_6ghz_capab));
    ...
    if (!is_mesh_interface(drv->nlmode))
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "nl80211: Ignore ASSOC/AUTH flags since driver doesn't support full AP client state");
    ...
    if (params->plink_state &&
        nla_put_u8(msg, NL80211_ATTR_STA_PLINK_STATE, sta_plink_state_nl80211(params->plink_state)))
        goto fail;
#endif /* CONFIG_MESH */
...
static void rtnl_neigh_delete_fdb_entry(struct i802_bss *bss, const u8 *addr)
{
    ...
    if (err < 0)
    {
        wpa_printf(MSG_DEBUG, "nl80211: bridge FDB entry delete for " MACSTR " ifindex=%d failed: %s", MAC2STR(addr),
...
static int wpa_driver_nl80211_sta_remove(struct i802_bss *bss, const u8 *addr, int deauth, u16 reason_code)
{
    ...
    if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_DEL_STATION)) || nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, addr) ||
    ...
    ret = send_and_recv_msgs(drv, msg, NULL, NULL, NULL, NULL);
    wpa_printf(MSG_DEBUG, "nl80211: sta_remove -> DEL_STATION %s " MACSTR " --> %d (%s)", bss->ifname, MAC2STR(addr),
               ret, strerror(-ret));
...
void nl80211_remove_iface(struct wpa_driver_nl80211_data *drv, int ifidx)
{
    ...
    dl_list_for_each(drv2, &drv->global->interfaces, struct wpa_driver_nl80211_data, list)
    {
                                     ...
                                     void *arg)
{
    ...
    if ((addr && iftype == NL80211_IFTYPE_P2P_DEVICE) && nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, addr))
        goto fail;
     ...
     */
    if (drv->hostapd || iftype == NL80211_IFTYPE_AP_VLAN || iftype == NL80211_IFTYPE_WDS ||
...
static int nl80211_setup_ap(struct i802_bss *bss)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Setup AP(%s) - device_ap_sme=%d use_monitor=%d", bss->ifname, drv->device_ap_sme,
               drv->use_monitor);
...
static void nl80211_teardown_ap(struct i802_bss *bss)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Teardown AP(%s) - device_ap_sme=%d use_monitor=%d", bss->ifname, drv->device_ap_sme,
               drv->use_monitor);
    if (drv->device_ap_sme)
    {
        wpa_driver_nl80211_probe_req_report(bss, 0);
...
static int nl80211_tx_control_port(void *priv, const u8 *dest, u16 proto, const u8 *buf, size_t len, int no_encrypt)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Send over control port dest=" MACSTR " proto=0x%04x len=%u no_encrypt=%d",
    ...
    if (!msg || nla_put_u16(msg, NL80211_ATTR_CONTROL_PORT_ETHERTYPE, proto) ||
        nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, dest) || nla_put(msg, NL80211_ATTR_FRAME, len, buf) ||
    ...
    ret              = send_and_recv_msgs(bss->drv, msg, NULL, NULL, ack_handler_cookie, &ext_arg);
    if (ret)
    {
        wpa_printf(MSG_DEBUG, "nl80211: tx_control_port failed: ret=%d (%s)", ret, strerror(-ret));
    }
    else
    {
        struct wpa_driver_nl80211_data *drv = bss->drv;

        wpa_printf(MSG_DEBUG, "nl80211: tx_control_port cookie=0x%llx", (long long unsigned int)cookie);
    ...
    void *priv, const u8 *addr, const u8 *data, size_t data_len, int encrypt, const u8 *own_addr, u32 flags)
{
    ...
    if (drv->control_port_ap && (drv->capa.flags & WPA_DRIVER_FLAGS_CONTROL_PORT))
        return nl80211_tx_control_port(bss, addr, ETH_P_EAPOL, data, data_len, !encrypt);
    ...
    if (res < 0)
    {
        wpa_printf(MSG_ERROR, "hapd_send_eapol - packet len: %lu - failed", (unsigned long)len);
    }
    os_free(hdr);
...
static int wpa_driver_nl80211_sta_set_flags(
    void *priv, const u8 *addr, unsigned int total_flags, unsigned int flags_or, unsigned int flags_and)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               bss->ifname, MAC2STR(addr), total_flags, flags_or, flags_and, !!(total_flags & WPA_STA_AUTHORIZED));
    ...
    if (!flags || ((total_flags & WPA_STA_AUTHORIZED) && nla_put_flag(msg, NL80211_STA_FLAG_AUTHORIZED)) ||
        ((total_flags & WPA_STA_WMM) && nla_put_flag(msg, NL80211_STA_FLAG_WME)) ||
        ((total_flags & WPA_STA_SHORT_PREAMBLE) && nla_put_flag(msg, NL80211_STA_FLAG_SHORT_PREAMBLE)) ||
...
static int driver_nl80211_sta_set_airtime_weight(void *priv, const u8 *addr, unsigned int weight)
{
    struct i802_bss *bss = priv;
    struct nl_msg *msg;
    int ret;

    wpa_printf(MSG_DEBUG, "nl80211: Set STA airtime weight - ifname=%s addr=" MACSTR " weight=%u", bss->ifname,
               MAC2STR(addr), weight);
    ...
    if (ret)
    {
        wpa_printf(MSG_DEBUG, "nl80211: SET_STATION[AIRTIME_WEIGHT] failed: ret=%d (%s)", ret, strerror(-ret));
    }
    return ret;
fail:
    nlmsg_free(msg);
...
static int nl80211_ht_vht_overrides(struct nl_msg *msg, struct wpa_driver_associate_params *params)
{
    ...
    if (params->vhtcaps && params->vhtcaps_mask)
    {
        ...
        if (nla_put(msg, NL80211_ATTR_VHT_CAPABILITY, sz, params->vhtcaps) ||
            nla_put(msg, NL80211_ATTR_VHT_CAPABILITY_MASK, sz, params->vhtcaps_mask))
            return -1;
    }
...
retry:
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_JOIN_IBSS)) || params->ssid == NULL ||
        params->ssid_len > sizeof(drv->ssid))
        goto fail;
    ...
    if (params->key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X || params->key_mgmt_suite == WPA_KEY_MGMT_PSK ||
                                           ...
                                           struct nl_msg *msg)
{
    ...
    {
        wpa_hexdump_ascii(MSG_DEBUG, "  * FILS ERP EMSKname/username", params->fils_erp_username,
                          params->fils_erp_username_len);
        if (nla_put(msg, NL80211_ATTR_FILS_ERP_USERNAME, params->fils_erp_username_len, params->fils_erp_username))
            return -1;
    }

    if (params->fils_erp_realm_len)
    {
        wpa_hexdump_ascii(MSG_DEBUG, "  * FILS ERP Realm", params->fils_erp_realm, params->fils_erp_realm_len);
        if (nla_put(msg, NL80211_ATTR_FILS_ERP_REALM, params->fils_erp_realm_len, params->fils_erp_realm))
            return -1;
    }

    if (params->fils_erp_rrk_len)
    {
        wpa_printf(MSG_DEBUG, "  * FILS ERP next seq %u", params->fils_erp_next_seq_num);
        if (nla_put_u16(msg, NL80211_ATTR_FILS_ERP_NEXT_SEQ_NUM, params->fils_erp_next_seq_num))
            return -1;

        wpa_printf(MSG_DEBUG, "  * FILS ERP rRK (len=%lu)", (unsigned long)params->fils_erp_rrk_len);
        if (nla_put(msg, NL80211_ATTR_FILS_ERP_RRK, params->fils_erp_rrk_len, params->fils_erp_rrk))
            return -1;
    }
                                  ...
                                  struct nl_msg *msg)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "  * EDMG configuration: channels=0x%x bw_config=%d", params->freq.edmg.channels,
                   params->freq.edmg.bw_config);
        if (nla_put_u8(msg, NL80211_ATTR_WIPHY_EDMG_CHANNELS, params->freq.edmg.channels) ||
            nla_put_u8(msg, NL80211_ATTR_WIPHY_EDMG_BW_CONFIG, params->freq.edmg.bw_config))
            return -1;
    }
    ...
    if (params->key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X || params->key_mgmt_suite == WPA_KEY_MGMT_PSK ||
        params->key_mgmt_suite == WPA_KEY_MGMT_FT_IEEE8021X || params->key_mgmt_suite == WPA_KEY_MGMT_FT_PSK ||
        params->key_mgmt_suite == WPA_KEY_MGMT_CCKM || params->key_mgmt_suite == WPA_KEY_MGMT_OSEN ||
        params->key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X_SHA256 || params->key_mgmt_suite == WPA_KEY_MGMT_PSK_SHA256 ||
        params->key_mgmt_suite == WPA_KEY_MGMT_SAE || params->key_mgmt_suite == WPA_KEY_MGMT_FT_SAE ||
        params->key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X_SUITE_B ||
        params->key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X_SUITE_B_192 ||
        params->key_mgmt_suite == WPA_KEY_MGMT_FT_IEEE8021X_SHA384 ||
        params->key_mgmt_suite == WPA_KEY_MGMT_FILS_SHA256 || params->key_mgmt_suite == WPA_KEY_MGMT_FILS_SHA384 ||
        params->key_mgmt_suite == WPA_KEY_MGMT_FT_FILS_SHA256 ||
        params->key_mgmt_suite == WPA_KEY_MGMT_FT_FILS_SHA384 || params->key_mgmt_suite == WPA_KEY_MGMT_OWE ||
    ...
    if (params->psk && (drv->capa.flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE_PSK))
    {
    ...
    if (params->key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X_NO_WPA &&
        (params->pairwise_suite == WPA_CIPHER_NONE || params->pairwise_suite == WPA_CIPHER_WEP104 ||
    ...
    if ((params->auth_alg & WPA_AUTH_ALG_FILS) && nl80211_put_fils_connect_params(drv, params, msg) != 0)
        return -1;

    if ((params->key_mgmt_suite == WPA_KEY_MGMT_SAE || params->key_mgmt_suite == WPA_KEY_MGMT_FT_SAE) &&
        (!(drv->capa.flags & WPA_DRIVER_FLAGS_SME)) && nla_put_flag(msg, NL80211_ATTR_EXTERNAL_AUTH_SUPPORT))
        return -1;

    return 0;
}

static int wpa_driver_nl80211_try_connect(struct wpa_driver_nl80211_data *drv,
                                          struct wpa_driver_associate_params *params,
                                          struct i802_bss *bss)
{
    struct nl_msg *msg;
    ...
    if (params->req_key_mgmt_offload && params->psk &&
        (params->key_mgmt_suite == WPA_KEY_MGMT_PSK || params->key_mgmt_suite == WPA_KEY_MGMT_PSK_SHA256 ||
                                      ...
                                      struct i802_bss *bss)
{
    ...
    if (ret == -EALREADY)
    {
        ...
        wpa_printf(MSG_DEBUG,
                   ...
                   "attempt");
        if (wpa_driver_nl80211_disconnect(drv, WLAN_REASON_PREV_AUTH_NOT_VALID, bss))
            return -1;
        ret = wpa_driver_nl80211_try_connect(drv, params, bss);
    }
    return ret;
...
static int wpa_driver_nl80211_associate(void *priv, struct wpa_driver_associate_params *params)
{
    ...
    if (params->fils_kek)
    {
        wpa_printf(MSG_DEBUG, "  * FILS KEK (len=%u)", (unsigned int)params->fils_kek_len);
        if (nla_put(msg, NL80211_ATTR_FILS_KEK, params->fils_kek_len, params->fils_kek))
            goto fail;
    }
    if (params->fils_nonces)
    {
        wpa_hexdump(MSG_DEBUG, "  * FILS nonces (for AAD)", params->fils_nonces, params->fils_nonces_len);
        if (nla_put(msg, NL80211_ATTR_FILS_NONCES, params->fils_nonces_len, params->fils_nonces))
            goto fail;
    }
                                            ...
                                            struct hostapd_freq_params *desired_freq_params)
{
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        if (i == 0 && was_ap && !is_ap_interface(nlmode) && bss->brname[0] &&
            (bss->added_if_into_bridge || bss->already_in_bridge))
        {
            ...
            if (linux_br_del_if(drv->global->ioctl_sock, bss->brname, bss->ifname) < 0)
                wpa_printf(MSG_INFO, "nl80211: Failed to remove interface %s from bridge %s: %s", bss->ifname,
                           bss->brname, strerror(errno));
        }
    ...
    if (!bss->in_deinit && !is_ap_interface(nlmode) && !is_mesh_interface(nlmode) &&
...
void nl80211_restore_ap_mode(struct i802_bss *bss)
{
    ...
    if (!was_ap && is_ap_interface(drv->ap_scan_as_station) && bss->brname[0] &&
        (bss->added_if_into_bridge || bss->already_in_bridge))
    {
        wpa_printf(MSG_DEBUG, "nl80211: Add AP interface %s back into the bridge %s", bss->ifname, bss->brname);
        if (linux_br_add_if(drv->global->ioctl_sock, bss->brname, bss->ifname) < 0)
        {
            wpa_printf(MSG_WARNING, "nl80211: Failed to add interface %s into bridge %s: %s", bss->ifname, bss->brname,
                       strerror(errno));
        }
    }
    drv->ap_scan_as_station = NL80211_IFTYPE_UNSPECIFIED;
...
static int wpa_driver_nl80211_set_operstate(void *priv, int state)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Set %s operstate %d->%d (%s)", bss->ifname, drv->operstate, state,
               state ? "UP" : "DORMANT");
    drv->operstate = state;
...
static int wpa_driver_nl80211_set_supp_port(void *priv, int authorized)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Set supplicant port %sauthorized for " MACSTR, authorized ? "" : "un",
               MAC2STR(drv->bssid));
...
static void get_sta_tid_stats(struct hostap_sta_driver_data *data, struct nlattr *attr)
{
    ...
    {
        if (nla_parse_nested(tid_stats, NL80211_TID_STATS_MAX, tidattr, NULL) != 0 ||
            !tid_stats[NL80211_TID_STATS_TXQ_STATS] ||
            nla_parse_nested(txq_stats, NL80211_TXQ_STATS_MAX, tid_stats[NL80211_TID_STATS_TXQ_STATS],
                             txq_stats_policy) != 0)
            continue;
        /* sum the backlogs over all TIDs for station */
        if (txq_stats[NL80211_TXQ_STATS_BACKLOG_BYTES])
            data->backlog_bytes += nla_get_u32(txq_stats[NL80211_TXQ_STATS_BACKLOG_BYTES]);
...
static int get_sta_handler(struct nl_msg *msg, void *arg)
{
    ...
    static struct nla_policy stats_policy[NL80211_STA_INFO_MAX + 1] = {
        [NL80211_STA_INFO_INACTIVE_TIME] = {.type = NLA_U32},  [NL80211_STA_INFO_RX_BYTES] = {.type = NLA_U32},
        [NL80211_STA_INFO_TX_BYTES] = {.type = NLA_U32},       [NL80211_STA_INFO_RX_PACKETS] = {.type = NLA_U32},
        [NL80211_STA_INFO_TX_PACKETS] = {.type = NLA_U32},     [NL80211_STA_INFO_TX_FAILED] = {.type = NLA_U32},
        [NL80211_STA_INFO_RX_BYTES64] = {.type = NLA_U64},     [NL80211_STA_INFO_TX_BYTES64] = {.type = NLA_U64},
        [NL80211_STA_INFO_SIGNAL] = {.type = NLA_U8},          [NL80211_STA_INFO_ACK_SIGNAL] = {.type = NLA_U8},
        [NL80211_STA_INFO_RX_DURATION] = {.type = NLA_U64},    [NL80211_STA_INFO_TX_DURATION] = {.type = NLA_U64},
        [NL80211_STA_INFO_CONNECTED_TIME] = {.type = NLA_U32},
    };
    struct nlattr *rate[NL80211_RATE_INFO_MAX + 1];
    static struct nla_policy rate_policy[NL80211_RATE_INFO_MAX + 1] = {
        [NL80211_RATE_INFO_BITRATE] = {.type = NLA_U16},   [NL80211_RATE_INFO_BITRATE32] = {.type = NLA_U32},
        [NL80211_RATE_INFO_MCS] = {.type = NLA_U8},        [NL80211_RATE_INFO_VHT_MCS] = {.type = NLA_U8},
        [NL80211_RATE_INFO_SHORT_GI] = {.type = NLA_FLAG}, [NL80211_RATE_INFO_VHT_NSS] = {.type = NLA_U8},
    };
    ...
    if (stats[NL80211_STA_INFO_RX_BYTES64] && stats[NL80211_STA_INFO_TX_BYTES64])
    {
        /*
    ...
    {
        data->last_ack_rssi = nla_get_u8(stats[NL80211_STA_INFO_ACK_SIGNAL]);
        data->flags |= STA_DRV_DATA_LAST_ACK_RSSI;
    }

    if (stats[NL80211_STA_INFO_CONNECTED_TIME])
    {
        data->connected_sec = nla_get_u32(stats[NL80211_STA_INFO_CONNECTED_TIME]);
        ...
        nla_parse_nested(rate, NL80211_RATE_INFO_MAX, stats[NL80211_STA_INFO_TX_BITRATE], rate_policy) == 0)
    {
        if (rate[NL80211_RATE_INFO_BITRATE32])
            data->current_tx_rate = nla_get_u32(rate[NL80211_RATE_INFO_BITRATE32]);
        else if (rate[NL80211_RATE_INFO_BITRATE])
            data->current_tx_rate = nla_get_u16(rate[NL80211_RATE_INFO_BITRATE]);
        ...
        {
            data->tx_vhtmcs = nla_get_u8(rate[NL80211_RATE_INFO_VHT_MCS]);
        ...
        {
            data->tx_vht_nss = nla_get_u8(rate[NL80211_RATE_INFO_VHT_NSS]);
    ...
    {
        if (rate[NL80211_RATE_INFO_BITRATE32])
            data->current_rx_rate = nla_get_u32(rate[NL80211_RATE_INFO_BITRATE32]);
        else if (rate[NL80211_RATE_INFO_BITRATE])
            data->current_rx_rate = nla_get_u16(rate[NL80211_RATE_INFO_BITRATE]);
        ...
        if (rate[NL80211_RATE_INFO_VHT_MCS])
        {
            data->rx_vhtmcs = nla_get_u8(rate[NL80211_RATE_INFO_VHT_MCS]);
        ...
        if (rate[NL80211_RATE_INFO_VHT_NSS])
        {
            data->rx_vht_nss = nla_get_u8(rate[NL80211_RATE_INFO_VHT_NSS]);
...
static int i802_read_sta_data(struct i802_bss *bss, struct hostap_sta_driver_data *data, const u8 *addr)
{
    ...
    return send_and_recv_msgs(bss->drv, msg, get_sta_handler, data, NULL, NULL);
}
...
static int i802_set_tx_queue_params(void *priv, int queue, int aifs, int cw_min, int cw_max, int burst_time)
{
    ...
    if (nla_put_u16(msg, NL80211_TXQ_ATTR_TXOP, (burst_time * 100 + 16) / 32) ||
        nla_put_u16(msg, NL80211_TXQ_ATTR_CWMIN, cw_min) || nla_put_u16(msg, NL80211_TXQ_ATTR_CWMAX, cw_max) ||
    ...
    wpa_printf(MSG_DEBUG, "nl80211: TX queue param set: queue=%d aifs=%d cw_min=%d cw_max=%d burst_time=%d --> res=%d",
...
static int i802_set_sta_vlan(struct i802_bss *bss, const u8 *addr, const char *ifname, int vlan_id)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: %s[%d]: set_sta_vlan(" MACSTR ", ifname=%s[%d], vlan_id=%d)", bss->ifname,
               if_nametoindex(bss->ifname), MAC2STR(addr), ifname, if_nametoindex(ifname), vlan_id);
    if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_SET_STATION)) || nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, addr) ||
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_ERROR, "nl80211: NL80211_ATTR_STA_VLAN (addr=" MACSTR " ifname=%s vlan_id=%d) failed: %d (%s)",
...
static int i802_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr, u16 reason)
{
    ...
    return wpa_driver_nl80211_send_mlme(bss, (u8 *)&mgmt, IEEE80211_HDRLEN + sizeof(mgmt.u.deauth), 0, 0, 0, 0, 0, NULL,
                                        0, 0);
}
...
static void dump_ifidx(struct wpa_driver_nl80211_data *drv)
{
    ...
    for (i = 0; i < drv->num_if_indices; i++)
    {
        if (!drv->if_indices[i].ifindex)
            continue;
        res = os_snprintf(pos, end - pos, " %d(%d)", drv->if_indices[i].ifindex, drv->if_indices[i].reason);
...
static void add_ifidx(struct wpa_driver_nl80211_data *drv, int ifidx, int ifidx_reason)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Add own interface ifindex %d (ifidx_reason %d)", ifidx, ifidx_reason);
    ...
    drv->if_indices = os_realloc_array(old, drv->num_if_indices + 1, sizeof(*old));
    if (!drv->if_indices)
    {
        if (!old)
            drv->if_indices = drv->default_if_indices;
...
static void del_ifidx(struct wpa_driver_nl80211_data *drv, int ifidx, int ifidx_reason)
{
    ...
    for (i = 0; i < drv->num_if_indices; i++)
    {
        if ((drv->if_indices[i].ifindex == ifidx || ifidx == IFIDX_ANY) &&
            (drv->if_indices[i].reason == ifidx_reason || ifidx_reason == IFIDX_ANY))
        {
...
static int i802_set_wds_sta(void *priv, const u8 *addr, int aid, int val, const char *bridge_ifname, char *ifname_wds)
{
    ...
    if (ret >= (int)sizeof(name))
        wpa_printf(MSG_WARNING, "nl80211: WDS interface name was truncated");
    else if (ret < 0)
        return ret;
    ...
    if (val)
    {
        if (!if_nametoindex(name))
        {
            ...
            wpa_supplicant_event(bss->ctx, EVENT_WDS_STA_INTERFACE_STATUS, &event);
        }
    ...
    {
        if (bridge_ifname && linux_br_del_if(drv->global->ioctl_sock, bridge_ifname, name) < 0)
            wpa_printf(MSG_INFO, "nl80211: Failed to remove interface %s from bridge %s: %s", name, bridge_ifname,
                       strerror(errno));
                             ...
                             const char *ifname)
{
    ...
    {
        wpa_printf(MSG_WARNING, "nl80211: Failed to add interface %s into bridge %s: %s", ifname, brname,
                   strerror(errno));
...
static void *i802_init(struct hostapd_data *hapd, struct wpa_init_params *params)
{
    ...
    bss =
        wpa_driver_nl80211_drv_init(hapd, params->ifname, params->global_priv, 1, params->bssid, params->driver_params);
    if (bss == NULL)
        return NULL;
    ...
    {
        wpa_printf(MSG_DEBUG, "nl80211: Interface %s is in bridge %s", params->ifname, master_ifname);
        br_ifindex = if_nametoindex(master_ifname);
        os_strlcpy(bss->brname, master_ifname, IFNAMSIZ);
    }
    else if ((params->num_bridge == 0 || !params->bridge[0]) && linux_master_get(master_ifname, params->ifname) == 0)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Interface %s is in master %s", params->ifname, master_ifname);
    ...
    if (bss->added_if_into_bridge || bss->already_in_bridge)
    {
        ...
        if (err)
        {
            wpa_printf(MSG_ERROR, "nl80211: Failed to connect nl_sock to NETLINK_ROUTE: %s", nl_geterror(err));
            goto failed;
        }
    }
#endif /* CONFIG_LIBNL3_ROUTE */
    ...
    {
        wpa_printf(MSG_DEBUG, "nl80211: Do not open EAPOL RX socket - using control port for RX");
                                     ...
                                     int setup_ap)
{
    ...
    if (nlmode == NL80211_IFTYPE_P2P_DEVICE)
    {
        ...
        wpa_printf(MSG_DEBUG, "nl80211: New P2P Device interface %s (0x%llx) created", ifname,
    ...
    if (!addr && (type == WPA_IF_P2P_CLIENT || type == WPA_IF_P2P_GROUP || type == WPA_IF_P2P_GO ||
                  type == WPA_IF_MESH || type == WPA_IF_STATION))
    {
     ...
     * the parent interface. This is used mainly with hostapd.
     */
    if (ifidx > 0 && (drv->hostapd || nlmode == NL80211_IFTYPE_AP_VLAN || nlmode == NL80211_IFTYPE_WDS ||
...
static int wpa_driver_nl80211_if_remove(struct i802_bss *bss, enum wpa_driver_if_type type, const char *ifname)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: %s(type=%d ifname=%s) ifindex=%d added_if=%d", __func__, type, ifname, ifindex,
               bss->added_if);
                                  ...
                                  size_t csa_offs_len)
{
    ...
    if (!(msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_FRAME)) ||
        ...
        (offchanok && ((drv->capa.flags & WPA_DRIVER_FLAGS_OFFCHANNEL_TX) || drv->test_use_roc_tx) &&
    ...
    else
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "nl80211: Drop oldest pending send frame cookie 0x%llx",
                       (long long unsigned int)drv->send_frame_cookies[0]);
            os_memmove(&drv->send_frame_cookies[0], &drv->send_frame_cookies[1],
                       (MAX_SEND_FRAME_COOKIES - 1) * sizeof(u64));
                                          ...
                                          int no_cck)
{
    ...
    if (is_ap_interface(drv->nlmode) && (int)freq == bss->freq && bss->beacon_set)
        offchanok = 0;
    ...
    {
        wpa_printf(MSG_DEBUG, "nl80211: Use random TA " MACSTR, MAC2STR(src));
    ...
    if (is_mesh_interface(drv->nlmode))
    {
        ...
        modes = nl80211_get_hw_feature_data(bss, &num_modes, &flags, &dfs_domain);
        if (dfs_domain != HOSTAPD_DFS_REGION_ETSI && ieee80211_is_dfs(bss->freq, modes, num_modes))
            offchanok = 0;
        if (modes)
        {
    ...
    if (is_ap_interface(drv->nlmode) && (!(drv->capa.flags & WPA_DRIVER_FLAGS_OFFCHANNEL_TX) ||
                                         (int)freq == bss->freq || drv->device_ap_sme || !drv->use_monitor))
        ret = wpa_driver_nl80211_send_mlme(bss, buf, 24 + data_len, 0, freq, no_cck, offchanok, wait_time, NULL, 0, 0);
    else
        ret = nl80211_send_frame_cmd(bss, freq, wait_time, buf, 24 + data_len, 1, no_cck, 0, offchanok, NULL, 0);
...
static void nl80211_frame_wait_cancel(struct i802_bss *bss, u64 cookie)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Cancel TX frame wait: cookie=0x%llx", (long long unsigned int)cookie);
...
static int wpa_driver_nl80211_remain_on_channel(void *priv, unsigned int freq, unsigned int duration)
{
    ...
    if (!(msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_REMAIN_ON_CHANNEL)) ||
        nla_put_u32(msg, NL80211_ATTR_WIPHY_FREQ, freq) || nla_put_u32(msg, NL80211_ATTR_DURATION, duration))
    {
        nlmsg_free(msg);
        return -1;
    }
...
static int wpa_driver_nl80211_probe_req_report(struct i802_bss *bss, int report)
{
    ...
    if (nl80211_register_frame(bss, bss->nl_preq, (WLAN_FC_TYPE_MGMT << 2) | (WLAN_FC_STYPE_PROBE_REQ << 4), NULL, 0,
                               false) < 0)
        goto out_err;

    nl80211_register_eloop_read(&bss->nl_preq, wpa_driver_nl80211_event_receive, bss->nl_cb, 0);
...
static int nl80211_disable_11b_rates(struct wpa_driver_nl80211_data *drv, int ifindex, int disabled)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: NL80211_CMD_SET_TX_BITRATE_MASK (ifindex=%d %s)", ifindex,
               disabled ? "NL80211_TXRATE_LEGACY=OFDM-only" : "no NL80211_TXRATE_LEGACY constraint");
...
static int nl80211_signal_monitor(void *priv, int threshold, int hysteresis)
{
    ...
    if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_SET_CQM)) || !(cqm = nla_nest_start(msg, NL80211_ATTR_CQM)) ||
...
static int nl80211_signal_poll(void *priv, struct wpa_signal_info *si)
{
    ...
    if (res)
    {
        if (drv->nlmode != NL80211_IFTYPE_ADHOC && drv->nlmode != NL80211_IFTYPE_MESH_POINT)
            return res;
        si->current_signal = 0;
    }
...
static int nl80211_set_param(void *priv, const char *param)
{
    ...
    if (os_strstr(param, "control_port=0"))
    {
        ...
        drv->capa.flags2 &= ~(WPA_DRIVER_FLAGS2_CONTROL_PORT_RX | WPA_DRIVER_FLAGS2_CONTROL_PORT_TX_STATUS);
    ...
    if (os_strstr(param, "no_rrm=1"))
    {
        ...
        if (!bss->in_deinit && !is_ap_interface(drv->nlmode) && !is_mesh_interface(drv->nlmode))
        {
            nl80211_mgmt_unsubscribe(bss, "no_rrm=1");
            if (nl80211_mgmt_subscribe_non_ap(bss) < 0)
                wpa_printf(MSG_DEBUG, "nl80211: Failed to re-register Action frame processing - ignore for now");
        }
    }
...
static int nl80211_pmkid(struct i802_bss *bss, int cmd, struct wpa_pmkid_params *params)
{
    struct nl_msg *msg;
    const size_t PMK_MAX_LEN = 48; /* current cfg80211 limit */

    if (!(msg = nl80211_bss_msg(bss, 0, cmd)) ||
        (params->pmkid && nla_put(msg, NL80211_ATTR_PMKID, 16, params->pmkid)) ||
        (params->bssid && nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, params->bssid)) ||
        (params->ssid_len && nla_put(msg, NL80211_ATTR_SSID, params->ssid_len, params->ssid)) ||
        (params->fils_cache_id && nla_put(msg, NL80211_ATTR_FILS_CACHE_ID, 2, params->fils_cache_id)) ||
        (params->pmk_lifetime && nla_put_u32(msg, NL80211_ATTR_PMK_LIFETIME, params->pmk_lifetime)) ||
        (params->pmk_reauth_threshold &&
         nla_put_u8(msg, NL80211_ATTR_PMK_REAUTH_THRESHOLD, params->pmk_reauth_threshold)) ||
        (cmd != NL80211_CMD_DEL_PMKSA && params->pmk_len && params->pmk_len <= PMK_MAX_LEN &&
...
static int nl80211_add_pmkid(void *priv, struct wpa_pmkid_params *params)
{
    ...
    if (params->bssid)
        wpa_printf(MSG_DEBUG, "nl80211: Add PMKID for " MACSTR, MAC2STR(params->bssid));
    else if (params->fils_cache_id && params->ssid_len)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Add PMKSA for cache id %02x%02x SSID %s", params->fils_cache_id[0],
                   params->fils_cache_id[1], wpa_ssid_txt(params->ssid, params->ssid_len));
    }

    ret = nl80211_pmkid(bss, NL80211_CMD_SET_PMKSA, params);
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG, "nl80211: NL80211_CMD_SET_PMKSA failed: %d (%s)", ret, strerror(-ret));
    }
...
static int nl80211_remove_pmkid(void *priv, struct wpa_pmkid_params *params)
{
    ...
    if (params->bssid)
        wpa_printf(MSG_DEBUG, "nl80211: Delete PMKID for " MACSTR, MAC2STR(params->bssid));
    else if (params->fils_cache_id && params->ssid_len)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Delete PMKSA for cache id %02x%02x SSID %s", params->fils_cache_id[0],
                   params->fils_cache_id[1], wpa_ssid_txt(params->ssid, params->ssid_len));
    }
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG, "nl80211: NL80211_CMD_DEL_PMKSA failed: %d (%s)", ret, strerror(-ret));
    }
...
static void add_survey(struct nlattr **sinfo, u32 ifidx, struct dl_list *survey_list)
{
    ...
    wpa_printf(MSG_DEBUG,
               "nl80211: Freq survey dump event (freq=%d MHz noise=%d channel_time=%ld busy_time=%ld tx_time=%ld "
...
static int wpa_driver_nl80211_get_survey(void *priv, unsigned int freq)
{
    ...
    do
    {
        ...
        err = send_and_recv_msgs(drv, msg, survey_handler, survey_results, NULL, NULL);
    } while (err > 0);
...
static void nl80211_set_rekey_info(
    void *priv, const u8 *kek, size_t kek_len, const u8 *kck, size_t kck_len, const u8 *replay_ctr)
{
    ...
    if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_SET_REKEY_OFFLOAD)) ||
        ...
        nla_put(msg, NL80211_REKEY_DATA_REPLAY_CTR, NL80211_REPLAY_CTR_LEN, replay_ctr))
    {
        nl80211_nlmsg_clear(msg);
        nlmsg_free(msg);
        return;
...
static void nl80211_send_null_frame(struct i802_bss *bss, const u8 *own_addr, const u8 *addr, int qos)
{
    ...
    if (wpa_driver_nl80211_send_mlme(bss, (u8 *)&nulldata, size, 0, 0, 0, 0, 0, NULL, 0, 0) < 0)
        wpa_printf(MSG_DEBUG,
...
static void nl80211_poll_client(void *priv, const u8 *own_addr, const u8 *addr, int qos)
{
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Client probe request for " MACSTR " failed: ret=%d (%s)", MAC2STR(addr), ret,
                   strerror(-ret));
    }
    else
    {
        wpa_printf(MSG_DEBUG, "nl80211: Client probe request addr=" MACSTR " cookie=%llu", MAC2STR(addr),
...
static int nl80211_set_power_save(struct i802_bss *bss, int enabled)
{
    ...
    if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_SET_POWER_SAVE)) ||
        nla_put_u32(msg, NL80211_ATTR_PS_STATE, enabled ? NL80211_PS_ENABLED : NL80211_PS_DISABLED))
    {
        nlmsg_free(msg);
        return -ENOBUFS;
    }
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Setting PS state %s failed: %d (%s)", enabled ? "enabled" : "disabled", ret,
                   strerror(-ret));
    }
    return ret;
}
...
static int nl80211_start_radar_detection(void *priv, struct hostapd_freq_params *freq)
{
    ...
    wpa_printf(MSG_DEBUG,
               "nl80211: Start radar detection (CAC) %d MHz (ht_enabled=%d, vht_enabled=%d, he_enabled=%d, "
...
static int nl80211_add_peer_capab(struct nl_msg *msg, enum tdls_peer_capability capa)
{
    u32 peer_capab = 0;
    ...
    return nla_put_u32(msg, NL80211_ATTR_TDLS_PEER_CAPABILITY, peer_capab);
}

static int nl80211_send_tdls_mgmt(void *priv,
                                  const u8 *dst,
                                  u8 action_code,
                                  u8 dialog_token,
                                  u16 status_code,
                                  ...
                                  size_t len)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_TDLS_MGMT)) || nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, dst) ||
        nla_put_u8(msg, NL80211_ATTR_TDLS_ACTION, action_code) ||
        nla_put_u8(msg, NL80211_ATTR_TDLS_DIALOG_TOKEN, dialog_token) ||
        nla_put_u16(msg, NL80211_ATTR_STATUS_CODE, status_code) || nl80211_add_peer_capab(msg, peer_capab) ||
        (initiator && nla_put_flag(msg, NL80211_ATTR_TDLS_INITIATOR)) || nla_put(msg, NL80211_ATTR_IE, len, buf))
        goto fail;
...
static int nl80211_tdls_oper(void *priv, enum tdls_oper oper, const u8 *peer)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_TDLS_OPER)) ||
        nla_put_u8(msg, NL80211_ATTR_TDLS_OPERATION, nl80211_oper) || nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, peer))
    {
        nlmsg_free(msg);
        return -ENOBUFS;
    }
    ...
    res = send_and_recv_msgs(drv, msg, NULL, NULL, NULL, NULL);
    wpa_printf(MSG_DEBUG, "nl80211: TDLS_OPER: oper=%d mac=" MACSTR " --> res=%d (%s)", nl80211_oper, MAC2STR(peer),
               res, strerror(-res));
    return res;
}

static int nl80211_tdls_enable_channel_switch(void *priv,
                                              const u8 *addr,
                                              u8 oper_class,
                                              const struct hostapd_freq_params *params)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Enable TDLS channel switch " MACSTR " oper_class=%u freq=%u", MAC2STR(addr),
               oper_class, params->freq);
    msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_TDLS_CHANNEL_SWITCH);
    if (!msg || nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, addr) ||
        nla_put_u8(msg, NL80211_ATTR_OPER_CLASS, oper_class) || (ret = nl80211_put_freq_params(msg, params)))
    {
        nlmsg_free(msg);
...
static int driver_nl80211_set_key(void *priv, struct wpa_driver_set_key_params *params)
{
    struct i802_bss *bss = priv;
...
static int driver_nl80211_deauthenticate(void *priv, const u8 *addr, u16 reason_code)
{
    struct i802_bss *bss = priv;
...
static int driver_nl80211_send_mlme(void *priv,
                                    const u8 *data,
                                    size_t data_len,
                                    int noack,
                                    unsigned int freq,
...
{
    struct i802_bss *bss = priv;
    return wpa_driver_nl80211_send_mlme(bss, data, data_len, noack, freq, 0, 0, wait, csa_offs, csa_offs_len,
                                        no_encrypt);
}
...
static int driver_nl80211_send_action(void *priv,
                                      unsigned int freq,
                                      unsigned int wait_time,
                                      const u8 *dst,
                                      const u8 *src,
...
static int wpa_driver_nl80211_update_ft_ies(void *priv, const u8 *md, const u8 *ies, size_t ies_len)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_UPDATE_FT_IES)) || nla_put(msg, NL80211_ATTR_IE, ies_len, ies) ||
...
static int nl80211_update_dh_ie(void *priv, const u8 *peer_mac, u16 reason_code, const u8 *ie, size_t ie_len)
{
    int ret;
    struct nl_msg *msg;
    struct i802_bss *bss                = priv;
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Updating DH IE peer: " MACSTR " reason %u", MAC2STR(peer_mac), reason_code);
    if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_UPDATE_OWE_INFO)) ||
        nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, peer_mac) || nla_put_u16(msg, NL80211_ATTR_STATUS_CODE, reason_code) ||
    ...
    if (ret)
    {
        wpa_printf(MSG_DEBUG, "nl80211: update_dh_ie failed err=%d (%s)", ret, strerror(-ret));
    }
...
static int wpa_driver_nl80211_status(void *priv, char *buf, size_t buflen)
{
    ...
    res = os_snprintf(
        ...
        "freq=%d\n"
        "%s%s%s%s%s%s",
        bss->ifindex, bss->ifname, bss->brname, MAC2STR(bss->addr), bss->freq, bss->beacon_set ? "beacon_set=1\n" : "",
    ...
    res = os_snprintf(
        ...
        drv->phyname, MAC2STR(drv->perm_addr), drv->ifindex, drv->operstate, scan_state_str(drv->scan_state),
        MAC2STR(drv->auth_bssid), MAC2STR(drv->auth_attempt_bssid), MAC2STR(drv->bssid), MAC2STR(drv->prev_bssid),
        drv->associated, drv->assoc_freq, drv->monitor_sock, drv->monitor_ifidx, drv->monitor_refcount,
        drv->last_mgmt_freq, drv->eapol_tx_sock, drv->ignore_if_down_event ? "ignore_if_down_event=1\n" : "",
    ...
    if (drv->has_capability)
    {
        res = os_snprintf(pos, end - pos,
                          ...
                          drv->capa.key_mgmt, drv->capa.enc, drv->capa.auth, (unsigned long long)drv->capa.flags,
                          drv->capa.rrm_flags, drv->capa.max_scan_ssids, drv->capa.max_sched_scan_ssids,
                          drv->capa.sched_scan_supported, drv->capa.max_match_sets, drv->capa.max_remain_on_chan,
                          drv->capa.max_stations, drv->capa.probe_resp_offloads, drv->capa.max_acl_mac_addrs,
                          drv->capa.num_multichan_concurrent, drv->capa.mac_addr_rand_sched_scan_supported,
                          drv->capa.mac_addr_rand_scan_supported, drv->capa.conc_capab, drv->capa.max_conc_chan_2_4,
                          drv->capa.max_conc_chan_5_0, drv->capa.max_sched_scan_plans,
                          drv->capa.max_sched_scan_plan_interval, drv->capa.max_sched_scan_plan_iterations);
        if (os_snprintf_error(end - pos, res))
            return pos - buf;
        pos += res;
    }
    ...
    msg = nlmsg_alloc();
    if (msg && nl80211_cmd(drv, msg, 0, NL80211_CMD_GET_REG) &&
        nla_put_u32(msg, NL80211_ATTR_WIPHY, drv->wiphy_idx) == 0)
    {
        if (send_and_recv_msgs(drv, msg, nl80211_get_country, alpha2, NULL, NULL) == 0 && alpha2[0])
        {
...
{
    if ((settings->head && nla_put(msg, NL80211_ATTR_BEACON_HEAD, settings->head_len, settings->head)) ||
        (settings->tail && nla_put(msg, NL80211_ATTR_BEACON_TAIL, settings->tail_len, settings->tail)) ||
        (settings->beacon_ies && nla_put(msg, NL80211_ATTR_IE, settings->beacon_ies_len, settings->beacon_ies)) ||
        (settings->proberesp_ies &&
         nla_put(msg, NL80211_ATTR_IE_PROBE_RESP, settings->proberesp_ies_len, settings->proberesp_ies)) ||
        (settings->assocresp_ies &&
         nla_put(msg, NL80211_ATTR_IE_ASSOC_RESP, settings->assocresp_ies_len, settings->assocresp_ies)) ||
...
static int nl80211_switch_channel(void *priv, struct csa_settings *settings)
{
    ...
    wpa_printf(MSG_DEBUG,
               "nl80211: Channel switch request (cs_count=%u block_tx=%u freq=%d channel=%d sec_channel_offset=%d "
               "width=%d cf1=%d cf2=%d%s%s%s)",
               settings->cs_count, settings->block_tx, settings->freq_params.freq, settings->freq_params.channel,
               settings->freq_params.sec_channel_offset, settings->freq_params.bandwidth,
               settings->freq_params.center_freq1, settings->freq_params.center_freq2,
               settings->freq_params.ht_enabled ? " ht" : "", settings->freq_params.vht_enabled ? " vht" : "",
    ...
    if (nla_put(msg, NL80211_ATTR_CSA_C_OFF_BEACON, csa_off_len * sizeof(u16), settings->counter_offset_beacon) ||
        (settings->beacon_csa.probe_resp &&
         nla_put(msg, NL80211_ATTR_CSA_C_OFF_PRESP, csa_off_len * sizeof(u16), settings->counter_offset_presp)))
        goto fail;
...
static int nl80211_add_ts(void *priv, u8 tsid, const u8 *addr, u8 user_priority, u16 admitted_time)
{
    ...
    if (!msg || nla_put_u8(msg, NL80211_ATTR_TSID, tsid) || nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, addr) ||
...
static int nl80211_del_ts(void *priv, u8 tsid, const u8 *addr)
{
    ...
    if (!(msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_DEL_TX_TS)) || nla_put_u8(msg, NL80211_ATTR_TSID, tsid) ||
...
static bool is_cmd_with_nested_attrs(unsigned int vendor_id, unsigned int subcmd)
{
...
static int nl80211_vendor_cmd(void *priv,
                              ...
                              struct wpabuf *buf)
{
    ...
    if (vendor_id == 0xffffffff)
    {
        ...
        ret = send_and_recv_msgs_owner(drv, msg, get_connect_handle(bss), 0, cmd_reply_handler, buf, NULL, NULL);
        if (ret)
    ...
    else if (nested_attr == NESTED_ATTR_UNSPECIFIED && is_cmd_with_nested_attrs(vendor_id, subcmd))
        nla_flag = NLA_F_NESTED;
    else
        nla_flag = 0;

    if (!(msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, vendor_id) ||
...
static int nl80211_get_wowlan(void *priv)
{
    ...
    ret = send_and_recv_msgs(drv, msg, get_wowlan_handler, &wowlan_enabled, NULL, NULL);
    if (ret)
    {
    ...
    wpa_printf(MSG_DEBUG, "nl80211: wowlan is %s", wowlan_enabled ? "enabled" : "disabled");
...
static int nl80211_set_wowlan(void *priv, const struct wowlan_triggers *triggers)
{
    ...
    if (!(msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_SET_WOWLAN)) ||
        ...
        (triggers->disconnect && nla_put_flag(msg, NL80211_WOWLAN_TRIG_DISCONNECT)) ||
        (triggers->magic_pkt && nla_put_flag(msg, NL80211_WOWLAN_TRIG_MAGIC_PKT)) ||
        (triggers->gtk_rekey_failure && nla_put_flag(msg, NL80211_WOWLAN_TRIG_GTK_REKEY_FAILURE)) ||
        (triggers->eap_identity_req && nla_put_flag(msg, NL80211_WOWLAN_TRIG_EAP_IDENT_REQUEST)) ||
        (triggers->four_way_handshake && nla_put_flag(msg, NL80211_WOWLAN_TRIG_4WAY_HANDSHAKE)) ||
        (triggers->rfkill_release && nla_put_flag(msg, NL80211_WOWLAN_TRIG_RFKILL_RELEASE)))
    {
        nlmsg_free(msg);
        return -ENOBUFS;
    }
...
static int nl80211_roaming(void *priv, int allowed, const u8 *bssid)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_ROAMING) ||
        !(params = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) ||
        nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_ROAMING_POLICY,
                    allowed ? QCA_ROAMING_ALLOWED_WITHIN_ESS : QCA_ROAMING_NOT_ALLOWED) ||
...
static int nl80211_disable_fils(void *priv, int disable)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_SET_WIFI_CONFIGURATION) ||
        !(params = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) ||
        nla_put_u8(msg, QCA_WLAN_VENDOR_ATTR_CONFIG_DISABLE_FILS, disable))
    {
        nlmsg_free(msg);
        return -1;
    }
...
static int nl80211_set_bssid_tmp_disallow(void *priv, unsigned int num_bssid, const u8 *bssid)
{
    struct i802_bss *bss                = priv;
    struct wpa_driver_nl80211_data *drv = bss->drv;
    struct nl_msg *msg;
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Set temporarily disallowed BSSIDs (num=%u)", num_bssid);
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        ...
        !(params = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) ||
        nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_ROAMING_SUBCMD, QCA_WLAN_VENDOR_ROAMING_SUBCMD_SET_BLACKLIST_BSSID) ||
        nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_ROAMING_REQ_ID, WPA_SUPPLICANT_CLIENT_ID) ||
        nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_ROAMING_PARAM_SET_BSSID_PARAMS_NUM_BSSID, num_bssid))
        goto fail;

    nlbssids = nla_nest_start(msg, QCA_WLAN_VENDOR_ATTR_ROAMING_PARAM_SET_BSSID_PARAMS);
    if (!nlbssids)
        goto fail;
    ...
    for (i = 0; i < num_bssid; i++)
    {
        ...
        if (!attr)
            goto fail;
        if (nla_put(msg, QCA_WLAN_VENDOR_ATTR_ROAMING_PARAM_SET_BSSID_PARAMS_BSSID, ETH_ALEN, &bssid[i * ETH_ALEN]))
            goto fail;
        wpa_printf(MSG_DEBUG, "nl80211:   BSSID[%u]: " MACSTR, i, MAC2STR(&bssid[i * ETH_ALEN]));
        nla_nest_end(msg, attr);
    }
...
static int nl80211_add_sta_node(void *priv, const u8 *addr, u16 auth_alg)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_ADD_STA_NODE) ||
        !(params = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) ||
        (addr && nla_put(msg, QCA_WLAN_VENDOR_ATTR_ADD_STA_NODE_MAC_ADDR, ETH_ALEN, addr)) ||
        nla_put_u16(msg, QCA_WLAN_VENDOR_ATTR_ADD_STA_NODE_AUTH_ALGO, auth_alg))
    {
        nlmsg_free(msg);
        wpa_printf(MSG_ERROR, "%s: err in adding vendor_cmd and vendor_data", __func__);
        return -1;
    }
    nla_nest_end(msg, params);
...
static int nl80211_put_mesh_id(struct nl_msg *msg, const u8 *mesh_id, size_t mesh_id_len)
{
    if (mesh_id)
    {
        wpa_printf(MSG_DEBUG, "  * Mesh ID (SSID)=%s", wpa_ssid_txt(mesh_id, mesh_id_len));
...
static int nl80211_put_mesh_config(struct nl_msg *msg, struct wpa_driver_mesh_bss_params *params)
{
    ...
    if (((params->flags & WPA_DRIVER_MESH_CONF_FLAG_AUTO_PLINKS) &&
         nla_put_u8(msg, NL80211_MESHCONF_AUTO_OPEN_PLINKS, params->auto_plinks)) ||
        ((params->flags & WPA_DRIVER_MESH_CONF_FLAG_FORWARDING) &&
         nla_put_u8(msg, NL80211_MESHCONF_FORWARDING, params->forwarding)) ||
        ((params->flags & WPA_DRIVER_MESH_CONF_FLAG_MAX_PEER_LINKS) &&
         nla_put_u16(msg, NL80211_MESHCONF_MAX_PEER_LINKS, params->max_peer_links)) ||
        ((params->flags & WPA_DRIVER_MESH_CONF_FLAG_RSSI_THRESHOLD) &&
         nla_put_u32(msg, NL80211_MESHCONF_RSSI_THRESHOLD, params->rssi_threshold)))
        return -1;
    ...
    if ((params->flags & WPA_DRIVER_MESH_CONF_FLAG_PEER_LINK_TIMEOUT) &&
        nla_put_u32(msg, NL80211_MESHCONF_PLINK_TIMEOUT, params->peer_link_timeout))
    {
...
static int nl80211_join_mesh(struct i802_bss *bss, struct wpa_driver_mesh_join_params *params)
{
    ...
    if (!msg || nl80211_put_freq_params(msg, &params->freq) || nl80211_put_basic_rates(msg, params->basic_rates) ||
        nl80211_put_mesh_id(msg, params->meshid, params->meshid_len) ||
        nl80211_put_beacon_int(msg, params->beacon_int) || nl80211_put_dtim_period(msg, params->dtim_period))
        goto fail;
...
static int wpa_driver_nl80211_leave_mesh(void *priv)
{
    ...
    if (drv->start_mode_sta && wpa_driver_nl80211_set_mode(drv->first_bss, NL80211_IFTYPE_STATION))
    {
...
static int nl80211_probe_mesh_link(void *priv, const u8 *addr, const u8 *eth, size_t len)
{
    struct i802_bss *bss                = priv;
    struct wpa_driver_nl80211_data *drv = bss->drv;
    struct nl_msg *msg;
    ...
    if (!msg || nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, addr) || nla_put(msg, NL80211_ATTR_FRAME, len, eth))
    {
        nlmsg_free(msg);
        return -ENOBUFS;
    }
    ...
    if (ret)
    {
        wpa_printf(MSG_DEBUG, "nl80211: mesh link probe to " MACSTR " failed: ret=%d (%s)", MAC2STR(addr), ret,
                   strerror(-ret));
    }
    else
    {
        wpa_printf(MSG_DEBUG, "nl80211: Mesh link to " MACSTR " probed successfully", MAC2STR(addr));
    }
...
static int wpa_driver_br_add_ip_neigh(void *priv, u8 version, const u8 *ipaddr, int prefixlen, const u8 *addr)
{
    ...
    if (res)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Adding bridge ip neigh failed: %s", nl_geterror(res));
    }
errout:
    if (nl_lladdr)
...
static int wpa_driver_br_delete_ip_neigh(void *priv, u8 version, const u8 *ipaddr)
{
    ...
    if (res)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Deleting bridge ip neigh failed: %s", nl_geterror(res));
    }
errout:
    if (nl_ipaddr)
        nl_addr_put(nl_ipaddr);
...
static int add_acs_ch_list(struct nl_msg *msg, const int *freq_list)
{
    ...
    for (i = 0; i < num_freqs; i++)
    {
         ...
         * compatibility.
         */
        if (!(freq >= 2412 && freq <= 2484) && !(freq >= 5180 && freq <= 5900) && !(freq >= 5945 && freq <= 7115))
            continue;
    ...
    if (num_channels)
        ret = nla_put(msg, QCA_WLAN_VENDOR_ATTR_ACS_CH_LIST, num_channels, ch_list);
...
static int add_acs_freq_list(struct nl_msg *msg, const int *freq_list)
{
    ...
    for (i = 0; i < len; i++)
        freqs[i] = freq_list[i];
    ret = nla_put(msg, QCA_WLAN_VENDOR_ATTR_ACS_FREQ_LIST, sizeof(u32) * len, freqs);
    os_free(freqs);
    return ret;
}

static int nl80211_qca_do_acs(struct wpa_driver_nl80211_data *drv, struct drv_acs_params *params)
{
    struct nl_msg *msg;
    struct nlattr *data;
    int ret;
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_DO_ACS) ||
        !(data = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) ||
        nla_put_u8(msg, QCA_WLAN_VENDOR_ATTR_ACS_HW_MODE, mode) ||
        (params->ht_enabled && nla_put_flag(msg, QCA_WLAN_VENDOR_ATTR_ACS_HT_ENABLED)) ||
        (params->ht40_enabled && nla_put_flag(msg, QCA_WLAN_VENDOR_ATTR_ACS_HT40_ENABLED)) ||
        (params->vht_enabled && nla_put_flag(msg, QCA_WLAN_VENDOR_ATTR_ACS_VHT_ENABLED)) ||
        nla_put_u16(msg, QCA_WLAN_VENDOR_ATTR_ACS_CHWIDTH, params->ch_width) ||
        add_acs_ch_list(msg, params->freq_list) || add_acs_freq_list(msg, params->freq_list) ||
        (params->edmg_enabled && nla_put_flag(msg, QCA_WLAN_VENDOR_ATTR_ACS_EDMG_ENABLED)))
    {
        nlmsg_free(msg);
        return -ENOBUFS;
    }
    ...
    wpa_printf(MSG_DEBUG, "nl80211: ACS Params: HW_MODE: %d HT: %d HT40: %d VHT: %d BW: %d EDMG: %d", params->hw_mode,
               params->ht_enabled, params->ht40_enabled, params->vht_enabled, params->ch_width, params->edmg_enabled);

    ret = send_and_recv_msgs(drv, msg, NULL, NULL, NULL, NULL);
    if (ret)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Failed to invoke driver ACS function: %s", strerror(-ret));
    }
    return ret;
}
...
static int nl80211_set_band(void *priv, u32 band_mask)
{
    ...
    if (!drv->setband_vendor_cmd_avail || (band_mask > (WPA_SETBAND_2G | WPA_SETBAND_5G | WPA_SETBAND_6G)))
        return -1;
     ...
     */
    qca_band_value = ((qca_band_mask & QCA_SETBAND_5G) && (qca_band_mask & QCA_SETBAND_2G)) ?
                         QCA_SETBAND_AUTO :
                         qca_band_mask & ~QCA_SETBAND_6G;

    wpa_printf(MSG_DEBUG, "nl80211: QCA_BAND_MASK = 0x%x, QCA_BAND_VALUE = %d", qca_band_mask, qca_band_value);

    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_SETBAND) ||
        !(data = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) ||
        nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_SETBAND_VALUE, qca_band_value) ||
        nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_SETBAND_MASK, qca_band_mask))
    {
        nlmsg_free(msg);
        return -ENOBUFS;
    }
    ...
    if (ret)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Driver setband function failed: %s", strerror(-ret));
    }
    return ret;
}
                                      ...
                                      unsigned int *freq_list)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_IFINDEX, drv->ifindex) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_GET_PREFERRED_FREQ_LIST) ||
        !(params = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) ||
    ...
    ret = send_and_recv_msgs(drv, msg, preferred_freq_info_handler, &param, NULL, NULL);
    if (ret)
    {
    ...
    *num = param.num;
    ...
    for (i = 0; i < *num; i++)
    {
        wpa_printf(MSG_DEBUG, "nl80211: preferred_channel_list[%d]=%d", i, freq_list[i]);
    }
...
static int nl80211_set_prob_oper_freq(void *priv, unsigned int freq)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Set P2P probable operating freq %u for ifindex %d", freq, bss->ifindex);

    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_SET_PROBABLE_OPER_CHANNEL) ||
        !(params = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) ||
        nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_PROBABLE_OPER_CHANNEL_IFACE_TYPE, QCA_IFACE_TYPE_P2P_CLIENT) ||
...
static int nl80211_p2p_lo_start(void *priv,
                                unsigned int freq,
                                unsigned int period,
                                ...
                                size_t ies_len)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Start P2P Listen offload: freq=%u, period=%u, interval=%u, count=%u", freq, period,
               interval, count);
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_P2P_LISTEN_OFFLOAD_START))
        goto fail;
    ...
    if (nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_CHANNEL, freq) ||
        nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_PERIOD, period) ||
        nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_INTERVAL, interval) ||
        nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_COUNT, count) ||
        nla_put(msg, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_DEVICE_TYPES, dev_types_len, device_types) ||
        nla_put(msg, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_VENDOR_IE, ies_len, ies))
        goto fail;
    ...
    if (ret)
    {
        wpa_printf(MSG_DEBUG, "nl80211: Failed to send P2P Listen offload vendor command");
        goto fail;
    }
...
static int nl80211_p2p_lo_stop(void *priv)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_P2P_LISTEN_OFFLOAD_STOP))
    {
        nlmsg_free(msg);
        return -1;
    }
...
static int nl80211_set_tdls_mode(void *priv, int tdls_external_control)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Set TDKS mode: tdls_external_control=%d", tdls_external_control);

    if (tdls_external_control == 1)
        tdls_mode = QCA_WLAN_VENDOR_TDLS_TRIGGER_MODE_IMPLICIT | QCA_WLAN_VENDOR_TDLS_TRIGGER_MODE_EXTERNAL;
    else
        tdls_mode = QCA_WLAN_VENDOR_TDLS_TRIGGER_MODE_EXPLICIT;

    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_CONFIGURE_TDLS))
        goto fail;
    ...
    if (nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_TDLS_CONFIG_TRIGGER_MODE, tdls_mode))
        goto fail;
    ...
    if (ret)
    {
        wpa_printf(MSG_ERROR, "nl80211: Set TDLS mode failed: ret=%d (%s)", ret, strerror(-ret));
        goto fail;
    }
    return 0;
fail:
...
static enum mbo_transition_reject_reason nl80211_mbo_reject_reason_mapping(enum qca_wlan_btm_candidate_status status)
{
    switch (status)
    {
...
static void nl80211_parse_btm_candidate_info(struct candidate_list *candidate, struct nlattr *tb[], int num)
{
    enum qca_wlan_btm_candidate_status status;
    char buf[50];

    os_memcpy(candidate->bssid, nla_data(tb[QCA_WLAN_VENDOR_ATTR_BTM_CANDIDATE_INFO_BSSID]), ETH_ALEN);

    status                   = nla_get_u32(tb[QCA_WLAN_VENDOR_ATTR_BTM_CANDIDATE_INFO_STATUS]);
    ...
    if (candidate->is_accept)
        ...
        os_snprintf(buf, sizeof(buf), "Rejected, Reject_reason: %d", candidate->reject_reason);
    wpa_printf(MSG_DEBUG, "nl80211:   BSSID[%d]: " MACSTR " %s", num, MAC2STR(candidate->bssid), buf);
}

static int nl80211_get_bss_transition_status_handler(struct nl_msg *msg, void *arg)
{
    ...
    static struct nla_policy policy[QCA_WLAN_VENDOR_ATTR_BTM_CANDIDATE_INFO_MAX + 1] = {
        [QCA_WLAN_VENDOR_ATTR_BTM_CANDIDATE_INFO_BSSID] = {.minlen = ETH_ALEN},
    ...
    if (!tb_msg[NL80211_ATTR_VENDOR_DATA] ||
        nla_parse_nested(tb_vendor, QCA_WLAN_VENDOR_ATTR_MAX, tb_msg[NL80211_ATTR_VENDOR_DATA], NULL) ||
        !tb_vendor[QCA_WLAN_VENDOR_ATTR_BTM_CANDIDATE_INFO])
        return NL_SKIP;

    wpa_printf(MSG_DEBUG, "nl80211: WNM Candidate list received from driver");
    nla_for_each_nested(attr, tb_vendor[QCA_WLAN_VENDOR_ATTR_BTM_CANDIDATE_INFO], rem)
    {
        if (info->num >= num || nla_parse_nested(tb, QCA_WLAN_VENDOR_ATTR_BTM_CANDIDATE_INFO_MAX, attr, policy) ||
            !tb[QCA_WLAN_VENDOR_ATTR_BTM_CANDIDATE_INFO_BSSID] || !tb[QCA_WLAN_VENDOR_ATTR_BTM_CANDIDATE_INFO_STATUS])
            break;
...
static struct wpa_bss_candidate_info *nl80211_get_bss_transition_status(void *priv, struct wpa_bss_trans_info *params)
{
    struct i802_bss *bss                = priv;
    struct wpa_driver_nl80211_data *drv = bss->drv;
    struct nl_msg *msg;
    ...
    info->candidates = os_calloc(params->n_candidates, sizeof(*info->candidates));
    if (!info->candidates)
    {
        os_free(info);
        return NULL;
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_FETCH_BSS_TRANSITION_STATUS))
        goto fail;
    ...
    if (nla_put_u8(msg, QCA_WLAN_VENDOR_ATTR_BTM_MBO_TRANSITION_REASON, params->mbo_transition_reason))
        goto fail;
    ...
    {
        wpa_printf(MSG_DEBUG, "nl80211:   BSSID[%d]: " MACSTR, i, MAC2STR(pos));
        attr2 = nla_nest_start(msg, i);
        if (!attr2 || nla_put(msg, QCA_WLAN_VENDOR_ATTR_BTM_CANDIDATE_INFO_BSSID, ETH_ALEN, pos))
            goto fail;
        pos += ETH_ALEN;
    ...
    ret = send_and_recv_msgs(drv, msg, nl80211_get_bss_transition_status_handler, info, NULL, NULL);
    msg = NULL;
    if (ret)
    {
        wpa_printf(MSG_ERROR, "nl80211: WNM Get BSS transition status failed: ret=%d (%s)", ret, strerror(-ret));
        goto fail;
    }
    return info;
...
static int nl80211_ignore_assoc_disallow(void *priv, int ignore_disallow)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Set ignore_assoc_disallow %d", ignore_disallow);
    if (nla_put_u8(msg, QCA_WLAN_VENDOR_ATTR_CONFIG_IGNORE_ASSOC_DISALLOWED, ignore_disallow))
        goto fail;
    ...
    if (ret)
    {
        wpa_printf(MSG_ERROR, "nl80211: Set ignore_assoc_disallow failed: ret=%d (%s)", ret, strerror(-ret));
        goto fail;
    }
...
static int wpa_driver_do_broadcom_acs(struct wpa_driver_nl80211_data *drv, struct drv_acs_params *params)
{
    struct nl_msg *msg;
    struct nlattr *data;
    int freq_list_len;
    ...
    wpa_printf(MSG_DEBUG, "%s: freq_list_len=%d", __func__, freq_list_len);

    msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR);
    if (!msg || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_BRCM) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, BRCM_VENDOR_SCMD_ACS) ||
        !(data = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) ||
        nla_put_u8(msg, BRCM_VENDOR_ATTR_ACS_HW_MODE, params->hw_mode) ||
        nla_put_u8(msg, BRCM_VENDOR_ATTR_ACS_HT_ENABLED, params->ht_enabled) ||
        nla_put_u8(msg, BRCM_VENDOR_ATTR_ACS_HT40_ENABLED, params->ht40_enabled) ||
        nla_put_u8(msg, BRCM_VENDOR_ATTR_ACS_VHT_ENABLED, params->vht_enabled) ||
        nla_put_u16(msg, BRCM_VENDOR_ATTR_ACS_CHWIDTH, params->ch_width) ||
        (freq_list_len > 0 &&
         nla_put(msg, BRCM_VENDOR_ATTR_ACS_FREQ_LIST, sizeof(int) * freq_list_len, params->freq_list)))
        goto fail;
    nla_nest_end(msg, data);

    wpa_printf(MSG_DEBUG, "nl80211: ACS Params: HW_MODE: %d HT: %d HT40: %d VHT: %d BW: %d", params->hw_mode,
               params->ht_enabled, params->ht40_enabled, params->vht_enabled, params->ch_width);

    ret = send_and_recv_msgs(drv, msg, NULL, NULL, NULL, NULL);
    if (ret)
    {
        wpa_printf(MSG_ERROR, "nl80211: BRCM Failed to invoke driver ACS function: %s", strerror(errno));
    }
...
static int nl80211_write_to_file(const char *name, unsigned int val)
{
    ...
    if (fd < 0)
    {
        ...
        wpa_printf(level, "nl80211: Failed to open %s: %s", name, strerror(errno));
        return ret;
    }
    ...
    {
        ret = -1;
        wpa_printf(MSG_ERROR, "nl80211: Failed to write to %s: %s", name, strerror(errno));
    }
    close(fd);
...
static int nl80211_configure_data_frame_filters(void *priv, u32 filter_flags)
{
    ...
    ret = os_snprintf(path, sizeof(path), "/proc/sys/net/ipv4/conf/%s/drop_unicast_in_l2_multicast", bss->ifname);
    ...
    if (ret)
    {
        wpa_printf(MSG_ERROR, "nl80211: Failed to set IPv4 unicast in multicast filter");
        return ret;
    }

    os_snprintf(path, sizeof(path), "/proc/sys/net/ipv6/conf/%s/drop_unicast_in_l2_multicast", bss->ifname);
    ret = nl80211_write_to_file(path, !!(filter_flags & WPA_DATA_FRAME_FILTER_FLAG_GTK));

    if (ret)
    {
        wpa_printf(MSG_ERROR, "nl80211: Failed to set IPv6 unicast in multicast filter");
        return ret;
    }

    /* Configure filtering of unicast frame encrypted using GTK */
    os_snprintf(path, sizeof(path), "/proc/sys/net/ipv4/conf/%s/drop_gratuitous_arp", bss->ifname);
    ret = nl80211_write_to_file(path, !!(filter_flags & WPA_DATA_FRAME_FILTER_FLAG_ARP));
    if (ret)
    {
    ...
    os_snprintf(path, sizeof(path), "/proc/sys/net/ipv6/conf/%s/drop_unsolicited_na", bss->ifname);
    ret = nl80211_write_to_file(path, !!(filter_flags & WPA_DATA_FRAME_FILTER_FLAG_NA));
    if (ret)
    {
...
static int nl80211_get_ext_capab(
...
static int nl80211_update_connection_params(void *priv,
                                            struct wpa_driver_associate_params *params,
                                            enum wpa_drv_update_connect_params_mask mask)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Update connection params (ifindex=%d)", drv->ifindex);
    ...
    if ((mask & WPA_DRV_UPDATE_FILS_ERP_INFO) && nl80211_put_fils_connect_params(drv, params, msg))
        goto fail;

    ret = send_and_recv_msgs(drv, msg, NULL, NULL, NULL, NULL);
    msg = NULL;
    if (ret)
        wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: Update connect params command failed: ret=%d (%s)", ret, strerror(-ret));
...
static int nl80211_send_external_auth_status(void *priv, struct external_auth *params)
{
    struct i802_bss *bss                = priv;
    struct wpa_driver_nl80211_data *drv = bss->drv;
    struct nl_msg *msg                  = NULL;
     ...
     */
    if ((is_ap_interface(drv->nlmode) && !bss->drv->device_ap_sme) || (drv->capa.flags & WPA_DRIVER_FLAGS_SME))
        return -1;

    wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: External auth status: %u", params->status);

    msg = nl80211_drv_msg(drv, 0, NL80211_CMD_EXTERNAL_AUTH);
    if (!msg || nla_put_u16(msg, NL80211_ATTR_STATUS_CODE, params->status) ||
        (params->ssid && params->ssid_len && nla_put(msg, NL80211_ATTR_SSID, params->ssid_len, params->ssid)) ||
        (params->pmkid && nla_put(msg, NL80211_ATTR_PMKID, PMKID_LEN, params->pmkid)) ||
        (params->bssid && nla_put(msg, NL80211_ATTR_BSSID, ETH_ALEN, params->bssid)))
        goto fail;
    ret = send_and_recv_msgs(drv, msg, NULL, NULL, NULL, NULL);
    msg = NULL;
    if (ret)
    {
        wpa_printf(MSG_DEBUG, "nl80211: External Auth status update failed: ret=%d (%s)", ret, strerror(-ret));
        goto fail;
    }
fail:
    nlmsg_free(msg);
...
static int nl80211_set_4addr_mode(void *priv, const char *bridge_ifname, int val)
{
    struct i802_bss *bss                = priv;
    struct wpa_driver_nl80211_data *drv = bss->drv;
    struct nl_msg *msg;
    ...
    wpa_printf(MSG_DEBUG, "nl80211: %s 4addr mode (bridge_ifname: %s)", val ? "Enable" : "Disable", bridge_ifname);
    ...
    {
        if (linux_br_del_if(drv->global->ioctl_sock, bridge_ifname, bss->ifname))
        {
            wpa_printf(MSG_ERROR, "nl80211: Failed to remove interface %s from bridge %s", bss->ifname, bridge_ifname);
            return -1;
        }
    ...
    {
        wpa_printf(MSG_DEBUG, "nl80211: 4addr mode was already enabled");
        ret = 0;
    }
    if (!ret)
    {
        if (bridge_ifname[0] && val && i802_check_bridge(drv, bss, bridge_ifname, bss->ifname) < 0)
            return -1;
        return 0;
    }
...
static int nl80211_dpp_listen(void *priv, bool enable)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Update DPP Public Action frame registration (%s multicast RX)",
               enable ? "enable" : "disable");
    handle = (void *)(((intptr_t)bss->nl_mgmt) ^ ELOOP_SOCKET_INVALID);
    return nl80211_register_frame(bss, handle, type, (u8 *)"\x04\x09\x50\x6f\x9a\x1a", 6, enable);
}
#endif /* CONFIG_DPP */

#ifdef CONFIG_TESTING_OPTIONS

static int testing_nl80211_register_frame(void *priv, u16 type, const u8 *match, size_t match_len, bool multicast)
{
    struct i802_bss *bss = priv;
    struct nl_sock *handle;
    ...
    if (!bss->nl_mgmt)
        return -1;
    handle = (void *)(((intptr_t)bss->nl_mgmt) ^ ELOOP_SOCKET_INVALID);
    return nl80211_register_frame(bss, handle, type, match, match_len, multicast);
}
...
static int testing_nl80211_radio_disable(void *priv, int disabled)
{
    ...
    return wpa_driver_nl80211_mlme(drv, drv->bssid, NL80211_CMD_DEAUTHENTICATE, WLAN_REASON_PREV_AUTH_NOT_VALID, 1,
                                   drv->first_bss);
}
