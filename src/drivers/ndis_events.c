                                                       ...
                                                       IEnumWbemClassObject **ppEnum)
{
    ...
    hr = IWbemServices_ExecQuery(pSvc, bsQueryLanguage, bsQuery, lFlags, pCtx, ppEnum);
                                                                        ...
                                                                        IWbemObjectSink *pResponseHandler)
{
    ...
    hr = IWbemServices_ExecNotificationQueryAsync(pSvc, bsQueryLanguage, bsQuery, lFlags, pCtx, pResponseHandler);
                                                          ...
                                                          IWbemServices **ppNamespace)
{
    ...
    hr = IWbemLocator_ConnectServer(pLoc, bsNetworkResource, bsUser, bsPassword, bsLocale, lSecurityFlags, bsAuthority,
                                    pCtx, ppNamespace);
...
static void ndis_events_media_specific(struct ndis_events_data *events, IWbemClassObject *pObj)
{
    ...
    hr = IWbemClassObject_Get(pObj, L"NdisStatusMediaSpecificIndication", 0, &vt, NULL, NULL);
    if (FAILED(hr))
    {
        wpa_printf(MSG_DEBUG,
                   "Could not get "
                                                      ...
                                                      IWbemClassObject __RPC_FAR *__RPC_FAR *ppObjArray)
{
    ...
    for (i = 0; i < lObjectCount; i++)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "Unexpected event - __CLASS: '%S'", vtClass.bstrVal);
        }
...
static HRESULT STDMETHODCALLTYPE ndis_events_set_status(
    IWbemObjectSink *this, long lFlags, HRESULT hResult, BSTR strParam, IWbemClassObject __RPC_FAR *pObjParam)
{
    return WBEM_NO_ERROR;
}
...
static int notification_query(IWbemObjectSink *pDestSink, IWbemServices *pSvc, const char *class_name)
{
    ...
    hr = call_IWbemServices_ExecNotificationQueryAsync(pSvc, L"WQL", query, 0, 0, pDestSink);
    if (FAILED(hr))
    {
        wpa_printf(MSG_DEBUG,
...
static int register_async_notification(IWbemObjectSink *pDestSink, IWbemServices *pSvc)
{
    ...
    const char *class_list[] = {
        "MSNdis_StatusMediaConnect",   "MSNdis_StatusMediaDisconnect", "MSNdis_StatusMediaSpecificIndication",
        "MSNdis_NotifyAdapterArrival", "MSNdis_NotifyAdapterRemoval",  NULL};
...
#define MAX_QUERY_LEN 256
    ...
    hr = call_IWbemLocator_ConnectServer(events->pLoc, L"ROOT\\CIMV2", NULL, NULL, 0, 0, 0, 0, &pSvc);
    if (FAILED(hr))
    {
    ...
    hr = call_IWbemServices_ExecQuery(pSvc, L"WQL", query, WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, NULL,
                                      &pEnumerator);
    if (!SUCCEEDED(hr))
    {
        wpa_printf(MSG_DEBUG,
    ...
    uReturned = 0;
    hr        = IEnumWbemClassObject_Next(pEnumerator, WBEM_INFINITE, 1, &pObj, &uReturned);
    ...
    wpa_printf(MSG_DEBUG, "ndis_events: Win32_NetworkAdapter::Name='%S'", vt.bstrVal);
    ...
    len = _snwprintf(query, MAX_QUERY_LEN, L"SELECT Name FROM Win32_PnPEntity WHERE DeviceID='");
...
struct ndis_events_data *ndis_events_init(HANDLE *read_pipe, HANDLE *event_avail, const char *ifname, const char *desc)
{
    ...
    wpa_printf(MSG_DEBUG, "ndis_events: use adapter descriptor '%S'", events->adapter_desc);

    hr = call_IWbemLocator_ConnectServer(events->pLoc, L"ROOT\\WMI", NULL, NULL, 0, 0, 0, 0, &events->pSvc);
    if (FAILED(hr))
    {
        wpa_printf(MSG_ERROR,
