...
static u32 get_nl80211_protocol_features(struct wpa_driver_nl80211_data *drv)
{
    ...
    if (send_and_recv_msgs(drv, msg, protocol_feature_handler, &feat, NULL, NULL) == 0)
        return feat;
...
static void get_iface_akm_suites_info(struct wiphy_info_data *info, struct nlattr *nl_akms)
{
    ...
    nla_for_each_nested(nl_iftype, tb[NL80211_IFTYPE_AKM_ATTR_IFTYPES], i)
    {
        switch (nla_type(nl_iftype))
        {
            ...
            case NL80211_IFTYPE_STATION:
                info->drv->capa.key_mgmt_iftype[WPA_IF_STATION] = key_mgmt;
                break;
            case NL80211_IFTYPE_AP:
                info->drv->capa.key_mgmt_iftype[WPA_IF_AP_BSS] = key_mgmt;
                break;
            case NL80211_IFTYPE_AP_VLAN:
                info->drv->capa.key_mgmt_iftype[WPA_IF_AP_VLAN] = key_mgmt;
                break;
            case NL80211_IFTYPE_MESH_POINT:
                info->drv->capa.key_mgmt_iftype[WPA_IF_MESH] = key_mgmt;
                break;
            case NL80211_IFTYPE_P2P_CLIENT:
                info->drv->capa.key_mgmt_iftype[WPA_IF_P2P_CLIENT] = key_mgmt;
                break;
            case NL80211_IFTYPE_P2P_GO:
                info->drv->capa.key_mgmt_iftype[WPA_IF_P2P_GO] = key_mgmt;
                break;
            case NL80211_IFTYPE_P2P_DEVICE:
                info->drv->capa.key_mgmt_iftype[WPA_IF_P2P_DEVICE] = key_mgmt;
                break;
            case NL80211_IFTYPE_NAN:
                info->drv->capa.key_mgmt_iftype[WPA_IF_NAN] = key_mgmt;
                break;
        }
        wpa_printf(MSG_DEBUG, "nl80211: %s supported key_mgmt 0x%x", nl80211_iftype_str(nla_type(nl_iftype)), key_mgmt);
    }
}

static void wiphy_info_iftype_akm_suites(struct wiphy_info_data *info, struct nlattr *tb)
{
    struct nlattr *nl_if;
    int rem_if;
    ...
    nla_for_each_nested(nl_if, tb, rem_if) get_iface_akm_suites_info(info, nl_if);
}

static void wiphy_info_akm_suites(struct wiphy_info_data *info, struct nlattr *tb)
{
    if (!tb)
        return;
    ...
    wpa_printf(MSG_DEBUG, "nl80211: wiphy supported key_mgmt 0x%x", info->capa->key_mgmt);
}
...
static void wiphy_info_cipher_suites(struct wiphy_info_data *info, struct nlattr *tb)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        wpa_printf(MSG_DEBUG, "nl80211: Supported cipher %02x-%02x-%02x:%d", c >> 24, (c >> 16) & 0xff, (c >> 8) & 0xff,
                   c & 0xff);
        switch (c)
        {
...
static void wiphy_info_ext_feature_flags(struct wiphy_info_data *info, struct nlattr *tb)
{
    ...
    if (ext_feature_isset(ext_features, len, NL80211_EXT_FEATURE_CONTROL_PORT_OVER_NL80211_TX_STATUS))
...
static void wiphy_info_extended_capab(struct wpa_driver_nl80211_data *drv, struct nlattr *tb)
{
    int rem = 0, i;
    ...
    nla_for_each_nested(attr, tb, rem)
    {
        ...
        if (!tb1[NL80211_ATTR_IFTYPE] || !tb1[NL80211_ATTR_EXT_CAPA] || !tb1[NL80211_ATTR_EXT_CAPA_MASK])
            continue;

        capa         = &drv->iface_ext_capa[drv->num_iface_ext_capa];
        capa->iftype = nla_get_u32(tb1[NL80211_ATTR_IFTYPE]);
        wpa_printf(MSG_DEBUG, "nl80211: Driver-advertised extended capabilities for interface type %s",
        ...
        wpa_hexdump(MSG_DEBUG, "nl80211: Extended capabilities", capa->ext_capa, capa->ext_capa_len);
        ...
        wpa_hexdump(MSG_DEBUG, "nl80211: Extended capabilities mask", capa->ext_capa_mask, capa->ext_capa_len);
...
static int wiphy_info_handler(struct nl_msg *msg, void *arg)
{
    ...
    {
        u32 ap_sme_features_flags = nla_get_u32(tb[NL80211_ATTR_DEVICE_AP_SME]);
    ...
    if (tb[NL80211_ATTR_EXT_CAPA] && tb[NL80211_ATTR_EXT_CAPA_MASK] && drv->extended_capa == NULL)
    {
        ...
        if (drv->extended_capa)
        {
            ...
            wpa_hexdump(MSG_DEBUG, "nl80211: Driver-advertised extended capabilities (default)", drv->extended_capa,
                        drv->extended_capa_len);
        }
        drv->extended_capa_mask = os_malloc(nla_len(tb[NL80211_ATTR_EXT_CAPA_MASK]));
        if (drv->extended_capa_mask)
        {
            os_memcpy(drv->extended_capa_mask, nla_data(tb[NL80211_ATTR_EXT_CAPA_MASK]),
                      nla_len(tb[NL80211_ATTR_EXT_CAPA_MASK]));
            wpa_hexdump(MSG_DEBUG, "nl80211: Driver-advertised extended capabilities mask (default)",
    ...
    if (tb[NL80211_ATTR_VENDOR_DATA])
    {
        ...
        nla_for_each_nested(nl, tb[NL80211_ATTR_VENDOR_DATA], rem)
        {
            ...
            else if (vinfo->vendor_id == OUI_BRCM)
            {
                switch (vinfo->subcmd)
                {
                    case BRCM_VENDOR_SCMD_ACS:
                        drv->capa.flags |= WPA_DRIVER_FLAGS_ACS_OFFLOAD;
                        wpa_printf(MSG_DEBUG, "Enabled BRCM ACS");
                        drv->brcm_do_acs = 1;
                        break;
                }
            ...
            wpa_printf(MSG_DEBUG, "nl80211: Supported vendor command: vendor_id=0x%x subcmd=%u", vinfo->vendor_id,
                       vinfo->subcmd);
        }
    }
    ...
    if (tb[NL80211_ATTR_VENDOR_EVENTS])
    {
        ...
        nla_for_each_nested(nl, tb[NL80211_ATTR_VENDOR_EVENTS], rem)
        {
            ...
            vinfo = nla_data(nl);
            wpa_printf(MSG_DEBUG, "nl80211: Supported vendor event: vendor_id=0x%x subcmd=%u", vinfo->vendor_id,
                       vinfo->subcmd);
        }
    }
...
static void qca_nl80211_check_dfs_capa(struct wpa_driver_nl80211_data *drv)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_DFS_CAPABILITY))
    {
        nlmsg_free(msg);
        return;
    }

    ret = send_and_recv_msgs(drv, msg, dfs_info_handler, &dfs_capability, NULL, NULL);
...
static void qca_nl80211_get_features(struct wpa_driver_nl80211_data *drv)
{
    ...
    if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
        nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, QCA_NL80211_VENDOR_SUBCMD_GET_FEATURES))
    {
        nlmsg_free(msg);
        return;
    }
    ...
    os_memset(&info, 0, sizeof(info));
    info.capa = &drv->capa;
    ret       = send_and_recv_msgs(drv, msg, features_info_handler, &info, NULL, NULL);
    if (ret || !info.flags)
        return;
...
int wpa_driver_nl80211_capa(struct wpa_driver_nl80211_data *drv)
{
    ...
    if (!drv->has_driver_key_mgmt)
    {
        drv->capa.key_mgmt = WPA_DRIVER_CAPA_KEY_MGMT_WPA | WPA_DRIVER_CAPA_KEY_MGMT_WPA_PSK |
                             ...
                             WPA_DRIVER_CAPA_KEY_MGMT_SUITE_B | WPA_DRIVER_CAPA_KEY_MGMT_OWE |
                             WPA_DRIVER_CAPA_KEY_MGMT_DPP;

        if (drv->capa.enc & (WPA_DRIVER_CAPA_ENC_CCMP_256 | WPA_DRIVER_CAPA_ENC_GCMP_256))
        ...
        if (drv->capa.flags & WPA_DRIVER_FLAGS_SME)
            drv->capa.key_mgmt |= WPA_DRIVER_CAPA_KEY_MGMT_FILS_SHA256 | WPA_DRIVER_CAPA_KEY_MGMT_FILS_SHA384 |
                                  WPA_DRIVER_CAPA_KEY_MGMT_FT_FILS_SHA256 | WPA_DRIVER_CAPA_KEY_MGMT_FT_FILS_SHA384 |
    ...
    wpa_printf(MSG_DEBUG,
               "nl80211: key_mgmt=0x%x enc=0x%x auth=0x%x flags=0x%llx rrm_flags=0x%x probe_resp_offloads=0x%x "
               "max_stations=%u max_remain_on_chan=%u max_scan_ssids=%d",
               drv->capa.key_mgmt, drv->capa.enc, drv->capa.auth, (unsigned long long)drv->capa.flags,
               drv->capa.rrm_flags, drv->capa.probe_resp_offloads, drv->capa.max_stations, drv->capa.max_remain_on_chan,
...
static void phy_info_ht_capa(struct hostapd_hw_modes *mode,
                             struct nlattr *capa,
...
static int phy_info_edmg_capa(struct hostapd_hw_modes *mode, struct nlattr *bw_config, struct nlattr *channels)
{
...
static void phy_info_freq(struct hostapd_hw_modes *mode, struct hostapd_channel_data *chan, struct nlattr *tb_freq[])
{
    ...
    if (tb_freq[NL80211_FREQUENCY_ATTR_WMM])
    {
        ...
        nla_for_each_nested(nl_wmm, tb_freq[NL80211_FREQUENCY_ATTR_WMM], rem_wmm)
        {
            ...
            if (!tb_wmm[NL80211_WMMR_CW_MIN] || !tb_wmm[NL80211_WMMR_CW_MAX] || !tb_wmm[NL80211_WMMR_AIFSN] ||
...
static int phy_info_iftype(struct hostapd_hw_modes *mode, struct nlattr *nl_iftype)
{
    ...
    if (nla_parse_nested(tb_flags, NL80211_IFTYPE_MAX, tb[NL80211_BAND_IFTYPE_ATTR_IFTYPES], NULL))
        return NL_STOP;
...
static int phy_info_band(struct phy_info_arg *phy_info, struct nlattr *nl_band)
{
    ...
    phy_info_ht_capa(mode, tb_band[NL80211_BAND_ATTR_HT_CAPA], tb_band[NL80211_BAND_ATTR_HT_AMPDU_FACTOR],
                     tb_band[NL80211_BAND_ATTR_HT_AMPDU_DENSITY], tb_band[NL80211_BAND_ATTR_HT_MCS_SET]);
    phy_info_vht_capa(mode, tb_band[NL80211_BAND_ATTR_VHT_CAPA], tb_band[NL80211_BAND_ATTR_VHT_MCS_SET]);
    ret = phy_info_edmg_capa(mode, tb_band[NL80211_BAND_ATTR_EDMG_BW_CONFIG], tb_band[NL80211_BAND_ATTR_EDMG_CHANNELS]);
    if (ret == NL_OK)
...
static struct hostapd_hw_modes *wpa_driver_nl80211_postprocess_modes(struct hostapd_hw_modes *modes, u16 *num_modes)
{
    ...
    while (m < *num_modes)
    {
        if (modes[m].mode == NUM_HOSTAPD_MODES)
        {
            ...
            if (m + 1 < *num_modes)
                os_memmove(&modes[m], &modes[m + 1], sizeof(struct hostapd_hw_modes) * (*num_modes - (m + 1)));
            (*num_modes)--;
            continue;
        }
        m++;
...
static int nl80211_get_reg(struct nl_msg *msg, void *arg)
{
    ...
    nla_for_each_nested(nl_rule, tb_msg[NL80211_ATTR_REG_RULES], rem_rule)
    {
        ...
        wpa_printf(MSG_DEBUG, "nl80211: %u-%u @ %u MHz %u mBm%s%s%s%s%s%s%s%s", start, end, max_bw, max_eirp,
                   flags & NL80211_RRF_NO_OFDM ? " (no OFDM)" : "", flags & NL80211_RRF_NO_CCK ? " (no CCK)" : "",
                   flags & NL80211_RRF_NO_INDOOR ? " (no indoor)" : "",
                   flags & NL80211_RRF_NO_OUTDOOR ? " (no outdoor)" : "", flags & NL80211_RRF_DFS ? " (DFS)" : "",
                   flags & NL80211_RRF_PTP_ONLY ? " (PTP only)" : "",
                   flags & NL80211_RRF_PTMP_ONLY ? " (PTMP only)" : "", flags & NL80211_RRF_NO_IR ? " (no IR)" : "");
        if (max_bw >= 40)
...
static int nl80211_set_regulatory_flags(struct wpa_driver_nl80211_data *drv, struct phy_info_arg *results)
{
    ...
    return send_and_recv_msgs(drv, msg, nl80211_get_reg, results, NULL, NULL);
}
...
static void nl80211_dump_chan_list(struct hostapd_hw_modes *modes, u16 num_modes)
{
    ...
    for (i = 0; i < num_modes; i++)
    {
        ...
        for (j = 0; j < mode->num_channels; j++)
        {
            ...
            res = os_snprintf(
                pos, end - pos, " %d%s%s%s", chan->freq, (chan->flag & HOSTAPD_CHAN_DISABLED) ? "[DISABLED]" : "",
                (chan->flag & HOSTAPD_CHAN_NO_IR) ? "[NO_IR]" : "", (chan->flag & HOSTAPD_CHAN_RADAR) ? "[RADAR]" : "");
        ...
        *pos = '\0';
        wpa_printf(MSG_DEBUG, "nl80211: Mode IEEE %s:%s", modestr(mode->mode), str);
    }
}
...
struct hostapd_hw_modes *nl80211_get_hw_feature_data(void *priv, u16 *num_modes, u16 *flags, u8 *dfs_domain)
{
    ...
    *dfs_domain = 0;
    ...
    if (send_and_recv_msgs(drv, msg, phy_info_handler, &result, NULL, NULL) == 0)
    {
        struct hostapd_hw_modes *modes;
