...
static void wpa_driver_ndis_event_process(struct wpa_driver_ndis_data *drv, u8 *buf, size_t len)
{
    u8 *pos, *data = NULL;
...
void wpa_driver_ndis_event_pipe_cb(void *eloop_data, void *user_data)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "%s: ReadFile() failed: %d", __func__, (int)GetLastError());
    }
}
