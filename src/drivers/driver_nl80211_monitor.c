    ...
    while (1)
    {
        ...
        switch (iter.this_arg_index)
        {
            ...
            case IEEE80211_RADIOTAP_TX_FLAGS:
                injected = 1;
                failed   = le_to_host16((*(le16 *)iter.this_arg)) & IEEE80211_RADIOTAP_F_TX_FAIL;
                break;
...
int nl80211_create_monitor_interface(struct wpa_driver_nl80211_data *drv)
{
    ...
    else
    {
        ...
        ret = os_snprintf(buf, IFNAMSIZ, "mon.%s", drv->first_bss->ifname);
        if (ret >= (int)sizeof(buf))
            wpa_printf(MSG_DEBUG, "nl80211: Monitor interface name has been truncated to %s", buf);
        else if (ret < 0)
            return ret;
    }
