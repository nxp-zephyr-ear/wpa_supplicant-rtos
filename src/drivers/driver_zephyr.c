...
static void wpa_drv_zep_event_proc_mgmt_rx(struct zep_drv_if_ctx *if_ctx, union wpa_event_data *event);
...
void hostapd_event_wrapper(void *ctx, enum wpa_event_type event, union wpa_event_data *data)
{
    ...
    if (data)
    {
        ...
        if (!msg.data)
        {
            wpa_printf(MSG_ERROR, "Failed to allocated forevent: %d", event);
            return;
        }
        os_memcpy(msg.data, data, sizeof(*data));
    }
    ...
    send_wpa_supplicant_event(&msg);
    // send_hostapd_event(&msg);
}

static int wpa_drv_zep_abort_scan(void *priv, u64 scan_cookie)
{
...
void wpa_drv_zep_event_proc_scan_abort(struct zep_drv_if_ctx *if_ctx)
{
    union wpa_event_data event;
    struct scan_info *info = NULL;

    wpa_printf(MSG_ERROR, "%s: Scan aborted by driver", __func__);
...
void wpa_drv_zep_event_proc_scan_done(struct zep_drv_if_ctx *if_ctx, union wpa_event_data *event)
{
    eloop_cancel_timeout(wpa_drv_zep_scan_timeout, if_ctx, if_ctx->supp_if_ctx);
...
void wpa_drv_zep_event_proc_auth_resp(struct zep_drv_if_ctx *if_ctx, union wpa_event_data *event)
{
...
void wpa_drv_zep_event_proc_deauth(struct zep_drv_if_ctx *if_ctx, union wpa_event_data *event)
{
    wpa_supplicant_event_wrapper(if_ctx->supp_if_ctx, EVENT_DEAUTH, event);
}

void wpa_drv_zep_event_proc_disassoc(struct zep_drv_if_ctx *if_ctx, union wpa_event_data *event)
{
...
static void wpa_drv_zep_event_proc_unprot_deauth(struct zep_drv_if_ctx *if_ctx, union wpa_event_data *event)
{
    wpa_supplicant_event_wrapper(if_ctx->supp_if_ctx, EVENT_UNPROT_DEAUTH, event);
}

static void wpa_drv_zep_event_proc_unprot_disassoc(struct zep_drv_if_ctx *if_ctx, union wpa_event_data *event)
{
    wpa_supplicant_event_wrapper(if_ctx->supp_if_ctx, EVENT_UNPROT_DISASSOC, event);
}

void wpa_drv_zep_event_proc_remain_on_channel(struct zep_drv_if_ctx *if_ctx,
                                                   int cancel_event,
                                                   union wpa_event_data *event)
{
...
void wpa_drv_zep_event_proc_survey_res(struct zep_drv_if_ctx *if_ctx,
 ...
 */
static void *wpa_drv_zep_init(void *ctx, const char *ifname, void *global_priv)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    ...
    if_ctx->dev_ops = if_ctx->dev_ctx->config;

    dev_ops = if_ctx->dev_ops;
    ...
    callbk_fns.scan_abort        = wpa_drv_zep_event_proc_scan_abort;
    ...
    callbk_fns.remain_on_channel = wpa_drv_zep_event_proc_remain_on_channel;
    callbk_fns.mgmt_rx           = wpa_drv_zep_event_proc_mgmt_rx;
...
static int wpa_drv_zep_scan2(void *priv, struct wpa_driver_scan_params *params)
{
    ...
    eloop_register_timeout(timeout, 0, wpa_drv_zep_scan_timeout, if_ctx, if_ctx->supp_if_ctx);
...
int wpa_drv_zep_sched_scan(void *priv, struct wpa_driver_scan_params *params)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int ret                                         = -1;
...
int wpa_drv_zep_stop_sched_scan(void *priv)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int ret                                         = -1;
...
static int wpa_drv_zep_deauthenticate(void *priv, const unsigned char *addr, u16 reason_code)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int ret                                         = -1;
...
static int wpa_drv_zep_authenticate(void *priv, struct wpa_driver_auth_params *params)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
...
static int wpa_drv_zep_associate(void *priv, struct wpa_driver_associate_params *params)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int ret                                         = -1;
...
static int wpa_drv_zep_set_key(void *priv, struct wpa_driver_set_key_params *params)
{
    ...
    return _wpa_drv_zep_set_key(priv, (const unsigned char *)params->ifname, params->alg, params->addr,
...
static int wpa_drv_zep_get_capa(void *priv, struct wpa_driver_capa *capa)
{
    ...
    if_ctx = (struct zep_drv_if_ctx *)priv;
...
static int wpa_drv_zep_get_ext_capab(
...
static int wpa_drv_zep_set_mac_addr(void *priv, const u8 *addr)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int ret                                         = -1;
...
static int wpa_drv_zep_send_action(void *priv,
                                        unsigned int freq,
...
static int wpa_drv_zep_remain_on_channel(void *priv, unsigned int freq, unsigned int duration)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int ret                                         = -1;
...
static int wpa_drv_zep_cancel_remain_on_channel(void *priv)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int ret                                         = -1;
...
static int wpa_drv_zep_get_survey(void *priv, unsigned int freq)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int ret                                         = -1;
...
int wpa_drv_zep_set_ap(void *priv, struct wpa_driver_ap_params *params)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int ret                                         = -1;
...
static int wpa_drv_zep_send_mlme(void *priv,
                                      const u8 *data,
                                      size_t data_len,
                                      int noack,
                                      unsigned int freq,
...
static int wpa_drv_zep_sta_add(void *priv, struct hostapd_sta_add_params *params)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int ret                                         = -1;
...
static int wpa_drv_zep_sta_remove(void *priv, const u8 *addr)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int ret                                         = -1;
...
static int wpa_drv_zep_send_eapol(
...
static int wpa_drv_zep_set_freq(void *priv, struct hostapd_freq_params *freq)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int status                                      = -1;
...
static int wpa_drv_zep_set_rts(void *priv, int rts_threshold)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int status                                      = -1;
...
static int wpa_drv_zep_set_frag(void *priv, int frag_threshold)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int status                                      = -1;
...
int wpa_drv_zep_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr, u16 reason)
{
    ...
    return wpa_drv_zep_send_mlme(priv, (u8 *)&mgmt, IEEE80211_HDRLEN + sizeof(mgmt.u.deauth), 0, 0, NULL, 0, 0, 0);
}

static int wpa_drv_zep_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr, u16 reason)
{
    struct ieee80211_mgmt mgmt;
    ...
    return wpa_drv_zep_send_mlme(priv, (u8 *)&mgmt, IEEE80211_HDRLEN + sizeof(mgmt.u.disassoc), 0, 0, NULL, 0, 0,
                                      0);
}

static int wpa_drv_zep_stop_ap(void *if_priv)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int status                                      = -1;
...
static int wpa_drv_zep_deinit_ap(void *if_priv)
{
    return wpa_drv_zep_stop_ap(if_priv);
}

static int wpa_drv_zep_set_acl(void *if_priv, struct hostapd_acl_params *params)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int status                                      = -1;
...
static void zep_set_ht40_mode(struct hostapd_hw_modes *mode, int start, int end)
{
    int c;
...
static void zep_set_ht40_mode_sec(struct hostapd_hw_modes *mode, int start, int end)
{
    int c;
...
static void zep_set_vht_mode(struct hostapd_hw_modes *mode, int start, int end, int max_bw)
{
    int c;
...
static void zep_reg_rule_max_eirp(struct hostapd_hw_modes *mode, int start, int end, int max_eirp)
{
    int c;
...
static struct hostapd_hw_modes *wpa_drv_zep_get_hw_feature_data(void *if_priv, u16 *num_modes, u16 *flags, u8 *dfs)
{
    struct zep_drv_if_ctx *if_ctx              = NULL;
    const struct zep_wpa_supp_dev_ops *dev_ops = NULL;
    int status                                      = -1;
    ...
    *dfs       = 0;
    ...
    zep_set_ht40_mode(&modes[0], start, end);
    zep_set_ht40_mode_sec(&modes[0], start, end);
    zep_reg_rule_max_eirp(&modes[0], start, end, 20);
    ...
    modes[1].num_channels = MAX_NUM_CHANNEL_5G;
    modes[1].num_rates    = 8;
    ...
    for (i = 0; i < 4; i++)
    {
        modes[1].channels[k].chan       = 36 + (i * 4);
        modes[1].channels[k].freq       = 5180 + (i * 20);
        modes[1].channels[k].flag       = 0;
        modes[1].channels[k].allowed_bw = HOSTAPD_CHAN_WIDTH_20 | HOSTAPD_CHAN_WIDTH_40P | HOSTAPD_CHAN_WIDTH_40M |
    ...
    for (i = 0; i < 4; i++)
    {
        modes[1].channels[k].chan       = 52 + (i * 4);
        modes[1].channels[k].freq       = 5260 + (i * 20);
        modes[1].channels[k].flag       = 0;
        modes[1].channels[k].allowed_bw = HOSTAPD_CHAN_WIDTH_20 | HOSTAPD_CHAN_WIDTH_40P | HOSTAPD_CHAN_WIDTH_40M |
                                          HOSTAPD_CHAN_WIDTH_80 | HOSTAPD_CHAN_WIDTH_160;
        k++;
    ...
    for (i = 0; i < 11; i++)
    {
        modes[1].channels[k].chan       = 100 + (i * 4);
        modes[1].channels[k].freq       = 5500 + (i * 20);
        modes[1].channels[k].flag       = 0;
        modes[1].channels[k].allowed_bw = HOSTAPD_CHAN_WIDTH_20 | HOSTAPD_CHAN_WIDTH_40P | HOSTAPD_CHAN_WIDTH_40M |
                                          HOSTAPD_CHAN_WIDTH_80 | HOSTAPD_CHAN_WIDTH_160;
        k++;
    ...
    for (i = 0; i < 5; i++)
    {
        modes[1].channels[k].chan       = 149 + (i * 4);
        modes[1].channels[k].freq       = 5745 + (i * 20);
        modes[1].channels[k].flag       = 0;
        modes[1].channels[k].allowed_bw = HOSTAPD_CHAN_WIDTH_20 | HOSTAPD_CHAN_WIDTH_40P | HOSTAPD_CHAN_WIDTH_40M |
                                          HOSTAPD_CHAN_WIDTH_80 | HOSTAPD_CHAN_WIDTH_160;
        k++;
    ...
    zep_set_ht40_mode(&modes[1], start, end);
    zep_set_ht40_mode_sec(&modes[1], start, end);
    zep_reg_rule_max_eirp(&modes[1], start, end, 20);
    zep_set_vht_mode(&modes[1], start, end, 160);
...
const struct wpa_driver_ops wpa_driver_zep_ops = {
    .name                     = "zep",
    .desc                     = "zep wpa_supplicant driver",
    ...
    .set_mac_addr             = wpa_drv_zep_set_mac_addr,
    ...
    .get_survey               = wpa_drv_zep_get_survey,
    ...
    .hapd_send_eapol = wpa_drv_zep_send_eapol,
    .set_freq        = wpa_drv_zep_set_freq,
    .set_rts         = wpa_drv_zep_set_rts,
    .set_frag        = wpa_drv_zep_set_frag,
