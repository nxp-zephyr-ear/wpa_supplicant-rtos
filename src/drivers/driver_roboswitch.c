...
static int wpa_driver_roboswitch_reg(struct wpa_driver_roboswitch_data *drv, u8 page, u8 reg, u8 op)
{
    ...
    /* set page number */
    wpa_driver_roboswitch_mdio_write(drv, ROBO_MII_PAGE, (page << 8) | ROBO_MII_PAGE_ENABLE);
...
static int wpa_driver_roboswitch_read(struct wpa_driver_roboswitch_data *drv, u8 page, u8 reg, u16 *val, int len)
{
    ...
    if (len > ROBO_MII_DATA_MAX || wpa_driver_roboswitch_reg(drv, page, reg, ROBO_MII_ADDR_READ) < 0)
        return -1;
...
static void wpa_driver_roboswitch_receive(void *priv, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    if (len > 14 && WPA_GET_BE16(buf + 12) == ETH_P_EAPOL && os_memcmp(buf, drv->own_addr, ETH_ALEN) == 0)
...
static int wpa_driver_roboswitch_set_param(void *priv, const char *param)
{
    ...
    if (param == NULL || os_strstr(param, "multicast_only=1") == NULL)
    {
        ...
        *sep    = '.';
        drv->l2 = l2_packet_init(drv->ifname, NULL, ETH_P_ALL, wpa_driver_roboswitch_receive, drv, 1);
        if (drv->l2 == NULL)
        {
...
static int wpa_driver_roboswitch_join(struct wpa_driver_roboswitch_data *drv, u16 ports, const u8 *addr)
{
    ...
    if (wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_CONF, read1, 1) < 0)
        return -1;
    if (!(read1[0] & (1 << 4)))
    {
        /* multiport addresses are not yet enabled */
        read1[0] |= 1 << 4;
        wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_1, addr_be16, 3);
        wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_1, &ports, 1);
        wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_2, addr_be16, 3);
        wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_2, &ports, 1);
        wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_CONF, read1, 1);
    }
    else
    {
        /* if both multiport addresses are the same we can add */
        if (wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_1, read1, 3) ||
            wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_2, read2, 3) ||
            os_memcmp(read1, read2, 6) != 0)
            return -1;
        if (wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_1, read1, 1) ||
            wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_2, read2, 1) || read1[0] != read2[0])
            return -1;
        wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_1, addr_be16, 3);
        wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_1, &ports, 1);
    }
...
static int wpa_driver_roboswitch_leave(struct wpa_driver_roboswitch_data *drv, u16 ports, const u8 *addr)
{
    ...
    if (wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_CONF, &_read, 1) < 0)
        return -1;
    /* If ARL control is disabled, there is nothing to leave. */
    if (!(_read & (1 << 4)))
        return -1;

    if (wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_1, addr_read, 3) < 0 ||
        wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_1, &ports_read, 1) < 0)
        return -1;
    /* check if we occupy multiport address 1 */
    if (os_memcmp(addr_read, addr_be16, 6) == 0 && ports_read == ports)
    {
        if (wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_2, addr_read, 3) < 0 ||
            wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_2, &ports_read, 1) < 0)
            return -1;
        /* and multiport address 2 */
        if (os_memcmp(addr_read, addr_be16, 6) == 0 && ports_read == ports)
        {
            _read &= ~(1 << 4);
            wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_CONF, &_read, 1);
        }
        else
        {
            wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_1, addr_read, 3);
            wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_1, &ports_read, 1);
            wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_2, addr_read, 3);
            wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_2, &ports_read, 1);
        }
    }
    else
    {
        ...
        if (os_memcmp(addr_read, addr_be16, 6) == 0 && ports_read == ports)
        {
            wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_1, addr_read, 3);
            wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_1, &ports_read, 1);
        }
        else
            return -1;
    }
...
static void *wpa_driver_roboswitch_init(void *ctx, const char *ifname)
{
    ...
    *sep = '\0';
    ...
    if (if_mii(&drv->ifr)->phy_id != ROBO_PHY_ADDR && if_mii(&drv->ifr)->phy_id != 0)
    {
        wpa_printf(MSG_INFO,
    ...
    wpa_driver_roboswitch_write(drv, ROBO_VLAN_PAGE, ROBO_VLAN_ACCESS_5350, _read, 1);
    wpa_driver_roboswitch_read(drv, ROBO_VLAN_PAGE, ROBO_VLAN_ACCESS_5350, _read + 1, 1);
    drv->is_5350 = _read[0] == _read[1];

    /* set the read bit */
    vlan |= 1 << 13;
    wpa_driver_roboswitch_write(drv, ROBO_VLAN_PAGE, drv->is_5350 ? ROBO_VLAN_ACCESS_5350 : ROBO_VLAN_ACCESS, &vlan, 1);
    wpa_driver_roboswitch_read(drv, ROBO_VLAN_PAGE, ROBO_VLAN_READ, _read, drv->is_5350 ? 2 : 1);
