...
static void *eap_pwd_init(struct eap_sm *sm)
{
    ...
    if (data->salt_len)
    {
        ...
        if (!data->salt)
        {
            wpa_printf(MSG_INFO, "EAP-pwd: Memory allocation of salt failed");
            bin_clear_free(data->id_server, data->id_server_len);
...
static void eap_pwd_build_id_req(struct eap_sm *sm, struct eap_pwd_data *data, u8 id)
{
    ...
    wpa_hexdump_key(MSG_DEBUG, "EAP-pwd (server): password", data->password, data->password_len);
    if (data->salt_len)
        wpa_hexdump(MSG_DEBUG, "EAP-pwd (server): salt", data->salt, data->salt_len);
    ...
    if (data->salt_len)
    {
        switch (data->password_len)
        {
            ...
            case 64:
                ...
                break;
            default:
                wpa_printf(MSG_INFO, "EAP-pwd (server): bad size %d for salted password", (int)data->password_len);
                eap_pwd_state(data, FAILURE);
                return;
        }
    }
    else
    {
        ...
        data->password_prep = data->password_hash ? EAP_PWD_PREP_MS : EAP_PWD_PREP_NONE;
    }
...
static void eap_pwd_build_commit_req(struct eap_sm *sm, struct eap_pwd_data *data, u8 id)
{
    ...
    data->outbuf = wpabuf_alloc(2 * prime_len + order_len + (data->salt ? 1 + data->salt_len : 0));
    if (data->outbuf == NULL)
        goto fin;
...
static struct wpabuf *eap_pwd_build_req(struct eap_sm *sm, void *priv, u8 id)
{
     ...
     */
    req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PWD, EAP_PWD_HDR_SIZE + len + (totlen ? sizeof(u16) : 0),
                        EAP_CODE_REQUEST, id);
    if (req == NULL)
    {
        eap_pwd_state(data, FAILURE);
...
static bool eap_pwd_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    struct eap_pwd_data *data = priv;
    const u8 *pos;
    size_t len;
...
static void eap_pwd_process_id_resp(struct eap_sm *sm, struct eap_pwd_data *data, const u8 *payload, size_t payload_len)
{
    ...
    if ((data->group_num != be_to_host16(id->group_num)) || (id->random_function != EAP_PWD_DEFAULT_RAND_FUNC) ||
        (os_memcmp(id->token, (u8 *)&data->token, sizeof(data->token))) || (id->prf != EAP_PWD_DEFAULT_PRF) ||
    ...
    res = compute_password_element(data->grp, data->group_num, password, password_len, data->id_server,
                                   data->id_server_len, data->id_peer, data->id_peer_len, (u8 *)&data->token);
    ...
    wpa_printf(MSG_DEBUG, "EAP-PWD (server): computed %d bit PWE...", (int)crypto_ec_prime_len_bits(data->grp->group));
                                        ...
                                        size_t payload_len)
{
    ...
    {
        wpa_printf(MSG_INFO, "EAP-pwd: Unexpected Commit payload length %u (expected %u)", (unsigned int)payload_len,
    ...
    if (crypto_bignum_cmp(data->my_scalar, data->peer_scalar) == 0 ||
        crypto_ec_point_cmp(data->grp->group, data->my_element, data->peer_element) == 0)
    {
        wpa_printf(MSG_INFO, "EAP-PWD (server): detected reflection attack!");
        goto fin;
    }

    /* compute the shared key, k */
    if ((crypto_ec_point_mul(data->grp->group, data->grp->pwe, data->peer_scalar, K) < 0) ||
        (crypto_ec_point_add(data->grp->group, K, data->peer_element, K) < 0) ||
                                         ...
                                         size_t payload_len)
{
    ...
    *ptr = EAP_PWD_DEFAULT_PRF;
    ...
    if (compute_keys(data->grp, data->k, data->peer_scalar, data->my_scalar, conf, data->my_confirm, &cs, data->msk,
...
static void eap_pwd_process(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    ...
    if ((pos == NULL) || (len < 1))
    {
        wpa_printf(MSG_INFO, "Bad EAP header! pos %s and len = %d", (pos == NULL) ? "is NULL" : "is not NULL",
                   (int)len);
        return;
    }
    ...
    if (EAP_PWD_GET_MORE_BIT(lm_exch) || data->in_frag_pos)
    {
        ...
        if ((data->in_frag_pos + len) > wpabuf_size(data->inbuf))
        {
            wpa_printf(MSG_DEBUG,
                       ...
                       (int)data->in_frag_pos, (int)len, (int)wpabuf_size(data->inbuf));
            eap_pwd_state(data, FAILURE);
            return;
        }
