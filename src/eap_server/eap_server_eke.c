...
{
    wpa_printf(MSG_DEBUG, "EAP-EKE: Failure - code 0x%lx", code);
...
static void *eap_eke_init(struct eap_sm *sm)
{
    ...
    {
        if (sm->cfg->server_id[i] == '.' && data->serverid_type == EAP_EKE_ID_OPAQUE)
...
{
    if (pos[0] == EAP_EKE_DHGROUP_EKE_16 && pos[1] == EAP_EKE_ENCR_AES128_CBC && pos[2] == EAP_EKE_PRF_HMAC_SHA2_256 &&
        pos[3] == EAP_EKE_MAC_HMAC_SHA2_256)
        return 1;

    if (pos[0] == EAP_EKE_DHGROUP_EKE_15 && pos[1] == EAP_EKE_ENCR_AES128_CBC && pos[2] == EAP_EKE_PRF_HMAC_SHA2_256 &&
        pos[3] == EAP_EKE_MAC_HMAC_SHA2_256)
        return 1;

    if (pos[0] == EAP_EKE_DHGROUP_EKE_14 && pos[1] == EAP_EKE_ENCR_AES128_CBC && pos[2] == EAP_EKE_PRF_HMAC_SHA2_256 &&
        pos[3] == EAP_EKE_MAC_HMAC_SHA2_256)
        return 1;

    if (pos[0] == EAP_EKE_DHGROUP_EKE_14 && pos[1] == EAP_EKE_ENCR_AES128_CBC && pos[2] == EAP_EKE_PRF_HMAC_SHA1 &&
...
static struct wpabuf *eap_eke_build_failure(struct eap_eke_data *data, u8 id)
{
    ...
    wpa_printf(MSG_DEBUG, "EAP-EKE: Request/Failure: Failure-Code=0x%lx", data->failure_code);
...
static struct wpabuf *eap_eke_build_commit(struct eap_sm *sm, struct eap_eke_data *data, u8 id)
{
    ...
    if (eap_eke_derive_key(&data->sess, sm->user->password, sm->user->password_len, sm->cfg->server_id,
                           sm->cfg->server_id_len, data->peerid, data->peerid_len, data->key) < 0)
    {
    ...
    if (eap_eke_dhcomp(&data->sess, data->key, pub, wpabuf_put(msg, data->sess.dhcomp_len)) < 0)
    {
...
static struct wpabuf *eap_eke_build_confirm(struct eap_sm *sm, struct eap_eke_data *data, u8 id)
{
    ...
    if (eap_eke_derive_ka(&data->sess, sm->cfg->server_id, sm->cfg->server_id_len, data->peerid, data->peerid_len,
...
static bool eap_eke_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    struct eap_eke_data *data = priv;
    size_t len;
    const u8 *pos;
...
static void eap_eke_process_identity(
    struct eap_sm *sm, struct eap_eke_data *data, const struct wpabuf *respData, const u8 *payload, size_t payloadlen)
{
    const u8 *pos, *end;
    int i;
...
static void eap_eke_process_commit(
    struct eap_sm *sm, struct eap_eke_data *data, const struct wpabuf *respData, const u8 *payload, size_t payloadlen)
{
    ...
    if (eap_eke_shared_secret(&data->sess, data->key, data->dh_priv, dhcomp) < 0)
    {
    ...
    if (eap_eke_derive_ke_ki(&data->sess, sm->cfg->server_id, sm->cfg->server_id_len, data->peerid, data->peerid_len) <
        0)
    {
...
static void eap_eke_process_confirm(
    struct eap_sm *sm, struct eap_eke_data *data, const struct wpabuf *respData, const u8 *payload, size_t payloadlen)
{
    size_t decrypt_len;
    ...
    if (eap_eke_derive_msk(&data->sess, sm->cfg->server_id, sm->cfg->server_id_len, data->peerid, data->peerid_len,
                           data->nonce_s, data->nonce_p, data->msk, data->emsk) < 0)
    {
...
static void eap_eke_process_failure(
    struct eap_sm *sm, struct eap_eke_data *data, const struct wpabuf *respData, const u8 *payload, size_t payloadlen)
{
    u32 code;
    ...
    wpa_printf(MSG_DEBUG, "EAP-EKE: Peer reported failure code 0x%lx", code);
