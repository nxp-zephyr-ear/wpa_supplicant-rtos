...
static struct wpabuf *eap_sake_build_identity(struct eap_sm *sm, struct eap_sake_data *data, u8 id)
{
    ...
    eap_sake_add_attr(msg, EAP_SAKE_AT_SERVERID, sm->cfg->server_id, sm->cfg->server_id_len);
...
static struct wpabuf *eap_sake_build_confirm(struct eap_sm *sm, struct eap_sake_data *data, u8 id)
{
    ...
    if (eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p, sm->cfg->server_id, sm->cfg->server_id_len,
                             data->peerid, data->peerid_len, 0, wpabuf_head(msg), wpabuf_len(msg), mic, mic))
    {
...
static bool eap_sake_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    struct eap_sake_data *data = priv;
...
static void eap_sake_process_identity(
    struct eap_sm *sm, struct eap_sake_data *data, const struct wpabuf *respData, const u8 *payload, size_t payloadlen)
{
    ...
    struct eap_sm *sm, struct eap_sake_data *data, const struct wpabuf *respData, const u8 *payload, size_t payloadlen)
{
    ...
    if (eap_sake_derive_keys(sm->user->password, sm->user->password + EAP_SAKE_ROOT_SECRET_LEN, data->rand_s,
                             data->rand_p, (u8 *)&data->tek, data->msk, data->emsk) < 0)
    {
    ...
    if (eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p, sm->cfg->server_id, sm->cfg->server_id_len,
                             data->peerid, data->peerid_len, 1, wpabuf_head(respData), wpabuf_len(respData), attr.mic_p,
                             mic_p) < 0)
    {
        wpa_printf(MSG_INFO, "EAP-SAKE: Failed to compute MIC");
        data->state = FAILURE;
...
static void eap_sake_process_confirm(
    struct eap_sm *sm, struct eap_sake_data *data, const struct wpabuf *respData, const u8 *payload, size_t payloadlen)
{
    struct eap_sake_parse_attr attr;
    u8 mic_p[EAP_SAKE_MIC_LEN];
...
static void eap_sake_process_auth_reject(
    struct eap_sm *sm, struct eap_sake_data *data, const struct wpabuf *respData, const u8 *payload, size_t payloadlen)
{
