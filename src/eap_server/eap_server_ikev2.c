...
static void *eap_ikev2_init(struct eap_sm *sm)
{
    ...
    if (data == NULL)
        return NULL;
    data->state           = MSG;
    data->fragment_size   = sm->cfg->fragment_size > 0 ? sm->cfg->fragment_size : IKEV2_FRAGMENT_SIZE;
...
static struct wpabuf *eap_ikev2_build_msg(struct eap_ikev2_data *data, u8 id)
{
    ...
    if (flags & IKEV2_FLAGS_ICV_INCLUDED)
    {
        ...
        ikev2_integ_hash(data->ikev2.proposal.integ, data->ikev2.keys.SK_ai, data->ikev2.keys.SK_integ_len, msg, len,
...
static bool eap_ikev2_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    const u8 *pos;
    size_t len;
...
static int eap_ikev2_process_icv(
    struct eap_ikev2_data *data, const struct wpabuf *respData, u8 flags, const u8 *pos, const u8 **end, int frag_ack)
{
    if (flags & IKEV2_FLAGS_ICV_INCLUDED)
    {
        int icv_len = eap_ikev2_validate_icv(data->ikev2.proposal.integ, &data->ikev2.keys, 0, respData, pos, *end);
        if (icv_len < 0)
            return -1;
...
static int eap_ikev2_process_fragment(
    struct eap_ikev2_data *data, u8 flags, u32 message_length, const u8 *buf, size_t len)
{
...
{
    if (eap_ikev2_derive_keymat(data->ikev2.proposal.prf, &data->ikev2.keys, data->ikev2.i_nonce,
                                data->ikev2.i_nonce_len, data->ikev2.r_nonce, data->ikev2.r_nonce_len,
                                data->keymat) < 0)
    {
        wpa_printf(MSG_DEBUG,
    ...
    if (flags & IKEV2_FLAGS_LENGTH_INCLUDED)
    {
        ...
        if (message_length < (u32)(end - pos))
        {
            wpa_printf(MSG_DEBUG,
                       ...
                       "Length (%ld; %ld remaining in this msg)",
