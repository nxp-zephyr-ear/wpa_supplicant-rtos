...
static struct wpabuf *eap_pax_build_std_1(struct eap_sm *sm, struct eap_pax_data *data, u8 id)
{
    ...
    req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PAX, sizeof(*pax) + 2 + EAP_PAX_RAND_LEN + EAP_PAX_ICV_LEN,
                        EAP_CODE_REQUEST, id);
    if (req == NULL)
    {
    ...
    if (eap_pax_mac(data->mac_id, (u8 *)"", 0, wpabuf_mhead(req), wpabuf_len(req) - EAP_PAX_ICV_LEN, NULL, 0, NULL, 0,
                    pos) < 0)
    {
...
static struct wpabuf *eap_pax_build_std_3(struct eap_sm *sm, struct eap_pax_data *data, u8 id)
{
    ...
    req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PAX, sizeof(*pax) + 2 + EAP_PAX_MAC_LEN + EAP_PAX_ICV_LEN,
                        EAP_CODE_REQUEST, id);
    if (req == NULL)
    {
        wpa_printf(MSG_ERROR,
    ...
    pos = wpabuf_put(req, EAP_PAX_MAC_LEN);
    if (eap_pax_mac(data->mac_id, data->ck, EAP_PAX_CK_LEN, data->rand.r.y, EAP_PAX_RAND_LEN, (u8 *)data->cid,
                    data->cid_len, NULL, 0, pos) < 0)
    {
    ...
    pos = wpabuf_put(req, EAP_PAX_MAC_LEN);
    if (eap_pax_mac(data->mac_id, data->ick, EAP_PAX_ICK_LEN, wpabuf_mhead(req), wpabuf_len(req) - EAP_PAX_ICV_LEN,
                    NULL, 0, NULL, 0, pos) < 0)
    {
        wpa_printf(MSG_ERROR, "EAP-PAX: Failed to calculate ICV");
        data->state = FAILURE;
...
static bool eap_pax_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    struct eap_pax_data *data = priv;
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "public_key_id 0x%x",
               resp->op_code, resp->flags, resp->mac_id, resp->dh_group_id, resp->public_key_id);
    ...
    if (data->keys_set)
    {
        ...
        if (eap_pax_mac(data->mac_id, data->ick, EAP_PAX_ICK_LEN, wpabuf_mhead(respData),
                        wpabuf_len(respData) - EAP_PAX_ICV_LEN, NULL, 0, NULL, 0, icvbuf) < 0)
        {
...
static void eap_pax_process_std_2(struct eap_sm *sm, struct eap_pax_data *data, struct wpabuf *respData)
{
    ...
    if (eap_pax_initial_key_derivation(data->mac_id, data->ak, data->rand.e, data->mk, data->ck, data->ick, data->mid) <
        0)
    {
        wpa_printf(MSG_INFO,
    ...
    if (eap_pax_mac(data->mac_id, data->ck, EAP_PAX_CK_LEN, data->rand.r.x, EAP_PAX_RAND_LEN, data->rand.r.y,
                    EAP_PAX_RAND_LEN, (u8 *)data->cid, data->cid_len, mac) < 0)
    {
    ...
    wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: ICV", pos, EAP_PAX_ICV_LEN);
    if (eap_pax_mac(data->mac_id, data->ick, EAP_PAX_ICK_LEN, wpabuf_head(respData),
                    wpabuf_len(respData) - EAP_PAX_ICV_LEN, NULL, 0, NULL, 0, icvbuf) < 0)
    {
        wpa_printf(MSG_INFO, "EAP-PAX: Failed to calculate ICV");
        return;
...
static u8 *eap_pax_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
    ...
    *len = EAP_MSK_LEN;
    eap_pax_kdf(data->mac_id, data->mk, EAP_PAX_MK_LEN, "Master Session Key", data->rand.e, 2 * EAP_PAX_RAND_LEN,
                EAP_MSK_LEN, key);
...
static u8 *eap_pax_get_emsk(struct eap_sm *sm, void *priv, size_t *len)
{
    ...
    *len = EAP_EMSK_LEN;
    eap_pax_kdf(data->mac_id, data->mk, EAP_PAX_MK_LEN, "Extended Master Session Key", data->rand.e,
                2 * EAP_PAX_RAND_LEN, EAP_EMSK_LEN, key);
