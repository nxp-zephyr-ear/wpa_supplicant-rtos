...
static struct wpabuf *eap_gpsk_build_gpsk_1(struct eap_sm *sm, struct eap_gpsk_data *data, u8 id)
{
    ...
    len = 1 + 2 + sm->cfg->server_id_len + EAP_GPSK_RAND_LEN + 2 + data->csuite_count * sizeof(struct eap_gpsk_csuite);
...
static struct wpabuf *eap_gpsk_build_gpsk_3(struct eap_sm *sm, struct eap_gpsk_data *data, u8 id)
{
    ...
    len    = 1 + 2 * EAP_GPSK_RAND_LEN + 2 + sm->cfg->server_id_len + sizeof(struct eap_gpsk_csuite) + 2 + miclen;
    req    = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_GPSK, len, EAP_CODE_REQUEST, id);
    if (req == NULL)
    {
        wpa_printf(MSG_ERROR,
...
static bool eap_gpsk_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    struct eap_gpsk_data *data = priv;
    const u8 *pos;
    size_t len;
...
static void eap_gpsk_process_gpsk_2(struct eap_sm *sm, struct eap_gpsk_data *data, const u8 *payload, size_t payloadlen)
{
    ...
    if (alen != sm->cfg->server_id_len || os_memcmp(pos, sm->cfg->server_id, alen) != 0)
    {
        wpa_printf(MSG_DEBUG,
    ...
    if (eap_gpsk_derive_keys(sm->user->password, sm->user->password_len, data->vendor, data->specifier, data->rand_peer,
                             data->rand_server, data->id_peer, data->id_peer_len, sm->cfg->server_id,
                             sm->cfg->server_id_len, data->msk, data->emsk, data->sk, &data->sk_len, data->pk,
                             &data->pk_len) < 0)
    {
    ...
    if (eap_gpsk_derive_session_id(sm->user->password, sm->user->password_len, data->vendor, data->specifier,
                                   data->rand_peer, data->rand_server, data->id_peer, data->id_peer_len,
                                   sm->cfg->server_id, sm->cfg->server_id_len, EAP_TYPE_GPSK, data->session_id,
                                   &data->id_len) < 0)
    {
    ...
    if (eap_gpsk_compute_mic(data->sk, data->sk_len, data->vendor, data->specifier, payload, pos - payload, mic) < 0)
    {
