                                      ...
                                      u8 next_payload)
{
    ...
    decrypted = ikev2_decrypt_payload(data->proposal.encr, data->proposal.integ, &data->keys, 0, hdr, encrypted,
...
static int ikev2_process_sa_init(struct ikev2_initiator_data *data,
                                 ...
                                 struct ikev2_payloads *pl)
{
    if (ikev2_process_sar1(data, pl->sa, pl->sa_len) < 0 || ikev2_process_ker(data, pl->ke, pl->ke_len) < 0 ||
...
static int ikev2_process_auth_secret(struct ikev2_initiator_data *data, u8 method, const u8 *auth, size_t auth_len)
{
    ...
    if (ikev2_derive_auth_data(data->proposal.prf, data->r_sign_msg, data->IDr, data->IDr_len, data->IDr_type,
                               &data->keys, 0, data->shared_secret, data->shared_secret_len, data->i_nonce,
                               data->i_nonce_len, data->key_pad, data->key_pad_len, auth_data) < 0)
    {
                                           ...
                                           size_t payload_len)
{
    ...
    if (ikev2_process_idr(data, pl.idr, pl.idr_len) < 0 || ikev2_process_cert(data, pl.cert, pl.cert_len) < 0 ||
                                 ...
                                 struct ikev2_payloads *pl)
{
    ...
    decrypted = ikev2_decrypt_payload(data->proposal.encr, data->proposal.integ, &data->keys, 0, hdr, pl->encrypted,
...
static void ikev2_build_hdr(
    struct ikev2_initiator_data *data, struct wpabuf *msg, u8 exchange_type, u8 next_payload, u32 message_id)
{
...
static int ikev2_build_auth(struct ikev2_initiator_data *data, struct wpabuf *msg, u8 next_payload)
{
    ...
    if (ikev2_derive_auth_data(data->proposal.prf, data->i_sign_msg, data->IDi, data->IDi_len, ID_KEY_ID, &data->keys,
                               1, data->shared_secret, data->shared_secret_len, data->r_nonce, data->r_nonce_len,
...
static struct wpabuf *ikev2_build_sa_init(struct ikev2_initiator_data *data)
{
    ...
    if (ikev2_build_sai(data, msg, IKEV2_PAYLOAD_KEY_EXCHANGE) || ikev2_build_kei(data, msg, IKEV2_PAYLOAD_NONCE) ||
...
static struct wpabuf *ikev2_build_sa_auth(struct ikev2_initiator_data *data)
{
    ...
    if (ikev2_build_idi(data, plain, IKEV2_PAYLOAD_AUTHENTICATION) ||
        ...
        ikev2_build_encrypted(data->proposal.encr, data->proposal.integ, &data->keys, 1, msg, plain, IKEV2_PAYLOAD_IDi))
    {
        wpabuf_free(plain);
        wpabuf_free(msg);
        return NULL;
