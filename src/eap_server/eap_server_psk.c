...
static struct wpabuf *eap_psk_build_1(struct eap_sm *sm, struct eap_psk_data *data, u8 id)
{
    ...
    req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PSK, sizeof(*psk) + sm->cfg->server_id_len, EAP_CODE_REQUEST, id);
    if (req == NULL)
    {
...
static bool eap_psk_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    struct eap_psk_data *data = priv;
    size_t len;
    u8 t;
