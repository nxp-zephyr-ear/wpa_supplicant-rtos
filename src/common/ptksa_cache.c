...
static void ptksa_cache_free_entry(struct ptksa_cache *ptksa, struct ptksa_cache_entry *entry)
{
    ptksa->n_ptksa--;
...
static void ptksa_cache_expire(void *eloop_ctx, void *timeout_ctx)
{
    ...
    dl_list_for_each_safe(e, next, &ptksa->ptksa, struct ptksa_cache_entry, list)
    {
        if (e->expiration > now.sec)
            continue;

        wpa_printf(MSG_DEBUG, "Expired PTKSA cache entry for " MACSTR, MAC2STR(e->addr));
...
void ptksa_cache_deinit(struct ptksa_cache *ptksa)
{
    ...
    dl_list_for_each_safe(e, next, &ptksa->ptksa, struct ptksa_cache_entry, list) ptksa_cache_free_entry(ptksa, e);
...
struct ptksa_cache_entry *ptksa_cache_get(struct ptksa_cache *ptksa, const u8 *addr, u32 cipher)
{
    ...
    {
        if ((!addr || os_memcmp(e->addr, addr, ETH_ALEN) == 0) && (cipher == WPA_CIPHER_NONE || cipher == e->cipher))
            return e;
    }
...
int ptksa_cache_list(struct ptksa_cache *ptksa, char *buf, size_t len)
{
    ...
    ret = os_snprintf(pos, buf + len - pos, "Index / ADDR / Cipher / expiration (secs) / TK / KDK\n");
    ...
    dl_list_for_each(e, &ptksa->ptksa, struct ptksa_cache_entry, list)
    {
        ret = os_snprintf(pos, buf + len - pos, "%u " MACSTR, i, MAC2STR(e->addr));
        if (os_snprintf_error(buf + len - pos, ret))
            return pos - buf;
        pos += ret;

        ret = os_snprintf(pos, buf + len - pos, " %s %lu ", wpa_cipher_txt(e->cipher), e->expiration - now.sec);
        if (os_snprintf_error(buf + len - pos, ret))
            return pos - buf;
        pos += ret;

        ret = wpa_snprintf_hex(pos, buf + len - pos, e->ptk.tk, e->ptk.tk_len);
        if (os_snprintf_error(buf + len - pos, ret))
            return pos - buf;
        pos += ret;
        ...
        ret = wpa_snprintf_hex(pos, buf + len - pos, e->ptk.kdk, e->ptk.kdk_len);
        if (os_snprintf_error(buf + len - pos, ret))
            return pos - buf;
        pos += ret;
...
void ptksa_cache_flush(struct ptksa_cache *ptksa, const u8 *addr, u32 cipher)
{
    ...
    dl_list_for_each_safe(e, next, &ptksa->ptksa, struct ptksa_cache_entry, list)
    {
        if ((!addr || os_memcmp(e->addr, addr, ETH_ALEN) == 0) && (cipher == WPA_CIPHER_NONE || cipher == e->cipher))
        {
            wpa_printf(MSG_DEBUG, "Flush PTKSA cache entry for " MACSTR, MAC2STR(e->addr));
    ...
    struct ptksa_cache *ptksa, const u8 *addr, u32 cipher, u32 life_time, const struct wpa_ptk *ptk)
{
    ...
    ptksa->n_ptksa++;
    wpa_printf(MSG_DEBUG, "Added PTKSA cache entry addr=" MACSTR " cipher=%u", MAC2STR(addr), cipher);
