...
#define MAX_MISSING_SAK_USE                      \
    10 /* Accept up to 10 inbound MKPDUs without \
...
static const char *sci_txt(const struct ieee802_1x_mka_sci *sci)
{
    ...
    os_snprintf(txt, sizeof(txt), MACSTR "@%u", MAC2STR(sci->addr), be_to_host16(sci->port));
    return txt;
}
...
static void ieee802_1x_mka_dump_basic_body(struct ieee802_1x_mka_basic_body *body)
{
    ...
    wpa_printf(MSG_DEBUG, "\tMACsec Capability: %d", body->macsec_capability);
    wpa_printf(MSG_DEBUG, "\tParameter set body length: %zu", body_len);
    wpa_printf(MSG_DEBUG, "\tSCI: %s", sci_txt(&body->actor_sci));
    wpa_printf(MSG_DEBUG, "\tActor's Member Identifier: %s", mi_txt(body->actor_mi));
    wpa_printf(MSG_DEBUG, "\tActor's Message Number: %d", be_to_host32(body->actor_mn));
    wpa_printf(MSG_DEBUG, "\tAlgorithm Agility: %s", algo_agility_txt(body->algo_agility));
    wpa_hexdump(MSG_DEBUG, "\tCAK Name", body->ckn, body_len + MKA_HDR_LEN - sizeof(*body));
}
...
static void ieee802_1x_mka_dump_peer_body(struct ieee802_1x_mka_peer_body *body)
{
    ...
    for (i = 0; i < body_len; i += MI_LEN + sizeof(mn))
    {
        ...
        wpa_printf(MSG_DEBUG, "\tMember Id: %s  Message Number: %d", mi_txt(mi), be_to_host32(mn));
    }
}
...
static void ieee802_1x_mka_dump_dist_sak_body(struct ieee802_1x_mka_dist_sak_body *body)
{
    ...
    wpa_printf(MSG_DEBUG, "\tConfidentiality Offset: %d", body->confid_offset);
    wpa_printf(MSG_DEBUG, "\tBody Length...........: %zu", body_len);
    if (!body_len)
        return;

    wpa_printf(MSG_DEBUG, "\tKey Number............: %d", be_to_host32(body->kn));
    if (body_len == 28)
    {
    ...
    {
        wpa_hexdump(MSG_DEBUG, "\tMACsec Cipher Suite...:", body->sak, CS_ID_LEN);
        wpa_hexdump(MSG_DEBUG, "\tAES Key Wrap of SAK...:", body->sak + CS_ID_LEN,
 ...
 */
static struct ieee802_1x_mka_participant *ieee802_1x_kay_get_participant(struct ieee802_1x_kay *kay,
                                                                         const u8 *ckn,
                                                                         size_t len)
{
    ...
    {
        if (participant->ckn.len == len && os_memcmp(participant->ckn.name, ckn, participant->ckn.len) == 0)
            return participant;
    }
 ...
 */
static struct ieee802_1x_kay_peer *ieee802_1x_kay_get_potential_peer(struct ieee802_1x_mka_participant *participant,
                                                                     const u8 *mi)
{
 ...
 */
static struct ieee802_1x_kay_peer *ieee802_1x_kay_get_live_peer(struct ieee802_1x_mka_participant *participant,
                                                                const u8 *mi)
{
 ...
 */
static bool ieee802_1x_kay_is_in_potential_peer(struct ieee802_1x_mka_participant *participant, const u8 *mi)
{
 ...
 */
static bool ieee802_1x_kay_is_in_live_peer(struct ieee802_1x_mka_participant *participant, const u8 *mi)
{
 ...
 */
static struct macsec_ciphersuite *ieee802_1x_kay_get_cipher_suite(struct ieee802_1x_mka_participant *participant,
                                                                  const u8 *cs_id,
                                                                  unsigned int *idx)
{
    unsigned int i;
...
static bool sci_equal(const struct ieee802_1x_mka_sci *a, const struct ieee802_1x_mka_sci *b)
{
 ...
 */
static struct ieee802_1x_kay_peer *ieee802_1x_kay_get_peer_sci(struct ieee802_1x_mka_participant *participant,
                                                         ...
                                                         struct data_key *key)
{
    ...
    wpa_printf(MSG_DEBUG, "KaY: Create receive SA(an: %hhu lowest_pn: %u) of SC", an, lowest_pn);
 ...
 */
static struct receive_sc *ieee802_1x_kay_init_receive_sc(const struct ieee802_1x_mka_sci *psci)
{
    ...
    wpa_printf(MSG_DEBUG, "KaY: Create receive SC: SCI %s", sci_txt(&psc->sci));
...
{
    wpa_printf(MSG_DEBUG, "\tMI: %s  MN: %d  SCI: %s", mi_txt(peer->mi), peer->mn, sci_txt(&peer->sci));
}
 ...
 */
static struct ieee802_1x_kay_peer *ieee802_1x_kay_create_live_peer(struct ieee802_1x_mka_participant *participant,
                                                                   const u8 *mi,
                                                                   u32 mn)
{
    struct ieee802_1x_kay_peer *peer;
 ...
 */
static struct ieee802_1x_kay_peer *ieee802_1x_kay_create_potential_peer(struct ieee802_1x_mka_participant *participant,
                                                                        const u8 *mi,
                                                                        u32 mn)
{
    struct ieee802_1x_kay_peer *peer;
 ...
 */
static struct ieee802_1x_kay_peer *ieee802_1x_kay_move_live_peer(struct ieee802_1x_mka_participant *participant,
                                                                 u8 *mi,
                                                                 u32 mn)
{
    struct ieee802_1x_kay_peer *peer;
 ...
 */
static bool ieee802_1x_mka_basic_body_present(struct ieee802_1x_mka_participant *participant)
{
    return true;
}
...
static bool reset_participant_mi(struct ieee802_1x_mka_participant *participant)
{
 ...
 */
static struct ieee802_1x_mka_participant *ieee802_1x_mka_decode_basic_body(struct ieee802_1x_kay *kay,
                                                                           const u8 *mka_msg,
                                                                           size_t msg_len)
{
    struct ieee802_1x_mka_participant *participant;
    ...
    {
        wpa_printf(MSG_DEBUG, "KaY: Peer's version(%d) greater than MKA current version(%d)", body->version,
                   MKA_VERSION_ID);
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "KaY: Too small body length %zu", body_len);
        return NULL;
    }
    ...
    if (!participant)
    {
        wpa_printf(MSG_DEBUG, "KaY: Peer is not included in my CA - ignore MKPDU");
        return NULL;
    }
    ...
    if (os_memcmp(body->actor_mi, participant->mi, MI_LEN) == 0)
    {
        if (!reset_participant_mi(participant))
            return NULL;
        wpa_printf(MSG_DEBUG, "KaY: Peer using my MI - selected a new random MI: %s", mi_txt(participant->mi));
    }
    ...
    if (!peer)
    {
        ...
        if (!peer)
        {
            wpa_printf(MSG_DEBUG, "KaY: No potential peer entry found - ignore MKPDU");
            return NULL;
        }
    ...
    {
        wpa_printf(MSG_WARNING, "KaY: The peer MN did not increase - ignore MKPDU");
        return NULL;
    }
 ...
 */
static bool ieee802_1x_mka_live_peer_body_present(struct ieee802_1x_mka_participant *participant)
{
...
static int ieee802_1x_mka_get_live_peer_length(struct ieee802_1x_mka_participant *participant)
{
    ...
    dl_list_for_each(peer, &participant->live_peers, struct ieee802_1x_kay_peer, list) len +=
 ...
 */
static bool ieee802_1x_mka_potential_peer_body_present(struct ieee802_1x_mka_participant *participant)
{
...
static int ieee802_1x_mka_get_potential_peer_length(struct ieee802_1x_mka_participant *participant)
{
    ...
    dl_list_for_each(peer, &participant->potential_peers, struct ieee802_1x_kay_peer, list) len +=
        sizeof(struct ieee802_1x_mka_peer_id);
 ...
 */
static bool ieee802_1x_mka_i_in_peerlist(struct ieee802_1x_mka_participant *participant,
                                         const u8 *mka_msg,
                                         size_t msg_len)
{
    ...
    for (pos = mka_msg, left_len = msg_len; left_len > MKA_HDR_LEN + DEFAULT_ICV_LEN;
         left_len -= MKA_ALIGN_LENGTH(body_len) + MKA_HDR_LEN, pos += MKA_ALIGN_LENGTH(body_len) + MKA_HDR_LEN)
    {
        ...
        if (left_len < (MKA_HDR_LEN + MKA_ALIGN_LENGTH(body_len) + DEFAULT_ICV_LEN))
        {
            wpa_printf(MSG_ERROR,
                       "KaY: MKA Peer Packet Body Length (%zu bytes) is less than the Parameter Set Header Length (%zu "
                       "bytes) + the Parameter Set Body Length (%zu bytes) + %d bytes of ICV",
                       left_len, MKA_HDR_LEN, MKA_ALIGN_LENGTH(body_len), DEFAULT_ICV_LEN);
            return false;
        }
        ...
        if ((body_len % 16) != 0)
        {
            wpa_printf(MSG_ERROR, "KaY: MKA Peer Packet Body Length (%zu bytes) should be a multiple of 16 octets",
                       body_len);
            continue;
        }
        ...
        for (i = 0; i < body_len; i += sizeof(struct ieee802_1x_mka_peer_id))
        {
            ...
            if (os_memcmp(peer_mi->mi, participant->mi, MI_LEN) == 0)
            {
                ...
                wpa_printf(MSG_DEBUG, "KaY: My MI - received MN %u, most recently transmitted MN %u", mn,
                           participant->mn);
                 ...
                 * have sent). */
                if (mn == participant->mn || (participant->mn > 1 && mn == participant->mn - 1))
                    return true;
            }
        }
    }
 ...
 */
static int ieee802_1x_mka_decode_live_peer_body(struct ieee802_1x_mka_participant *participant,
                                                const u8 *peer_msg,
                                                size_t msg_len)
{
    ...
    for (i = 0; i < body_len; i += sizeof(struct ieee802_1x_mka_peer_id))
    {
        ...
        peer_mi = (const struct ieee802_1x_mka_peer_id *)(peer_msg + MKA_HDR_LEN + i);
        ...
        else if (!ieee802_1x_kay_create_potential_peer(participant, peer_mi->mi, peer_mn))
        {
            return -1;
        }
    }
 ...
 */
static int ieee802_1x_mka_decode_potential_peer_body(struct ieee802_1x_mka_participant *participant,
                                                     const u8 *peer_msg,
                                                     size_t msg_len)
{
    const struct ieee802_1x_mka_hdr *hdr;
 ...
 */
static bool ieee802_1x_mka_sak_use_body_present(struct ieee802_1x_mka_participant *participant)
{
 ...
 */
static int ieee802_1x_mka_decode_sak_use_body(struct ieee802_1x_mka_participant *participant,
                                              const u8 *mka_msg,
                                              size_t msg_len)
{
    struct ieee802_1x_mka_hdr *hdr;
    ...
    if (!peer)
    {
        wpa_printf(MSG_WARNING, "KaY: The peer (%s) is not my live peer - ignore MACsec SAK Use parameter set",
    ...
    {
        wpa_printf(MSG_ERROR, "KaY: MKA Use SAK Packet Body Length (%zu bytes) should be 0, 40, or more octets",
                   body_len);
        return -1;
    }
    ...
    if (body->delay_protect && (!be_to_host32(body->llpn) || !be_to_host32(body->olpn)))
    {
        wpa_printf(MSG_WARNING, "KaY: Lowest packet number should be greater than 0 when delay_protect is TRUE");
        return -1;
    }
    ...
    if (participant->is_key_server)
    {
        ...
        dl_list_for_each(peer_iter, &participant->live_peers, struct ieee802_1x_kay_peer, list)
        {
 ...
 */
static bool ieee802_1x_mka_dist_sak_body_present(struct ieee802_1x_mka_participant *participant)
{
    return participant->is_key_server && participant->to_dist_sak && participant->new_key;
}
...
static int ieee802_1x_mka_encode_dist_sak_body(struct ieee802_1x_mka_participant *participant, struct wpabuf *buf)
{
    ...
    if (!sak)
    {
        wpa_printf(MSG_DEBUG, "KaY: No SAK available to build Distributed SAK parameter set");
        return -1;
    }
    ...
    if (aes_wrap(participant->kek.key, participant->kek.len, cipher_suite_tbl[cs_index].sak_len / 8, sak->key,
                 body->sak + sak_pos))
    {
 ...
 */
static int ieee802_1x_mka_decode_dist_sak_body(struct ieee802_1x_mka_participant *participant,
                                               const u8 *mka_msg,
                                               size_t msg_len)
{
    struct ieee802_1x_mka_hdr *hdr;
    ...
    {
        wpa_printf(MSG_ERROR, "KaY: MKA Use SAK Packet Body Length (%zu bytes) should be 0, 28, 36, or more octets",
                   body_len);
        return -1;
    }
    ...
    if (participant->is_key_server)
    {
        wpa_printf(MSG_ERROR, "KaY: Reject distributed SAK since I'm a key server");
        return -1;
    }
    if (!kay->macsec_desired || kay->macsec_capable == MACSEC_CAP_NOT_IMPLEMENTED)
    {
    ...
    {
        if (os_memcmp(sa_key->key_identifier.mi, participant->current_peer_id.mi, MI_LEN) == 0 &&
            sa_key->key_identifier.kn == be_to_host32(body->kn))
        {
            wpa_printf(MSG_DEBUG, "KaY: SAK has already been installed - do not set it again");
            return 0;
        }
    }
    ...
    else
    {
        ...
        cs = ieee802_1x_kay_get_cipher_suite(participant, body->sak, &idx);
        if (!cs)
        {
    ...
    if (aes_unwrap(participant->kek.key, participant->kek.len, sak_len >> 3, wrap_sak, unwrap_sak))
    {
 ...
 */
static bool ieee802_1x_mka_icv_body_present(struct ieee802_1x_mka_participant *participant)
{
    return true;
}
...
static int ieee802_1x_mka_encode_icv_body(struct ieee802_1x_mka_participant *participant, struct wpabuf *buf)
{
    ...
    if (mka_alg_tbl[participant->kay->mka_algindex].icv_hash(participant->ick.key, participant->ick.len,
 ...
 */
static const u8 *ieee802_1x_mka_decode_icv_body(struct ieee802_1x_mka_participant *participant,
                                                const u8 *mka_msg,
                                                size_t msg_len)
{
    const struct ieee802_1x_mka_hdr *hdr;
 ...
 */
static int ieee802_1x_mka_decode_dist_cak_body(struct ieee802_1x_mka_participant *participant,
                                               const u8 *mka_msg,
                                               size_t msg_len)
{
    struct ieee802_1x_mka_hdr *hdr;
    ...
    if (body_len < 28)
    {
        wpa_printf(MSG_ERROR, "KaY: MKA Use CAK Packet Body Length (%zu bytes) should be 28 or more octets", body_len);
        return -1;
    }
 ...
 */
static int ieee802_1x_mka_decode_kmd_body(struct ieee802_1x_mka_participant *participant,
                                          const u8 *mka_msg,
                                          size_t msg_len)
{
    struct ieee802_1x_mka_hdr *hdr;
    ...
    if (body_len < 5)
    {
        wpa_printf(MSG_ERROR, "KaY: MKA Use KMD Packet Body Length (%zu bytes) should be 5 or more octets", body_len);
        return -1;
    }
 ...
 */
static int ieee802_1x_mka_decode_announce_body(struct ieee802_1x_mka_participant *participant,
                                               const u8 *mka_msg,
                                               size_t msg_len)
{
    return 0;
...
static struct mka_param_body_handler mka_body_handler[] = {
    ...
    {.body_tx = NULL, .body_rx = ieee802_1x_mka_decode_dist_cak_body, .body_length = NULL, .body_present = NULL},

    /* KMD parameter set */
    {.body_tx = NULL, .body_rx = ieee802_1x_mka_decode_kmd_body, .body_length = NULL, .body_present = NULL},

    /* Announcement parameter set */
    {.body_tx = NULL, .body_rx = ieee802_1x_mka_decode_announce_body, .body_length = NULL, .body_present = NULL},
...
static int ieee802_1x_kay_generate_new_sak(struct ieee802_1x_mka_participant *participant)
{
    ...
    {
        wpa_printf(MSG_ERROR, "KaY: Live peers list must not be empty when generating fresh SAK");
        return -1;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "KaY: Life time has not elapsed since prior SAK distributed");
        return -1;
    }
    ...
    {
        if (ieee802_1x_sak_aes_cmac(participant->cak.key, participant->cak.len, context, ctx_len, key, key_len))
        {
    ...
    {
        wpa_printf(MSG_ERROR, "KaY: SAK Length(%u) not supported", key_len);
        goto fail;
    }
    ...
    dl_list_for_each(peer, &participant->live_peers, struct ieee802_1x_kay_peer, list) peer->sak_used = false;
...
static int compare_priorities(const struct ieee802_1x_kay_peer *peer, const struct ieee802_1x_kay_peer *other)
{
...
static int ieee802_1x_kay_elect_key_server(struct ieee802_1x_mka_participant *participant)
{
    ...
    if (key_server && participant->can_be_key_server)
    {
        ...
        {
            wpa_printf(MSG_WARNING, "KaY: Cannot elect key server between me and peer, duplicate MAC detected");
            key_server = NULL;
        }
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "KaY: Peer %s was elected as the key server", mi_txt(key_server->mi));
...
static int ieee802_1x_kay_decide_macsec_use(struct ieee802_1x_mka_participant *participant)
{
    ...
    dl_list_for_each(peer, &participant->live_peers, struct ieee802_1x_kay_peer, list)
    {
        ...
        less_capability = (less_capability < peer->macsec_capability) ? less_capability : peer->macsec_capability;
        has_peer        = true;
    }
...
static int ieee802_1x_kay_encode_mkpdu(struct ieee802_1x_mka_participant *participant, struct wpabuf *pbuf)
{
    ...
    wpa_printf(MSG_DEBUG, "KaY: Ethernet header: DA=" MACSTR " SA=" MACSTR " Ethertype=0x%x", MAC2STR(ether_hdr->dest),
               MAC2STR(ether_hdr->src), be_to_host16(ether_hdr->ethertype));
    ...
    wpa_printf(MSG_DEBUG, "KaY: Common EAPOL PDU structure: Protocol Version=%u Packet Type=%u Packet Body Length=%u",
               eapol_hdr->version, eapol_hdr->type, be_to_host16(eapol_hdr->length));

    for (i = 0; i < ARRAY_SIZE(mka_body_handler); i++)
    {
        if (mka_body_handler[i].body_present && mka_body_handler[i].body_present(participant))
        {
...
static int ieee802_1x_participant_send_mkpdu(struct ieee802_1x_mka_participant *participant)
{
    ...
    wpa_printf(MSG_DEBUG, "KaY: Encode and send an MKPDU (ifname=%s)", kay->if_name);
...
static void ieee802_1x_participant_timer(void *eloop_ctx, void *timeout_ctx)
{
    ...
    kay         = participant->kay;
    wpa_printf(MSG_DEBUG, "KaY: Participant timer (ifname=%s)", kay->if_name);
    ...
    if (participant->retry_count < MAX_RETRY_CNT || participant->mode == PSK)
    {
        ieee802_1x_participant_send_mkpdu(participant);
        participant->retry_count++;
    }

    eloop_register_timeout(kay->mka_hello_time / 1000, 0, ieee802_1x_participant_timer, participant, NULL);
 ...
 */
static struct transmit_sa *ieee802_1x_kay_init_transmit_sa(struct transmit_sc *psc,
                                                           u8 an,
                                                           u32 next_PN,
                                                           struct data_key *key)
{
    ...
    dl_list_add(&psc->sa_list, &psa->list);
    wpa_printf(MSG_DEBUG, "KaY: Create transmit SA(an: %hhu, next_pn: %u) of SC", an, next_PN);
 ...
 * init_transmit_sc -
 */
static struct transmit_sc *ieee802_1x_kay_init_transmit_sc(const struct ieee802_1x_mka_sci *sci)
{
    ...
    dl_list_init(&psc->sa_list);
    wpa_printf(MSG_DEBUG, "KaY: Create transmit SC - SCI: %s", sci_txt(&psc->sci));
 ...
 * MKPDUs)
 */
static int ieee802_1x_kay_mkpdu_validity_check(struct ieee802_1x_kay *kay, const u8 *buf, size_t len)
{
    ...
    wpa_printf(MSG_DEBUG, "KaY: Ethernet header: DA=" MACSTR " SA=" MACSTR " Ethertype=0x%x", MAC2STR(eth_hdr->dest),
               MAC2STR(eth_hdr->src), be_to_host16(eth_hdr->ethertype));

    /* the destination address shall not be an individual address */
    if (os_memcmp(eth_hdr->dest, pae_group_addr, ETH_ALEN) != 0)
    {
        wpa_printf(MSG_DEBUG, "KaY: ethernet destination address is not PAE group address");
        return -1;
    }
    ...
    if ((mka_msg_len % 4) != 0)
    {
        wpa_printf(MSG_DEBUG, "KaY: MKPDU is not multiple of 4 octets");
        return -1;
    }

    wpa_hexdump(MSG_MSGDUMP, "KaY: EAPOL-MKA Packet Body (MKPDU)", mka_hdr, mka_msg_len);
    ...
    {
        wpa_printf(MSG_ERROR,
                   "KaY: Received EAPOL-MKA Packet Body Length (%zu bytes) is less than the Basic Parameter Set Header "
                   "Length (%zu bytes) + the Basic Parameter Set Body Length (%zu bytes) + %d bytes of ICV",
    ...
    {
        wpa_printf(MSG_WARNING, "KaY: Received EAPOL-MKA CKN Length (%zu bytes) is out of range (<= %u bytes)", ckn_len,
                   MAX_CKN_LEN);
        return -1;
    }
    ...
    {
        wpa_printf(MSG_INFO, "KaY: Peer's algorithm agility (%s) not supported", algo_agility_txt(body->algo_agility));
        return -1;
    }
    ...
    if (len < mka_alg_tbl[kay->mka_algindex].icv_len ||
        mka_alg_tbl[kay->mka_algindex].icv_hash(participant->ick.key, participant->ick.len, buf,
    ...
    msg_icv = ieee802_1x_mka_decode_icv_body(participant, (const u8 *)mka_hdr, mka_msg_len);
    if (!msg_icv)
    {
        wpa_printf(MSG_WARNING, "KaY: No ICV in MKPDU - ignore it");
        return -1;
    }
    wpa_hexdump(MSG_DEBUG, "KaY: Received ICV", msg_icv, mka_alg_tbl[kay->mka_algindex].icv_len);
    if (os_memcmp_const(msg_icv, icv, mka_alg_tbl[kay->mka_algindex].icv_len) != 0)
    {
        wpa_printf(MSG_WARNING, "KaY: Computed ICV is not equal to Received ICV");
        wpa_hexdump(MSG_DEBUG, "KaY: Calculated ICV", icv, mka_alg_tbl[kay->mka_algindex].icv_len);
        return -1;
    }
...
static int ieee802_1x_kay_decode_mkpdu(struct ieee802_1x_kay *kay, const u8 *buf, size_t len)
{
    ...
    wpa_printf(MSG_DEBUG, "KaY: Decode received MKPDU (ifname=%s)", kay->if_name);
    ...
    i_in_peerlist   = ieee802_1x_mka_i_in_peerlist(participant, pos, left_len);
    is_in_live_peer = ieee802_1x_kay_is_in_live_peer(participant, participant->current_peer_id.mi);
    wpa_printf(MSG_DEBUG, "KaY: i_in_peerlist=%s is_in_live_peer=%s", yes_no(i_in_peerlist), yes_no(is_in_live_peer));
    if (i_in_peerlist && !is_in_live_peer)
    {
        /* accept the peer as live peer */
        is_in_potential_peer = ieee802_1x_kay_is_in_potential_peer(participant, participant->current_peer_id.mi);
        if (is_in_potential_peer)
        {
            if (!ieee802_1x_kay_move_live_peer(participant, participant->current_peer_id.mi,
                                               be_to_host32(participant->current_peer_id.mn)))
                return -1;
        }
        else if (!ieee802_1x_kay_create_live_peer(participant, participant->current_peer_id.mi,
                                                  be_to_host32(participant->current_peer_id.mn)))
        {
            return -1;
        }
    ...
    handled[0] = true;
    for (; left_len > MKA_HDR_LEN + DEFAULT_ICV_LEN; pos += body_len + MKA_HDR_LEN, left_len -= body_len + MKA_HDR_LEN)
    {
        hdr       = (struct ieee802_1x_mka_hdr *)pos;
        body_len  = MKA_ALIGN_LENGTH(get_mka_param_body_len(hdr));
        body_type = get_mka_param_body_type(hdr);
        ...
        {
            wpa_printf(MSG_DEBUG, "KaY: Ignore duplicated body type %u", body_type);
            continue;
        }

        handled[body_type] = true;
        if (body_type < ARRAY_SIZE(mka_body_handler) && mka_body_handler[body_type].body_rx)
        {
            if (mka_body_handler[body_type].body_rx(participant, pos, left_len) != 0)
            {
                /* Handle parameter set failure */
                if (body_type != MKA_SAK_USE)
                {
                    wpa_printf(MSG_INFO, "KaY: Discarding Rx MKPDU: decode of parameter set type (%d) failed",
                               body_type);
                    return -1;
                }
        ...
        {
            wpa_printf(MSG_ERROR, "KaY: The body type %d is not supported in this MKA version %d", body_type,
                       MKA_VERSION_ID);
        }
    }
    ...
    {
        wpa_printf(MSG_INFO, "KaY: Discarding Rx MKPDU: decode of parameter set type (%d) failed", MKA_SAK_USE);
        if (!reset_participant_mi(participant))
            wpa_printf(MSG_DEBUG, "KaY: Could not update mi");
        else
            wpa_printf(MSG_DEBUG, "KaY: Selected a new random MI: %s", mi_txt(participant->mi));
        return -1;
    }
...
static void kay_l2_receive(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    eth_hdr   = (struct ieee8023_hdr *)buf;
    eapol_hdr = (struct ieee802_1x_hdr *)(eth_hdr + 1);
    calc_len  = sizeof(*eth_hdr) + sizeof(*eapol_hdr) + be_to_host16(eapol_hdr->length);
    if (len < calc_len)
    {
        wpa_printf(MSG_MSGDUMP, "KaY: EAPOL MPDU is invalid: (received len %lu, calculated len %lu, EAPOL length %u)",
                   (unsigned long)len, (unsigned long)calc_len, be_to_host16(eapol_hdr->length));
        return;
    }
    if (len > calc_len)
    {
        wpa_hexdump(MSG_DEBUG, "KaY: Ignore extra octets following the Packey Body field", &buf[calc_len],
                    len - calc_len);
        len = calc_len;
    }
    ...
    if (be_to_host16(eth_hdr->ethertype) != ETH_P_PAE || eapol_hdr->type != IEEE802_1X_TYPE_EAPOL_MKA)
    ...
    {
        wpa_printf(MSG_ERROR, "KaY: No MKA participant instance - ignore EAPOL-MKA");
        return;
    }
                                           ...
                                           const u8 *addr)
{
    ...
    wpa_printf(MSG_DEBUG, "KaY: Initialize - ifname=%s addr=" MACSTR " port=%u priority=%u", ifname, MAC2STR(addr),
               port, priority);
    ...
    wpa_printf(MSG_DEBUG, "KaY: Generated SCI: %s", sci_txt(&kay->actor_sci));
    ...
    if (policy != DO_NOT_SECURE && secy_get_capability(kay, &kay->macsec_capable) < 0)
        goto error;

    if (policy == DO_NOT_SECURE || kay->macsec_capable == MACSEC_CAP_NOT_IMPLEMENTED)
    {
    ...
    else
    {
        ...
        if (kay->macsec_capable >= MACSEC_CAP_INTEG_AND_CONF && policy == SHOULD_ENCRYPT)
        {
                                                             ...
                                                             bool is_authenticator)
{
    ...
    wpa_printf(MSG_DEBUG, "KaY: Create MKA (ifname=%s mode=%s authenticator=%s)", kay->if_name, mode_txt(mode),
    ...
    {
        wpa_printf(MSG_ERROR, "KaY: Unexpected CAK length %u", (unsigned int)cak->len);
        return NULL;
    }
    ...
    wpa_hexdump(MSG_DEBUG, "KaY: CKN", participant->ckn.name, participant->ckn.len);
    participant->cak.len = cak->len;
    os_memcpy(participant->cak.key, cak->key, cak->len);
    wpa_hexdump_key(MSG_DEBUG, "KaY: CAK", participant->cak.key, participant->cak.len);
    if (life)
    ...
    if (!reset_participant_mi(participant))
        goto fail;
    wpa_printf(MSG_DEBUG, "KaY: Selected random MI: %s", mi_txt(participant->mi));
    ...
    if (mka_alg_tbl[kay->mka_algindex].kek_trfm(participant->cak.key, participant->cak.len, participant->ckn.name,
                                                participant->ckn.len, participant->kek.key, participant->kek.len))
    {
    ...
    if (mka_alg_tbl[kay->mka_algindex].ick_trfm(participant->cak.key, participant->cak.len, participant->ckn.name,
                                                participant->ckn.len, participant->ick.key, participant->ick.len))
    {
...
int ieee802_1x_kay_get_status(struct ieee802_1x_kay *kay, char *buf, size_t buflen)
{
    ...
    res = os_snprintf(pos, end - pos,
                      ...
                      "MKA Hello Time=%u\n",
                      kay->active ? "Active" : "Not-Active", kay->authenticated ? "Yes" : "No",
                      kay->secured ? "Yes" : "No", kay->failed ? "Yes" : "No", kay->actor_priority,
                      kay->key_server_priority, kay->is_key_server ? "Yes" : "No", kay->dist_kn - 1, kay->rcvd_keys,
                      kay->mka_hello_time);
    if (os_snprintf_error(buflen, res))
        return 0;
    pos += res;

    res = os_snprintf(pos, end - pos, "actor_sci=%s\n", sci_txt(&kay->actor_sci));
    if (os_snprintf_error(buflen, res))
        return end - pos;
    pos += res;

    res = os_snprintf(pos, end - pos, "key_server_sci=%s\n", sci_txt(&kay->key_server_sci));
    if (os_snprintf_error(buflen, res))
        return end - pos;
    pos += res;

    count = 0;
    dl_list_for_each(p, &kay->participant_list, struct ieee802_1x_mka_participant, list)
    {
        char *pos2 = pos;
        ...
        pos2 += wpa_snprintf_hex(pos2, end - pos2, p->ckn.name, p->ckn.len);
        ...
        res = os_snprintf(pos2, end - pos2,
                          ...
                          "is_key_server=%s\n"
                          "is_elected=%s\n",
                          mi_txt(p->mi), p->mn, yes_no(p->active), yes_no(p->participant), yes_no(p->retain),
                          dl_list_len(&p->live_peers), dl_list_len(&p->potential_peers), yes_no(p->is_key_server),
                          yes_no(p->is_elected));
        if (os_snprintf_error(buflen, res))
            return end - pos;
        pos2 += res;
...
static char *mka_mib_peer(struct dl_list *peers, bool live, char *buf, char *end)
{
    char *pos = buf;
    ...
    dl_list_for_each(p, peers, struct ieee802_1x_kay_peer, list)
    {
        res = os_snprintf(pos, end - pos,
                          ...
                          mi_txt(p->mi), p->mn, live ? 1 : 2, sci_txt(&p->sci));
...
int ieee802_1x_kay_get_mib(struct ieee802_1x_kay *kay, char *buf, size_t buflen)
{
    ...
    dl_list_for_each(p, &kay->participant_list, struct ieee802_1x_mka_participant, list)
    {
        ...
        res = os_snprintf(pos2, end - pos2,
                          ...
                          true_false(p->cached), true_false(p->active), true_false(p->retain),
                          activate_control_txt(p->activate), true_false(p->principal));
        if (os_snprintf_error(buflen, res))
            return end - pos;
        pos2 += res;
        pos = pos2;
