...
static int aes_kdf(
    const u8 *kdk, size_t kdk_bits, const char *label, const u8 *context, int ctx_bits, int ret_bits, u8 *ret)
{
    const int h = 128;
    const int r = 8;
    int i, n;
...
int ieee802_1x_cak_aes_cmac(const u8 *msk, size_t msk_bytes, const u8 *mac1, const u8 *mac2, u8 *cak, size_t cak_bytes)
{
    ...
    return aes_kdf(msk, 8 * msk_bytes, "IEEE8021 EAP CAK", context, sizeof(context) * 8, 8 * cak_bytes, cak);
}
    ...
    const u8 *msk, size_t msk_bytes, const u8 *mac1, const u8 *mac2, const u8 *sid, size_t sid_bytes, u8 *ckn)
{
    ...
    res = aes_kdf(msk, 8 * msk_bytes, "IEEE8021 EAP CKN", context, ctx_len * 8, 128, ckn);
    os_free(context);
    return res;
}
 ...
 */
int ieee802_1x_kek_aes_cmac(const u8 *cak, size_t cak_bytes, const u8 *ckn, size_t ckn_bytes, u8 *kek, size_t kek_bytes)
{
    u8 context[16];
    ...
    return aes_kdf(cak, 8 * cak_bytes, "IEEE8021 KEK", context, sizeof(context) * 8, 8 * kek_bytes, kek);
}
 ...
 */
int ieee802_1x_ick_aes_cmac(const u8 *cak, size_t cak_bytes, const u8 *ckn, size_t ckn_bytes, u8 *ick, size_t ick_bytes)
{
    u8 context[16];
    ...
    return aes_kdf(cak, 8 * cak_bytes, "IEEE8021 ICK", context, sizeof(context) * 8, 8 * ick_bytes, ick);
}
...
int ieee802_1x_icv_aes_cmac(const u8 *ick, size_t ick_bytes, const u8 *msg, size_t msg_bytes, u8 *icv)
{
    ...
    if (res)
    {
        wpa_printf(MSG_ERROR, "MKA: AES-CMAC failed for ICV calculation");
        return -1;
    }
    return 0;
}
 ...
 */
int ieee802_1x_sak_aes_cmac(const u8 *cak, size_t cak_bytes, const u8 *ctx, size_t ctx_bytes, u8 *sak, size_t sak_bytes)
{
    return aes_kdf(cak, cak_bytes * 8, "IEEE8021 SAK", ctx, ctx_bytes * 8, sak_bytes * 8, sak);
}
