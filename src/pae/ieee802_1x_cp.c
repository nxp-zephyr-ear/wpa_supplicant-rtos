...
{
    return sm->confidentiality_offset != sm->cipher_offset || sm->current_cipher_suite != sm->cipher_suite;
}
...
SM_STEP(CP)
{
    ...
    switch (sm->CP_state)
    {
        ...
        case CP_RECEIVING:
            ...
            if (!sm->elected_self)
                SM_ENTER(CP, READY);
            if (sm->elected_self && (sm->all_receiving || !sm->controlled_port_enabled || !sm->transmit_when))
