...
{
    if (wpa_sm_set_key(sm, WPA_ALG_NONE, peer->addr, 0, 0, NULL, 0, NULL, 0, KEY_FLAG_PAIRWISE) < 0)
    {
        wpa_printf(MSG_WARNING,
                   "TDLS: Failed to delete TPK-TK from "
                   "the driver");
        return -1;
    }
...
static int wpa_tdls_set_key(struct wpa_sm *sm, struct wpa_tdls_peer *peer)
{
    ...
    wpa_printf(MSG_DEBUG, "TDLS: Configure pairwise key for peer " MACSTR, MAC2STR(peer->addr));
    if (wpa_sm_set_key(sm, alg, peer->addr, 0, 1, rsc, sizeof(rsc), peer->tpk.tk, key_len, KEY_FLAG_PAIRWISE_RX_TX) < 0)
    {
        wpa_printf(MSG_WARNING,
                   "TDLS: Failed to set TPK to the "
                   "driver");
        return -1;
    }
    peer->tk_set = 1;
...
static int wpa_tdls_send_tpk_msg(struct wpa_sm *sm,
                                 const u8 *dst,
                                 u8 action_code,
                                 u8 dialog_token,
                                 u16 status_code,
...
static int wpa_tdls_tpk_send(struct wpa_sm *sm,
                             const u8 *dest,
                             u8 action_code,
                             u8 dialog_token,
                             u16 status_code,
                             ...
                             size_t msg_len)
{
    ...
    wpa_printf(MSG_DEBUG,
               "TDLS: TPK send dest=" MACSTR
               " action_code=%u "
               "dialog_token=%u status_code=%u peer_capab=%u initiator=%d "
               "msg_len=%u",
               MAC2STR(dest), action_code, dialog_token, status_code, peer_capab, initiator, (unsigned int)msg_len);
    ...
    if (action_code == WLAN_TDLS_SETUP_CONFIRM || action_code == WLAN_TDLS_TEARDOWN ||
        action_code == WLAN_TDLS_DISCOVERY_REQUEST || action_code == WLAN_TDLS_DISCOVERY_RESPONSE)
        return 0; /* No retries */
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "(action_code=%u)",
               action_code);
    eloop_register_timeout(peer->sm_tmr.timer / 1000, (peer->sm_tmr.timer % 1000) * 1000, wpa_tdls_tpk_retry_timeout,
                           sm, peer);
    return 0;
}
...
static void wpa_tdls_tpk_retry_timeout(void *eloop_ctx, void *timeout_ctx)
{
    ...
    if (peer->sm_tmr.count)
    {
        ...
        if (wpa_tdls_send_tpk_msg(sm, peer->sm_tmr.dest, peer->sm_tmr.action_code, peer->sm_tmr.dialog_token,
                                  peer->sm_tmr.status_code, peer->sm_tmr.peer_capab, peer->initiator, peer->sm_tmr.buf,
                                  peer->sm_tmr.buf_len))
        {
            wpa_printf(MSG_INFO,
                       "TDLS: Failed to retry "
                       "transmission");
        }

        eloop_cancel_timeout(wpa_tdls_tpk_retry_timeout, sm, peer);
        eloop_register_timeout(peer->sm_tmr.timer / 1000, (peer->sm_tmr.timer % 1000) * 1000,
 ...
 */
static int wpa_tdls_ftie_mic(
    const u8 *kck, u8 trans_seq, const u8 *lnkid, const u8 *rsnie, const u8 *timeoutie, const u8 *ftie, u8 *mic)
{
    u8 *buf, *pos;
...
static int wpa_tdls_key_mic_teardown(
    const u8 *kck, u8 trans_seq, u16 rcode, u8 dtoken, const u8 *lnkid, const u8 *ftie, u8 *mic)
{
    u8 *buf, *pos;
    struct wpa_tdls_ftie *_ftie;
    int ret;
...
static int wpa_supplicant_verify_tdls_mic(
    u8 trans_seq, struct wpa_tdls_peer *peer, const u8 *lnkid, const u8 *timeoutie, const struct wpa_tdls_ftie *ftie)
{
    u8 mic[16];
    ...
    {
        wpa_printf(MSG_WARNING,
                   "TDLS: Could not verify TDLS MIC, "
...
static int wpa_supplicant_verify_tdls_mic_teardown(
    u8 trans_seq, u16 rcode, u8 dtoken, struct wpa_tdls_peer *peer, const u8 *lnkid, const struct wpa_tdls_ftie *ftie)
{
    u8 mic[16];
    ...
    {
        wpa_printf(MSG_INFO,
                   "TDLS: Could not verify TDLS Teardown "
...
static int wpa_tdls_send_teardown(struct wpa_sm *sm, const u8 *addr, u16 reason_code)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "TDLS: Testing - add extra subelem to "
                   "FTIE");
        ftie->ie_len += 170;
...
static int wpa_tdls_recv_teardown(struct wpa_sm *sm, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    wpa_printf(MSG_DEBUG, "TDLS: TDLS Teardown Request from " MACSTR " (reason code %u)", MAC2STR(src_addr),
               reason_code);
    ...
    {
        wpa_printf(MSG_INFO,
                   "TDLS: No Link Identifier IE in TDLS "
                   "Teardown");
        return -1;
    }
...
static int wpa_tdls_send_error(
    struct wpa_sm *sm, const u8 *dst, u8 tdls_action, u8 dialog_token, int initiator, u16 status)
{
...
static int wpa_tdls_send_tpk_m1(struct wpa_sm *sm, struct wpa_tdls_peer *peer)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "TDLS: Use alternative RSN IE for "
                   "testing");
        rsn_capab = WPA_CAPABILITY_PEERKEY_ENABLED;
    }
#endif /* CONFIG_TDLS_TESTING */
...
skip_ies:
    ...
    if (tdls_testing & TDLS_TESTING_DIFF_BSSID)
    {
        ...
        wpa_printf(MSG_DEBUG,
                   "TDLS: Testing - use incorrect BSSID in "
                   "Link Identifier");
...
skip_ies:
    ...
    status = wpa_tdls_tpk_send(sm, src_addr, WLAN_TDLS_SETUP_CONFIRM, dtoken, 0, peer_capab, peer->initiator, rbuf,
                               pos - rbuf);
    os_free(rbuf);
...
static int wpa_tdls_process_discovery_request(struct wpa_sm *sm, const u8 *addr, const u8 *buf, size_t len)
{
     ...
     */
    if (wpa_supplicant_parse_ies(buf + sizeof(struct wpa_tdls_frame) + 1, len - (sizeof(struct wpa_tdls_frame) + 1),
                                 &kde) < 0)
    {
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "TDLS: Link ID not found in Discovery "
                   "Request");
        return -1;
    }
...
static int copy_supp_rates(const struct wpa_eapol_ie_parse *kde, struct wpa_tdls_peer *peer)
{
    ...
    peer->supp_rates_len = merge_byte_arrays(
        peer->supp_rates, sizeof(peer->supp_rates), kde->supp_rates + 2, kde->supp_rates_len - 2,
        kde->ext_supp_rates ? kde->ext_supp_rates + 2 : NULL, kde->ext_supp_rates ? kde->ext_supp_rates_len - 2 : 0);
    return 0;
}
...
static int copy_peer_ht_capab(const struct wpa_eapol_ie_parse *kde, struct wpa_tdls_peer *peer)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "TDLS: No supported ht capabilities "
                   "received");
        return 0;
    }
    ...
    wpa_hexdump(MSG_DEBUG, "TDLS: Peer HT capabilities", (u8 *)peer->ht_capabilities,
...
static int copy_peer_vht_capab(const struct wpa_eapol_ie_parse *kde, struct wpa_tdls_peer *peer)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "TDLS: No supported vht capabilities "
                   "received");
        return 0;
    }
    ...
    wpa_hexdump(MSG_DEBUG, "TDLS: Peer VHT capabilities", (u8 *)peer->vht_capabilities,
                sizeof(struct ieee80211_vht_capabilities));

    return 0;
}

static int copy_peer_he_capab(const struct wpa_eapol_ie_parse *kde, struct wpa_tdls_peer *peer)
{
    ...
    os_free(peer->he_capabilities);
    peer->he_capab_len    = 0;
    peer->he_capabilities = os_memdup(kde->he_capabilities, kde->he_capab_len);
    if (!peer->he_capabilities)
        return -1;

    peer->he_capab_len = kde->he_capab_len;
    wpa_hexdump(MSG_DEBUG, "TDLS: Peer HE capabilities", peer->he_capabilities, peer->he_capab_len);

    return 0;
}

static int copy_peer_he_6ghz_band_capab(const struct wpa_eapol_ie_parse *kde, struct wpa_tdls_peer *peer)
{
    if (!kde->he_6ghz_capabilities)
    {
        wpa_printf(MSG_DEBUG, "TDLS: No HE 6 GHz band capabilities received");
        return 0;
    }

    if (!peer->he_6ghz_band_capabilities)
    {
        peer->he_6ghz_band_capabilities = os_zalloc(sizeof(struct ieee80211_he_6ghz_band_cap));
        if (peer->he_6ghz_band_capabilities == NULL)
            return -1;
    }

    os_memcpy(peer->he_6ghz_band_capabilities, kde->he_6ghz_capabilities, sizeof(struct ieee80211_he_6ghz_band_cap));

    wpa_hexdump(MSG_DEBUG, "TDLS: Peer 6 GHz band HE capabilities", peer->he_6ghz_band_capabilities,
...
static int copy_peer_supp_oper_classes(const struct wpa_eapol_ie_parse *kde, struct wpa_tdls_peer *peer)
{
    ...
    wpa_hexdump(MSG_DEBUG, "TDLS: Peer Supported Operating Classes", (u8 *)peer->supp_oper_classes,
...
static int wpa_tdls_addset_peer(struct wpa_sm *sm, struct wpa_tdls_peer *peer, int add)
{
    return wpa_sm_tdls_peer_addset(
        sm, peer->addr, add, peer->aid, peer->capability, peer->supp_rates, peer->supp_rates_len, peer->ht_capabilities,
        peer->vht_capabilities, peer->he_capabilities, peer->he_capab_len, peer->he_6ghz_band_capabilities,
        peer->qos_info, peer->wmm_capable, peer->ext_capab, peer->ext_capab_len, peer->supp_channels,
        peer->supp_channels_len, peer->supp_oper_classes, peer->supp_oper_classes_len);
}
...
static int wpa_tdls_process_tpk_m1(struct wpa_sm *sm, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    {
        wpa_printf(MSG_INFO,
                   "TDLS: No valid Link Identifier IE in "
                   "TPK M1");
        goto error;
    }
    ...
    if (copy_peer_vht_capab(&kde, peer) < 0 || copy_peer_he_capab(&kde, peer) < 0 ||
    ...
    if (tdls_testing & TDLS_TESTING_CONCURRENT_INIT)
    {
        ...
        if (peer == NULL)
            goto error;
        wpa_printf(MSG_DEBUG,
                   "TDLS: Testing concurrent initiation of "
                   "TDLS setup - send own request");
        peer->initiator = 1;
        wpa_sm_tdls_peer_addset(sm, peer->addr, 1, 0, 0, NULL, 0, NULL, NULL, NULL, 0, NULL, 0, 0, NULL, 0, NULL, 0,
                                NULL, 0);
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "TDLS: Testing - ignore AP prohibition "
                   "on TDLS");
        tdls_prohibited = 0;
    }
#endif /* CONFIG_TDLS_TESTING */
    ...
    {
        wpa_printf(MSG_INFO,
                   "TDLS: Too long Initiator RSN IE in "
                   "TPK M1");
    ...
    if ((ie.capabilities & (WPA_CAPABILITY_NO_PAIRWISE | WPA_CAPABILITY_PEERKEY_ENABLED)) !=
        WPA_CAPABILITY_PEERKEY_ENABLED)
    {
        wpa_printf(MSG_INFO,
                   "TDLS: Invalid RSN Capabilities in "
                   "TPK M1");
...
skip_rsn:
    ...
    if (os_memcmp(peer->inonce, ftie->Snonce, WPA_NONCE_LEN) != 0 || !tdls_nonce_set(peer->inonce))
    {
        /*
...
static int wpa_tdls_enable_link(struct wpa_sm *sm, struct wpa_tdls_peer *peer)
{
    ...
    {
        wpa_printf(MSG_INFO,
                   "TDLS: Could not configure key to the "
                   "driver");
        return -1;
    }
    peer->reconfig_key = 0;
...
static int wpa_tdls_process_tpk_m2(struct wpa_sm *sm, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    if (peer->tpk_success)
    {
        wpa_printf(MSG_INFO, "TDLS: Ignore incoming TPK M2 retry, from " MACSTR " as TPK M3 was already sent",
                   MAC2STR(src_addr));
        return 0;
    }
    ...
    {
        wpa_printf(MSG_INFO,
                   "TDLS: FTIE SNonce in TPK M2 does "
    ...
    if (wpa_tdls_set_key(sm, peer) < 0)
    {
         ...
         */
        wpa_printf(MSG_DEBUG,
                   "TDLS: Try to configure TPK again after "
...
skip_rsn:
    ...
    wpa_printf(MSG_DEBUG,
               "TDLS: Sending TDLS Setup Confirm / "
...
static int wpa_tdls_process_tpk_m3(struct wpa_sm *sm, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    {
        wpa_printf(MSG_INFO,
                   "TDLS: RSN IE in TPK M3 does not match "
    ...
    {
        wpa_printf(MSG_INFO,
                   "TDLS: FTIE ANonce in TPK M3 does "
    ...
    {
        wpa_printf(MSG_INFO,
                   "TDLS: FTIE SNonce in TPK M3 does not "
...
int wpa_tdls_start(struct wpa_sm *sm, const u8 *addr)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "TDLS: TDLS is prohibited in this BSS - "
    ...
    if (wpa_sm_tdls_peer_addset(sm, peer->addr, 1, 0, 0, NULL, 0, NULL, NULL, NULL, 0, NULL, 0, 0, NULL, 0, NULL, 0,
                                NULL, 0))
    {
        wpa_tdls_disable_peer_link(sm, peer);
        return -1;
...
static void wpa_supplicant_rx_tdls(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "TDLS: Discard message - TDLS disabled "
    ...
    if (tf->payloadtype != 2 /* TDLS_RFTYPE */ || tf->category != WLAN_ACTION_TDLS)
    {
        wpa_printf(MSG_INFO,
...
int wpa_tdls_init(struct wpa_sm *sm)
{
    ...
    sm->l2_tdls = l2_packet_init(sm->bridge_ifname ? sm->bridge_ifname : sm->ifname, sm->own_addr, ETH_P_80211_ENCAP,
                                 wpa_supplicant_rx_tdls, sm, 0);
    if (sm->l2_tdls == NULL)
    {
        wpa_printf(MSG_ERROR,
                   "TDLS: Failed to open l2_packet "
                   "connection");
        return -1;
    }
...
void wpa_tdls_ap_ies(struct wpa_sm *sm, const u8 *ies, size_t len)
{
    ...
    if (ies == NULL || ieee802_11_parse_elems(ies, len, &elems, 0) == ParseFailed || elems.ext_capab == NULL ||
        elems.ext_capab_len < 5)
        return;
