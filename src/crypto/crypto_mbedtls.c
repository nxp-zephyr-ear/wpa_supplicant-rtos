...
static int digest_vector(mbedtls_md_type_t md_type, size_t num_elem, const u8 *addr[], const size_t *len, u8 *mac)
{
    size_t i;
                                  ...
                                  mbedtls_operation_t operation)
{
    ...
    if (mbedtls_cipher_setkey(ctx, key, cipher_info->MBEDTLS_PRIVATE(key_bitlen), operation) != 0)
    {
...
static mbedtls_cipher_type_t alg_to_mbedtls_cipher(enum crypto_cipher_alg alg, size_t key_len)
{
    switch (alg)
    {
#ifdef MBEDTLS_ARC4_C
...
struct crypto_cipher *crypto_cipher_init(enum crypto_cipher_alg alg, const u8 *iv, const u8 *key, size_t key_len)
{
    ...
    if (crypto_init_cipher_ctx(&ctx->ctx_enc, cipher_info, iv, key, MBEDTLS_ENCRYPT) < 0)
    {
        goto cleanup;
    }

    if (crypto_init_cipher_ctx(&ctx->ctx_dec, cipher_info, iv, key, MBEDTLS_DECRYPT) < 0)
    {
        goto cleanup;
    }
...
int aes_wrap(const u8 *kek, size_t kek_len, int n, const u8 *plain, u8 *cipher)
{
    ...
    ret = mbedtls_nist_kw_setkey(&ctx, MBEDTLS_CIPHER_ID_AES, kek, kek_len * 8, 1);
    if (ret != 0)
    {
        return ret;
    }

    ret = mbedtls_nist_kw_wrap(&ctx, MBEDTLS_KW_MODE_KW, plain, n * 8, cipher, &olen, (n + 1) * 8);
...
int aes_unwrap(const u8 *kek, size_t kek_len, int n, const u8 *cipher, u8 *plain)
{
    ...
    ret = mbedtls_nist_kw_setkey(&ctx, MBEDTLS_CIPHER_ID_AES, kek, kek_len * 8, 0);
    if (ret != 0)
    {
        return ret;
    }

    ret = mbedtls_nist_kw_unwrap(&ctx, MBEDTLS_KW_MODE_KW, cipher, (n + 1) * 8, plain, &olen, (n * 8));
...
int crypto_mod_exp(const uint8_t *base,
                   size_t base_len,
                   const uint8_t *power,
                   size_t power_len,
                   ...
                   size_t *result_len)
{
    ...
    MBEDTLS_MPI_CHK(mbedtls_mpi_exp_mod(&bn_result, &bn_base, &bn_exp, &bn_modulus, &bn_rinv));
...
int pbkdf2_sha1(const char *passphrase, const u8 *ssid, size_t ssid_len, int iterations, u8 *buf, size_t buflen)
{
    ...
    ret = mbedtls_pkcs5_pbkdf2_hmac(&sha1_ctx, (const u8 *)passphrase, os_strlen(passphrase), ssid, ssid_len,
                                    iterations, 32, buf);
    if (ret != 0)
    {
        ret = -1;
               ...
               u8 *auth)
{
    ...
    ret = mbedtls_ccm_encrypt_and_tag(&ccm, plain_len, nonce, 13, aad, aad_len, plain, crypt, auth, M);
               ...
               u8 *plain)
{
    ...
    ret = mbedtls_ccm_star_auth_decrypt(&ccm, crypt_len, nonce, 13, aad, aad_len, crypt, plain, auth, M);
...
int rc4_skip(const u8 *key, size_t keylen, size_t skip, u8 *data, size_t data_len)
{
    ...
    while (skip >= sizeof(skip_buf_in))
    {
        ...
        if ((ret = mbedtls_arc4_crypt(&ctx, len, skip_buf_in, skip_buf_out)) != 0)
        {
