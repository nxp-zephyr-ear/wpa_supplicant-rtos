...
{
    return sha512_prf_bits(key, key_len, label, data, data_len, buf, buf_len * 8);
}
    ...
    const u8 *key, size_t key_len, const char *label, const u8 *data, size_t data_len, u8 *buf, size_t buf_len_bits)
{
    ...
    while (pos < buf_len)
    {
        ...
        {
            if (hmac_sha512_vector(key, key_len, 4, addr, len, &buf[pos]) < 0)
                return -1;
            pos += SHA512_MAC_LEN;
        }
        else
        {
            if (hmac_sha512_vector(key, key_len, 4, addr, len, hash) < 0)
                return -1;
