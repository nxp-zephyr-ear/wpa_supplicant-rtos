...
ASN1_SEQUENCE(ResponseBytes) = {
    ...
    ASN1_SIMPLE(ResponseBytes, response, ASN1_OCTET_STRING)} ASN1_SEQUENCE_END(ResponseBytes);

ASN1_SEQUENCE(OCSPResponse) = {
    ASN1_SIMPLE(OCSPResponse, responseStatus, ASN1_ENUMERATED),
    ASN1_EXP_OPT(OCSPResponse, responseBytes, ResponseBytes, 0)} ASN1_SEQUENCE_END(OCSPResponse);
...
ASN1_SEQUENCE(CertID) = {ASN1_SIMPLE(CertID, hashAlgorithm, X509_ALGOR),
...
ASN1_CHOICE(ResponderID) = {ASN1_EXP(ResponderID, value.byName, X509_NAME, 1),
                            ASN1_EXP(ResponderID, value.byKey, ASN1_OCTET_STRING, 2)} ASN1_CHOICE_END(ResponderID);

ASN1_SEQUENCE(RevokedInfo) = {
    ASN1_SIMPLE(RevokedInfo, revocationTime, ASN1_GENERALIZEDTIME),
    ASN1_EXP_OPT(RevokedInfo, revocationReason, ASN1_ENUMERATED, 0)} ASN1_SEQUENCE_END(RevokedInfo);

ASN1_CHOICE(CertStatus) = {ASN1_IMP(CertStatus, value.good, ASN1_NULL, 0),
                           ASN1_IMP(CertStatus, value.revoked, RevokedInfo, 1),
                           ASN1_IMP(CertStatus, value.unknown, ASN1_NULL, 2)} ASN1_CHOICE_END(CertStatus);

ASN1_SEQUENCE(SingleResponse) = {
    ASN1_SIMPLE(SingleResponse, certID, CertID), ASN1_SIMPLE(SingleResponse, certStatus, CertStatus),
    ASN1_SIMPLE(SingleResponse, thisUpdate, ASN1_GENERALIZEDTIME),
    ASN1_EXP_OPT(SingleResponse, nextUpdate, ASN1_GENERALIZEDTIME, 0),
    ASN1_EXP_SEQUENCE_OF_OPT(SingleResponse, singleExtensions, X509_EXTENSION, 1)} ASN1_SEQUENCE_END(SingleResponse);

ASN1_SEQUENCE(ResponseData) = {
    ASN1_EXP_OPT(ResponseData, version, ASN1_INTEGER, 0), ASN1_SIMPLE(ResponseData, responderID, ResponderID),
    ASN1_SIMPLE(ResponseData, producedAt, ASN1_GENERALIZEDTIME),
    ASN1_SEQUENCE_OF(ResponseData, responses, SingleResponse),
    ASN1_EXP_SEQUENCE_OF_OPT(ResponseData, responseExtensions, X509_EXTENSION, 1)} ASN1_SEQUENCE_END(ResponseData);
...
ASN1_SEQUENCE(BasicOCSPResponse) = {
    ...
    ASN1_EXP_SEQUENCE_OF_OPT(BasicOCSPResponse, certs, X509, 0)} ASN1_SEQUENCE_END(BasicOCSPResponse);

IMPLEMENT_ASN1_FUNCTIONS(BasicOCSPResponse);

#define sk_SingleResponse_num(sk) sk_num(CHECKED_CAST(_STACK *, STACK_OF(SingleResponse) *, sk))

#define sk_SingleResponse_value(sk, i) \
    ((SingleResponse *)sk_value(CHECKED_CAST(_STACK *, STACK_OF(SingleResponse) *, sk), (i)))
...
static int ocsp_resp_valid(ASN1_GENERALIZEDTIME *thisupd, ASN1_GENERALIZEDTIME *nextupd)
{
    time_t now, tmp;
    ...
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: OCSP response nextUpdate before thisUpdate");
        return 0;
    }
...
static int issuer_match(X509 *cert, X509 *issuer, CertID *certid)
{
    ...
    if (!dgst)
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: Could not find matching hash algorithm for OCSP");
        return -1;
    }
    ...
    if (!ikey || !EVP_Digest(ikey->data, ikey->length, md, &len, dgst, NULL) || !ASN1_OCTET_STRING_set(hash, md, len))
    {
        ASN1_OCTET_STRING_free(hash);
        return -1;
    }
...
enum ocsp_result check_ocsp_resp(SSL_CTX *ssl_ctx, SSL *ssl, X509 *cert, X509 *issuer, X509 *issuer_issuer)
{
    ...
    if (txt)
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: Searching OCSP response for peer certificate serialNumber: %s", txt);
        os_free(txt);
    }
    ...
    if (basic->certs)
    {
        ...
        for (i = 0; i < num; i++)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "OpenSSL: Could not add certificate to the untrusted stack");
            }
        }
    }
    ...
    if (ASN1_item_verify(ASN1_ITEM_rptr(ResponseData), basic->signatureAlgorithm, basic->signature,
    ...
    wpa_printf(MSG_DEBUG, "OpenSSL: Found OCSP signer certificate %s and verified BasicOCSPResponse signature", buf);
    ...
    if (ret <= 0)
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: Could not validate OCSP signer certificate");
        goto fail;
    }
    ...
    if (!signer_trusted)
    {
        ...
        if ((signer->ex_flags & EXFLAG_XKUSAGE) && (signer->ex_xkusage & XKU_OCSP_SIGN))
        {
    ...
    wpa_printf(MSG_DEBUG, "OpenSSL: OCSP version: %lu", ASN1_INTEGER_get(rd->version));
    ...
    if (num_resp == 0)
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: No OCSP SingleResponse within BasicOCSPResponse");
        result = OCSP_NO_RESPONSE;
        goto fail;
    }
    ...
    for (j = 0; j < num_resp; j++)
    {
        ...
        wpa_printf(MSG_DEBUG, "OpenSSL: OCSP SingleResponse %u/%u", j + 1, num_resp);
        ...
        cid2 = sresp->certID;
        if (j > 0 && (OBJ_cmp(cid1->hashAlgorithm->algorithm, cid2->hashAlgorithm->algorithm) != 0 ||
                      ASN1_OCTET_STRING_cmp(cid1->issuerNameHash, cid2->issuerNameHash) != 0 ||
                      ASN1_OCTET_STRING_cmp(cid1->issuerKeyHash, cid2->issuerKeyHash) != 0))
        {
            wpa_printf(MSG_DEBUG,
                       "OpenSSL: Different OCSP response issuer information between SingleResponse values within "
                       "BasicOCSPResponse");
            goto fail;
        }

        if (!matching_resp && issuer &&
            ASN1_INTEGER_cmp(sresp->certID->serialNumber, X509_get_serialNumber(cert)) == 0 &&
    ...
    if (!matching_resp)
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: Could not find OCSP response that matches the peer certificate");
        result = OCSP_NO_RESPONSE;
        goto fail;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: OCSP response indicated that the peer certificate has been revoked");
        result = OCSP_REVOKED;
        goto fail;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: OCSP response did not indicate good status");
        result = OCSP_NO_RESPONSE;
        goto fail;
    }
