...
static void wolfssl_reset_in_data(struct tls_in_data *in, const struct wpabuf *buf)
{
...
static void remove_session_cb(WOLFSSL_CTX *ctx, WOLFSSL_SESSION *sess)
{
    ...
    if (!buf)
        return;
    wpa_printf(MSG_DEBUG, "wolfSSL: Free application session data %p (sess %p)", buf, sess);
    wpabuf_free(buf);
...
void *tls_init(const struct tls_config *conf)
{
    ...
    if (conf->tls_session_lifetime > 0)
    {
        ...
        wolfSSL_CTX_set_session_cache_mode(ssl_ctx, SSL_SESS_CACHE_SERVER);
        wolfSSL_CTX_set_timeout(ssl_ctx, conf->tls_session_lifetime);
        wolfSSL_CTX_sess_set_remove_cb(ssl_ctx, remove_session_cb);
    }
    else
    {
        wolfSSL_CTX_set_session_cache_mode(ssl_ctx, SSL_SESS_CACHE_CLIENT);
    }
    ...
    {
        wpa_printf(MSG_ERROR, "wolfSSL: Failed to set cipher string '%s'", ciphers);
        tls_deinit(ssl_ctx);
        return NULL;
    }
...
static int tls_connection_dh(struct tls_connection *conn, const char *dh_file, const u8 *dh_blob, size_t blob_len)
{
    if (!dh_file && !dh_blob)
        return 0;
    ...
    if (dh_blob)
    {
        if (wolfSSL_SetTmpDH_buffer(conn->ssl, dh_blob, blob_len, SSL_FILETYPE_ASN1) < 0)
        {
    ...
    if (dh_file)
    {
        ...
        if (wolfSSL_SetTmpDH_file(conn->ssl, dh_file, SSL_FILETYPE_PEM) < 0)
        {
            ...
            if (wolfSSL_SetTmpDH_file(conn->ssl, dh_file, SSL_FILETYPE_ASN1) < 0)
            {
                                      ...
                                      size_t blob_len)
{
    ...
    if (client_cert_blob)
    {
        if (wolfSSL_use_certificate_chain_buffer_format(conn->ssl, client_cert_blob, blob_len, SSL_FILETYPE_ASN1) !=
            SSL_SUCCESS)
        {
    ...
    if (client_cert)
    {
        if (wolfSSL_use_certificate_chain_file(conn->ssl, client_cert) != SSL_SUCCESS)
        {
            wpa_printf(MSG_INFO, "SSL: use client cert PEM file failed");
            if (wolfSSL_use_certificate_chain_file_format(conn->ssl, client_cert, SSL_FILETYPE_ASN1) != SSL_SUCCESS)
            {
                                      ...
                                      size_t blob_len)
{
    ...
    if (private_key_blob)
    {
        if (wolfSSL_use_PrivateKey_buffer(conn->ssl, private_key_blob, blob_len, SSL_FILETYPE_ASN1) <= 0)
        {
    ...
    if (!ok && private_key)
    {
        if (wolfSSL_use_PrivateKey_file(conn->ssl, private_key, SSL_FILETYPE_PEM) <= 0)
        {
            wpa_printf(MSG_INFO, "SSL: use private key PEM file failed");
            if (wolfSSL_use_PrivateKey_file(conn->ssl, private_key, SSL_FILETYPE_ASN1) <= 0)
            {
...
static int tls_match_alt_subject_component(WOLFSSL_X509 *cert, int type, const char *value, size_t len)
{
...
static int domain_suffix_match(const char *val, size_t len, const char *match, size_t match_len, int full)
{
    size_t i;
...
static int tls_match_suffix_helper(WOLFSSL_X509 *cert, const char *match, size_t match_len, int full)
{
    WOLFSSL_GENERAL_NAME *gen;
    void *ext;
    int i;
    ...
    for (j = 0; ext && j < wolfSSL_sk_num(ext); j++)
    {
        ...
        if (!gen || gen->type != ASN_DNS_TYPE)
            continue;
        dns_name++;
        wpa_hexdump_ascii(MSG_DEBUG, "TLS: Certificate dNSName", wolfSSL_ASN1_STRING_data(gen->d.ia5),
                          wolfSSL_ASN1_STRING_length(gen->d.ia5));
        if (domain_suffix_match((const char *)wolfSSL_ASN1_STRING_data(gen->d.ia5),
                                wolfSSL_ASN1_STRING_length(gen->d.ia5), match, match_len, full) == 1)
        {
    ...
    for (;;)
    {
        ...
        i = wolfSSL_X509_NAME_get_index_by_NID(name, ASN_COMMON_NAME, i);
        if (i == -1)
            break;
                                   ...
                                   enum tls_fail_reason reason)
{
    ...
    ev.cert_fail.reason     = reason != TLS_FAIL_UNSPECIFIED ? reason : wolfssl_tls_fail_reason(err);
...
static void wolfssl_tls_cert_event(struct tls_connection *conn, WOLFSSL_X509 *err_cert, int depth, const char *subject)
{
    struct wpabuf *cert = NULL;
    union tls_event_data ev;
    struct tls_context *context = conn->context;
    ...
    for (i = 0; ext && i < wolfSSL_sk_num(ext); i++)
    {
        ...
        if (!gen || (gen->type != GEN_EMAIL && gen->type != GEN_DNS && gen->type != GEN_URI))
            continue;
        ...
        os_memcpy(pos, wolfSSL_ASN1_STRING_data(gen->d.ia5), wolfSSL_ASN1_STRING_length(gen->d.ia5));
...
static int tls_verify_cb(int preverify_ok, WOLFSSL_X509_STORE_CTX *x509_ctx)
{
    ...
    ssl   = wolfSSL_X509_STORE_CTX_get_ex_data(x509_ctx, wolfSSL_get_ex_data_X509_STORE_CTX_idx());
    wolfSSL_X509_NAME_oneline(wolfSSL_X509_get_subject_name(err_cert), buf, sizeof(buf));
    ...
    if (!preverify_ok && (conn->flags & TLS_CONN_DISABLE_TIME_CHECKS) &&
        (err == X509_V_ERR_CERT_HAS_EXPIRED || err == ASN_AFTER_DATE_E || err == ASN_BEFORE_DATE_E ||
         err == X509_V_ERR_CERT_NOT_YET_VALID))
    {
        wpa_printf(MSG_DEBUG, "wolfSSL: Ignore certificate validity time mismatch");
        preverify_ok = 1;
    }
    ...
    if (depth == 0 && conn->server_cert_only)
    {
        ...
        if (!cert)
        {
            wpa_printf(MSG_DEBUG, "wolfSSL: Could not fetch server certificate data");
            preverify_ok = 0;
        }
        else
        {
            ...
            else if (!preverify_ok)
            {
                 ...
                 */
                wpa_printf(MSG_DEBUG, "wolfSSL: Ignore validation issues for a pinned server certificate");
                preverify_ok = 1;
            }
            wpabuf_free(cert);
        }
    ...
    if (!preverify_ok)
    {
        wpa_printf(MSG_WARNING, "TLS: Certificate verification failed, error %d (%s) depth %d for '%s'", err, err_str,
                   depth, buf);
        wolfssl_tls_fail_event(conn, err_cert, err, depth, buf, err_str, TLS_FAIL_UNSPECIFIED);
        return preverify_ok;
    }

    wpa_printf(MSG_DEBUG, "TLS: %s - preverify_ok=%d err=%d (%s) ca_cert_verify=%d depth=%d buf='%s'", __func__,
    ...
    if (depth == 0 && match && os_strstr(buf, match) == NULL)
    {
        ...
        preverify_ok = 0;
        wolfssl_tls_fail_event(conn, err_cert, err, depth, buf, "Subject mismatch", TLS_FAIL_SUBJECT_MISMATCH);
    }
    else if (depth == 0 && altmatch && !tls_match_alt_subject(err_cert, altmatch))
    {
        wpa_printf(MSG_WARNING, "TLS: altSubjectName match '%s' not found", altmatch);
        preverify_ok = 0;
        wolfssl_tls_fail_event(conn, err_cert, err, depth, buf, "AltSubject mismatch", TLS_FAIL_ALTSUBJECT_MISMATCH);
    }
    else if (depth == 0 && suffix_match && !tls_match_suffix(err_cert, suffix_match, 0))
    {
        ...
        preverify_ok = 0;
        wolfssl_tls_fail_event(conn, err_cert, err, depth, buf, "Domain suffix mismatch",
    ...
    else if (depth == 0 && domain_match && !tls_match_suffix(err_cert, domain_match, 1))
    {
        ...
        preverify_ok = 0;
        wolfssl_tls_fail_event(conn, err_cert, err, depth, buf, "Domain mismatch", TLS_FAIL_DOMAIN_MISMATCH);
    }
    else
    {
    ...
    {
        wpa_printf(MSG_DEBUG, "wolfSSL: Reject server certificate on probe-only run");
        preverify_ok = 0;
        wolfssl_tls_fail_event(conn, err_cert, err, depth, buf, "Server certificate chain probe",
    ...
    if (depth == 0 && (conn->flags & TLS_CONN_REQUEST_OCSP) && preverify_ok)
    {
        ...
        {
            preverify_ok = 0;
            wolfssl_tls_fail_event(conn, err_cert, err, depth, buf, "certificate revoked", TLS_FAIL_REVOKED);
            if (err == X509_V_OK)
        ...
        {
            preverify_ok = 0;
            wolfssl_tls_fail_event(conn, err_cert, err, depth, buf, "bad certificate status response",
...
static int tls_connection_ca_cert(void *tls_ctx,
                                  struct tls_connection *conn,
                                  const char *ca_cert,
                                  ...
                                  const char *ca_path)
{
    ...
    if (ca_cert && os_strncmp(ca_cert, "hash://", 7) == 0)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "wolfSSL: Unsupported ca_cert hash value '%s'", ca_cert);
            return -1;
        }
        pos += 14;
        if (os_strlen(pos) != 32 * 2)
        {
            wpa_printf(MSG_DEBUG, "wolfSSL: Unexpected SHA256 hash length in ca_cert '%s'", ca_cert);
            return -1;
        }
        if (hexstr2bin(pos, conn->srv_cert_hash, 32) < 0)
        {
            wpa_printf(MSG_DEBUG, "wolfSSL: Invalid SHA256 hash value in ca_cert '%s'", ca_cert);
            return -1;
        }
        conn->server_cert_only = 1;
        wpa_printf(MSG_DEBUG, "wolfSSL: Checking only server certificate match");
        return 0;
#else  /* CONFIG_SHA256 */
    ...
    if (ca_cert_blob)
    {
        if (wolfSSL_CTX_load_verify_buffer(ctx, ca_cert_blob, blob_len, SSL_FILETYPE_ASN1) != SSL_SUCCESS)
        {
    ...
    if (ca_cert || ca_path)
    {
        ...
        if (wolfSSL_CTX_load_verify_locations(ctx, ca_cert, ca_path) != SSL_SUCCESS)
        {
            ...
            if (wolfSSL_CTX_der_load_verify_locations(ctx, ca_cert, SSL_FILETYPE_ASN1) != SSL_SUCCESS)
            {
...
int tls_connection_set_params(void *tls_ctx, struct tls_connection *conn, const struct tls_connection_params *params)
{
    ...
    if (tls_connection_ca_cert(tls_ctx, conn, params->ca_cert, params->ca_cert_blob, params->ca_cert_blob_len,
                               params->ca_path) < 0)
    {
    ...
    if (tls_connection_client_cert(conn, params->client_cert, params->client_cert_blob, params->client_cert_blob_len) <
        0)
    {
    ...
    if (tls_connection_private_key(tls_ctx, conn, params->private_key, params->private_key_passwd,
                                   params->private_key_blob, params->private_key_blob_len) < 0)
    {
        wpa_printf(MSG_INFO, "Error setting private key");
        return -1;
    }

    if (tls_connection_dh(conn, params->dh_file, params->dh_blob, params->dh_blob_len) < 0)
    {
        wpa_printf(MSG_INFO, "Error setting DH");
        return -1;
    }

    if (params->openssl_ciphers && wolfSSL_set_cipher_list(conn->ssl, params->openssl_ciphers) != 1)
    {
        wpa_printf(MSG_INFO, "wolfSSL: Failed to set cipher string '%s'", params->openssl_ciphers);
        return -1;
    }
    ...
    {
        if (wolfSSL_UseOCSPStapling(conn->ssl, WOLFSSL_CSR_OCSP, WOLFSSL_CSR_OCSP_USE_NONCE) != SSL_SUCCESS)
            return -1;
    ...
    if (params->flags & TLS_CONN_REQUEST_OCSP)
    {
        if (wolfSSL_UseOCSPStaplingV2(conn->ssl, WOLFSSL_CSR2_OCSP_MULTI, 0) != SSL_SUCCESS)
            return -1;
        wolfSSL_CTX_EnableOCSP(tls_ctx, 0);
    }
    ...
    {
        wpa_printf(MSG_INFO, "wolfSSL: No OCSP support included - reject configuration");
        return -1;
    }
    if (params->flags & TLS_CONN_REQUEST_OCSP)
    {
        wpa_printf(MSG_DEBUG, "wolfSSL: No OCSP support included - allow optional OCSP case to continue");
    }
#endif /* HAVE_OCSP */
#endif /* !HAVE_CERTIFICATE_STATUS_REQUEST && \
...
static int tls_global_client_cert(void *ssl_ctx, const char *client_cert)
{
    ...
    if (wolfSSL_CTX_use_certificate_chain_file_format(ctx, client_cert, SSL_FILETYPE_ASN1) != SSL_SUCCESS &&
        wolfSSL_CTX_use_certificate_chain_file(ctx, client_cert) != SSL_SUCCESS)
    {
...
static int tls_global_private_key(void *ssl_ctx, const char *private_key, const char *private_key_passwd)
{
    WOLFSSL_CTX *ctx = ssl_ctx;
    char *passwd     = NULL;
    int ret          = 0;
    ...
    if (wolfSSL_CTX_use_PrivateKey_file(ctx, private_key, SSL_FILETYPE_ASN1) != 1 &&
        wolfSSL_CTX_use_PrivateKey_file(ctx, private_key, SSL_FILETYPE_PEM) != 1)
    {
...
static int tls_global_dh(void *ssl_ctx, const char *dh_file, const u8 *dh_blob, size_t blob_len)
{
    WOLFSSL_CTX *ctx = ssl_ctx;
    ...
    if (dh_blob)
    {
        if (wolfSSL_CTX_SetTmpDH_buffer(ctx, dh_blob, blob_len, SSL_FILETYPE_ASN1) < 0)
        {
    ...
    if (dh_file)
    {
        if (wolfSSL_CTX_SetTmpDH_file(ctx, dh_file, SSL_FILETYPE_PEM) < 0)
        {
            ...
            if (wolfSSL_CTX_SetTmpDH_file(ctx, dh_file, SSL_FILETYPE_ASN1) < 0)
            {
...
int ocsp_status_cb(
    void *unused, const char *url, int url_sz, unsigned char *request, int request_sz, unsigned char **response)
{
    size_t len;
    ...
    if (!url)
    {
        wpa_printf(MSG_DEBUG, "wolfSSL: OCSP status callback - no response configured");
        *response = NULL;
        return 0;
    }
    ...
    *response = (unsigned char *)os_readfile(url, &len);
    if (!*response)
    {
        wpa_printf(MSG_DEBUG, "wolfSSL: OCSP status callback - could not read response file");
        return -1;
    }
    wpa_printf(MSG_DEBUG, "wolfSSL: OCSP status callback - send cached response");
    return len;
}
...
int tls_global_set_params(void *tls_ctx, const struct tls_connection_params *params)
{
    ...
    {
        wpa_printf(MSG_INFO, "SSL: Failed to load ca cert file '%s'", params->ca_cert);
        return -1;
    }

    if (tls_global_client_cert(tls_ctx, params->client_cert) < 0)
    {
        wpa_printf(MSG_INFO, "SSL: Failed to load client cert file '%s'", params->client_cert);
        return -1;
    }

    if (tls_global_private_key(tls_ctx, params->private_key, params->private_key_passwd) < 0)
    {
        wpa_printf(MSG_INFO, "SSL: Failed to load private key file '%s'", params->private_key);
        return -1;
    }

    if (tls_global_dh(tls_ctx, params->dh_file, params->dh_blob, params->dh_blob_len) < 0)
    {
        wpa_printf(MSG_INFO, "SSL: Failed to load DH file '%s'", params->dh_file);
        return -1;
    }

    if (params->openssl_ciphers && wolfSSL_CTX_set_cipher_list(tls_ctx, params->openssl_ciphers) != 1)
    {
        wpa_printf(MSG_INFO, "wolfSSL: Failed to set cipher string '%s'", params->openssl_ciphers);
        return -1;
    }
    ...
    {
        wpa_printf(MSG_INFO, "wolfSSL: openssl_ecdh_curves not supported");
        return -1;
    }
    ...
    {
        wolfSSL_CTX_SetOCSP_OverrideURL(tls_ctx, params->ocsp_stapling_response);
        wolfSSL_CTX_SetOCSP_Cb(tls_ctx, ocsp_status_cb, ocsp_resp_free_cb, NULL);
    }
#endif /* HAVE_OCSP */
                              ...
                              size_t session_ctx_len)
{
    ...
    {
        conn->ca_cert_verify = 1;
        wolfSSL_set_verify(conn->ssl, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, tls_verify_cb);
    }
    else
    {
        conn->ca_cert_verify = 0;
...
static struct wpabuf *wolfssl_handshake(struct tls_connection *conn, const struct wpabuf *in_data, int server)
{
    int res;
    ...
    if (res != 1)
    {
        ...
        else
        {
            ...
            wpa_printf(MSG_DEBUG, "SSL: wolfSSL_connect - failed %s", wolfSSL_ERR_error_string(err, msg));
            conn->failed++;
        }
    }
...
static struct wpabuf *wolfssl_get_appl_data(struct tls_connection *conn, size_t max_len)
{
    int res;
    ...
    res = wolfSSL_read(conn->ssl, wpabuf_mhead(appl_data), wpabuf_size(appl_data));
    if (res < 0)
    {
        int err = wolfSSL_get_error(conn->ssl, res);
        ...
        else
        {
            ...
            wpa_printf(MSG_DEBUG, "Failed to read possible Application Data %s", wolfSSL_ERR_error_string(err, msg));
        }
                                                   ...
                                                   int server)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "wolfSSL: Handshake finished - resumed=%d", tls_connection_resumed(NULL, conn));
...
struct wpabuf *tls_connection_encrypt(void *tls_ctx, struct tls_connection *conn, const struct wpabuf *in_data)
{
    ...
    if (res < 0)
    {
        ...
        wpa_printf(MSG_INFO, "Encryption failed - SSL_write: %s", wolfSSL_ERR_error_string(err, msg));
        return NULL;
    }
...
{
    return os_snprintf(buf, buf_len, "wolfSSL build=%s run=%s", WOLFSSL_VERSION, wolfSSL_lib_version());
}
...
int tls_connection_get_random(void *ssl_ctx, struct tls_connection *conn, struct tls_random *keys)
{
    ...
    keys->client_random_len = wolfSSL_get_client_random(ssl, conn->client_random, sizeof(conn->client_random));
    keys->server_random     = conn->server_random;
    keys->server_random_len = wolfSSL_get_server_random(ssl, conn->server_random, sizeof(conn->server_random));
...
int tls_connection_get_eap_fast_key(void *tls_ctx, struct tls_connection *conn, u8 *out, size_t out_len)
{
    ...
    skip = 2 * (wolfSSL_GetKeySize(ssl) + wolfSSL_GetHmacSize(ssl) + wolfSSL_GetIVSize(ssl));
    ...
    wolfSSL_get_keys(ssl, &master_key, &master_key_len, &server_random, &server_len, &client_random, &client_len);
    ...
    {
        tls_prf_sha256(master_key, master_key_len, "key expansion", seed, sizeof(seed), _out, skip + out_len);
        ret = 0;
    }
    else
    {
        ret = tls_prf_sha1_md5(master_key, master_key_len, "key expansion", seed, sizeof(seed), _out, skip + out_len);
    }
...
static int tls_sess_sec_cb(WOLFSSL *s, void *secret, int *secret_len, void *arg)
{
    ...
    if (wolfSSL_get_client_random(s, client_random, sizeof(client_random)) == 0 ||
        wolfSSL_get_server_random(s, server_random, sizeof(server_random)) == 0 ||
    ...
    ret = conn->session_ticket_cb(conn->session_ticket_cb_ctx, conn->session_ticket, ticket_len, client_random,
                                         ...
                                         void *ctx)
{
    ...
    if (cb)
    {
        if (wolfSSL_set_session_secret_cb(conn->ssl, tls_sess_sec_cb, conn) != 1)
            return -1;
    }
    else
    {
...
void tls_connection_remove_session(struct tls_connection *conn)
{
    ...
    wpa_printf(MSG_DEBUG, "wolfSSL: Removed cached session to disable session resumption");
}
