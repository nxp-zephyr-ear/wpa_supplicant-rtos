...
void *tls_init(const struct tls_config *conf)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "GnuTLS: Library version %s (runtime) - %s (build)", gnutls_check_version(NULL),
                   GNUTLS_VERSION);
    }
...
int tls_connection_set_params(void *tls_ctx, struct tls_connection *conn, const struct tls_connection_params *params)
{
    ...
    if (params->flags & (TLS_CONN_DISABLE_TLSv1_0 | TLS_CONN_DISABLE_TLSv1_1 | TLS_CONN_DISABLE_TLSv1_2))
    {
        os_snprintf(prio_buf, sizeof(prio_buf), "NORMAL:-VERS-SSL3.0%s%s%s",
                    params->flags & TLS_CONN_DISABLE_TLSv1_0 ? ":-VERS-TLS1.0" : "",
                    params->flags & TLS_CONN_DISABLE_TLSv1_1 ? ":-VERS-TLS1.1" : "",
                    params->flags & TLS_CONN_DISABLE_TLSv1_2 ? ":-VERS-TLS1.2" : "");
        prio = prio_buf;
    }
    ...
    if (params->openssl_ciphers)
    {
        ...
        else if (os_strcmp(params->openssl_ciphers, "SUITEB192") == 0)
        {
            prio = "SUITEB192";
        }
        else if ((params->flags & TLS_CONN_SUITEB) &&
                 os_strcmp(params->openssl_ciphers, "ECDHE-RSA-AES256-GCM-SHA384") == 0)
        {
            prio = "NONE:+VERS-TLS1.2:+AEAD:+ECDHE-RSA:+AES-256-GCM:+SIGN-RSA-SHA384:+CURVE-SECP384R1:+COMP-NULL";
        }
        else if (os_strcmp(params->openssl_ciphers, "ECDHE-RSA-AES256-GCM-SHA384") == 0)
        {
            prio = "NONE:+VERS-TLS1.2:+AEAD:+ECDHE-RSA:+AES-256-GCM:+SIGN-RSA-SHA384:+CURVE-SECP384R1:+COMP-NULL";
        }
        else if (os_strcmp(params->openssl_ciphers, "DHE-RSA-AES256-GCM-SHA384") == 0)
        {
            prio =
                "NONE:+VERS-TLS1.2:+AEAD:+DHE-RSA:+AES-256-GCM:+SIGN-RSA-SHA384:+CURVE-SECP384R1:+COMP-NULL:%PROFILE_"
                "HIGH";
        }
        else if (os_strcmp(params->openssl_ciphers, "ECDHE-ECDSA-AES256-GCM-SHA384") == 0)
        {
    ...
    {
        prio =
            "NONE:+VERS-TLS1.2:+AEAD:+ECDHE-ECDSA:+ECDHE-RSA:+DHE-RSA:+AES-256-GCM:+SIGN-RSA-SHA384:+CURVE-SECP384R1:+"
            "COMP-NULL:%PROFILE_HIGH";
    }
    ...
    if (params->ca_cert)
    {
        ...
        if (ret < 0)
        {
            wpa_printf(MSG_DEBUG, "GnuTLS: Failed to read CA cert '%s' in DER format (%s) - try in PEM format",
            ...
            if (ret < 0)
            {
                wpa_printf(MSG_DEBUG, "Failed to read CA cert '%s' in PEM format: %s", params->ca_cert,
                           gnutls_strerror(ret));
                return -1;
            }
            wpa_printf(MSG_DEBUG, "GnuTLS: Successfully read CA cert '%s' in PEM format", params->ca_cert);
        }
        else
        {
            wpa_printf(MSG_DEBUG, "GnuTLS: Successfully read CA cert '%s' in DER format", params->ca_cert);
        }
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "GnuTLS: Try to parse client cert '%s' and key '%s' in DER format", params->client_cert,
                   params->private_key);
#if GNUTLS_VERSION_NUMBER >= 0x03010b
        ret = gnutls_certificate_set_x509_key_file2(conn->xcred, params->client_cert, params->private_key,
                                                    GNUTLS_X509_FMT_DER, params->private_key_passwd, 0);
#else
        /* private_key_passwd not (easily) supported here */
        ret = gnutls_certificate_set_x509_key_file(conn->xcred, params->client_cert, params->private_key,
                                                   GNUTLS_X509_FMT_DER);
#endif
        if (ret < 0)
        {
            wpa_printf(MSG_DEBUG, "GnuTLS: Failed to read client cert/key in DER format (%s) - try in PEM format",
                       gnutls_strerror(ret));
#if GNUTLS_VERSION_NUMBER >= 0x03010b
            ret = gnutls_certificate_set_x509_key_file2(conn->xcred, params->client_cert, params->private_key,
            ...
            wpa_printf(MSG_DEBUG, "GnuTLS: Successfully read client cert/key in PEM format");
        }
        else
        {
            wpa_printf(MSG_DEBUG, "GnuTLS: Successfully read client cert/key in DER format");
        }
    }
    else if (params->private_key)
    {
        ...
        wpa_printf(MSG_DEBUG, "GnuTLS: Try to parse client cert/key '%s'in PKCS#12 DER format", params->private_key);
        ret = gnutls_certificate_set_x509_simple_pkcs12_file(conn->xcred, params->private_key, GNUTLS_X509_FMT_DER,
    ...
    else if (params->client_cert_blob && params->private_key_blob)
    {
...
#if GNUTLS_VERSION_NUMBER >= 0x03010b
        ret = gnutls_certificate_set_x509_key_mem2(conn->xcred, &cert, &key, GNUTLS_X509_FMT_DER,
        ...
        if (ret < 0)
        {
            wpa_printf(MSG_DEBUG,
                       ...
                       gnutls_strerror(ret));
#if GNUTLS_VERSION_NUMBER >= 0x03010b
            ret = gnutls_certificate_set_x509_key_mem2(conn->xcred, &cert, &key, GNUTLS_X509_FMT_PEM,
                                                       params->private_key_passwd, 0);
#else
            /* private_key_passwd not (easily) supported here */
    ...
    else if (params->private_key_blob)
    {
        ...
        /* Try to load in PKCS#12 format */
        ret = gnutls_certificate_set_x509_simple_pkcs12_mem(conn->xcred, &key, GNUTLS_X509_FMT_DER,
                                                            params->private_key_passwd);
        if (ret != 0)
        {
            wpa_printf(MSG_DEBUG,
...
#if GNUTLS_VERSION_NUMBER >= 0x030103
static int server_ocsp_status_req(gnutls_session_t session, void *ptr, gnutls_datum_t *resp)
{
    ...
    if (!cached)
    {
        wpa_printf(MSG_DEBUG, "GnuTLS: OCSP status callback - could not read response file (%s)",
...
int tls_global_set_params(void *tls_ctx, const struct tls_connection_params *params)
{
    ...
    if (params->client_cert && params->private_key)
    {
        ...
        ret = gnutls_certificate_set_x509_key_file(global->xcred, params->client_cert, params->private_key,
                                                   GNUTLS_X509_FMT_DER);
        if (ret < 0)
        {
            wpa_printf(MSG_DEBUG,
...
#ifdef PKCS12_FUNCS
        /* Try to load in PKCS#12 format */
        ret = gnutls_certificate_set_x509_simple_pkcs12_file(global->xcred, params->private_key, GNUTLS_X509_FMT_DER,
                                                             params->private_key_passwd);
        if (ret != 0)
        {
            wpa_printf(MSG_DEBUG,
    ...
    gnutls_certificate_set_ocsp_status_request_function(global->xcred, server_ocsp_status_req, global);
#endif /* 3.1.3 */
                              ...
                              size_t out_len)
{
    ...
    return gnutls_prf_rfc5705(conn->session, os_strlen(label), label, context_len, (const char *)context, out_len,
                              (char *)out);
#else  /* 3.4.4 */
    if (context)
        return -1;
    return gnutls_prf(conn->session, os_strlen(label), label, 0 /* client_random first */, 0, NULL, out_len,
                      (char *)out);
#endif /* 3.4.4 */
}
...
static int tls_match_suffix_helper(gnutls_x509_crt_t cert, const char *match, int full)
{
    int res = -1;

#if GNUTLS_VERSION_NUMBER >= 0x030300
    if (full)
        res = gnutls_x509_crt_check_hostname2(cert, match, GNUTLS_VERIFY_DO_NOT_ALLOW_WILDCARDS);
#endif /* >= 3.3.0 */
    if (res == -1)
        res = gnutls_x509_crt_check_hostname(cert, match);

    wpa_printf(MSG_DEBUG, "TLS: Match domain against %s%s --> res=%d", full ? "" : "suffix ", match, res);
    return res;
}

static int tls_match_suffix(gnutls_x509_crt_t cert, const char *match, int full)
{
...
static int tls_connection_verify_peer(gnutls_session_t session)
{
    ...
    for (i = 0; i < num_certs; i++)
    {
        ...
        if (i == 0)
        {
            if (conn->suffix_match && !tls_match_suffix(cert, conn->suffix_match, 0))
            {
...
#if GNUTLS_VERSION_NUMBER >= 0x030300
            if (conn->domain_match && !tls_match_suffix(cert, conn->domain_match, 1))
            {
        ...
        if (!conn->disable_time_checks && (gnutls_x509_crt_get_expiration_time(cert) < now.sec ||
...
int tls_get_cipher(void *ssl_ctx, struct tls_connection *conn, char *buf, size_t buflen)
{
    ...
    if (kx == GNUTLS_KX_RSA)
        ...
        res = os_snprintf(buf, buflen, "%s-%s-%s", kx_str, cipher_str, mac_str);
