...
#if !defined(CONFIG_FIPS) && (defined(EAP_FAST) || defined(EAP_FAST_DYNAMIC) || defined(EAP_SERVER_FAST))
...
static size_t SSL_SESSION_get_master_key(const SSL_SESSION *session, unsigned char *out, size_t outlen)
{
    if (!session || session->master_key_length < 0 || (size_t)session->master_key_length > outlen)
        return 0;
...
static const CERT_CONTEXT *cryptoapi_find_cert(const char *name, DWORD store)
{
    ...
    if (strncmp(name, "cert://", 7) == 0)
    {
        ...
        ret = CertFindCertificateInStore(cs, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0, CERT_FIND_SUBJECT_STR, wbuf,
                                         NULL);
    }
    else if (strncmp(name, "hash://", 7) == 0)
    {
        ...
        if (buf && hexstr2bin(hash, buf, len) == 0)
        {
            ...
            blob.pbData = buf;
            ret =
                CertFindCertificateInStore(cs, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0, CERT_FIND_HASH, &blob, NULL);
        }
        os_free(buf);
    }
...
static int tls_cryptoapi_cert(SSL *ssl, const char *name)
{
    ...
    if (!CryptAcquireCertificatePrivateKey(priv->cert, CRYPT_ACQUIRE_COMPARE_KEY_FLAG, NULL, &priv->crypt_prov,
                                           &priv->key_spec, &priv->free_crypt_prov))
    {
        cryptoapi_error(
            "Failed to acquire a private key for the "
            "certificate");
        goto err;
    }
...
static int tls_engine_load_dynamic_pkcs11(const char *pkcs11_so_path, const char *pkcs11_module_path)
{
    ...
    const char *pre_cmd[]  = {"SO_PATH", NULL /* pkcs11_so_path */, "ID", NULL /* engine_id */, "LIST_ADD", "1",
                             /* "NO_VCHECK", "1", */
                             "LOAD", NULL, NULL, NULL};
    const char *post_cmd[] = {"MODULE_PATH", NULL /* pkcs11_module_path */, NULL, NULL};
...
static int tls_engine_load_dynamic_opensc(const char *opensc_so_path)
{
    ...
    const char *pre_cmd[] = {
        "SO_PATH", NULL /* opensc_so_path */, "ID", NULL /* engine_id */, "LIST_ADD", "1", "LOAD", NULL, NULL, NULL};
...
static void remove_session_cb(SSL_CTX *ctx, SSL_SESSION *sess)
{
    ...
    if (!buf)
        return;
    wpa_printf(MSG_DEBUG, "OpenSSL: Free application session data %p (sess %p)", buf, sess);
    wpabuf_free(buf);
...
void *tls_init(const struct tls_config *conf)
{
    ...
    if (tls_ex_idx_session < 0)
    {
        tls_ex_idx_session = SSL_SESSION_get_ex_new_index(0, NULL, NULL, NULL, NULL);
        if (tls_ex_idx_session < 0)
        {
            tls_deinit(data);
            return NULL;
    ...
    if (conf && (conf->opensc_engine_path || conf->pkcs11_engine_path || conf->pkcs11_module_path))
    {
        if (tls_engine_load_dynamic_opensc(conf->opensc_engine_path) ||
            tls_engine_load_dynamic_pkcs11(conf->pkcs11_engine_path, conf->pkcs11_module_path))
        {
            tls_deinit(data);
            return NULL;
        }
...
static int tls_is_pin_error(unsigned int err)
{
    return ERR_GET_LIB(err) == ERR_LIB_PKCS11 &&
           (ERR_GET_REASON(err) == CKR_PIN_INCORRECT || ERR_GET_REASON(err) == CKR_PIN_INVALID ||
...
#error "This code depends on OPENSSL_NO_ENGINE being defined by BoringSSL."
    ...
    if (!conn->private_key)
    {
        wpa_printf(MSG_ERROR, "ENGINE: cannot load private key with id '%s' [%s]", key_id,
...
static const char *openssl_handshake_type(int content_type, const u8 *buf, size_t len)
{
...
static void check_server_hello(struct tls_connection *conn, const u8 *pos, const u8 *end)
{
    ...
    wpa_printf(MSG_DEBUG, "OpenSSL: Server selected cipher suite 0x%x", conn->cipher_suite);
}

static void check_server_key_exchange(SSL *ssl, struct tls_connection *conn, const u8 *pos, const u8 *end)
{
    size_t payload_len;
    u16 dh_len;
    BIGNUM *p;
    ...
    wpa_printf(MSG_DEBUG, "OpenSSL: Server DH prime length: %d bits", conn->server_dh_prime_len);
}
...
static void tls_msg_cb(int write_p, int version, int content_type, const void *buf, size_t len, SSL *ssl, void *arg)
{
    ...
    if (write_p == 2)
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: session ver=0x%x content_type=%d", version, content_type);
        wpa_hexdump_key(MSG_MSGDUMP, "OpenSSL: Data", buf, len);
        return;
    }

    wpa_printf(MSG_DEBUG, "OpenSSL: %s ver=0x%x content_type=%d (%s/%s)", write_p ? "TX" : "RX", version, content_type,
               openssl_content_type(content_type), openssl_handshake_type(content_type, buf, len));
...
struct tls_connection *tls_connection_init(void *ssl_ctx)
{
    ...
    if (data->crl_reload_interval > 0 && os_get_reltime(&now) == 0 &&
        os_reltime_expired(&now, &data->crl_last_reload, data->crl_reload_interval))
    {
        wpa_printf(MSG_INFO, "OpenSSL: Flushing X509 store with ca_cert file");
        new_cert_store = tls_crl_cert_reload(data->ca_cert, data->check_crl);
        if (!new_cert_store)
        {
            wpa_printf(MSG_ERROR, "OpenSSL: Error replacing X509 store with ca_cert file");
        }
        else
        {
            /* Replace old store */
...
char *tls_connection_peer_serial_num(void *tls_ctx, struct tls_connection *conn)
{
    ...
    if (!serial_num)
        return NULL;
    wpa_snprintf_hex_uppercase(serial_num, len, ASN1_STRING_get0_data(ser), ASN1_STRING_length(ser));
    return serial_num;
}
...
static int tls_match_altsubject(X509 *cert, const char *match)
{
    ...
    do
    {
        ...
        while (end)
        {
            if (os_strncmp(end + 1, "EMAIL:", 6) == 0 || os_strncmp(end + 1, "DNS:", 4) == 0 ||
...
static int domain_suffix_match(const u8 *val, size_t len, const char *match, size_t match_len, int full)
{
    size_t i;
...
static int get_dn_field_index(const struct tls_dn_field_order_cnt *dn_cnt, int nid)
{
    switch (nid)
    {
        case NID_commonName:
        ...
        case NID_pkcs9_emailAddress:
            return dn_cnt->email;
        default:
            wpa_printf(MSG_ERROR, "TLS: Unknown NID '%d' in check_cert_subject", nid);
            return -1;
    }
}
 ...
 */
static int match_dn_field(
    const X509 *cert, int nid, const char *field, const char *value, const struct tls_dn_field_order_cnt *dn_cnt)
{
    ...
    for (i = -1; (i = X509_NAME_get_index_by_NID(name, nid, i)) > -1;)
    {
        ...
        if (len > 0 && value[len - 1] == '*')
        {
             ...
             * length. */
            ret = ASN1_STRING_length(cn) >= len - 1 && os_memcmp(ASN1_STRING_get0_data(cn), value, len - 1) == 0;
        }
        else
        {
            /* Compare actual certificate DN field value with
             * configuration DN field value */
            ret = ASN1_STRING_length(cn) == len && os_memcmp(ASN1_STRING_get0_data(cn), value, len) == 0;
        }
        if (!ret)
        {
            wpa_printf(MSG_ERROR, "OpenSSL: Failed to match %s '%s' with certificate DN field value '%s'", field, value,
 ...
 * Returns: 1 on success and 0 on failure
 */
static int get_value_from_field(const X509 *cert, char *field_str, struct tls_dn_field_order_cnt *dn_cnt)
{
    int nid;
    ...
    {
        wpa_printf(MSG_ERROR, "TLS: Unknown field '%s' in check_cert_subject", name);
        return 0;
    }
    ...
    if (!value)
    {
        wpa_printf(MSG_ERROR, "TLS: Distinguished Name field '%s' value is not defined in check_cert_subject", name);
        return 0;
    }
...
static int tls_match_dn_field(X509 *cert, const char *match)
{
    ...
    while ((token = cstr_token(match, "/", &last)))
    {
        ...
        {
            wpa_printf(MSG_ERROR, "OpenSSL: Too long DN matching field value in '%s'", match);
            return 0;
        }
        ...
        {
            wpa_printf(MSG_DEBUG, "OpenSSL: No match for DN '%s'", field);
            return 0;
        }
    }
...
#ifndef CONFIG_NATIVE_WINDOWS
static int tls_match_suffix_helper(X509 *cert, const char *match, size_t match_len, int full)
{
    GENERAL_NAME *gen;
    void *ext;
    int i;
    ...
    for (j = 0; ext && j < sk_GENERAL_NAME_num(ext); j++)
    {
        ...
        if (domain_suffix_match(gen->d.dNSName->data, gen->d.dNSName->length, match, match_len, full) == 1)
        {
    ...
    for (;;)
    {
        ...
        if (domain_suffix_match(cn->data, cn->length, match, match_len, full) == 1)
        {
...
static void openssl_tls_cert_event(struct tls_connection *conn, X509 *err_cert, int depth, const char *subject)
{
    ...
    os_memset(&ev, 0, sizeof(ev));
    if (conn->cert_probe || (conn->flags & TLS_CONN_EXT_CERT_CHECK) || context->cert_in_cb)
    {
        cert              = get_x509_cert(err_cert);
    ...
    if (ser)
    {
        wpa_snprintf_hex_uppercase(serial_num, sizeof(serial_num), ASN1_STRING_get0_data(ser), ASN1_STRING_length(ser));
...
static int tls_verify_cb(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
    ...
    if (!preverify_ok && !conn->data->check_crl_strict &&
        (err == X509_V_ERR_CRL_HAS_EXPIRED || err == X509_V_ERR_CRL_NOT_YET_VALID))
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: Ignore certificate validity CRL time mismatch");
        preverify_ok = 1;
    }
    ...
    if (!preverify_ok)
    {
        if (depth > 0)
        {
            ...
            if (chain && sk_X509_num(chain) > 0)
            {
                ...
                X509_NAME_oneline(X509_get_subject_name(cert), buf2, sizeof(buf2));
    ...
    if (check_cert_subject)
    {
        if (depth == 0 && !tls_match_dn_field(err_cert, check_cert_subject))
        {
            preverify_ok = 0;
            openssl_tls_fail_event(conn, err_cert, err, depth, buf, "Distinguished Name", TLS_FAIL_DN_MISMATCH);
        }
    }
    ...
    else if (depth == 0 && suffix_match && !tls_match_suffix(err_cert, suffix_match, 0))
    {
        ...
        preverify_ok = 0;
        openssl_tls_fail_event(conn, err_cert, err, depth, buf, "Domain suffix mismatch",
    ...
    if (conn->cert_probe && preverify_ok && depth == 0)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   "on probe-only run");
        preverify_ok = 0;
        openssl_tls_fail_event(conn, err_cert, err, depth, buf, "Server certificate chain probe",
    ...
    if (conn->flags & TLS_CONN_SUITEB)
    {
        ...
        if (len >= 0)
        {
            wpa_printf(MSG_DEBUG, "OpenSSL: RSA modulus size: %d bits", len);
            if (len < 3072)
            {
                preverify_ok = 0;
                openssl_tls_fail_event(conn, err_cert, err, depth, buf, "Insufficient RSA modulus size",
    ...
    if (depth == 0 && (conn->flags & TLS_CONN_REQUEST_OCSP) && preverify_ok)
    {
        ...
        res = check_ocsp_resp(conn->ssl_ctx, conn->ssl, err_cert, conn->peer_issuer, conn->peer_issuer_issuer);
        if (res == OCSP_REVOKED)
        {
            preverify_ok = 0;
            openssl_tls_fail_event(conn, err_cert, err, depth, buf, "certificate revoked", TLS_FAIL_REVOKED);
            if (err == X509_V_OK)
        ...
        {
            preverify_ok = 0;
            openssl_tls_fail_event(conn, err_cert, err, depth, buf, "bad certificate status response",
                                  ...
                                  const char *ca_path)
{
    ...
    if (ca_cert_blob)
    {
        X509 *cert = d2i_X509(NULL, (const unsigned char **)&ca_cert_blob, ca_cert_blob_len);
        if (cert == NULL)
        {
    ...
    {
        if (tls_add_ca_from_keystore(SSL_CTX_get_cert_store(ssl_ctx), &ca_cert[11]) < 0)
            return -1;
        SSL_set_verify(conn->ssl, SSL_VERIFY_PEER, tls_verify_cb);
        return 0;
    }
    ...
    if (ca_cert && os_strncmp("keystores://", ca_cert, 12) == 0)
    {
        ...
        {
            if (tls_add_ca_from_keystore_encoded(SSL_CTX_get_cert_store(ssl_ctx), alias))
            {
                wpa_printf(MSG_WARNING, "OpenSSL: %s - Failed to add ca_cert %s from keystore", __func__, alias);
                rc = -1;
                break;
            }
        }
...
static int suiteb_cert_cb(SSL *ssl, void *arg)
{
    ...
    wpa_printf(MSG_DEBUG, "OpenSSL: Server DH prime length (%d bits) not sufficient for Suite B RSA - reject handshake",
...
static int tls_set_conn_flags(struct tls_connection *conn, unsigned int flags, const char *openssl_ciphers)
{
    SSL *ssl = conn->ssl;
...
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
    if (flags & (TLS_CONN_ENABLE_TLSv1_0 | TLS_CONN_ENABLE_TLSv1_1 | TLS_CONN_ENABLE_TLSv1_2))
    {
        int version = 0;
        ...
        if (!version)
        {
            wpa_printf(MSG_DEBUG, "OpenSSL: Invalid TLS version configuration");
            return -1;
        }

        if (SSL_set_min_proto_version(ssl, version) != 1)
        {
            wpa_printf(MSG_DEBUG, "OpenSSL: Failed to set minimum TLS version");
            return -1;
        }
    }
#endif /* >= 1.1.0 */
#if OPENSSL_VERSION_NUMBER >= 0x10100000L && !defined(LIBRESSL_VERSION_NUMBER) && !defined(OPENSSL_IS_BORINGSSL)
    {
        ...
        if ((flags & (TLS_CONN_ENABLE_TLSv1_0 | TLS_CONN_ENABLE_TLSv1_1)) && SSL_get_security_level(ssl) > need_level)
        {
            /*
    ...
    if (flags & TLS_CONN_SUITEB_NO_ECDH)
    {
        ...
        if (openssl_ciphers)
        {
            wpa_printf(MSG_DEBUG, "OpenSSL: Override ciphers for Suite B (no ECDH): %s", openssl_ciphers);
            ciphers = openssl_ciphers;
        }
        if (SSL_set_cipher_list(ssl, ciphers) != 1)
        {
            wpa_printf(MSG_INFO, "OpenSSL: Failed to set Suite B ciphers");
            return -1;
        }
    }
    else if (flags & TLS_CONN_SUITEB)
    {
        ...
        if (openssl_ciphers)
        {
            wpa_printf(MSG_DEBUG, "OpenSSL: Override ciphers for Suite B: %s", openssl_ciphers);
            ciphers = openssl_ciphers;
        }
        if (SSL_set_cipher_list(ssl, ciphers) != 1)
        {
        ...
        {
            wpa_printf(MSG_INFO, "OpenSSL: Failed to set Suite B curves");
            return -1;
        }
        ...
        {
            EC_KEY_free(ecdh);
            wpa_printf(MSG_INFO, "OpenSSL: Failed to set ECDH parameter");
            return -1;
        }
        EC_KEY_free(ecdh);
    }
    if (flags & (TLS_CONN_SUITEB | TLS_CONN_SUITEB_NO_ECDH))
    {
        ...
        if (SSL_CTX_set_verify_algorithm_prefs(conn->ssl_ctx, sigalgs, 1) != 1)
        {
            wpa_printf(MSG_INFO, "OpenSSL: Failed to set Suite B sigalgs");
            return -1;
        }
    ...
    if (flags & (TLS_CONN_SUITEB | TLS_CONN_SUITEB_NO_ECDH))
    {
        wpa_printf(MSG_ERROR, "OpenSSL: Suite B RSA case not supported with this OpenSSL version");
        return -1;
    }
#endif /* OPENSSL_VERSION_NUMBER */
...
#else  /* OPENSSL_IS_BORINGSSL */
    if (!(flags & (TLS_CONN_SUITEB | TLS_CONN_SUITEB_NO_ECDH)) && openssl_ciphers &&
        SSL_set_cipher_list(ssl, openssl_ciphers) != 1)
    {
        wpa_printf(MSG_INFO, "OpenSSL: Failed to set openssl_ciphers '%s'", openssl_ciphers);
        return -1;
    }
#endif /* OPENSSL_IS_BORINGSSL */
#else  /* CONFIG_SUITEB */
...
#define TEAP_DH_ANON_CS                                        \
    ...
    "ADH-AES256-SHA256:ADH-AES128-SHA256:ADH-AES256-SHA:ADH-AES128-SHA"
        ...
        wpa_printf(MSG_DEBUG, "OpenSSL: Enable cipher suites for anonymous EAP-TEAP provisioning: %s", cs);
                              ...
                              size_t session_ctx_len)
{
    ...
    {
        conn->ca_cert_verify = 1;
        SSL_set_verify(conn->ssl, SSL_VERIFY_PEER | SSL_VERIFY_CLIENT_ONCE, tls_verify_cb);
    }
    else if (verify_peer)
    {
        conn->ca_cert_verify = 1;
        SSL_set_verify(conn->ssl, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT | SSL_VERIFY_CLIENT_ONCE,
                       tls_verify_cb);
    }
    else
    {
...
static int tls_global_client_cert(struct tls_data *data, const char *client_cert)
{
#ifndef OPENSSL_NO_STDIO
    SSL_CTX *ssl_ctx = data->ssl;
    ...
    if (SSL_CTX_use_certificate_file(ssl_ctx, client_cert, SSL_FILETYPE_ASN1) != 1 &&
        SSL_CTX_use_certificate_chain_file(ssl_ctx, client_cert) != 1 &&
        SSL_CTX_use_certificate_file(ssl_ctx, client_cert, SSL_FILETYPE_PEM) != 1)
    {
...
#ifdef PKCS12_FUNCS
static int tls_parse_pkcs12(struct tls_data *data, SSL *ssl, PKCS12 *p12, const char *passwd)
{
    EVP_PKEY *pkey;
    X509 *cert;
    STACK_OF(X509) * certs;
    ...
    if (certs)
    {
        ...
        while ((cert = sk_X509_pop(certs)) != NULL)
        {
            ...
            wpa_printf(MSG_DEBUG,
                       ...
                       buf);
            if ((ssl && SSL_add1_chain_cert(ssl, cert) != 1) || (!ssl && SSL_CTX_add1_chain_cert(data->ssl, cert) != 1))
            {
                tls_show_errors(MSG_DEBUG, __func__, "Failed to add additional certificate");
                res = -1;
                X509_free(cert);
                break;
            }
        ...
        if (ssl)
            res = SSL_build_cert_chain(ssl, SSL_BUILD_CHAIN_FLAG_CHECK | SSL_BUILD_CHAIN_FLAG_IGNORE_ERROR);
        else
            res = SSL_CTX_build_cert_chain(data->ssl, SSL_BUILD_CHAIN_FLAG_CHECK | SSL_BUILD_CHAIN_FLAG_IGNORE_ERROR);
        if (!res)
        {
...
static int tls_read_pkcs12(struct tls_data *data, SSL *ssl, const char *private_key, const char *passwd)
{
#ifdef PKCS12_FUNCS
    FILE *f;
    PKCS12 *p12;
...
static int tls_read_pkcs12_blob(struct tls_data *data, SSL *ssl, const u8 *blob, size_t len, const char *passwd)
{
#ifdef PKCS12_FUNCS
    PKCS12 *p12;
...
static int tls_connection_engine_ca_cert(struct tls_data *data, struct tls_connection *conn, const char *ca_cert_id)
{
#ifndef OPENSSL_NO_ENGINE
    X509 *cert;
    SSL_CTX *ssl_ctx = data->ssl;
...
static int tls_use_private_key_file(struct tls_data *data,
                                    SSL *ssl,
                                    ...
                                    const char *private_key_passwd)
{
    ...
    else
    {
        ...
        if (!bio)
            return -1;
        pkey = PEM_read_bio_PrivateKey(bio, NULL, tls_passwd_cb, (void *)private_key_passwd);
        BIO_free(bio);
        if (!pkey)
            return -1;
                                      ...
                                      size_t private_key_blob_len)
{
    ...
    while (private_key_blob)
    {
        ...
        if (SSL_use_PrivateKey_ASN1(EVP_PKEY_EC, conn->ssl, (u8 *)private_key_blob, private_key_blob_len) == 1)
        {
            wpa_printf(MSG_DEBUG, "OpenSSL: SSL_use_PrivateKey_ASN1(EVP_PKEY_EC) --> OK");
            ok = 1;
            break;
        }
#endif /* OPENSSL_NO_EC */
        ...
        bio = BIO_new_mem_buf((u8 *)private_key_blob, private_key_blob_len);
        if (bio)
        {
            EVP_PKEY *pkey;
        ...
        if (tls_read_pkcs12_blob(data, conn->ssl, private_key_blob, private_key_blob_len, private_key_passwd) == 0)
        {
            wpa_printf(MSG_DEBUG,
    ...
    {
        if (tls_use_private_key_file(data, conn->ssl, private_key, private_key_passwd) == 0)
        {
            ok = 1;
            break;
        }

        if (tls_read_pkcs12(data, conn->ssl, private_key, private_key_passwd) == 0)
        {
            wpa_printf(MSG_DEBUG,
...
static int tls_global_private_key(struct tls_data *data, const char *private_key, const char *private_key_passwd)
{
    SSL_CTX *ssl_ctx = data->ssl;
    ...
    if (tls_use_private_key_file(data, NULL, private_key, private_key_passwd) &&
...
int tls_connection_get_random(void *ssl_ctx, struct tls_connection *conn, struct tls_random *keys)
{
    ...
    keys->client_random_len = SSL_get_client_random(ssl, conn->client_random, sizeof(conn->client_random));
    keys->server_random     = conn->server_random;
    keys->server_random_len = SSL_get_server_random(ssl, conn->server_random, sizeof(conn->server_random));
...
static int openssl_get_keyblock_size(SSL *ssl)
{
    ...
    if (EVP_CIPHER_mode(c) == EVP_CIPH_GCM_MODE || EVP_CIPHER_mode(c) == EVP_CIPH_CCM_MODE)
    ...
    wpa_printf(MSG_DEBUG, "OpenSSL: keyblock size: mac_key_len=%d enc_key_len=%d fixed_iv_len=%d", mac_key_len,
...
int tls_connection_export_key(void *tls_ctx,
                              ...
                              size_t out_len)
{
    if (!conn || SSL_export_keying_material(conn->ssl, out, out_len, label, os_strlen(label), context, context_len,
                                            context != NULL) != 1)
        return -1;
    return 0;
}
...
int tls_connection_get_eap_fast_key(void *tls_ctx, struct tls_connection *conn, u8 *out, size_t out_len)
{
    ...
    master_key_len = SSL_SESSION_get_master_key(sess, master_key, sizeof(master_key));
    ...
    {
        tls_prf_sha256(master_key, master_key_len, "key expansion", rnd, 2 * SSL3_RANDOM_SIZE, _out, skip + out_len);
        ret = 0;
    }
    else if (tls_prf_sha1_md5(master_key, master_key_len, "key expansion", rnd, 2 * SSL3_RANDOM_SIZE, _out,
                              skip + out_len) == 0)
    {
        ret = 0;
    }
    ...
    wpa_printf(MSG_ERROR, "OpenSSL: EAP-FAST keys cannot be exported in FIPS mode");
    return -1;
#endif /* OPENSSL_NEED_EAP_FAST_PRF */
}

static struct wpabuf *openssl_handshake(struct tls_connection *conn, const struct wpabuf *in_data)
{
    int res;
     ...
     * for processing.
     */
    if (in_data && wpabuf_len(in_data) > 0 && BIO_write(conn->ssl_in, wpabuf_head(in_data), wpabuf_len(in_data)) < 0)
    {
    ...
    if (res != 1)
    {
        ...
        else
        {
            ...
            if (!conn->server && !conn->client_hello_generated)
            {
                 ...
                 * available. */
                wpa_printf(MSG_DEBUG, "OpenSSL: Could not generate ClientHello");
                conn->write_alerts++;
                return NULL;
            }
        }
...
#ifdef CONFIG_SUITEB
    if ((conn->flags & TLS_CONN_SUITEB) && !conn->server && os_strncmp(SSL_get_cipher(conn->ssl), "DHE-", 4) == 0 &&
                                                   ...
                                                   struct wpabuf **appl_data)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: Handshake finished - resumed=%d", tls_connection_resumed(conn->ssl_ctx, conn));
        if (conn->server)
        {
            char *buf;
            size_t buflen = 2000;
            ...
            if (buf)
            {
                if (SSL_get_shared_ciphers(conn->ssl, buf, buflen))
                {
                    buf[buflen - 1] = '\0';
                    wpa_printf(MSG_DEBUG, "OpenSSL: Shared ciphers: %s", buf);
                }
                os_free(buf);
            }
        }
...
int tls_connection_client_hello_ext(
    void *ssl_ctx, struct tls_connection *conn, int ext_type, const u8 *data, size_t data_len)
{
...
static int ocsp_resp_cb(SSL *s, void *arg)
{
    ...
    if (!id)
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: Could not create OCSP certificate identifier (SHA256)");
        OCSP_BASICRESP_free(basic);
        OCSP_RESPONSE_free(rsp);
        return 0;
    }

    res = OCSP_resp_find_status(basic, id, &status, &reason, &produced_at, &this_update, &next_update);
    if (!res)
    {
        OCSP_CERTID_free(id);
        ...
        if (!id)
        {
            wpa_printf(MSG_DEBUG, "OpenSSL: Could not create OCSP certificate identifier (SHA1)");
            OCSP_BASICRESP_free(basic);
            OCSP_RESPONSE_free(rsp);
            return 0;
        }
    ...
    if (!res)
    {
        wpa_printf(MSG_INFO, "OpenSSL: Could not find current server certificate from OCSP response%s",
                   (conn->flags & TLS_CONN_REQUIRE_OCSP) ? "" : " (OCSP not required)");
        OCSP_CERTID_free(id);
        OCSP_BASICRESP_free(basic);
        OCSP_RESPONSE_free(rsp);
        return (conn->flags & TLS_CONN_REQUIRE_OCSP) ? 0 : 1;
...
static int is_tpm2_key(const char *path)
{
    ...
    static const char *tpm2_tags[] = {"-----BEGIN TSS2 PRIVATE KEY-----", "-----BEGIN TSS2 KEY BLOB-----", NULL};
...
int tls_connection_set_params(void *tls_ctx, struct tls_connection *conn, const struct tls_connection_params *params)
{
    ...
    if (params->private_key && (!engine_id || os_strcmp(engine_id, "tpm2") == 0) && is_tpm2_key(params->private_key))
    {
    ...
    if (tls_connection_set_subject_match(conn, params->subject_match, params->altsubject_match, params->suffix_match,
                                         params->domain_match, params->check_cert_subject))
        return -1;
    ...
    else if (tls_connection_ca_cert(data, conn, params->ca_cert, params->ca_cert_blob, params->ca_cert_blob_len,
                                    params->ca_path))
        return -1;
    ...
    else if (tls_connection_client_cert(conn, params->client_cert, params->client_cert_blob,
    ...
    else if (tls_connection_private_key(data, conn, params->private_key, params->private_key_passwd,
    ...
    {
        wpa_printf(MSG_INFO, "OpenSSL: Failed to set cipher string '%s'", ciphers);
        return -1;
    }
    ...
    if (!params->openssl_ecdh_curves)
    {
        ...
        {
            wpa_printf(MSG_INFO, "OpenSSL: Failed to set ECDH curves to auto");
            return -1;
        }
#endif /* >= 1.0.2 && < 1.1.0 */
#endif /* OPENSSL_NO_EC */
    ...
    else if (params->openssl_ecdh_curves[0])
    {
        ...
        wpa_printf(MSG_INFO, "OpenSSL: ECDH configuration nnot supported");
        return -1;
#else /* OPENSSL_IS_BORINGSSL || < 1.0.2 */
#ifndef OPENSSL_NO_EC
        if (SSL_set1_curves_list(conn->ssl, params->openssl_ecdh_curves) != 1)
        {
            wpa_printf(MSG_INFO, "OpenSSL: Failed to set ECDH curves '%s'", params->openssl_ecdh_curves);
            return -1;
        }
#else  /* OPENSSL_NO_EC */
    ...
    if (tls_set_conn_flags(conn, params->flags, params->openssl_ciphers) < 0)
        return -1;
    ...
    {
        wpa_printf(MSG_INFO, "OpenSSL: No OCSP support included - reject configuration");
        return -1;
    }
    if (params->flags & TLS_CONN_REQUEST_OCSP)
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: No OCSP support included - allow optional OCSP case to continue");
    }
#endif /* HAVE_OCSP */
#endif /* OPENSSL_IS_BORINGSSL */
...
static void openssl_debug_dump_cipher_list(SSL_CTX *ssl_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "OpenSSL: Enabled cipher suites in priority order");
    for (i = 0;; i++)
    {
        const char *cipher;
...
static void openssl_debug_dump_certificate(int i, X509 *cert)
{
    ...
    wpa_printf(MSG_DEBUG, "%d: %s (%s) %s", i, buf, openssl_pkey_type_str(pkey), serial_num);
    EVP_PKEY_free(pkey);
}
...
static void openssl_debug_dump_certificates(SSL_CTX *ssl_ctx)
{
    ...
    if (SSL_CTX_get0_chain_certs(ssl_ctx, &certs) == 1)
    {
        ...
        for (i = sk_X509_num(certs); i > 0; i--)
            openssl_debug_dump_certificate(i, sk_X509_value(certs, i - 1));
    }
...
static void openssl_debug_dump_certificate_chains(SSL_CTX *ssl_ctx)
{
    ...
    for (res = SSL_CTX_set_current_cert(ssl_ctx, SSL_CERT_SET_FIRST); res == 1;
...
int tls_global_set_params(void *tls_ctx, const struct tls_connection_params *params)
{
    ...
    {
        data->check_cert_subject = os_strdup(params->check_cert_subject);
        if (!data->check_cert_subject)
            return -1;
    }

    if (tls_global_ca_cert(data, params->ca_cert) || tls_global_client_cert(data, params->client_cert) ||
        tls_global_private_key(data, params->private_key, params->private_key_passwd) ||
        tls_global_client_cert(data, params->client_cert2) ||
        tls_global_private_key(data, params->private_key2, params->private_key_passwd2) ||
...
static int tls_sess_sec_cb(
    SSL *s, void *secret, int *secret_len, STACK_OF(SSL_CIPHER) * peer_ciphers, const SSL_CIPHER **cipher, void *arg)
#else  /* OPENSSL_IS_BORINGSSL */
static int tls_sess_sec_cb(
    SSL *s, void *secret, int *secret_len, STACK_OF(SSL_CIPHER) * peer_ciphers, SSL_CIPHER **cipher, void *arg)
#endif /* OPENSSL_IS_BORINGSSL */
{
    struct tls_connection *conn = arg;
    int ret;
    ...
    ret = conn->session_ticket_cb(conn->session_ticket_cb_ctx, conn->session_ticket, conn->session_ticket_len,
                                  s->s3->client_random, s->s3->server_random, secret);
#else
    unsigned char client_random[SSL3_RANDOM_SIZE];
    unsigned char server_random[SSL3_RANDOM_SIZE];
...
{
#if OPENSSL_VERSION_NUMBER >= 0x10100000L && !defined(LIBRESSL_VERSION_NUMBER)
    return os_snprintf(buf, buf_len, "OpenSSL build=%s run=%s", OPENSSL_VERSION_TEXT, OpenSSL_version(OPENSSL_VERSION));
#else
