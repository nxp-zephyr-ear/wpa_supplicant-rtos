...
int des_encrypt(const u8 *clear, const u8 *key, u8 *cypher)
{
    ...
    if (ctx && EVP_EncryptInit_ex(ctx, EVP_des_ecb(), NULL, pkey, NULL) == 1 &&
        EVP_CIPHER_CTX_set_padding(ctx, 0) == 1 && EVP_EncryptUpdate(ctx, cypher, &plen, clear, 8) == 1 &&
...
int rc4_skip(const u8 *key, size_t keylen, size_t skip, u8 *data, size_t data_len)
{
    ...
    ctx = EVP_CIPHER_CTX_new();
    if (!ctx || !EVP_CipherInit_ex(ctx, EVP_rc4(), NULL, NULL, NULL, 1) || !EVP_CIPHER_CTX_set_padding(ctx, 0) ||
        !EVP_CIPHER_CTX_set_key_length(ctx, keylen) || !EVP_CipherInit_ex(ctx, NULL, NULL, key, NULL, 1))
        goto out;
...
void *aes_encrypt_init(const u8 *key, size_t len)
{
    ...
    if (!type)
    {
        wpa_printf(MSG_INFO, "%s: Unsupported len=%u", __func__, (unsigned int)len);
        return NULL;
    }
...
int aes_128_cbc_encrypt(const u8 *key, const u8 *iv, u8 *data, size_t data_len)
{
    ...
    clen = data_len;
    len  = sizeof(buf);
    if (EVP_EncryptInit_ex(ctx, EVP_aes_128_cbc(), NULL, key, iv) == 1 && EVP_CIPHER_CTX_set_padding(ctx, 0) == 1 &&
        EVP_EncryptUpdate(ctx, data, &clen, data, data_len) == 1 && clen == (int)data_len &&
...
int aes_128_cbc_decrypt(const u8 *key, const u8 *iv, u8 *data, size_t data_len)
{
    ...
    plen = data_len;
    len  = sizeof(buf);
    if (EVP_DecryptInit_ex(ctx, EVP_aes_128_cbc(), NULL, key, iv) == 1 && EVP_CIPHER_CTX_set_padding(ctx, 0) == 1 &&
        EVP_DecryptUpdate(ctx, data, &plen, data, data_len) == 1 && plen == (int)data_len &&
...
int crypto_dh_init(u8 generator, const u8 *prime, size_t prime_len, u8 *privkey, u8 *pubkey)
{
    ...
    if (crypto_mod_exp(&generator, 1, privkey, prime_len, prime, prime_len, pubkey, &pubkey_len) < 0)
        return -1;
                            ...
                            size_t *len)
{
    ...
    if (!pub || !p || BN_is_zero(pub) || BN_is_one(pub) || BN_cmp(pub, p) >= 0)
        goto fail;
    ...
    if (order)
    {
        ...
        ctx    = BN_CTX_new();
        tmp    = BN_new();
        failed = !q || !ctx || !tmp || !BN_mod_exp(tmp, pub, q, p, ctx) || !BN_is_one(tmp);
        BN_clear_free(q);
        BN_clear_free(tmp);
        BN_CTX_free(ctx);
        if (failed)
    ...
    res = crypto_mod_exp(pubkey, pubkey_len, privkey, privkey_len, prime, prime_len, secret, len);
fail:
    BN_clear_free(pub);
    BN_clear_free(p);
    return res;
                   ...
                   size_t *result_len)
{
    ...
    if (BN_mod_exp_mont_consttime(bn_result, bn_base, bn_exp, bn_modulus, ctx, NULL) != 1)
        goto error;
...
struct crypto_cipher *crypto_cipher_init(enum crypto_cipher_alg alg, const u8 *iv, const u8 *key, size_t key_len)
{
    ...
    if (!(ctx->enc = EVP_CIPHER_CTX_new()) || !EVP_EncryptInit_ex(ctx->enc, cipher, NULL, NULL, NULL) ||
        !EVP_CIPHER_CTX_set_padding(ctx->enc, 0) || !EVP_CIPHER_CTX_set_key_length(ctx->enc, key_len) ||
    ...
    if (!(ctx->dec = EVP_CIPHER_CTX_new()) || !EVP_DecryptInit_ex(ctx->dec, cipher, NULL, NULL, NULL) ||
        !EVP_CIPHER_CTX_set_padding(ctx->dec, 0) || !EVP_CIPHER_CTX_set_key_length(ctx->dec, key_len) ||
...
err:
    ...
    *publ = NULL;
    ...
    p = get_group5_prime();
    q = get_group5_order();
    if (!g || BN_set_word(g, 2) != 1 || !p || !q || DH_set0_pqg(dh, p, q, g) != 1)
        goto err;
    p = NULL;
    q = NULL;
    g = NULL;
...
{
    return openssl_hmac_vector(EVP_sha384(), key, key_len, num_elem, addr, len, mac, 48);
}
...
{
    return openssl_hmac_vector(EVP_sha512(), key, key_len, num_elem, addr, len, mac, 64);
}
                          ...
                          struct crypto_bignum *d)
{
    ...
    if (bnctx == NULL)
        return -1;
    res = BN_mod_exp_mont_consttime((BIGNUM *)d, (const BIGNUM *)a, (const BIGNUM *)b, (const BIGNUM *)c, bnctx, NULL);
    BN_CTX_free(bnctx);
                         ...
                         struct crypto_bignum *d)
{
    ...
    if (!bnctx)
        return -1;
    res = BN_mod_add((BIGNUM *)d, (const BIGNUM *)a, (const BIGNUM *)b, (const BIGNUM *)c, bnctx);
    BN_CTX_free(bnctx);
...
int crypto_bignum_sqrmod(const struct crypto_bignum *a, const struct crypto_bignum *b, struct crypto_bignum *c)
{
    ...
    if (!bnctx)
        return -1;
    res = BN_mod_sqr((BIGNUM *)c, (const BIGNUM *)a, (const BIGNUM *)b, bnctx);
    BN_CTX_free(bnctx);
...
int crypto_bignum_legendre(const struct crypto_bignum *a, const struct crypto_bignum *p)
{
    ...
    if (!exp || !tmp ||
        /* exp = (p-1) / 2 */
        !BN_sub(exp, (const BIGNUM *)p, BN_value_one()) || !BN_rshift1(exp, exp) ||
        !BN_mod_exp_mont_consttime(tmp, (const BIGNUM *)a, exp, (const BIGNUM *)p, bnctx, NULL))
        goto fail;
...
struct crypto_ec *crypto_ec_init(int group)
{
    ...
    e->a     = BN_new();
    e->b     = BN_new();
    if (e->group == NULL || e->bnctx == NULL || e->prime == NULL || e->order == NULL || e->a == NULL || e->b == NULL ||
...
{
    return EC_POINT_get_affine_coordinates_GFp(e->group, (const EC_POINT *)p, (BIGNUM *)x, NULL, e->bnctx) == 1 ? 0 :
                                                                                                                  -1;
}
                        ...
                        struct crypto_ec_point *res)
{
    if (TEST_FAIL())
        return -1;
    return EC_POINT_mul(e->group, (EC_POINT *)res, NULL, (const EC_POINT *)p, (const BIGNUM *)b, e->bnctx) ? 0 : -1;
}
...
struct crypto_bignum *crypto_ec_point_compute_y_sqr(struct crypto_ec *e, const struct crypto_bignum *x)
{
    ...
    if (tmp && BN_mod_sqr(tmp, (const BIGNUM *)x, e->prime, e->bnctx) && BN_mod_add_quick(tmp, e->a, tmp, e->prime) &&
        BN_mod_mul(tmp, tmp, (const BIGNUM *)x, e->prime, e->bnctx) && BN_mod_add_quick(tmp, tmp, e->b, e->prime))
...
void crypto_ec_point_debug_print(const struct crypto_ec *e, const struct crypto_ec_point *p, const char *title)
{
    ...
    x = BN_new();
    y = BN_new();
    if (!x || !y || EC_POINT_get_affine_coordinates_GFp(e->group, (const EC_POINT *)p, x, y, e->bnctx) != 1)
        goto fail;
...
struct crypto_ecdh *crypto_ecdh_init(int group)
{
    ...
    if (!ec_params)
    {
        wpa_printf(MSG_ERROR, "OpenSSL: Failed to generate EC_KEY parameters");
        goto fail;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "OpenSSL: Failed to generate EVP_PKEY parameters");
        goto fail;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "OpenSSL: EVP_PKEY_keygen_init failed: %s", ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }

    if (EVP_PKEY_keygen(kctx, &ecdh->pkey) != 1)
    {
        wpa_printf(MSG_ERROR, "OpenSSL: EVP_PKEY_keygen failed: %s", ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }
...
struct crypto_ecdh *crypto_ecdh_init2(int group, struct crypto_ec_key *own_key)
{
    ...
    if (!ecdh->pkey || EVP_PKEY_assign_EC_KEY(ecdh->pkey, EVP_PKEY_get1_EC_KEY((EVP_PKEY *)own_key)) != 1)
        goto fail;
...
struct wpabuf *crypto_ecdh_get_pubkey(struct crypto_ecdh *ecdh, int inc_y)
{
    ...
    if (EC_POINT_get_affine_coordinates_GFp(ecdh->ec->group, pubkey, x, y, ecdh->ec->bnctx) != 1)
    {
        wpa_printf(MSG_ERROR, "OpenSSL: EC_POINT_get_affine_coordinates_GFp failed: %s",
                   ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }

    res = crypto_bignum_to_bin((struct crypto_bignum *)x, wpabuf_put(buf, len), len, len);
    if (res < 0)
        goto fail;

    if (inc_y)
    {
        res = crypto_bignum_to_bin((struct crypto_bignum *)y, wpabuf_put(buf, len), len, len);
        if (res < 0)
            goto fail;
    }
...
struct wpabuf *crypto_ecdh_set_peerkey(struct crypto_ecdh *ecdh, int inc_y, const u8 *key, size_t len)
{
    ...
    if (inc_y)
    {
        ...
        if (!y)
            goto fail;
        if (!EC_POINT_set_affine_coordinates_GFp(ecdh->ec->group, pub, x, y, ecdh->ec->bnctx))
        {
            wpa_printf(MSG_ERROR, "OpenSSL: EC_POINT_set_affine_coordinates_GFp failed: %s",
                       ERR_error_string(ERR_get_error(), NULL));
            goto fail;
        }
    }
    else if (!EC_POINT_set_compressed_coordinates_GFp(ecdh->ec->group, pub, x, 0, ecdh->ec->bnctx))
    {
        wpa_printf(MSG_ERROR, "OpenSSL: EC_POINT_set_compressed_coordinates_GFp failed: %s",
                   ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "OpenSSL: ECDH peer public key is not on curve");
        goto fail;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "OpenSSL: EC_KEY_set_public_key failed: %s", ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }
    ...
    if (!ctx || EVP_PKEY_derive_init(ctx) != 1 || EVP_PKEY_derive_set_peer(ctx, peerkey) != 1 ||
        EVP_PKEY_derive(ctx, NULL, &secret_len) != 1)
    {
        wpa_printf(MSG_ERROR, "OpenSSL: EVP_PKEY_derive(1) failed: %s", ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "OpenSSL: EVP_PKEY_derive(2) failed: %s", ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }
    if (secret->size != secret_len)
        wpa_printf(MSG_DEBUG, "OpenSSL: EVP_PKEY_derive(2) changed secret_len %d -> %d", (int)secret->size,
                   (int)secret_len);
...
struct crypto_ec_key *crypto_ec_key_parse_priv(const u8 *der, size_t der_len)
{
    ...
    if (!eckey)
    {
        wpa_printf(MSG_INFO, "OpenSSL: d2i_ECPrivateKey() failed: %s", ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }
...
struct crypto_ec_key *crypto_ec_key_parse_pub(const u8 *der, size_t der_len)
{
    ...
    if (!pkey)
    {
        wpa_printf(MSG_INFO, "OpenSSL: d2i_PUBKEY() failed: %s", ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }
...
struct crypto_ec_key *crypto_ec_key_set_pub(int group, const u8 *buf_x, const u8 *buf_y, size_t len)
{
    ...
    {
        wpa_printf(MSG_ERROR, "OpenSSL: Failed to set EC_KEY: %s", ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }
...
struct crypto_ec_key *crypto_ec_key_set_pub_point(struct crypto_ec *ec, const struct crypto_ec_point *pub)
{
    ...
    eckey = EC_KEY_new();
    if (!eckey || EC_KEY_set_group(eckey, ec->group) != 1 || EC_KEY_set_public_key(eckey, (const EC_POINT *)pub) != 1)
    {
        wpa_printf(MSG_ERROR, "OpenSSL: Failed to set EC_KEY: %s", ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }
...
ASN1_SEQUENCE(EC_COMP_PUBKEY) = {
    ...
    ASN1_SIMPLE(EC_COMP_PUBKEY, pub_key, ASN1_BIT_STRING)} ASN1_SEQUENCE_END(EC_COMP_PUBKEY);
...
struct wpabuf *crypto_ec_key_get_subject_public_key(struct crypto_ec_key *key)
{
    ...
    if (!pubkey || X509_ALGOR_set0(pubkey->alg, OBJ_nid2obj(EVP_PKEY_EC), V_ASN1_OBJECT, (void *)OBJ_nid2obj(nid)) != 1)
        goto fail;
    ...
    if (!der)
        goto fail;
    len = EC_POINT_point2oct(group, point, POINT_CONVERSION_COMPRESSED, der, len, ctx);
    ...
    if (der_len <= 0)
    {
        wpa_printf(MSG_ERROR, "BoringSSL: Failed to build DER encoded public key");
        goto fail;
    }
...
fail:
    ...
    if (der_len <= 0)
    {
        wpa_printf(MSG_INFO, "OpenSSL: i2d_PUBKEY() failed: %s", ERR_error_string(ERR_get_error(), NULL));
        return NULL;
    }
...
struct wpabuf *crypto_ec_key_get_pubkey_point(struct crypto_ec_key *key, int prefix)
{
    ...
    if (len <= 0)
    {
        wpa_printf(MSG_ERROR, "OpenSSL: Failed to determine public key encoding length");
        EC_KEY_free(eckey);
        return NULL;
    }
    ...
    if (res != len)
    {
        wpa_printf(MSG_ERROR, "OpenSSL: Failed to encode public key (res=%d/%d)", res, len);
        wpabuf_free(buf);
        return NULL;
    }
...
struct wpabuf *crypto_ec_key_sign(struct crypto_ec_key *key, const u8 *data, size_t len)
{
    ...
    if (!pkctx || EVP_PKEY_sign_init(pkctx) <= 0 ||
        EVP_PKEY_sign(pkctx, wpabuf_put(sig_der, 0), &sig_len, data, len) <= 0)
    {
        wpabuf_free(sig_der);
        sig_der = NULL;
    }
...
struct wpabuf *crypto_ec_key_sign_r_s(struct crypto_ec_key *key, const u8 *data, size_t len)
{
    ...
    group = EC_KEY_get0_group(eckey);
    prime = BN_new();
    if (!prime || !group || !EC_GROUP_get_curve_GFp(group, prime, NULL, NULL, NULL))
        goto fail;
    ...
    if (crypto_bignum_to_bin((const struct crypto_bignum *)r, r_buf, prime_len, prime_len) < 0 ||
    ...
    struct crypto_ec_key *key, const u8 *data, size_t len, const u8 *r, size_t r_len, const u8 *s, size_t s_len)
{
    ...
    if (der_len <= 0)
    {
        wpa_printf(MSG_DEBUG, "OpenSSL: Could not DER encode signature");
        goto fail;
    }
...
int crypto_ec_key_group(struct crypto_ec_key *key)
{
    ...
    wpa_printf(MSG_ERROR, "OpenSSL: Unsupported curve (nid=%d) in EC key", nid);
    return -1;
}
...
struct wpabuf *crypto_pkcs7_get_certificates(const struct wpabuf *pkcs7)
{
    ...
    {
        wpa_printf(MSG_INFO, "OpenSSL: Could not parse PKCS#7 object: %s", ERR_error_string(ERR_get_error(), NULL));
        goto fail;
    }
#else  /* OPENSSL_IS_BORINGSSL */
    ...
    {
        wpa_printf(MSG_INFO, "OpenSSL: No certificates found in PKCS#7 object");
        goto fail;
    }
...
#if OPENSSL_VERSION_NUMBER < 0x10100000L
    if (!X509_NAME_add_entry_by_NID(n, nid, MBSTRING_UTF8, (unsigned char *)name, os_strlen(name), -1, 0))
        return -1;
#else
    if (!X509_NAME_add_entry_by_NID(n, nid, MBSTRING_UTF8, (const unsigned char *)name, os_strlen(name), -1, 0))
        return -1;
#endif
