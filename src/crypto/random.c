...
static void random_extract(u8 *out)
{
    ...
    hmac_sha1(stub_key, sizeof(stub_key), (const u8 *)pool, sizeof(pool), hash);
    ...
    hmac_sha1(stub_key, sizeof(stub_key), (const u8 *)buf, sizeof(buf), hash);
    /*
...
int random_pool_ready(void)
{
...
#ifdef CONFIG_GETRANDOM
    res = getrandom(stub_key + stub_key_avail, sizeof(stub_key) - stub_key_avail, GRND_NONBLOCK);
    if (res < 0)
    {
        if (errno == ENOSYS)
        {
            wpa_printf(MSG_DEBUG, "random: getrandom() not supported, falling back to /dev/random");
        }
        else
        {
            wpa_printf(MSG_INFO, "random: no data from getrandom(): %s", strerror(errno));
            res = 0;
        }
    }
    ...
    if (res < 0)
    {
        ...
        res = read(fd, stub_key + stub_key_avail, sizeof(stub_key) - stub_key_avail);
        if (res < 0)
        {
...
static void random_read_fd(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    res = read(sock, stub_key + stub_key_avail, sizeof(stub_key) - stub_key_avail);
    if (res < 0)
    {
        wpa_printf(MSG_ERROR,
    ...
    wpa_printf(MSG_DEBUG, "random: Got %u/%u bytes from /dev/random", (unsigned)res,
               (unsigned)(sizeof(stub_key) - stub_key_avail));
    stub_key_avail += res;
...
void random_init(const char *entropy_file)
{
...
#ifdef CONFIG_GETRANDOM
    {
        ...
        if (getrandom(&stub, 0, GRND_NONBLOCK) == 0 || errno != ENOSYS)
        {
            wpa_printf(MSG_DEBUG, "random: getrandom() support available");
            return;
        }
    }
#endif /* CONFIG_GETRANDOM */
