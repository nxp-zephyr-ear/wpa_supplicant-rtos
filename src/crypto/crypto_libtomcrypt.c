...
static int crypto_rsa_encrypt_pkcs1(
    int block_type, rsa_key *key, int key_type, const u8 *in, size_t inlen, u8 *out, size_t *outlen)
{
    ...
    struct crypto_public_key *key, const u8 *crypt, size_t crypt_len, u8 *plain, size_t *plain_len)
{
    ...
    if (len < 3 + 8 + 16 /* min hash len */ || plain[0] != 0x00 || plain[1] != 0x01 || plain[2] != 0xff)
    {
        wpa_printf(MSG_INFO,
...
int crypto_global_init(void)
{
    ...
    if (register_hash(&md4_desc) < 0 || register_hash(&md5_desc) < 0 || register_hash(&sha1_desc) < 0 ||
        register_cipher(&aes_desc) < 0 || register_cipher(&des_desc) < 0 || register_cipher(&des3_desc) < 0)
    {
        wpa_printf(MSG_ERROR,
...
{
    /* TODO: check pubkey */
    return crypto_mod_exp(pubkey, pubkey_len, privkey, privkey_len, prime, prime_len, secret, len);
}
