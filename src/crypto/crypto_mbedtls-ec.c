...
int crypto_ec_point_to_bin(struct crypto_ec *e, const struct crypto_ec_point *point, u8 *x, u8 *y)
{
    ...
    if (x)
    {
        if (crypto_bignum_to_bin((struct crypto_bignum *)&((mbedtls_ecp_point *)point)->MBEDTLS_PRIVATE(X), x, len,
                                 len) < 0)
        {
            return -1;
        }
    ...
    if (y)
    {
        if (crypto_bignum_to_bin((struct crypto_bignum *)&((mbedtls_ecp_point *)point)->MBEDTLS_PRIVATE(Y), y, len,
                                 len) < 0)
        {
            return -1;
        }
...
int crypto_ec_get_affine_coordinates(struct crypto_ec *e,
                                     struct crypto_ec_point *pt,
...
int crypto_ec_point_x(struct crypto_ec *e, const struct crypto_ec_point *p, struct crypto_bignum *x)
{
    ...
    MBEDTLS_MPI_CHK(mbedtls_mpi_copy((mbedtls_mpi *)x, &((mbedtls_ecp_point *)p)->MBEDTLS_PRIVATE(X)));
                        ...
                        struct crypto_ec_point *c)
{
    ...
    MBEDTLS_MPI_CHK(mbedtls_ecp_muladd(&e->group, (mbedtls_ecp_point *)c, &one, (const mbedtls_ecp_point *)a, &one,
                        ...
                        struct crypto_ec_point *res)
{
    ...
    MBEDTLS_MPI_CHK(mbedtls_ecp_mul(&e->group, (mbedtls_ecp_point *)res, (const mbedtls_mpi *)b,
...
int crypto_ec_point_is_on_curve(struct crypto_ec *e, const struct crypto_ec_point *p)
{
    ...
    MBEDTLS_MPI_CHK(
        mbedtls_mpi_exp_mod(&y_sqr_lhs, &((const mbedtls_ecp_point *)p)->MBEDTLS_PRIVATE(Y), &two, &e->group.P, NULL));

    y_sqr_rhs = (mbedtls_mpi *)crypto_ec_point_compute_y_sqr(
        e, (const struct crypto_bignum *)&((const mbedtls_ecp_point *)p)->MBEDTLS_PRIVATE(X));
...
void crypto_debug_print_point(const char *title, struct crypto_ec *e, const struct crypto_ec_point *point)
{
    u8 x[32], y[32];
...
struct crypto_key *crypto_ec_get_key(const u8 *privkey, size_t privkey_len)
{
    ...
    ret = mbedtls_pk_parse_key(kctx, privkey, privkey_len, NULL, 0); //, f_rng, NULL);
...
int crypto_ecdh(struct crypto_key *key_own, struct crypto_key *key_peer, u8 *secret, size_t *secret_len)
{
    ...
    *secret_len = 0;
    ...
    if (!ctx)
    {
        wpa_printf(MSG_ERROR, "DPP: EVP_PKEY_CTX_new failed: %s", __func__);
        return -1;
    }
...
void crypto_debug_print_ec_key(const char *title, struct crypto_key *key)
{
    ...
    int len =
        mbedtls_mpi_size((mbedtls_mpi *)crypto_ec_get_prime((struct crypto_ec *)crypto_ec_get_group_from_key(key)));
...
struct crypto_key *crypto_ec_gen_keypair(u16 ike_group)
{
    ...
    if (mbedtls_pk_setup(kctx, mbedtls_pk_info_from_type(MBEDTLS_PK_ECKEY)) != 0)
        goto fail;
...
static int pk_write_ec_pubkey_formatted(unsigned char **p, unsigned char *start, mbedtls_ecp_keypair *ec, int format)
{
    int ret    = 0;
    size_t len = 0;
    unsigned char buf[MBEDTLS_ECP_MAX_PT_LEN];

    if ((ret = mbedtls_ecp_point_write_binary(&ec->MBEDTLS_PRIVATE(grp), &ec->MBEDTLS_PRIVATE(Q), format, &len, buf,
                                              sizeof(buf))) != 0)
    {
        return (ret);
    }
...
int mbedtls_pk_write_pubkey_formatted(unsigned char **p,
                                      unsigned char *start,
