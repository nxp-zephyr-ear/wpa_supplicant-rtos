...
static int wolfssl_hmac_vector(int type,
                               const u8 *key,
                               size_t key_len,
                               size_t num_elem,
                               const u8 *addr[],
...
{
    return wolfssl_hmac_vector(WC_MD5, key, key_len, num_elem, addr, len, mac, 16);
}
...
{
    return wolfssl_hmac_vector(WC_SHA, key, key_len, num_elem, addr, len, mac, 20);
}
...
{
    return wolfssl_hmac_vector(WC_SHA256, key, key_len, num_elem, addr, len, mac, 32);
}
...
{
    return wolfssl_hmac_vector(WC_SHA384, key, key_len, num_elem, addr, len, mac, 48);
}
...
{
    return wolfssl_hmac_vector(WC_SHA512, key, key_len, num_elem, addr, len, mac, 64);
}
...
{
    if (wc_PBKDF2(buf, (const byte *)passphrase, os_strlen(passphrase), ssid, ssid_len, iterations, buflen, WC_SHA) !=
        0)
        return -1;
    return 0;
}
...
int aes_wrap(const u8 *kek, size_t kek_len, int n, const u8 *plain, u8 *cipher)
{
    ...
    ret = wc_AesKeyWrap(kek, kek_len, plain, n * 8, cipher, (n + 1) * 8, NULL);
...
int aes_unwrap(const u8 *kek, size_t kek_len, int n, const u8 *cipher, u8 *plain)
{
    ...
    ret = wc_AesKeyUnWrap(kek, kek_len, cipher, (n + 1) * 8, plain, n * 8, NULL);
    return ret != n * 8 ? -1 : 0;
}
...
struct crypto_cipher *crypto_cipher_init(enum crypto_cipher_alg alg, const u8 *iv, const u8 *key, size_t key_len)
{
    ...
    switch (alg)
    {
        ...
        case CRYPTO_CIPHER_ALG_3DES:
            if (key_len != DES3_KEYLEN || wc_Des3_SetKey(&ctx->enc.des3, key, iv, DES_ENCRYPTION) ||
...
void *dh5_init(struct wpabuf **priv, struct wpabuf **publ)
{
    ...
    *publ = NULL;
    ...
    if (wc_DhSetKey(dh, RFC3526_PRIME_1536, sizeof(RFC3526_PRIME_1536), RFC3526_GENERATOR_1536,
                    sizeof(RFC3526_GENERATOR_1536)) != 0)
        goto done;

    if (wc_DhGenerateKeyPair(dh, &rng, wpabuf_mhead(privkey), &priv_sz, wpabuf_mhead(pubkey), &pub_sz) != 0)
        goto done;
...
void *dh5_init_fixed(const struct wpabuf *priv, const struct wpabuf *publ)
{
    ...
    if (wc_DhAgree(dh, secret, &secret_sz, wpabuf_head(priv), wpabuf_len(priv), RFC3526_GENERATOR_1536,
                   sizeof(RFC3526_GENERATOR_1536)) != 0)
        goto done;

    if (secret_sz != wpabuf_len(publ) || os_memcmp(secret, wpabuf_head(publ), secret_sz) != 0)
        goto done;
...
struct wpabuf *dh5_derive_shared(void *ctx, const struct wpabuf *peer_public, const struct wpabuf *own_private)
{
    ...
    if (wc_DhAgree(ctx, wpabuf_mhead(secret), &secret_sz, wpabuf_head(own_private), wpabuf_len(own_private),
...
int crypto_dh_init(u8 generator, const u8 *prime, size_t prime_len, u8 *privkey, u8 *pubkey)
{
    ...
    if (wc_DhGenerateKeyPair(dh, &rng, privkey, &priv_sz, pubkey, &pub_sz) != 0)
        goto done;
                            ...
                            size_t *len)
{
    ...
    if (wc_DhAgree(dh, secret, &secret_sz, privkey, privkey_len, pubkey, pubkey_len) != 0)
        goto done;
...
int crypto_bignum_rand(struct crypto_bignum *r, const struct crypto_bignum *m)
{
    ...
    buf = os_malloc(len);
    if (!buf || wc_RNG_GenerateBlock(&rng, buf, len) != 0 || mp_read_unsigned_bin((mp_int *)r, buf, len) != MP_OKAY ||
...
int crypto_bignum_add(const struct crypto_bignum *a, const struct crypto_bignum *b, struct crypto_bignum *r)
{
    return mp_add((mp_int *)a, (mp_int *)b, (mp_int *)r) == MP_OKAY ? 0 : -1;
}

int crypto_bignum_mod(const struct crypto_bignum *a, const struct crypto_bignum *m, struct crypto_bignum *r)
{
                          ...
                          struct crypto_bignum *r)
{
    ...
    return mp_exptmod((mp_int *)b, (mp_int *)e, (mp_int *)m, (mp_int *)r) == MP_OKAY ? 0 : -1;
}

int crypto_bignum_inverse(const struct crypto_bignum *a, const struct crypto_bignum *m, struct crypto_bignum *r)
{
    if (TEST_FAIL())
        return -1;
...
int crypto_bignum_sub(const struct crypto_bignum *a, const struct crypto_bignum *b, struct crypto_bignum *r)
{
    if (TEST_FAIL())
        return -1;

    return mp_sub((mp_int *)a, (mp_int *)b, (mp_int *)r) == MP_OKAY ? 0 : -1;
}

int crypto_bignum_div(const struct crypto_bignum *a, const struct crypto_bignum *b, struct crypto_bignum *d)
{
    if (TEST_FAIL())
        return -1;
                         ...
                         struct crypto_bignum *d)
{
    ...
    return mp_addmod((mp_int *)a, (mp_int *)b, (mp_int *)c, (mp_int *)d) == MP_OKAY ? 0 : -1;
}
                         ...
                         struct crypto_bignum *d)
{
    ...
    return mp_mulmod((mp_int *)a, (mp_int *)b, (mp_int *)m, (mp_int *)d) == MP_OKAY ? 0 : -1;
}
...
int crypto_bignum_sqrmod(const struct crypto_bignum *a, const struct crypto_bignum *b, struct crypto_bignum *c)
{
    ...
    return mp_sqrmod((mp_int *)a, (mp_int *)b, (mp_int *)c) == MP_OKAY ? 0 : -1;
}
...
struct crypto_ec *crypto_ec_init(int group)
{
    ...
    if (wc_ecc_init(&e->key) != 0 || wc_ecc_set_curve(&e->key, 0, curve_id) != 0 || mp_init(&e->a) != MP_OKAY ||
        mp_init(&e->prime) != MP_OKAY || mp_init(&e->order) != MP_OKAY || mp_init(&e->b) != MP_OKAY ||
        mp_read_radix(&e->a, e->key.dp->Af, 16) != MP_OKAY || mp_read_radix(&e->b, e->key.dp->Bf, 16) != MP_OKAY ||
...
int crypto_ec_point_to_bin(struct crypto_ec *e, const struct crypto_ec_point *point, u8 *x, u8 *y)
{
    ...
    if (x)
    {
        if (crypto_bignum_to_bin((struct crypto_bignum *)p->x, x, e->key.dp->size, e->key.dp->size) <= 0)
            return -1;
    }

    if (y)
    {
        if (crypto_bignum_to_bin((struct crypto_bignum *)p->y, y, e->key.dp->size, e->key.dp->size) <= 0)
            return -1;
    }
                        ...
                        struct crypto_ec_point *c)
{
    ...
    if (!mp_isone(&mu))
    {
        ...
        if (mp_mulmod(pa->x, &mu, modulus, ta->x) != MP_OKAY || mp_mulmod(pa->y, &mu, modulus, ta->y) != MP_OKAY ||
            mp_mulmod(pa->z, &mu, modulus, ta->z) != MP_OKAY || mp_mulmod(pb->x, &mu, modulus, tb->x) != MP_OKAY ||
            mp_mulmod(pb->y, &mu, modulus, tb->y) != MP_OKAY || mp_mulmod(pb->z, &mu, modulus, tb->z) != MP_OKAY)
        {
            ret = -1;
            goto end;
        }
    ...
    ret = ecc_projective_add_point(pa, pb, (ecc_point *)c, &e->a, &e->prime, e->mont_b);
    if (ret != 0)
    {
        ret = -1;
        goto end;
                        ...
                        struct crypto_ec_point *res)
{
    ...
    ret = wc_ecc_mulmod((mp_int *)b, (ecc_point *)p, (ecc_point *)res, &e->a, &e->prime, 1);
    return ret == 0 ? 0 : -1;
}
...
struct crypto_bignum *crypto_ec_point_compute_y_sqr(struct crypto_ec *e, const struct crypto_bignum *x)
{
    ...
    if (mp_sqrmod((mp_int *)x, &e->prime, y2) != 0 || mp_mulmod((mp_int *)x, y2, &e->prime, y2) != 0 ||
        mp_mulmod((mp_int *)x, &e->a, &e->prime, &t) != 0 || mp_addmod(y2, &t, &e->prime, y2) != 0 ||
...
struct crypto_ecdh *crypto_ecdh_init(int group)
{
    ...
    ret = wc_ecc_make_key_ex(&rng, ecdh->ec->key.dp->size, &ecdh->ec->key, ecdh->ec->key.dp->id);
    if (ret < 0)
        goto fail;
...
struct wpabuf *crypto_ecdh_get_pubkey(struct crypto_ecdh *ecdh, int inc_y)
{
    ...
    ret = crypto_bignum_to_bin((struct crypto_bignum *)ecdh->ec->key.pubkey.x, wpabuf_put(buf, len), len, len);
    if (ret < 0)
        goto fail;
    if (inc_y)
    {
        ret = crypto_bignum_to_bin((struct crypto_bignum *)ecdh->ec->key.pubkey.y, wpabuf_put(buf, len), len, len);
        if (ret < 0)
            goto fail;
    }
...
struct wpabuf *crypto_ecdh_set_peerkey(struct crypto_ecdh *ecdh, int inc_y, const u8 *key, size_t len)
{
    ...
    ret = wc_ecc_import_point_der(wpabuf_mhead(pubkey), 1 + 2 * key_len, ecdh->ec->key.idx, point);
    if (ret != MP_OKAY)
        goto fail;
    ...
    ret = wc_ecc_shared_secret_ex(&ecdh->ec->key, point, wpabuf_put(secret, key_len), &key_len);
    if (ret != MP_OKAY)
        goto fail;
