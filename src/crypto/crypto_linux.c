...
static int linux_af_alg_socket(const char *type, const char *name)
{
    ...
    if (s < 0)
    {
        wpa_printf(MSG_ERROR, "%s: Failed to open AF_ALG socket: %s", __func__, strerror(errno));
        return -1;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "%s: Failed to bind AF_ALG socket(%s,%s): %s", __func__, type, name, strerror(errno));
        close(s);
        return -1;
    }
...
static int linux_af_alg_hash_vector(const char *alg,
                                    const u8 *key,
                                    size_t key_len,
                                    size_t num_elem,
                                    const u8 *addr[],
                                    ...
                                    size_t mac_len)
{
    ...
    {
        wpa_printf(MSG_ERROR, "%s: setsockopt(ALG_SET_KEY) failed: %s", __func__, strerror(errno));
        close(s);
        return -1;
    }
    ...
    if (t < 0)
    {
        wpa_printf(MSG_ERROR, "%s: accept on AF_ALG socket failed: %s", __func__, strerror(errno));
        close(s);
        return -1;
    }
    ...
    for (i = 0; i < num_elem; i++)
    {
        ...
        if (res < 0)
        {
            wpa_printf(MSG_ERROR, "%s: send on AF_ALG socket failed: %s", __func__, strerror(errno));
            goto fail;
        }
        if ((size_t)res < len[i])
        {
            wpa_printf(MSG_ERROR, "%s: send on AF_ALG socket did not accept full buffer (%d/%d)", __func__, (int)res,
                       (int)len[i]);
            goto fail;
        }
    }
    ...
    if (res < 0)
    {
        wpa_printf(MSG_ERROR, "%s: recv on AF_ALG socket failed: %s", __func__, strerror(errno));
        goto fail;
    }
    if ((size_t)res < mac_len)
    {
        wpa_printf(MSG_ERROR, "%s: recv on AF_ALG socket did not return full buffer (%d/%d)", __func__, (int)res,
                   (int)mac_len);
        goto fail;
    }
...
{
    return linux_af_alg_hash_vector("md4", NULL, 0, num_elem, addr, len, mac, 16);
}

int md5_vector(size_t num_elem, const u8 *addr[], const size_t *len, u8 *mac)
{
    return linux_af_alg_hash_vector("md5", NULL, 0, num_elem, addr, len, mac, MD5_MAC_LEN);
}

int sha1_vector(size_t num_elem, const u8 *addr[], const size_t *len, u8 *mac)
{
    return linux_af_alg_hash_vector("sha1", NULL, 0, num_elem, addr, len, mac, SHA1_MAC_LEN);
}

int sha256_vector(size_t num_elem, const u8 *addr[], const size_t *len, u8 *mac)
{
    return linux_af_alg_hash_vector("sha256", NULL, 0, num_elem, addr, len, mac, SHA256_MAC_LEN);
}

int sha384_vector(size_t num_elem, const u8 *addr[], const size_t *len, u8 *mac)
{
    return linux_af_alg_hash_vector("sha384", NULL, 0, num_elem, addr, len, mac, SHA384_MAC_LEN);
}

int sha512_vector(size_t num_elem, const u8 *addr[], const size_t *len, u8 *mac)
{
    return linux_af_alg_hash_vector("sha512", NULL, 0, num_elem, addr, len, mac, 64);
}

int hmac_md5_vector(const u8 *key, size_t key_len, size_t num_elem, const u8 *addr[], const size_t *len, u8 *mac)
{
    return linux_af_alg_hash_vector("hmac(md5)", key, key_len, num_elem, addr, len, mac, 16);
}
...
{
    return linux_af_alg_hash_vector("hmac(sha1)", key, key_len, num_elem, addr, len, mac, SHA1_MAC_LEN);
}
...
{
    return linux_af_alg_hash_vector("hmac(sha256)", key, key_len, num_elem, addr, len, mac, SHA256_MAC_LEN);
}
...
{
    return linux_af_alg_hash_vector("hmac(sha384)", key, key_len, num_elem, addr, len, mac, SHA384_MAC_LEN);
}
...
struct crypto_hash *crypto_hash_init(enum crypto_hash_alg alg, const u8 *key, size_t key_len)
{
    ...
    if (key && key_len && setsockopt(ctx->s, SOL_ALG, ALG_SET_KEY, key, key_len) < 0)
    {
        wpa_printf(MSG_ERROR, "%s: setsockopt(ALG_SET_KEY) failed: %s", __func__, strerror(errno));
        close(ctx->s);
        os_free(ctx);
...
static int linux_af_alg_skcipher_oper(struct linux_af_alg_skcipher *skcipher, int enc, const u8 *in, u8 *out)
{
    ...
    *op                = enc ? ALG_OP_ENCRYPT : ALG_OP_DECRYPT;
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_ERROR, "%s: sendmsg failed: %s", __func__, strerror(errno));
        return -1;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "%s: read did not return full data (%d/%d)", __func__, (int)ret, AES_BLOCK_SIZE);
        return -1;
    }
...
int rc4_skip(const u8 *key, size_t keylen, size_t skip, u8 *data, size_t data_len)
{
    ...
    *op                = ALG_OP_ENCRYPT;
    ...
    if (ret < 0)
    {
        wpa_printf(MSG_ERROR, "%s: recvmsg failed: %s", __func__, strerror(errno));
        linux_af_alg_skcipher_deinit(skcipher);
        return -1;
    }
    linux_af_alg_skcipher_deinit(skcipher);
    ...
    {
        wpa_printf(MSG_ERROR, "%s: recvmsg did not return full data (%d/%d)", __func__, (int)ret,
                   (int)(skip + data_len));
        return -1;
    }
...
int des_encrypt(const u8 *clear, const u8 *key, u8 *cypher)
{
    ...
    *op                = ALG_OP_ENCRYPT;
    ...
    if (ret < 8)
    {
        wpa_printf(MSG_ERROR, "%s: read did not return full data (%d/8)", __func__, (int)ret);
        goto fail;
    }
...
static int aes_128_cbc_oper(const u8 *key, int enc, const u8 *iv, u8 *data, size_t data_len)
{
    struct linux_af_alg_skcipher *skcipher;
    char buf[100];
    struct iovec io[1];
    ...
    os_memset(buf, 0, sizeof(buf));
    msg.msg_control    = buf;
    msg.msg_controllen = CMSG_SPACE(sizeof(u32)) + CMSG_SPACE(sizeof(*alg_iv) + iv_len);
    msg.msg_iov        = io;
    msg.msg_iovlen     = 1;
    ...
    *op             = enc ? ALG_OP_ENCRYPT : ALG_OP_DECRYPT;
    ...
    {
        wpa_printf(MSG_ERROR, "%s: recvmsg not return full data (%d/%d)", __func__, (int)ret, (int)data_len);
        linux_af_alg_skcipher_deinit(skcipher);
        return -1;
    }
...
{
    return linux_af_alg_hash_vector("cmac(aes)", key, key_len, num_elem, addr, len, mac, AES_BLOCK_SIZE);
}
...
int aes_unwrap(const u8 *kek, size_t kek_len, int n, const u8 *cipher, u8 *plain)
{
    ...
    *op             = ALG_OP_DECRYPT;
    ...
    if (ret < n * 8)
    {
        wpa_printf(MSG_ERROR, "%s: read not return full data (%d/%d)", __func__, (int)ret, n * 8);
        linux_af_alg_skcipher_deinit(skcipher);
        return -1;
    }
...
static int crypto_cipher_oper(struct crypto_cipher *ctx, u32 type, const u8 *in, u8 *out, size_t len)
{
    char buf[CMSG_SPACE(sizeof(u32))];
    struct iovec io[1];
    struct msghdr msg;
    ...
    *op                = type;
    ...
    if (ret < (ssize_t)len)
    {
        wpa_printf(MSG_ERROR, "%s: read did not return full data (%d/%d)", __func__, (int)ret, (int)len);
        return -1;
    }
