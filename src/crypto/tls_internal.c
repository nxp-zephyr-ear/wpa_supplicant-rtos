...
struct tls_connection *tls_connection_init(void *tls_ctx)
{
    ...
    if (!global->server)
    {
        ...
        tlsv1_client_set_cb(conn->client, global->event_cb, global->cb_ctx, global->cert_in_cb);
    }
#endif /* CONFIG_TLS_INTERNAL_CLIENT */
#ifdef CONFIG_TLS_INTERNAL_SERVER
    if (global->server)
...
int tls_connection_set_params(void *tls_ctx, struct tls_connection *conn, const struct tls_connection_params *params)
{
    ...
    if (tlsv1_set_private_key(cred, params->private_key, params->private_key_passwd, params->private_key_blob,
...
static int tls_connection_prf(void *tls_ctx,
                              struct tls_connection *conn,
                              const char *label,
                              const u8 *context,
                              size_t context_len,
                              ...
                              size_t out_len)
{
    ...
    if (conn->client)
    {
        ret = tlsv1_client_prf(conn->client, label, context, context_len, server_random_first, _out, skip + out_len);
    }
#endif /* CONFIG_TLS_INTERNAL_CLIENT */
#ifdef CONFIG_TLS_INTERNAL_SERVER
    if (conn->server)
    {
        ret = tlsv1_server_prf(conn->server, label, context, context_len, server_random_first, _out, skip + out_len);
    }
#endif /* CONFIG_TLS_INTERNAL_SERVER */
...
int tls_connection_export_key(void *tls_ctx,
                              ...
                              size_t out_len)
{
    return tls_connection_prf(tls_ctx, conn, label, context, context_len, 0, 0, out, out_len);
}

int tls_connection_get_eap_fast_key(void *tls_ctx, struct tls_connection *conn, u8 *out, size_t out_len)
{
    return tls_connection_prf(tls_ctx, conn, "key expansion", NULL, 0, 1, 1, out, out_len);
}
                                         ...
                                         int *need_more_data)
{
    ...
    ad  = NULL;
    res = tlsv1_client_handshake(conn->client, in_data ? wpabuf_head(in_data) : NULL, in_data ? wpabuf_len(in_data) : 0,
...
struct wpabuf *tls_connection_encrypt(void *tls_ctx, struct tls_connection *conn, const struct wpabuf *in_data)
{
    ...
    if (conn->client)
    {
        ...
        if (buf == NULL)
            return NULL;
        res = tlsv1_client_encrypt(conn->client, wpabuf_head(in_data), wpabuf_len(in_data), wpabuf_mhead(buf),
                                   wpabuf_size(buf));
        if (res < 0)
        {
            wpabuf_free(buf);
    ...
    if (conn->server)
    {
        ...
        if (buf == NULL)
            return NULL;
        res = tlsv1_server_encrypt(conn->server, wpabuf_head(in_data), wpabuf_len(in_data), wpabuf_mhead(buf),
                                   wpabuf_size(buf));
        if (res < 0)
        {
            wpabuf_free(buf);
                                       ...
                                       int *need_more_data)
{
    ...
    if (conn->server)
    {
        ...
        if (buf == NULL)
            return NULL;
        res = tlsv1_server_decrypt(conn->server, wpabuf_head(in_data), wpabuf_len(in_data), wpabuf_mhead(buf),
                                   wpabuf_size(buf));
        if (res < 0)
        {
            wpabuf_free(buf);
