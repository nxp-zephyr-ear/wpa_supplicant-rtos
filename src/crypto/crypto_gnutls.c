...
static int gnutls_digest_vector(int algo, size_t num_elem, const u8 *addr[], const size_t *len, u8 *mac)
{
    gcry_md_hd_t hd;
    unsigned char *p;
    size_t i;
...
static int gnutls_hmac_vector(
    int algo, const u8 *key, size_t key_len, size_t num_elem, const u8 *addr[], const size_t *len, u8 *mac)
{
    gcry_md_hd_t hd;
    unsigned char *p;
    size_t i;
...
{
    return gnutls_hmac_vector(GCRY_MD_MD5, key, key_len, num_elem, addr, len, mac);
}
...
{
    return gnutls_hmac_vector(GCRY_MD_SHA1, key, key_len, num_elem, addr, len, mac);
}
...
{
    return gnutls_hmac_vector(GCRY_MD_SHA256, key, key_len, num_elem, addr, len, mac);
}
...
{
    return gnutls_hmac_vector(GCRY_MD_SHA384, key, key_len, num_elem, addr, len, mac);
}
...
{
    return gnutls_hmac_vector(GCRY_MD_SHA512, key, key_len, num_elem, addr, len, mac);
}
                            ...
                            size_t *len)
{
    ...
    if (pubkey_len > prime_len || (pubkey_len == prime_len && os_memcmp(pubkey, prime, prime_len) >= 0))
        return -1;

    if (gcry_mpi_scan(&pub, GCRYMPI_FMT_USG, pubkey, pubkey_len, NULL) != GPG_ERR_NO_ERROR ||
    ...
    if (order)
    {
        ...
        failed = !tmp || gcry_mpi_scan(&p, GCRYMPI_FMT_USG, prime, prime_len, NULL) != GPG_ERR_NO_ERROR ||
                 gcry_mpi_scan(&q, GCRYMPI_FMT_USG, order, order_len, NULL) != GPG_ERR_NO_ERROR;
        if (!failed)
        {
                   ...
                   size_t *result_len)
{
    ...
    if (gcry_mpi_scan(&bn_base, GCRYMPI_FMT_USG, base, base_len, NULL) != GPG_ERR_NO_ERROR ||
        gcry_mpi_scan(&bn_exp, GCRYMPI_FMT_USG, power, power_len, NULL) != GPG_ERR_NO_ERROR ||
        gcry_mpi_scan(&bn_modulus, GCRYMPI_FMT_USG, modulus, modulus_len, NULL) != GPG_ERR_NO_ERROR)
        goto error;
