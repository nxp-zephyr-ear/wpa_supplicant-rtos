 ...
 * Cf.
 * https://sourceware.org/git/gitweb.cgi?p=binutils-gdb.git;a=commitdiff;h=fd3619828e94a24a92cddec42cbc0ab33352eeb4;hp=5dfda3562a69686c43aad4fb0269cc9d5ec010d5
 */
...
static void find_addr_sect(bfd *abfd, asection *section, void *obj)
{
    ...
    data->found =
        bfd_find_nearest_line(abfd, section, syms, data->pc - vma, &data->filename, &data->function, &data->line);
}
...
void wpa_trace_dump_func(const char *title, void **btrace, int btrace_num)
{
    ...
    for (i = 0; i < btrace_num; i++)
    {
        ...
        if (state == TRACE_HEAD && func &&
            (os_strcmp(func, "wpa_trace_add_ref_func") == 0 || os_strcmp(func, "wpa_trace_check_ref") == 0 ||
