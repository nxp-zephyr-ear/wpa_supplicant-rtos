...
static WINSCARDAPI LONG WINAPI (*dll_SCardEstablishContext)(IN DWORD dwScope,
...
static WINSCARDAPI LONG WINAPI (*dll_SCardListReadersA)(IN SCARDCONTEXT hContext,
                                                        IN LPCSTR mszGroups,
...
static WINSCARDAPI LONG WINAPI (*dll_SCardConnectA)(IN SCARDCONTEXT hContext,
                                                    IN LPCSTR szReader,
...
static WINSCARDAPI LONG WINAPI (*dll_SCardTransmit)(IN SCARDHANDLE hCard,
...
static int scard_read_record(
    struct scard_data *scard, unsigned char *data, size_t len, unsigned char recnum, unsigned char mode);
...
struct scard_data *scard_init(const char *reader)
{
    ...
    ret = SCardConnect(scard->ctx, &readers[pos], SCARD_SHARE_SHARED, SCARD_PROTOCOL_T0 | SCARD_PROTOCOL_T1,
    ...
    wpa_printf(MSG_DEBUG, "SCARD: card=0x%x active_protocol=%lu (%s)", (unsigned int)scard->card,
               (unsigned long)scard->protocol, scard->protocol == SCARD_PROTOCOL_T0 ? "T0" : "T1");
...
static long scard_transmit(
    struct scard_data *scard, unsigned char *_send, size_t send_len, unsigned char *_recv, size_t *recv_len)
{
    long ret;
#ifdef __APPLE__
    uint32_t rlen;
    ...
    rlen      = *recv_len;
    ret       = SCardTransmit(scard->card, scard->protocol == SCARD_PROTOCOL_T1 ? SCARD_PCI_T1 : SCARD_PCI_T0, _send,
                        (unsigned long)send_len, NULL, _recv, &rlen);
    *recv_len = rlen;
    if (ret == SCARD_S_SUCCESS)
    {
...
int scard_gsm_auth(struct scard_data *scard, const unsigned char *_rand, unsigned char *sres, unsigned char *kc)
{
    ...
    if ((scard->sim_type == SCARD_GSM_SIM && (len != 2 || resp[0] != 0x9f || resp[1] != 0x0c)) ||
        (scard->sim_type == SCARD_USIM && (len != 2 || resp[0] != 0x61 || resp[1] != 0x0e)))
    {
        wpa_printf(MSG_WARNING,
