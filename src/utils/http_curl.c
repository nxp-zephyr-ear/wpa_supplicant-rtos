...
ASN1_SEQUENCE(HashAlgAndValue) = {
    ...
    ASN1_SIMPLE(HashAlgAndValue, hashValue, ASN1_OCTET_STRING)} ASN1_SEQUENCE_END(HashAlgAndValue);

ASN1_SEQUENCE(LogotypeReference) = {
    ASN1_SEQUENCE_OF(LogotypeReference, refStructHash, HashAlgAndValue),
    ASN1_SEQUENCE_OF(LogotypeReference, refStructURI, ASN1_IA5STRING)} ASN1_SEQUENCE_END(LogotypeReference);
...
ASN1_SEQUENCE(LogotypeDetails) = {
    ...
    ASN1_SEQUENCE_OF(LogotypeDetails, logotypeURI, ASN1_IA5STRING)} ASN1_SEQUENCE_END(LogotypeDetails);

ASN1_CHOICE(LogotypeImageResolution) = {
    ASN1_IMP(LogotypeImageResolution, d.numBits, ASN1_INTEGER, 1),
    ASN1_IMP(LogotypeImageResolution, d.tableSize, ASN1_INTEGER, 2)} ASN1_CHOICE_END(LogotypeImageResolution);
...
ASN1_SEQUENCE(LogotypeImage) = {ASN1_SIMPLE(LogotypeImage, imageDetails, LogotypeDetails),
                                ASN1_OPT(LogotypeImage, imageInfo, LogotypeImageInfo)} ASN1_SEQUENCE_END(LogotypeImage);

ASN1_SEQUENCE(LogotypeAudioInfo) = {
    ASN1_SIMPLE(LogotypeAudioInfo, fileSize, ASN1_INTEGER), ASN1_SIMPLE(LogotypeAudioInfo, playTime, ASN1_INTEGER),
    ASN1_SIMPLE(LogotypeAudioInfo, channels, ASN1_INTEGER),
    ASN1_IMP_OPT(LogotypeAudioInfo, sampleRate, ASN1_INTEGER, 3),
    ASN1_IMP_OPT(LogotypeAudioInfo, language, ASN1_IA5STRING, 4)} ASN1_SEQUENCE_END(LogotypeAudioInfo);

ASN1_SEQUENCE(LogotypeAudio) = {ASN1_SIMPLE(LogotypeAudio, audioDetails, LogotypeDetails),
                                ASN1_OPT(LogotypeAudio, audioInfo, LogotypeAudioInfo)} ASN1_SEQUENCE_END(LogotypeAudio);

ASN1_SEQUENCE(LogotypeData) = {
    ASN1_SEQUENCE_OF_OPT(LogotypeData, image, LogotypeImage),
    ASN1_IMP_SEQUENCE_OF_OPT(LogotypeData, audio, LogotypeAudio, 1)} ASN1_SEQUENCE_END(LogotypeData);

ASN1_CHOICE(LogotypeInfo) = {ASN1_IMP(LogotypeInfo, d.direct, LogotypeData, 0),
                             ASN1_IMP(LogotypeInfo, d.indirect, LogotypeReference, 1)} ASN1_CHOICE_END(LogotypeInfo);

ASN1_SEQUENCE(OtherLogotypeInfo) = {
    ASN1_SIMPLE(OtherLogotypeInfo, logotypeType, ASN1_OBJECT),
    ASN1_SIMPLE(OtherLogotypeInfo, info, LogotypeInfo)} ASN1_SEQUENCE_END(OtherLogotypeInfo);

ASN1_SEQUENCE(LogotypeExtn) = {
    ASN1_EXP_SEQUENCE_OF_OPT(LogotypeExtn, communityLogos, LogotypeInfo, 0),
    ASN1_EXP_OPT(LogotypeExtn, issuerLogo, LogotypeInfo, 1), ASN1_EXP_OPT(LogotypeExtn, issuerLogo, LogotypeInfo, 2),
    ASN1_EXP_SEQUENCE_OF_OPT(LogotypeExtn, otherLogos, OtherLogotypeInfo, 3)} ASN1_SEQUENCE_END(LogotypeExtn);

IMPLEMENT_ASN1_FUNCTIONS(LogotypeExtn);

#ifdef OPENSSL_IS_BORINGSSL
#define sk_LogotypeInfo_num(st) sk_num(CHECKED_CAST(_STACK *, STACK_OF(LogotypeInfo) *, (st)))
#define sk_LogotypeInfo_value(st, i) \
    (LogotypeInfo *)sk_value(CHECKED_CAST(_STACK *, const STACK_OF(LogotypeInfo) *, (st)), (i))
#define sk_LogotypeImage_num(st) sk_num(CHECKED_CAST(_STACK *, STACK_OF(LogotypeImage) *, (st)))
#define sk_LogotypeImage_value(st, i) \
    (LogotypeImage *)sk_value(CHECKED_CAST(_STACK *, const STACK_OF(LogotypeImage) *, (st)), (i))
#define sk_LogotypeAudio_num(st) sk_num(CHECKED_CAST(_STACK *, STACK_OF(LogotypeAudio) *, (st)))
#define sk_LogotypeAudio_value(st, i) \
    (LogotypeAudio *)sk_value(CHECK_CAST(_STACK *, const STACK_OF(LogotypeAudio) *, (st)), (i))
#define sk_HashAlgAndValue_num(st) sk_num(CHECKED_CAST(_STACK *, STACK_OF(HashAlgAndValue) *, (st)))
#define sk_HashAlgAndValue_value(st, i) \
    (HashAlgAndValue *)sk_value(CHECKED_CAST(_STACK *, const STACK_OF(HashAlgAndValue) *, (st)), (i))
#define sk_ASN1_IA5STRING_num(st) sk_num(CHECKED_CAST(_STACK *, STACK_OF(ASN1_IA5STRING) *, (st)))
#define sk_ASN1_IA5STRING_value(st, i) \
    (ASN1_IA5STRING *)sk_value(CHECKED_CAST(_STACK *, const STACK_OF(ASN1_IA5STRING) *, (st)), (i))
...
static void add_logotype_ext(struct http_ctx *ctx, struct http_cert *hcert, X509 *cert)
{
    ...
    wpa_hexdump(MSG_DEBUG, "logotypeExtn", ASN1_STRING_get0_data(os), ASN1_STRING_length(os));
...
static int curl_cb_ssl_verify(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
    ...
    if (depth == 0 && ctx->ocsp != NO_OCSP && preverify_ok)
    {
        ...
        res = check_ocsp_resp(ssl_ctx, ssl, cert, ctx->peer_issuer, ctx->peer_issuer_issuer);
        if (res == OCSP_REVOKED)
        {
            preverify_ok = 0;
...
static CURL *setup_curl_post(struct http_ctx *ctx,
                             const char *address,
                             const char *ca_fname,
                             const char *username,
                             const char *password,
...
static int post_init_client(struct http_ctx *ctx,
                            const char *address,
                            const char *ca_fname,
                            const char *username,
                            const char *password,
