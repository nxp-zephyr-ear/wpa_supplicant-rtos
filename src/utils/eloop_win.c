...
static int eloop_prepare_handles(void)
{
    ...
    if (eloop.num_handles > eloop.reader_count + eloop.event_count + 8)
        return 0;
    n = os_realloc_array(eloop.handles, eloop.num_handles * 2, sizeof(eloop.handles[0]));
    if (n == NULL)
        return -1;
    eloop.handles = n;
    eloop.num_handles *= 2;
...
int eloop_register_read_sock(int sock, eloop_sock_handler handler, void *eloop_data, void *user_data)
{
    ...
    tmp = os_realloc_array(eloop.readers, eloop.reader_count + 1, sizeof(struct eloop_sock));
    if (tmp == NULL)
    {
...
void eloop_unregister_read_sock(int sock)
{
    ...
    {
        os_memmove(&eloop.readers[i], &eloop.readers[i + 1], (eloop.reader_count - i - 1) * sizeof(struct eloop_sock));
    }
    eloop.reader_count--;
    eloop.reader_table_changed = 1;
}
...
int eloop_register_event(void *event, size_t event_size, eloop_event_handler handler, void *eloop_data, void *user_data)
{
    ...
    tmp = os_realloc_array(eloop.events, eloop.event_count + 1, sizeof(struct eloop_event));
    if (tmp == NULL)
        return -1;
...
void eloop_unregister_event(void *event, size_t event_size)
{
    ...
    if (eloop.events == NULL || eloop.event_count == 0 || event_size != sizeof(HANDLE))
        return;
    ...
    {
        os_memmove(&eloop.events[i], &eloop.events[i + 1], (eloop.event_count - i - 1) * sizeof(struct eloop_event));
    }
    eloop.event_count--;
}
...
static void eloop_process_pending_signals(void)
{
    ...
    if (eloop.term_signal.signaled)
    {
        ...
        eloop.term_signal.handler(eloop.term_signal.sig, eloop.term_signal.user_data);
    }
}
...
{
#ifndef _WIN32_WCE
    if (SetConsoleCtrlHandler((PHANDLER_ROUTINE)eloop_handle_console_ctrl, TRUE) == 0)
    {
...
void eloop_run(void)
{
    ...
    while (!eloop.terminate && (!dl_list_empty(&eloop.timeout) || eloop.reader_count > 0 || eloop.event_count > 0))
    {
        ...
        {
            printf(
                "WaitForMultipleObjects: Too many events: "
...
#else  /* _WIN32_WCE */
        ret = WaitForMultipleObjectsEx(count, eloop.handles, FALSE, timeout_val, TRUE);
#endif /* _WIN32_WCE */
        err = GetLastError();
        ...
        {
            eloop.events[ret].handler(eloop.events[ret].eloop_data, eloop.events[ret].user_data);
        ...
        for (i = 0; i < eloop.reader_count; i++)
        {
            ...
            if (WSAEnumNetworkEvents(eloop.readers[i].sock, eloop.readers[i].event, &events) == 0 &&
                (events.lNetworkEvents & FD_READ))
            {
                eloop.readers[i].handler(eloop.readers[i].sock, eloop.readers[i].eloop_data,
