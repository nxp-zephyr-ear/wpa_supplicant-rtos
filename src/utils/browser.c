...
static void process_request_starting_uri(struct browser_context *ctx, const char *uri)
{
    int quit = 0;
...
static void view_cb_notify_estimated_load_progress(WebKitWebView *view, GParamSpec *pspec, struct browser_context *ctx)
{
                                      ...
                                      struct browser_context *ctx)
{
    ...
    switch (type)
    {
        ...
        case WEBKIT_POLICY_DECISION_TYPE_NAVIGATION_ACTION:
        {
            ...
            wpa_printf(MSG_DEBUG, "BROWSER:%s navigation action: uri=%s", __func__, uri);
            process_request_starting_uri(ctx, uri);
            break;
        }
        default:
...
static void view_cb_notify_title(WebKitWebView *view, GParamSpec *ps, struct browser_context *ctx)
{
    const char *title;
...
static gboolean view_cb_mime_type_policy_decision(WebKitWebView *view,
                                                  WebKitWebFrame *frame,
                                                  WebKitNetworkRequest *req,
                                                  gchar *mime,
...
static void view_cb_title_changed(WebKitWebView *view,
                                  WebKitWebFrame *frame,
                                  const char *title,
                                  struct browser_context *ctx)
{
...
int hs20_web_browser(const char *url, int ignore_tls)
{
    ...
    g_signal_connect(G_OBJECT(view), "notify::estimated-load-progress",
                     G_CALLBACK(view_cb_notify_estimated_load_progress), &ctx);
    g_signal_connect(G_OBJECT(view), "resource-load-started", G_CALLBACK(view_cb_resource_load_starting), &ctx);
    g_signal_connect(G_OBJECT(view), "decide-policy", G_CALLBACK(view_cb_decide_policy), &ctx);
    g_signal_connect(G_OBJECT(view), "mouse-target-changed", G_CALLBACK(view_cb_mouse_target_changed), &ctx);
    g_signal_connect(G_OBJECT(view), "notify::title", G_CALLBACK(view_cb_notify_title), &ctx);
#else  /* USE_WEBKIT2 */
